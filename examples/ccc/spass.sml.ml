  ...
  struct
    ...
    let rec atom =
      function 
               ...
               | ("mor", Some s_) -> (Mor s_)
               | ("obj", None) -> Obj
               | _ -> (What "?atom?")(* | atom (""mor"", SOME(S)) = Mor (""arrow"" ^ par (S)) *);;
    let rec arrow =
      function 
               | (QFProp a1_, QFProp a2_)
                   -> (QFProp ("implies" ^ (par ((a1_ ^ ", ") ^ a2_))))
               | _ -> (What "?arrow?")(* ?? *);;
    let rec pi =
      function 
    ...
    let rec root =
      function 
               ...
               | _ -> "?root?"(* morphism variables *)(* object variables *)(* constants *);;
    let rec lam _ = "?lam?";;
    let rec bvar x = x;;
    let rec const c = c;;
    let rec def d = d;;
...
;;begin
  ...
  (* Equality *)(* refl, then, sym *)(* identity and composition *)(* =@= *)(* products *)(* =pair= *)(* exponentials *)(* =cur= *)(* lemmas *);;