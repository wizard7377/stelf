(* Pass "curry" -- mechanical SML-tuple-style -> curried-style refactoring.

   Subcommands:
     refactor curry targets   list the tupled `val` declarations it will act on
     refactor curry locate    classify every site (AUTO / ESCALATE), write a report
     refactor curry patch     apply the AUTO edits in place

   See docs/curry-sites.txt for the report and docs/curry-reconciliation.md for
   the safety argument. *)
...
let main args =
  ...
  if cmd = "targets" then begin
    ...
    if cmd = "locate" then begin
      ...
      (Printf.eprintf "usage: refactor curry [targets|locate|patch] [scope] [--with-escalate]\n"; exit 1)
  end
