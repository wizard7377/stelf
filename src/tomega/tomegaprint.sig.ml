...
module type TOMEGAPRINT = sig
                          ...
                          val
                            formatFor : (Tomega.dec_
                                         IntSyn.ctx_ *
                                         Tomega.for_) ->
                                        Formatter.format
                          val
                            forToString : (Tomega.dec_
                                           IntSyn.ctx_ *
                                           Tomega.for_) ->
                                          string
                          val
                            formatFun : ((string list * Tomega.lemma list) *
                                         Tomega.prg_) ->
                                        Formatter.format
                          val
                            formatPrg : (Tomega.dec_
                                         IntSyn.ctx_ *
                                         Tomega.prg_) ->
                                        Formatter.format
                          (*  val formatLemmaDec: FunSyn.LemmaDec -> Formatter.format *)
                          val
                            funToString : ((string list * Tomega.lemma list) *
                                           Tomega.prg_) ->
                                          string
