...
module Redundant(Redundant__0: sig module Opsem : OPSEM end) : REDUNDANT =
  struct
    exception Error of string ;;
    (*
    ...
    let rec convert =
      function 
               ...
               | T.Case (T.Cases o_)
                   -> (T.Case ((T.Cases (convertCases o_))))
               | T.Let (d_, p1_, p2_)
    ...
    and caseEqual =
      function 
               | (((psi1_, t1, p1_) :: o1_),
                  (((psi2_, t2, p2_) :: o2_), tAfter))
                   -> let t2' = T.comp (T.invertSub tAfter, t2)
                        in let t = Opsem.createVarSub (psi1_, psi2_)
                             in let t' = T.comp (t2', t)
                                  in let doMatch =
                                       ...
                                       (* Note:  (Psi1 |- t1: Psi0) *)(* Psi1 |- t: Psi2 *)(* Psi1 |- t' : Psi_0 *)
               | ([], ([], t2)) -> true
    ...
    and prgEqual =
      function 
               ...
               | (T.Var x1, (T.Var x2, t2))
                   -> begin
                      match getFrontIndex (T.varSub (x2, t2))
                      with 
                           | None -> false
                           | Some i -> x1 = i
                      end
               ...
               | _ -> false(* there are no PClo created in converter *)(*      | prgEqual ((T.Root (H1, S1)), (T.Root (H2, S2), t2)) =
                (case (H1, H2)
...
*)
    and convertCases =
      function 
               | (a_ :: c_)
                   -> let ((psi_, t, p_), c'_) = removeRedundancy (a_, c_)
                        in ((psi_, t, convert p_) :: convertCases c'_)
               | c_ -> c_
    and removeRedundancy =
      function 
               | (c_, []) -> (c_, [])
               | (c_, (c'_ :: rest))
                   -> let (c''_ :: cs_) = mergeIfNecessary (c_, c'_)
                        in let (c'''_, rest') = removeRedundancy (c''_, rest)
                             in (c'''_, cs_ @ rest')
    and getFrontIndex =
      function 
               | T.Idx k -> (Some k)
               ...
               | I.EClo (u_, t)
                   -> begin
                      ...
                      with 
                           ...
                           | Some i
                               -> getFrontIndex
                                  (T.revCoerceFront (I.bvarSub (i, t)))
                      end
               | (I.Lam (I.Dec (_, u1_), u2_) as u_)
                   -> try (Some (Whnf.etaContract u_)) with 
                                                            | eta_ -> None
               | _ -> None
    and getBlockIndex = function 
                                 | I.Bidx k -> (Some k)
                                 | _ -> None
    and cleanSub =
      function 
    ...
    and mergeSpines =
      function 
               ...
               | (T.AppExp (e1_, s1_), (T.AppExp (e2_, s2_), t2)) -> begin
                   ...
                   raise ((Error "Spine not equal (AppExp)")) end
               | (T.AppBlock (b1_, s1_), (T.AppBlock (b2_, s2_), t2))
                   -> begin
                   if blockEqual (b1_, I.blockSub (b2_, T.coerceSub t2)) then
                   (T.AppBlock (b1_, mergeSpines (s1_, (s2_, t2)))) else
                   raise ((Error "Spine not equal (AppBlock)")) end
               | (T.AppPrg (p1_, s1_), (T.AppPrg (p2_, s2_), t2)) -> begin
                   if prgEqual (p1_, (p2_, t2)) then
                   (T.AppPrg (p1_, mergeSpines (s1_, (s2_, t2)))) else
                   raise ((Error "Prg (in App) not equal")) end
               | (T.SClo (s_, t1), (T.SClo (s, t2a), t2))
                   -> raise ((Error "SClo should not exist!"))
    ...
    and mergePrgs =
      function 
               | (T.Lam (d1_, p1_), (T.Lam (d2_, p2_), t2)) -> begin
                   ...
                   (decEqual (d1_, (d2_, t2))) &&
                     (prgEqual (p1_, (p2_, T.dot1 t2)))
                   then (T.Lam (d1_, p1_)) else
                   raise ((Error "Lambda don't match")) end
               | (T.New p1_, (T.New p2_, t2)) -> begin
                   if prgEqual (p1_, (p2_, t2)) then (T.New p1_) else
                   raise ((Error "New don't match")) end
               | (T.Choose p1_, (T.Choose p2_, t2)) -> begin
                   if prgEqual (p1_, (p2_, t2)) then (T.Choose p1_) else
                   raise ((Error "Choose don't match")) end
               | (T.PairExp (u1_, p1_), (T.PairExp (u2_, p2_), t2))
                   -> let t2' = T.coerceSub t2 in begin
                        if Conv.conv ((u1_, I.id), (u2_, t2')) then
                        (T.PairExp (u1_, mergePrgs (p1_, (p2_, t2)))) else
                        raise ((Error "cannot merge PairExp")) end
               | (T.PairBlock (b1_, p1_), (T.PairBlock (b2_, p2_), t2))
                   -> let b2'_ = I.blockSub (b2_, T.coerceSub t2) in begin
                        if blockEqual (b1_, b2'_) then
                        (T.PairBlock (b1_, mergePrgs (p1_, (p2_, t2)))) else
                        raise ((Error "cannot merge PairBlock")) end
               | (T.PairPrg (p1a_, p1b_), (T.PairPrg (p2a_, p2b_), t2))
                   -> begin
               ...
               | (T.Var x1, (T.Var x2, t2))
                   -> begin
                      ...
                      with 
                           | None
                               -> raise ((Error "Variables do not match."))
                           | Some i -> begin
                               if x1 = i then (T.Var x1) else
                               raise ((Error "Variables do not match.")) end
                      end
               | (T.Redex (p1_, s1_), (T.Redex (p2_, s2_), t2))
                   -> let newS = mergeSpines (s1_, (s2_, t2)) in begin
                        if prgEqual (p1_, (p2_, t2)) then
                        (T.Redex (p1_, newS)) else
                        raise ((Error "Redex Prgs don't match")) end
               | (T.Rec (d1_, p1_), (T.Rec (d2_, p2_), t2)) -> begin
                   if
                   (decEqual (d1_, (d2_, t2))) &&
                     (prgEqual (p1_, (p2_, T.dot1 t2)))
                   then (T.Rec (d1_, p1_)) else
                   raise ((Error "Rec's don't match")) end
               | (T.Case (T.Cases o1_), (T.Case (T.Cases ((c_ :: []))), t2))
                   -> (T.Case ((T.Cases (mergeCase (o1_, (c_, t2))))))
               | (T.Case o1_, (T.Case o2_, t2))
               ...
               | (T.Let (d1_, p1a_, p1b_), (T.Let (d2_, p2a_, p2b_), t2))
                   ...
                   else raise ((Error "Let don't match")) end
               | (T.EVar (psi1_, p1optRef_, f1_, _, _, _),
                  (T.EVar (psi2_, p2optref_, f2_, _, _, _), t2))
                   -> raise ((Error "No EVARs should exist!"))
               | _
                   -> raise
                      ((Error
                        "Redundancy in cases could not automatically be removed."))(* there are no PClo created in converter *)(* By invariant the second case should be a list of one *)(* three possible outcomes -
...
*)
    ...
    and printSub =
      function 
               ...
               | T.Dot (T.Prg (T.EVar _), s)
                   -> begin
                        print "PRG_EVAR (DOT) ";printSub s
                        end
               | T.Dot (T.Exp (I.EVar _), s)
                   -> begin
                        print "EXP_EVAR (DOT) ";printSub s
                        end
               | T.Dot (T.Prg p_, s)
                   -> begin
                        print "PRG (DOT) ";printSub s
                        end
               | T.Dot (T.Exp e_, s)
                   -> begin
                        print "EXP (DOT) ";printSub s
                        end
               | T.Dot (T.Block b_, s)
                   -> begin
    ...
    and mergeCase =
      function 
               | ([], c_)
                   -> raise ((Error "Case incompatible, cannot merge"))
               | ((((psi1_, t1, p1_) :: o_) as l_),
                  (((psi2_, t2, p2_), tAfter) as c_))
                   -> let tAfterInv = T.invertSub tAfter
                        in let t3 = T.comp (tAfterInv, t2)
                             in let t = Opsem.createVarSub (psi1_, psi2_)
                                  in let t' = T.comp (t3, t)
                                       in let doMatch =
                                            try begin
                                                  Opsem.matchSub
                                                  (psi1_, t1, t');true
                                            ...
                                            let newT = T.normalizeSub t
                                              in let stillMatch =
                                                   ...
                                                   (* We tried all the cases, and we can now add it *)(* Try other cases *)
                                                   end
        ...
        val _ = printCtx(Psi2)
         ...
         Psi1 |- t1 : Psi1'
         Psi2 |- t3 : Psi1'
         *)(* Psi1 |- t : Psi2 *)(* Psi1 |- t' : Psi1' *)(* If we can get this to match, then Psi1 |- P2[t] *)
    and mergeIfNecessary
      (((psi1_, s1, p1_) as c_), ((psi2_, s2, p2_) as c'_)) =
      let t = Opsem.createVarSub (psi1_, psi2_)
        in let t' = T.comp (s2, t)
             in let doMatch =
                  try begin
                        Opsem.matchSub (psi1_, s1, t');true
                        end
                  with 
                       | NoMatch -> false
                  in begin
