 ...
 *)
  ...
  let assignSubId : unit -> assSubsts = Red_black_set.RBSet.new_
  let cnstrSubId : unit -> cnstrSubsts = Red_black_set.RBSet.new_
  let querySubId : unit -> querySubsts = Red_black_set.RBSet.new_

  (* Identity substitution *)
  let rec isId s = Red_black_set.RBSet.isEmpty s

  (* Initialize substitution tree *)
  let rec makeTree () = ref (Node (nid (), Red_black_set.RBSet.new_ ()))

  (* Test if node has any children *)
  let rec noChildren c_ = Red_black_set.RBSet.isEmpty c_
  ...
  open! struct
    ...
    let rec compatible (label, t_, u_, rho_t, rho_u) =
      ...
      let rec genTop = function
        | label, (I.Root (h1_, s1_) as t_), (I.Root (h2_, s2_) as u_) -> begin
            if eqHeads (h1_, h2_) then
    ...
    let rec compatibleSub (nsub_t, nsub_e) =
      ...
      let _ =
        S.forall nsub_e (function nv, (l', e_) ->
            ...
            | Some (l, t_) -> begin
                if l = l' then begin
                  match compatible (l, t_, e_, rho_t, rho_e) with
                  | None -> begin
    ...
    and insert = function
      ...
      | (Node (_, children) as n_), nsub_e, ((g_clause2_, rc_) as gr_) -> begin
          if noChildren children then begin
    ...
    let rec retrieveChild
      ...
      let rec retrieve = function
        | ( Leaf (nsub, gclause_, residuals_),
            ...
            with
            | None -> ()
            | Some (nsub_query', cnstrSub', cnstr') -> begin
    ...
    let rec retrieveAll (num, child_, nsub_query, assignSub, cnstr, candSet) =
      ...
      let rec retrieve = function
        | ( Leaf (nsub, gclause_, residuals_),
            ...
            with
            | None -> ()
            | Some (nsub_query', (nsub_left', cnstrSub'), cnstr') -> begin
                if isId nsub_query' then begin
                  i := !i + 1;
                  begin
                    S.insert candSet
