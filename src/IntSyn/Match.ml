...
module MakeMatch (Whnf : WHNF) (Unify : UNIFY) (Trail : TRAIL) : MATCH = struct
  ...
  open! struct
    ...
    and pruneExpW (g, a, ss, rOccur) = match a with
      ...
      | ((AVar _ as x), s) -> raise (Match "Left-over AVar")
    ...
    let rec matchExpW (g, a, b) = match a, b with
      ...
      | ((Root (h1, s1_), s1) as us1), ((Root (h2, s2_), s2) as us2) ->
          ...
          | BVar k1, BVar k2 ->
              begin if k1 = k2 then matchSpine (g, (s1_, s1), (s2_, s2))
              else raise (Match "Bound variable clash")
              end
          | Const c1, Const c2 ->
              begin if c1 = c2 then matchSpine (g, (s1_, s1), (s2_, s2))
              else raise (Match "Constant clash")
              end
          | Proj (b1, i1), Proj (b2, i2) ->
              begin if i1 = i2 then begin
                matchBlock (g, b1, b2);
                matchSpine (g, (s1_, s1), (s2_, s2))
              end
          ...
          | FVar (n1, _, _), FVar (n2, _, _) ->
              begin if n1 = n2 then matchSpine (g, (s1_, s1), (s2_, s2))
              else raise (Match "Free variable clash")
              end
          | Def d1, Def d2 ->
              begin if d1 = d2 then matchSpine (g, (s1_, s1), (s2_, s2))
              else matchDefDefW (g, us1, us2)
              end
          | Def d1, Const c2 ->
              begin match defAncestor d1 with
              | Anc (_, _, None) -> matchExpW (g, Whnf.expandDef us1, us2)
              | Anc (_, _, Some c1) ->
                  begin if c1 = c2 then matchExpW (g, Whnf.expandDef us1, us2)
                  else raise (Match "Constant clash")
                  end
              end
          | Const c1, Def d2 ->
              begin match defAncestor d2 with
              | Anc (_, _, None) -> matchExpW (g, us1, Whnf.expandDef us2)
              | Anc (_, _, Some c2) ->
                  begin if c1 = c2 then matchExpW (g, us1, Whnf.expandDef us2)
                  else raise (Match "Constant clash")
                  end
              end
    ...
    and matchExp (g, ((e1, s1) as us1), ((e2, s2) as us2)) =
    ...
    and matchSpine (g, a, b) = match a, b with
      | (Nil, _), (Nil, _) -> ()
      | (SClo (s1_, s1'), s1), ss ->
          matchSpine (g, (s1_, comp s1' s1), ss)
      | ss, (SClo (s2_, s2'), s2) ->
          matchSpine (g, ss, (s2_, comp s2' s2))
    ...
    and matchBlock (g, a, b) = match a, b with
      | LVar ({ contents = Some b1 }, s, _), b2 ->
          matchBlock (g, blockSub b1 s, b2)
      | b1, LVar ({ contents = Some b2 }, s, _) ->
          matchBlock (g, b1, blockSub b2 s)
      | b1, b2 -> matchBlockW (g, b1, b2)

    and matchBlockW (g, b1, b2) = match b1, b2 with
      ...
      | LVar (r1, s1, (l1, t1)), b2 -> begin
          r1 := Some (blockSub b2 (Whnf.invert s1));
          ()
        end
      | b1, LVar (r2, s2, (l2, t2)) -> begin
    ...
    let matchW g us1 us2 =
      begin
        Unify.resetAwakenCnstrs ();
        match1W (g, us1, us2)
      end

    let match_ (g, us1, us2) =
      begin
        Unify.resetAwakenCnstrs ();
        match1 (g, us1, us2)
      end
