      ...
      struct
        ...
        let rec constCases =
          function 
                   ...
                   | (g_, vs_, (I.Const c :: sgn_), abstract, ops)
                       -> let (u_, vs'_) =
                            M.createAtomConst (g_, (I.Const c))
                            in constCases
        ...
        let rec paramCases =
          function 
                   ...
                   | (g_, vs_, k, abstract, ops)
                       -> let (u_, vs'_) = M.createAtomBVar (g_, k)
                            in paramCases
                               (g_, vs_, k - 1, abstract,
                                CSManager.trail
                                (function 
                   ...
                   | (g_, ((I.Root (I.Const c, _) as v_), s'), abstract)
                       -> constCases
                          (g_, (v_, s'), Index.lookup c, abstract,
                           paramCases
                           (g_, (v_, s'), I.ctxLength g_, abstract, []))
                                ...
                                function 
                                         | (name, u_)
                                             -> abstract
                                                (name, (I.Lam (d'_, u_))));;
        let rec split
          (M.Prefix (g_, m_, b_), ((I.Dec (_, v_) as d_), s), abstract) =
          lowerSplitDest
          (I.null_, (v_, s),
           function 
                    | (name', u'_)
                        -> abstract
                           (name', (M.Prefix (g_, m_, b_)),
                            (I.Dot ((I.Exp u'_), s))));;
        let rec occursInExp =
          function 
                   | (k, I.Uni _) -> false
        ...
        let rec checkExp =
          function 
                   ...
                   | (m_, I.Pi ((d_, p_), v_))
                       -> (checkDec (m_, d_)) &&
                            (checkExp ((I.Decl (m_, M.top_)), v_))
                   | (m_, I.Lam (d_, v_))
                       -> (checkDec (m_, d_)) &&
                            (checkExp ((I.Decl (m_, M.top_)), v_))
                   | (m_, I.Root (I.BVar k, s_))
                       -> (checkVar (m_, k)) && (checkSpine (m_, s_))
                   | (m_, I.Root (_, s_)) -> checkSpine (m_, s_)
        and checkSpine =
          function 
                   | (m_, nil_) -> true
                   | (m_, I.App (u_, s_))
                       -> (checkExp (m_, u_)) && (checkSpine (m_, s_))
        ...
        let rec inheritBelow =
          function 
                   ...
                   | (b', k', I.Root (I.BVar n', s'_), (b'_, d, d')) -> begin
                       if (n' = (k' + d')) && (n' > k') then
                       inheritBelowSpine
                       (b', k', s'_, ((I.Decl (b'_, b')), d, d' - 1)) else
                       inheritBelowSpine (b', k', s'_, (b'_, d, d')) end
                   | (b', k', I.Root (c_, s'_), bdd'_)
                       -> inheritBelowSpine (b', k', s'_, bdd'_)
        and inheritBelowSpine =
          function 
                   | (b', k', nil_, bdd'_) -> bdd'_
                   | (b', k', I.App (u'_, s'_), bdd'_)
                       -> inheritBelowSpine
                          (b', k', s'_, inheritBelow (b', k', u'_, bdd'_))
        and inheritBelowDec (b', k', I.Dec (x, v'_), bdd'_) =
          inheritBelow (b', k', v'_, bdd'_);;
        let rec skip =
          function 
                   | (k, I.Lam (d_, u_), bdd'_)
                       -> skip (k + 1, u_, skipDec (k, d_, bdd'_))
                   | (k, I.Pi ((d_, _), v_), bdd'_)
                       -> skip (k + 1, v_, skipDec (k, d_, bdd'_))
                   | (k, I.Root (I.BVar n, s_), (b'_, d, d')) -> begin
                       if (n = (k + d)) && (n > k) then
                       skipSpine (k, s_, (b'_, d - 1, d')) else
                       skipSpine (k, s_, (b'_, d, d')) end
                   | (k, I.Root (c_, s_), bdd'_) -> skipSpine (k, s_, bdd'_)
        and skipSpine =
          function 
                   | (k, nil_, bdd'_) -> bdd'_
                   | (k, I.App (u_, s_), bdd'_)
                       -> skipSpine (k, s_, skip (k, u_, bdd'_))
        ...
        let rec inheritExp =
          function 
                   | (b_, k, I.Lam (d_, u_), k', I.Lam (d'_, u'_), bdd'_)
                       -> inheritExp
                          (b_, k + 1, u_, k' + 1, u'_,
                           inheritDec (b_, k, d_, k', d'_, bdd'_))
                   | (b_, k, I.Pi ((d_, _), v_), k', I.Pi ((d'_, _), v'_),
                      bdd'_)
                       -> inheritExp
                          (b_, k + 1, v_, k' + 1, v'_,
                           inheritDec (b_, k, d_, k', d'_, bdd'_))
                   | (b_, k, (I.Root (I.BVar n, s_) as v_), k', v'_,
                      (b'_, d, d')) -> begin
                       if (n = (k + d)) && (n > k) then
                       skipSpine
                       (k, s_,
                       ...
                       let I.Root (c'_, s'_) = v'_
                         in inheritSpine (b_, k, s_, k', s'_, (b'_, d, d'))
                       end end
                   | (b_, k, I.Root (c_, s_), k', I.Root (c'_, s'_), bdd'_)
                       -> inheritSpine (b_, k, s_, k', s'_, bdd'_)
        and inheritNewRoot =
          function 
                   | (b_, b, k, I.Root (I.BVar n, s_), k',
                      (I.Root (I.BVar n', s'_) as v'_), (b'_, d, d'))
                       -> begin
                       if (n' = (k' + d')) && (n' > k') then
                       inheritBelow (b, k', v'_, (b'_, d - 1, d')) else
                       inheritBelow (b - 1, k', v'_, (b'_, d - 1, d')) end
        ...
        let rec inheritDTop =
          function 
                   | (b_, k, I.Pi ((I.Dec (_, v1_), no_), v2_), k', I.Pi
                      ((I.Dec (_, v1'_), no_), v2'_), bdd'_)
                       -> inheritG
                          (b_, k, v1_, k', v1'_,
                           inheritDTop (b_, k + 1, v2_, k' + 1, v2'_, bdd'_))
                   | (b_, k, (I.Root (I.Const cid, s_) as v_), k',
                      (I.Root (I.Const cid', s'_) as v'_), bdd'_)
                       -> let mS = valOf (ModeTable.modeLookup cid)
                            in inheritSpineMode
                               (M.top_, mS, b_, k, s_, k', s'_, bdd'_)
        and inheritDBot =
          function 
                   | (b_, k, I.Pi ((I.Dec (_, v1_), no_), v2_), k', I.Pi
                      ((I.Dec (_, v1'_), no_), v2'_), bdd'_)
                       -> inheritDBot (b_, k + 1, v2_, k' + 1, v2'_, bdd'_)
                   | (b_, k, I.Root (I.Const cid, s_), k', I.Root
                      (I.Const cid', s'_), bdd'_)
                       -> let mS = valOf (ModeTable.modeLookup cid)
                            in inheritSpineMode
                               (M.bot_, mS, b_, k, s_, k', s'_, bdd'_)
        and inheritG
          (b_, k, I.Root (I.Const cid, s_), k',
           (I.Root (I.Const cid', s'_) as v'_), bdd'_)
          =
          let mS = valOf (ModeTable.modeLookup cid)
            in inheritSpineMode
               (M.bot_, mS, b_, k, s_, k', s'_,
                inheritSpineMode (M.top_, mS, b_, k, s_, k', s'_, bdd'_))
        and inheritSpineMode =
          function 
                   | (mode, mnil_, b_, k, nil_, k', nil_, bdd'_) -> bdd'_
                   | (mode, ModeSyn.Mapp (m, mS), b_, k, I.App (u_, s_), k',
                      I.App (u'_, s'_), bdd'_) -> begin
                       if modeEq (m, mode) then
                       inheritSpineMode
                       (mode, mS, b_, k, s_, k', s'_,
                        inheritExp (b_, k, u_, k', u'_, bdd'_))
                       else
                       inheritSpineMode (mode, mS, b_, k, s_, k', s'_, bdd'_)
                       end;;
        let rec inheritSplitDepth
          ((M.State (_, M.Prefix (g_, m_, b_), v_) as s_),
           (M.State (name', M.Prefix (g'_, m'_, b'_), v'_) as s'_))
          =
          let d = I.ctxLength g_
            in let d' = I.ctxLength g'_
                 in let v_ = Whnf.normalize (v_, I.id)
                      in let v'_ = Whnf.normalize (v'_, I.id)
                           in let (b''_, 0, 0) =
                                inheritDBot
                                (b_, 0, v_, 0, v'_,
                                 inheritDTop
                                 (b_, 0, v_, 0, v'_, (I.null_, d, d')))
                                in (M.State
                                    (name', (M.Prefix (g'_, m'_, b''_)), v'_));;
        ...
        let rec expand' =
          function 
                   | (M.Prefix (null_, null_, null_), isIndex, abstract,
                      makeAddress)
                       -> ((M.Prefix (I.null_, I.null_, I.null_)), I.id, [])
                   | (M.Prefix
                      (I.Decl (g_, d_), I.Decl (m_, (top_ as mode)), I.Decl
                       (b_, b)),
                                 ...
                                 in let x_ =
                                      ...
                                      in let ops' = begin
                                           ...
                                           in ((M.Prefix (g'_, m'_, b'_)),
                                               ...
                                               ops')
                   | (M.Prefix
                      (I.Decl (g_, d_), I.Decl (m_, (bot_ as mode)), I.Decl
                       (b_, b)),
                      isIndex, abstract, makeAddress)
                       -> let (M.Prefix (g'_, m'_, b'_), s', ops) =
                            expand'
                            ...
                            in ((M.Prefix
                                 ((I.Decl (g'_, I.decSub (d_, s'))),
                                  (I.Decl (m'_, M.bot_)), (I.Decl (b'_, b)))),
                                I.dot1 s', ops);;
        ...
        let rec apply (_, sl_) =
          ...
          sl_;;
        let rec menu
          ((((M.State (name, M.Prefix (g_, m_, b_), v_), i), sl_) as op_)) =
          let rec active =
            function 
                     | ([], n) -> n
