module Macro (C : CST.CST) : MACRO.MACRO with module C = C = struct
  ...
  and go_cmd i args (cmd : C.cmd) : C.cmd =
    ...
    match !>cmd with
    | Symbol _ | Freeze _ | Thaw _ | Union _ | Deterministic _ | Prec _
    | Stop _ | ReplQuit _ | ReplHelp _ | ReplGet _ | ReplSet _ | ReplVersion _
    | Name _ | Require _ | Open _ -> cmd
  ...
  and go_term i args (term : C.term) : C.term =
    ...
    | Omitted _ | Typ _ | Internal _ -> term
    | Pi (loc, decls, body) ->
    ...
    | App (loc, head, arg_list) ->
      review (App (loc, go_term i args head, List.map (go_term i args) arg_list))
    | HasType (loc, tm, ty) ->
    ...
    | Foreign (loc, tm) ->
      review (Foreign (loc, go_term i args tm))
  ...
  and go_decl i args (decl : C.decl) : C.decl =
    ...
    | Decl0 (loc, names, ty) ->
      review (Decl0 (loc, names, go_term i args ty))
  ...
  and go_define i args (d : C.define) : C.define =
    ...
    | Define (loc, n, tm1, tm2_opt) ->
      review (Define (loc, n, go_term i args tm1, Option.map (go_term i args) tm2_opt))
  ...
  and go_mode_term i args (mt : C.modeTerm) : C.modeTerm =
    ...
    | ModeTerm _ -> mt
    | ModePi (loc, d, body, _) ->
      review (ModePi (loc, go_decl i args d, go_mode_term i args body,
