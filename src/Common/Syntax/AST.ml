(** {1 The Internal Syntax}
    ...
    The internal syntax of the LF logical framework. This module defines the
    core representation of LF terms, types, kinds, declarations, contexts,
    explicit substitutions, and the global signature.

    Types and terms share the same syntactic category ({!exp}), following the
    style of the Edinburgh Logical Framework where the distinction between terms
    and types is maintained by the type system rather than the grammar.
...
module type AST = sig
  ...
  *)
  ...
  (** Constraint-solver module identifier. Indexes into the foreign operation
      tables. *)

  type nonrec fgnExp = exn
  (** Foreign expression representation. Uses OCaml's extensible [exn] type so
      that constraint solvers can define their own expression forms. *)

  exception UnexpectedFgnExp of fgnExp
  (** Raised by a constraint solver when passed a foreign expression belonging
      to a different solver. *)

  type nonrec fgnCnstr = exn
  (** Foreign constraint representation. Uses OCaml's extensible [exn] type so
      that constraint solvers can define their own constraint forms. *)

  exception UnexpectedFgnCnstr of fgnCnstr
  (** Raised by a constraint solver when passed a foreign constraint belonging
      to a different solver. *)

  (** {2 Contexts} *)

  (** Contexts are snoc-lists of declarations, commonly written {m \Gamma}. They
      grow to the right: [Decl (Decl (Null, D1), D2)] represents
  ...
  (** Dependency information for function types. Tracks whether the bound
      variable actually appears in the body, which affects printing (arrow
      {m A \to B} vs. Pi {m \Pi x{:}A.\, B}) and reconstruction behavior. *)
  type depend =
    | No
        (** Non-dependent: the bound variable does not occur in the body.
            Printed as {m A \to B}. *)
    | Maybe
        (** Possibly dependent: not yet determined during reconstruction. *)
    | Meta
        (** Definitely dependent: the bound variable occurs in the body. Printed
            as {m \Pi x{:}A.\, B} or [{m \{x:A\} B}]. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Universes of LF. In LF, types classify terms and kinds classify types. *)
  type uni =
    | Kind  (** The universe of types. *)
    | Type  (** The universe of terms (objects). *)
  [@@deriving eq, ord, show { with_path = false }]

  (** LF expressions. Types and terms share the same syntactic category; the
      distinction is maintained by the type system.
  ...
  type exp =
    | Uni of uni
        (** A universe: [Uni Kind] classifies types, [Uni Type] classifies
            terms. *)
    | Pi of (dec * depend) * exp
        (** {m \Pi x{:}A.\, B}, the dependent function type. The first component
            is a declaration ({!dec}) paired with dependency information
            ({!depend}). The second component is the result type [B] in which
            the bound variable may occur. *)
    | Root of head * spine
        (** Atomic term: a {!head} applied to a {!spine} of arguments. Heads are
            canonical forms (constants, bound variables, projections). This is
            the normal form for applications where the head is not a lambda. See
            {!Redex} for non-canonical applications. *)
    | Redex of exp * spine
        (** Non-canonical application: an arbitrary expression applied to a
            spine. Unlike {!Root}, the head may be a lambda or other non-atomic
            expression. Reduced to {!Root} form during weak head-normal form
            computation. *)
    | Lam of dec * exp
        (** Lambda abstraction {m \lambda x{:}A.\, U}. The declaration
            introduces the bound variable and its type; the body may refer to it
            via de Bruijn index. *)
    | EVar of exp option ref * dec ctx * exp * cnstr_ ref list ref
        (** Existential (logic) variable {m X : \Gamma \vdash V}.
            - The [ref] cell holds [None] while uninstantiated, [Some U] once
              solved.
    ...
    | EClo of exp * sub
        (** Explicit closure {m U[s]}: an expression with a pending
            substitution. Substitutions are applied lazily; {!EClo} defers the
            work until the expression is inspected (e.g., during weak
            head-normal form computation). *)
    | AVar of exp option ref
        (** Assignable variable: a lightweight unification variable that carries
            ...
            reconstruction to represent variables produced as part of a
            constraint graph. *)
    | FgnExp of csid * fgnExp
        (** Foreign expression provided by an external constraint solver. The
            {!csid} identifies which solver owns this expression. Operations on
            it are dispatched through {!FgnExpStd}. *)
    | NVar of int
        (** Named/linear variable represented by a de Bruijn index. Used
            internally for fully-applied bound variables in certain specialized
            contexts (memo tables, subtree indexing). Distinct from the {!BVar}
            head constructor which appears inside {!Root}. *)
    | Tag of Tag.t * exp
        (** A tagged expression carrying position information and other
            metadata. Tags are transparent to equality and comparison (ignored
            by [deriving]).
            @since 2.0 *)
  [@@deriving eq, ord, show { with_path = false }]
  ...
  (** Heads of atomic terms ({!Root}).
      ...
      v} *)
  and head =
    | BVar of int  (** Bound variable with de Bruijn index [k] (1-indexed). *)
    | Const of cid
    ...
    | Def of cid
        (** Strict defined constant [d]: its definition is expanded eagerly
            during weak head-normal form computation. Refers to a {!ConDef} in
            the signature. *)
    | NSDef of cid
    ...
    | FgnConst of csid * conDec
        (** Foreign constant provided by constraint solver {!csid}. Carries its
            own {!conDec} rather than referencing the signature. *)
  [@@deriving eq, ord, show { with_path = false }]
  ...
  (** Explicit substitutions.
      ...
      substitution {m 1 \mapsto U,\; 2 \mapsto V,\; k \mapsto k+1} for
      {m k \geq 3}.
  ...
  and sub =
    ...
    | Dot of front * sub
        (** [Dot (Ft, s)] maps index 1 to [Ft] and index [k+1] to [s(k)]. This
            extends substitution [s] with a new binding for the most recently
            bound variable. *)
  [@@deriving eq, ord, show { with_path = false }]
  ...
  and front =
    | Idx of int
        (** A de Bruijn index. Represents a renaming: the variable is mapped to
            another variable rather than to a term. *)
    | Exp of exp  (** An expression to substitute in for the variable. *)
    | Axp of exp
        (** Assignable expression: like {!Exp} but marks the substituted term as
            assignable during unification. Historical; currently unused in the
            OCaml port. *)
    | Block of block
        (** A block value, used when substituting block variables. *)
    | Undef
        (** Undefined: marks a position where no valid substitution exists. Used
            in {!invShift} ([^-1]) and weakening substitutions where a variable
            is dropped from scope. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Declarations in LF contexts.

      The same declaration forms are used for both hypothetical (context-level)
      and top-level judgements.
  ...
  and dec =
    | Dec of string option * exp
        (** Standard variable declaration [x : V] with an optional name and a
            type expression. This is the most common declaration form. *)
    ...
    | NDec of string option
        (** Name-only declaration with no type information. Used as a
            placeholder during reconstruction when only the variable name is
            known. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Blocks: grouped variable declarations used in world checking and regular
      worlds.
  ...
  *)
  and block =
    | Bidx of int
        (** Block index: a de Bruijn index referencing a block variable in the
            current context. *)
    | LVar of block option ref * sub * (cid * sub)
        (** Block logic variable: an uninstantiated block variable.
            - The [ref] cell holds [None] while unsolved, [Some b] once
              instantiated.
            - The {!sub} is a suspended substitution.
            - The [(cid * sub)] pair references the block type declaration and
              its type substitution. *)
    | Inst of exp list
        (** Instantiated block: a concrete list of expressions representing the
            components of a fully resolved block. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Constraints: delayed unification problems attached to existential
      variables.
      ...
      v} *)
  and cnstr_ =
    | Solved  (** The constraint has been resolved. *)
    | Eqn of dec ctx * exp * exp
        (** An equality constraint: in context {m \Gamma}, expressions [U1] and
            [U2] must be equal. Delayed when one side contains an uninstantiated
            existential variable. *)
    | FgnCnstr of csid * fgnCnstr
        (** A foreign constraint managed by an external solver. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Status of a constant, controlling how it behaves during evaluation and
      unification. *)
  and status =
    | Normal  (** Inert constant: no special evaluation behavior. *)
    ...
    | Foreign of csid * (spine -> exp)
        (** Foreign constant: fully handled by an external solver. The function
            eagerly evaluates the application to produce a result. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Result of attempting foreign unification. *)
  and fgnUnify =
    | Succeed of fgnUnifyResidual list
        (** Unification succeeded, producing a (possibly empty) list of residual
            operations that must be performed. *)
    | Fail  (** Unification failed: the terms are not unifiable. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Residual operations produced by successful foreign unification. These are
      instructions for the main unification engine to carry out. *)
  and fgnUnifyResidual =
    | Assign of dec ctx * exp * exp * sub
        (** Assignment: perform {m \Gamma \vdash X = U[s]}, instantiating the
            existential variable. *)
    | Delay of exp * cnstr_ ref
        (** Delay: suspend a constraint, associating it with all rigid
            existential variables in the expression. The constraint will be
            re-awakened when those variables are instantiated. *)
  [@@deriving eq, ord, show { with_path = false }]

  (** Constant declarations in the global signature.

      Each variant stores the constant's name, optional parent structure, and
      variant-specific data. These are stored in the signature array and
      retrieved by {!sgnLookup}. *)
  and conDec =
    | ConDec of string * mid option * int * status * exp * uni
        (** Typed constant declaration [a : V : L]. Fields: name, parent
            structure, number of implicit arguments, status, type, and universe
            (Kind or Type). *)
    | ConDef of string * mid option * int * exp * exp * uni * ancestor
        (** Constant definition [d = U : V : L]. Fields: name, parent, implicit
            args, {e definition body} [U], type [V], universe, and ancestor
            information for termination. *)
    | AbbrevDef of string * mid option * int * exp * exp * uni
        (** Abbreviation definition [a = U : V : L]. Like {!ConDef} but without
            ancestor tracking. Abbreviations are expanded non-strictly (only
            when needed) and do not participate in termination analysis. Fields:
            name, parent, implicit args, definition body, type, universe. *)
    | BlockDec of string * mid option * dec ctx * dec list
        (** Block declaration [%block l : SOME G PI D1 ... Dn]. Fields: name,
            parent, the "some" context of parameters, and the list of "pi"
            declarations that form the block body. *)
    | BlockDef of string * mid option * cid list
        (** Block definition [%block l = (l1 | ... | ln)]. A union of existing
            block declarations. Fields: name, parent, list of block declaration
            {!cid}s. *)
    | SkoDec of string * mid option * int * exp * uni
        (** Skolem constant declaration. Introduced during coverage checking to
            witness universal quantification. Fields: name, parent, implicit
            args, type, universe. *)
  [@@deriving eq, ord, show { with_path = false }]
  ...
  (** Ancestor information for defined constants, used in termination checking.
      ...
      - [head] is [Some c] if expanding the definition one step yields an atomic
        type headed by [c], or [None] if it yields a function type.
      - [height] is the number of expansion steps to reach a non-definition
        head.
  ...
  (** [StrDec (name, parent)] declares a structure with the given name and
  ...
  (** A mutable constraint reference. Constraints are shared via [ref] cells so
      that solving one updates all references. *)
  ...
  (** Extract the number of implicit arguments. Returns [0] for {!BlockDec} and
      {!BlockDef}. *)

  val conDecStatus : conDec -> status
  (** Extract the status of a constant. Returns {!Normal} for all forms except
      {!ConDec}. *)
