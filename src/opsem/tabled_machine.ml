      ...
      let ((C.DProg (g_, dPool) as dProg), (p, s)) =
        ...
        in let (g'_, dAVars_, dEVars_, u'_, eqn', s') =
             ...
             in let _ = begin
                  ...
                  in let status = begin
                       if flag then TableParam.Complete else
                       TableParam.Incomplete end in begin
    ...
    and matchAtom
      ...
      let rec matchSig =
        ...
        in let rec matchDProg =
             ...
             in let rec matchConstraint (solve_fn, try_) =
                  let succeeded =
                    Cs_manager.trail
                    (function 
                              | ()
                    ...
                    in begin
                    if succeeded then matchConstraint (solve_fn, try_ + 1) else
                    () end
                  in begin
                     match I.constStatus (cidFromHead ha_)
                     with 
                          | I.Constraint (cs, solve_fn)
                              -> matchConstraint (solve_fn, 0)
  ...
  *);;
   ...
   *);;
    ...
    let rec solveQuery ((g, s), (C.DProg (g_, dPool) as dp), sc) =
      (!solve_fn_ref ((g, s), dp, sc))(* only works when query is atomic -- if query is not atomic,
