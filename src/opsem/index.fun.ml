...
module TableIndex (TableIndex__0 : sig
  ...
  open! struct
    ...
    let rec printTable () =
      let rec proofTerms = function
        | g_, d_, u_, [] -> print ""
        | g_, d_, u_, ((d'_, s'), _) :: s_ -> begin
            try
              print
                (Print.expToString
                   ( I.null_,
    ...
    let rec exceedsTermDepth i =
      begin match !termDepth with None -> false | Some n -> i > n
      end
    ...
    let rec exceedsCtxLength i =
      begin match !ctxLength with None -> false | Some n -> i > n
      end
    ...
    and countDecl = function
      | ctrType, ctrLength, I.Pi ((d_, _), v_) ->
          let ctrType' = countDec (0, d_) in
          begin if ctrType' > ctrType then
            countDecl (ctrType', ctrLength + 1, v_)
          else countDecl (ctrType, ctrLength + 1, v_)
          end
      | ctrType, ctrLength, u_ ->
          let ctrTerm = count (0, u_) in
          exceedsCtxDepth ctrType || exceedsCtxLength ctrLength
          || exceedsTermDepth (count (0, u_))

    and countDec = function
    ...
    and count = function
      ...
      | ctr, I.Pi ((d_, _), v_) ->
          ...
          let ctrType = countDec (ctr, d_) in
          max (ctrType, ctrTerm)
      | ctr, I.Root (f_, s_) ->
          let ctrDepth = countSpine (1, s_) in
          ctrDepth + 1 + ctr
      | ctr, I.Redex (u_, s_) ->
    ...
    let rec subsumes ((g_, d_, u_), (g'_, d'_, u'_)) =
      let upi_ = A.raiseType (g_, u_) in
      let upi'_ = A.raiseType (g'_, u'_) in
      let s' = reinstSub (g'_, d'_, I.id) in
      Cs_manager.trail (function () ->
          Unify.unifiable (d_, (upi'_, s'), (upi_, I.id)))
    ...
    let rec callCheckVariant (g_, d_, u_) =
      let upi_ = A.raiseType (concat (g_, d_), u_) in
      let rec lookup = function
        | (g_, d_, u_), [] -> begin
            table :=
              ((ref 1, g_, d_, u_), { solutions = []; lookup = 0 }) :: !table;
            begin
              ...
              begin
                added := true;
                begin if abstractionSet () then begin
        ...
        | (g_, d_, u_), (((k, g'_, d'_, u'_), answ) as h_) :: T -> begin
            if
              variant
                ((upi_, I.id), (A.raiseType (concat (g'_, d'_), u'_), I.id))
            then begin
              k := !k + 1;
              begin
                begin if !Global.chatter >= 5 then
      ...
      let rec lookup = function
        | (g_, d_, u_), [] -> begin
            ...
            begin
              ...
              begin
                added := true;
                begin if exceeds (A.raiseType (g_, u_)) then begin
        ...
        | (g_, d_, u_), ((k, g'_, d'_, u'_), answ) :: T -> begin
            if subsumes ((g_, d_, u_), (g'_, d'_, u'_)) then begin
              begin if !Global.chatter >= 5 then
                print
                  (("call "
                   ^ Print.expToString
    ...
    let rec member = function
      ...
      | (dk_, sk), ((d1_, s1), _) :: s_ -> begin
          if equalSub (sk, s1) && equalCtx (dk_, d1_) then true
          else member ((dk_, sk), s_)
        end
    ...
    let rec answCheckVariant (g_, d_, u_, s, o_) =
      ...
      in
      let rec lookup arg__1 arg__2 arg__3 =
        ...
        | ( (g_, d_, u_, s),
            (((k, g'_, d'_, u'_), { solutions = s_; lookup = i }) as h_) :: T,
            t'_ ) -> begin
            if
              variant
                ((upi_, I.id), (A.raiseType (concat (g'_, d'_), u'_), I.id))
            then
              let dk_, sk = A.abstractAnswSub s in
              begin if member ((dk_, sk), s_) then Repeated
              else begin
                table :=
                  rev t'_
                  @ ( (k, g'_, d'_, u'_),
                      { solutions = ((dk_, sk), o_) :: s_; lookup = i } )
                    :: T;
                begin
                  begin if !Global.chatter >= 5 then begin
                    print "\n Add solution  -- ";
    ...
    let rec memberSubsumes = function
      | (g_, d_, u_, s), (g'_, u'_, []) -> false
      | (g_, d_, u_, s), (g'_, u'_, ((d1_, s1), _) :: a_) ->
          let upi_ = A.raiseType (g_, u_) in
          let upi'_ = A.raiseType (g'_, u'_) in
          let s1' = reinstSub (g'_, d1_, I.id) in
          let vpis_ = (I.EClo (upi'_, s1), s1') in
          let b =
            Cs_manager.trail (function () ->
                Unify.unifiable (d_, (upi_, s), vpis_))
          in
          ...
          else memberSubsumes ((g_, d_, u_, s), (g'_, u'_, a_))
          end
    ...
    let rec answCheckSubsumes (g_, d_, u_, s, o_) =
      ...
      let rec lookup = function
        ...
        | ( (g_, d_, u_, s),
            ((k, g'_, d'_, u'_), { solutions = a_; lookup = i }) :: T,
            t'_ ) -> begin
            if subsumes ((g_, d_, u_), (g'_, d'_, u'_)) then
              let dk_, sk = A.abstractAnswSub s in
              begin if memberSubsumes ((g_, dk_, u_, sk), (g'_, u'_, a_)) then
                Repeated
              else
                let s' = reinstSub (g'_, d'_, I.id) in
                let _ =
                ...
                let _ =
                  ...
                  then begin
                    if !Global.chatter >= 4 then begin
                ...
                in
                let dk'_, sk' = A.abstractAnsw (dk_, s') in
                let rs = reinstSub (g'_, dk'_, I.id) in
                begin
                  begin match !Query_ with
                  | None -> ()
                  | Some (g1_, d1_, u1_, s1, sc1) -> begin
                      begin if !Global.chatter >= 4 then begin
                      ...
                      begin if subsumes ((g1_, d1_, u1_), (g'_, d'_, u'_)) then
                        Cs_manager.trail (function () ->
                            begin if
                              Unify.unifiable
                                ( d1_,
                                  (A.raiseType (g1_, u1_), s1),
                  ...
                  begin
                    table :=
                      rev t'_
                      @ ( (k, g'_, d'_, u'_),
                          { solutions = ((dk'_, sk'), o_) :: a_; lookup = i } )
                        :: T;
                    begin
                      begin if !Global.chatter >= 5 then begin
              ...
              lookup
                ( (g_, d_, u_, s),
                  T,
                  ((k, g'_, d'_, u'_), { solutions = a_; lookup = i }) :: t'_ )
          end
      in
      lookup ((g_, d_, u_, s), !table, [])
    ...
    let rec noAnswers = function
      | [] -> true
      | (((g'_, d'_, u'_), answ) as h_) :: l'_ -> begin
    ...
    let rec updateTable () =
      let rec update arg__4 arg__5 arg__6 =
        ...
        | [], T, flag_ -> (flag_, T)
        | ((k, g_, d_, u_), { solutions = s_; lookup = i }) :: T, t'_, flag_ ->
            let l = length s_ in
            begin if l = i then
              update T
                (((k, g_, d_, u_), { solutions = s_; lookup = List.length s_ })
                :: t'_)
                flag_
            else
              update T
      ...
      in
      let flag_, T = update !table [] false in
