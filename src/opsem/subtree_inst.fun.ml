...
module MemoTableInst(MemoTableInst__0: sig
   ...
   *);;
    ...
    type nonrec normalSubsts =
      ((int * IntSyn.exp_) RBSet.ordSet(* local depth *));;
      ...
      let rec del =
        function 
                 ...
                 | (x, (((y, e_) as h_) :: l_), l'_) -> begin
                     if x = y then (Some ((y, e_), (rev l'_) @ l_)) else
                     del (x, l_, (h_ :: l'_)) end
        in begin
           match del (x, ! l_, [])
           with 
  ...
  *);;
    ...
    type retrieval_ = | Variant of int * IntSyn.exp_ 
                      | NotCompatible ;;
    type compSub_ =
      | SplitSub of
        (ctx * normalSubsts) *
        (ctx * normalSubsts) *
        ((ctx * normalSubsts(* rho2 *)(* rho1 *)(* sigma *))) 
    ...
    open!
      struct
        ...
        let rec expToS (g_, u_) =
          try Print.expToString (g_, u_) with 
                                              | _ -> " <_ >";;
        let rec printSub =
          function 
                   | (g_, I.Shift n)
                       -> print (("I.Shift " ^ (Int.toString n)) ^ "\n")
                   | (g_, I.Dot (I.Idx n, s))
                       -> begin
                   ...
                   | (g_, I.Dot
                      (I.Exp
                       ...
                       -> begin
                            print
                            (("Exp ( EVar " ^ (expToS (g_, x_))) ^ ").");
                            printSub (g_, s)
                            end
                   | (g_, I.Dot (I.Exp ((I.EVar (_, _, _, _) as x_)), s))
                       -> begin
                            print
                            (("Exp ( EVar  " ^ (expToS (g_, x_))) ^ ").");
                            printSub (g_, s)
                            end
                   | (g_, I.Dot (I.Exp (I.AVar _), s))
                       -> begin
                            print "Exp (AVar _ ). ";printSub (g_, s)
                            end
                   | (g_, I.Dot
                      (I.Exp (I.EClo (I.AVar { contents = Some u_}, s')), s))
                       -> begin
                            print
                            (("Exp (AVar " ^ (expToS (g_, (I.EClo (u_, s')))))
                               ^ ").");
                            printSub (g_, s)
                            end
                   | (g_, I.Dot
                      (I.Exp
                       ((I.EClo (I.EVar ({ contents = Some u_}, _, _, _), s')
                          as x_)),
                       s))
                       -> begin
                            print
                            (("Exp (EVarClo " ^
                                (expToS (g_, (I.EClo (u_, s')))))
                               ^ ") ");
                            printSub (g_, s)
                            end
                   | (g_, I.Dot (I.Exp ((I.EClo (u_, s') as x_)), s))
                       -> begin
                            print
                            (("Exp (EClo " ^
                                (expToS (g_, Whnf.normalize (u_, s'))))
                               ^ ") ");
                            printSub (g_, s)
                            end
                   | (g_, I.Dot (I.Exp e_, s))
        ...
        let rec normalizeSub =
          function 
                   ...
                   | I.Dot
                       (I.Exp (I.EClo (I.AVar { contents = Some u_}, s')), s)
                       -> (I.Dot
                           ((I.Exp (Whnf.normalize (u_, s'))),
                            normalizeSub s))
                   | I.Dot
                       (I.Exp
                        (I.EClo
                         (I.EVar ({ contents = Some u_}, _, _, _), s')),
                        s)
                       -> (I.Dot
                           ((I.Exp (Whnf.normalize (u_, s'))),
                            normalizeSub s))
          ...
          function 
                   ...
                   | (I.Decl (g_, I.Dec (_, a_)), s)
                       -> let x_ = I.newEVar (I.null_, a_)
                            in (I.Dot ((I.Exp x_), ctxToEVarSub (g_, s)));;
        let rec lowerEVar' =
          function 
        ...
        let rec ctxToAVarSub =
          function 
                   ...
                   | (g'_, I.Decl (d_, I.Dec (_, a_)), s)
                       -> let (I.EVar (r, _, _, cnstr) as e_) =
                            I.newEVar (I.null_, a_)
                            in (I.Dot
                                ((I.Exp e_), ctxToAVarSub (g'_, d_, s)))
                   | (g'_, I.Decl (d_, I.ADec (_, d)), s)
                       -> let x_ = I.newAVar ()
                            in (I.Dot
                                ((I.Exp ((I.EClo (x_, (I.Shift (- d)))))),
                                 ctxToAVarSub (g'_, d_, s)));;
        let rec assign =
          function 
                   | (d, (I.Dec (n, v_) as dec1_),
                      (I.Root (I.BVar k, s1_) as e1_), u_, asub)
                       -> let (I.EVar (r, _, _, cnstr) as e_) =
                            I.newEVar (I.null_, v_)
                            in let x_ =
                                 lowerEVar1
                                 (e_, (I.EVar (r, I.null_, v_, cnstr)),
                                  Whnf.whnf (v_, I.id))
                                 in let _ = r := ((Some u_))
                                      in S.insert asub (k - d, (I.Exp x_))
                   | (d, (I.ADec (n, d') as dec1_),
                      (I.Root (I.BVar k, s1_) as e1_), u_, asub)
                       -> let (I.AVar r as a_) = I.newAVar ()
                            in let _ = r := ((Some u_))
                                 in let us_ =
                                      Whnf.whnf (u_, (I.Shift (- d')))
                                      in S.insert
                                         asub
                                         (k - d,
                                          (I.Exp
                       ...
                       -> begin
                          ...
                          with 
                               ...
                               | (I.Def c1, I.Def c2) -> begin
                                   ...
                                   let u1'_ =
                                     Whnf.normalize
                                     (Whnf.expandDef (u1_, I.id))
                                     in let u2'_ =
                                          Whnf.normalize
                                          (Whnf.expandDef (u2_, I.id))
                                          in assignExp
                                             (fasub, (ctxTotal, d),
                                              (d1_, u1'_), (d2_, u2'_))
                                   end
                               ...
                               | (_, I.Def c2)
                                   -> let u2'_ =
                                        ...
                                        (Whnf.expandDef (u2_, I.id))
                                        in assignExp
                                           (fasub, (ctxTotal, d), (d1_, u1_),
                                            (d2_, u2'_))
                               | (I.BVar k1, I.BVar k2) -> begin
                                   if k1 <= (r + d) then begin
                                   if k2 <= (r + d) then begin
                                   if k2 = k1 then fasub else
                                   raise ((Assignment "BVar clash")) end else
                                   raise ((Assignment "BVar - EVar clash"))
                                   end else
                                   begin
                                   match member ((k1 - d) + passed, d1_)
                                   with 
                                        | None
                                            -> raise
                                               ((Assignment
                               ...
                               | (I.Skonst c1, I.Skonst c2) -> begin
                                   ...
                                   raise
                                   ((Assignment "Skolem constant clash")) end
                               | _ -> raise ((Assignment "Head mismatch "))
                          end
                   | (fasub, (ctxTotal, d), (d1_, I.Lam (dec1_, u1_)),
                      (d2_, I.Lam (dec2_, u2_)))
                       -> assignExp
                          (fasub, (ctxTotal, d + 1), (d1_, u1_), (d2_, u2_))
                   | (fasub, (ctxTotal, d),
                      (d1_, I.Pi (((I.Dec (_, v1_) as dec1_), _), u1_)),
                      (d2_, I.Pi (((I.Dec (_, v2_) as dec2_), _), u2_)))
                       -> let fasub' =
                            assignExp
                            (fasub, (ctxTotal, d), (d1_, v1_), (d2_, v2_))
                            in assignExp
                               (fasub', (ctxTotal, d + 1), (d1_, u1_),
                                (d2_, u2_))
                   | (fasub, (ctxTotal, d),
                      (d1_, I.EClo (u_, (I.Shift 0 as s'))), (d2_, u2_))
                       -> assignExp
                          (fasub, (ctxTotal, d), (d1_, u_), (d2_, u2_))
                   | (fasub, (ctxTotal, d), (d1_, u1_),
                      (d2_, I.EClo (u_, (I.Shift 0 as s))))
                       -> assignExp
                          (fasub, (ctxTotal, d), (d1_, u1_), (d2_, u_))
        and assignSpine =
          function 
                   | (fasub, (ctxTotal, d), (d1_, nil_), (d2_, nil_))
                       -> fasub
                   | (fasub, (ctxTotal, d), (d1_, I.App (u1_, s1_)),
                      (d2_, I.App (u2_, s2_)))
                       -> let fasub' =
                            assignExp
                            (fasub, (ctxTotal, d), (d1_, u1_), (d2_, u2_))
                            in assignSpine
                               (fasub', (ctxTotal, d), (d1_, s1_),
                                (d2_, s2_));;
        let rec assignCtx =
          function 
                   | (fasub, ctxTotal, (d1_, null_), (d2_, null_)) -> fasub
                   | (fasub, ((r, passed) as ctxTotal),
                      (d1_, I.Decl (g1_, I.Dec (_, v1_))),
                      (d2_, I.Decl (g2_, I.Dec (_, v2_))))
                       -> let fasub' =
                            assignExp
                            (fasub, ((r - 1, passed + 1), 0), (d1_, v1_),
                             (d2_, v2_))
                            in assignCtx
                               (fasub', (r - 1, passed + 1), (d1_, g1_),
                                (d2_, g2_));;
        let nctr = ref 1;;
        let rec newNVar () =
          begin
        ...
        let rec equalDec =
          function 
                   | (I.Dec (_, u_), I.Dec (_, u'_))
                       -> Conv.conv ((u_, I.id), (u'_, I.id))
                   | (I.ADec (_, d), I.ADec (_, d')) -> d = d'
                   | (_, _) -> false;;
        let rec equalCtx =
          function 
                   | (null_, s, null_, s') -> true
                   | (I.Decl (g_, (I.Dec (_, a_) as d_)), s, I.Decl
                      (g'_, (I.Dec (_, a'_) as d'_)), s')
                       -> (Conv.convDec ((d_, s), (d'_, s'))) &&
                            (equalCtx (g_, I.dot1 s, g'_, I.dot1 s'))
                   | (_, s, _, s') -> false;;
        let rec equalEqn =
          function 
                   | (trivial_, trivial_) -> true
                   | (T.Unify (g_, x_, n_, eqn), T.Unify
                      (g'_, x'_, n'_, eqn'))
        ...
        let rec equalEqn' =
          function 
                   | (d, (d_, trivial_), (d'_, trivial_), asub) -> true
                   | (d,
                      (d_, T.Unify
                       (g_, (I.Root (I.BVar k, s_) as x_), n_, eqn)),
                      (d'_, T.Unify (g'_, x'_, n'_, eqn')), asub) -> begin
                       if
                       (equalCtx (g_, I.id, g'_, I.id)) &&
                         ((Conv.conv ((x_, I.id), (x'_, I.id))) &&
                            (Conv.conv ((n_, I.id), (n'_, I.id))))
                       then
                       let d' = d + (I.ctxLength g'_)
                         in begin
                              begin
                              if (k - d') > 0 then
                              begin
                              match member (k - d', d'_)
                              with 
                                   | None -> ()
                                   | Some (x, Dec)
                                       -> begin
                                          match RBSet.lookup asub (k - d')
                                          with 
                                               | None
                                                   -> begin
                                                        delete (x, d'_);
                              ...
                              begin
                                print
                                "Impossible -- Found BVar instead of EVar\n";
                                raise
                                ((Error
                                  "Impossibe -- Found BVar instead of EVar "))
                                
                                end
                              end;equalEqn' (d, (d_, eqn), (d'_, eqn'), asub)
                              end
                       else false end
                   | (d, _, _, asub) -> false;;
        let rec equalSub =
                   ...
                   | (I.Decl (dk_, I.Dec (_, a_)), I.Decl
                      (d1_, I.Dec (_, a1_)))
                       -> (Conv.conv ((a_, I.id), (a1_, I.id))) &&
                            (equalCtx' (dk_, d1_))
                   | (I.Decl (dk_, I.ADec (_, d')), I.Decl
                      (d1_, I.ADec (_, d)))
                       -> (d = d') && (equalCtx' (dk_, d1_))
                   | (_, _) -> false;;
        let rec instanceCtx (asub, (d1_, g1_), (d2_, g2_)) =
          let d1 = I.ctxLength g1_
            in let d2 = I.ctxLength g2_ in begin
                 if d1 = d2 then
                 try let fasub =
                       assignCtx
                       (function 
                                 | asub -> (),
                        (d1, 0), (d1_, g1_), (d2_, g2_))
                       in begin
                            fasub asub;true
                            end
                 with 
                      | Assignment msg -> false
                 else false end;;
        let rec collectEVar (d_, nsub) =
          let d'_ = emptyCtx ()
            in let rec collectExp =
                 function 
                          | (d, d'_, d_, I.Lam (_, u_))
                              -> collectExp (d + 1, d'_, d_, u_)
                          | (d, d'_, d_, I.Root (I.Const c, s_))
                              ...
                              -> begin
                                 ...
                                 with 
                                      ...
                                      | Some (x, Dec)
                                          -> begin
                                               delete (x - d, d_);
                                               insertList ((x - d, Dec), d'_)
                                               end
                                 end
                          | (d, d'_, d_, (I.Root (I.Def k, s_) as u_))
                              -> let u'_ =
                                   Whnf.normalize (Whnf.expandDef (u_, I.id))
                                   in collectExp (d, d'_, d_, u'_)
               and collectSpine =
                 function 
                          | (d, d'_, d_, nil_) -> ()
                          | (d, d'_, d_, I.App (u_, s_))
                              -> begin
                 ...
                 in begin
                      ...
                      (function 
                                | (nv, (du, u_))
                                    -> collectExp (0, d'_, d_, u_));
                      (d'_, d_)
                      end;;
        let rec convAssSub'
          (g_, idx_k, d_, asub, d, ((evars, avars) as evarsl)) =
          begin
          match RBSet.lookup asub d
          with 
               | None
                   -> begin
                      match member (d, d_)
                      with 
                           ...
                           | Some (x, IntSyn.Dec (n, v_))
                               -> let s =
                                    convAssSub'
                                    (g_, idx_k + 1, d_, asub, d + 1, evarsl)
                                    in let (I.EVar (r, _, _, cnstr) as e_) =
                                         I.newEVar (I.null_, v_)
                                         in (I.Dot
                                             ((I.Exp
                                               ((I.EClo
                                                 (e_,
                                                  (I.Shift (evars + avars)))))),
                                              s))
                           | Some (x, IntSyn.ADec (n, v_))
                               -> begin
                                    print
                                    "convAssSub' -- Found an uninstantiated AVAR\n";
                                    raise
                                    ((Error
                                      "Unassigned AVar -- should never happen\n"))
                                    
                                    end
                      end
               | Some ((I.Exp e_ as f_))
                   -> let e'_ = Whnf.normalize (e_, I.id)
                        in (I.Dot
                            ((I.Exp e'_),
                             convAssSub'
                             (g_, idx_k + 1, d_, asub, d + 1, evarsl)))
        ...
        let rec instance ((d_t_, (dt, T)), (d_u_, (du, u_)), rho_u, ac) =
          let rec instRoot =
            function 
                     | (depth, (I.Root ((I.Const k as h1_), s1_) as t_),
                        (I.Root (I.Const k', s2_) as u_), ac) -> begin
                         if k = k' then instSpine (depth, s1_, s2_, ac) else
                         raise ((Instance "Constant mismatch\n")) end
                     | (depth, (I.Root ((I.Def k as h1_), s1_) as t_),
                        (I.Root (I.Def k', s2_) as u_), ac) -> begin
                         if k = k' then instSpine (depth, s1_, s2_, ac) else
                         let t'_ = Whnf.normalize (Whnf.expandDef (T, I.id))
                           in let u'_ =
                                Whnf.normalize (Whnf.expandDef (u_, I.id))
                                in instExp (depth, t'_, u'_, ac)
                         end
                     | (depth, (I.Root ((I.Def k as h1_), s1_) as t_),
                        (I.Root (h2_, s2_) as u_), ac)
                         -> let t'_ =
                              Whnf.normalize (Whnf.expandDef (T, I.id))
                              in instExp (depth, t'_, u_, ac)
                     | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
                        (I.Root (I.BVar k', s2_) as u_), ac) -> begin
                         if (k > d) && (k' > d) then
                         let k1 = k - d
                           in let k2 = k' - d
                                in begin
                                   match (member (k1, d_t_),
                                          member (k2, d_u_))
                                   with 
                                        | (None, None) -> begin
                                            if k1 = k2 then
                                            instSpine (d, s1_, s2_, ac) else
                                            raise
                                            ((Instance
                                        ...
                                        | (Some (x, dec1_), Some (x', dec2_))
                                            ...
                                            if
                                            (k1 = k2) &&
                                              (equalDec (dec1_, dec2_))
                                            then
                                            let ac' =
                                              instSpine (d, s1_, s2_, ac)
                                              in let ac'' =
                                                   function 
                                                            | asub
                                                                -> begin
                     ...
                     | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
                        (I.Root (I.Const k', s2_) as u_), ac)
                         -> begin
                            match isExists (d, (I.BVar k), d_t_)
                            with 
                     ...
                     | (depth, (I.Root (h1_, s1_) as t_),
                        (I.Root (I.Def k', s2_) as u_), ac)
                         -> let u'_ =
                              Whnf.normalize (Whnf.expandDef (u_, I.id))
                              in instExp (depth, T, u'_, ac)
                     | (d, (I.Root (h1_, s1_) as t_),
                        (I.Root (h2_, s2_) as u_), ac)
                         -> raise ((Instance "Other Cases impossible\n"))
          and instExp =
            function 
                     | (d, (I.NVar n as t_), (I.Root (h_, s_) as u_), ac)
                         -> begin
                              S.insert rho_u (n, (d, u_));ac
                              end
                     | (d, (I.Root (h1_, s1_) as t_),
                        (I.Root (h2_, s2_) as u_), ac)
                         -> instRoot
                            (d, (I.Root (h1_, s1_)), (I.Root (h2_, s2_)), ac)
                     | (d, I.Lam ((I.Dec (_, a1_) as d1_), t1_), I.Lam
                        ((I.Dec (_, a2_) as d2_), u2_), ac)
          ...
          and instSpine =
            function 
                     | (d, nil_, nil_, ac) -> ac
                     | (d, I.App (T, s1_), I.App (u_, s2_), ac)
                         -> let ac' = instExp (d, T, u_, ac)
                              in let ac'' = instSpine (d, s1_, s2_, ac')
                                   in ac''
                     | (d, nil_, I.App (_, _), ac)
                         -> begin
                              print
        ...
        let rec compHeads =
          function 
                   ...
                   | ((d_1_, I.BVar k), (d_2_, I.BVar k'))
                       -> begin
                          match isExists (0, (I.BVar k), d_1_)
                          with 
                               | None -> k = k'
                               | Some (x, Dec) -> true
                          end
        ...
        let rec compatible'
          ((d_t_, (dt, T)), (d_u_, (du, u_)), ds_, rho_t, rho_u) =
          let rec genNVar ((rho_t, T), (rho_u, u_)) =
            begin
              S.insert rho_t ((! nctr) + 1, T);
              begin
                S.insert rho_u ((! nctr) + 1, u_);newNVar ()
                end
              
              end
            in let rec genRoot =
                 function 
                          | (d, (I.Root ((I.Const k as h1_), s1_) as t_),
                             (I.Root (I.Const k', s2_) as u_)) -> begin
                              if k = k' then
                              let s'_ = genSpine (d, s1_, s2_)
                                in (I.Root (h1_, s'_))
                              else
                              genNVar ((rho_t, (d, T)), (rho_u, (d, u_))) end
                          | (d, (I.Root ((I.Def k as h1_), s1_) as t_),
                          ...
                          | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
                             (I.Root (I.BVar k', s2_) as u_)) -> begin
                              ...
                              if k = k' then
                              try let s'_ = genSpine (d, s1_, s2_)
                                    in (I.Root (h1_, s'_))
                              with 
                                   | DifferentSpines
                                       -> genNVar
                                          ((rho_t, (d, T)), (rho_u, (d, u_)))
                              else
                              genNVar ((rho_t, (d, T)), (rho_u, (d, u_))) end
                              end
                          | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
                             (I.Root (I.Const k', s2_) as u_))
                              -> genNVar ((rho_t, (d, T)), (rho_u, (d, u_)))
                          | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
               ...
               and genExp =
                 function 
                          | (d, (I.NVar n as t_), (I.Root (h_, s_) as u_))
                              -> begin
                                   S.insert rho_u (n, (d, u_));T
                                   end
                          | (d, (I.Root (h1_, s1_) as t_),
                             (I.Root (h2_, s2_) as u_))
                              -> genRoot
                                 (d, (I.Root (h1_, s1_)),
                                  (I.Root (h2_, s2_)))
                          | (d, I.Lam ((I.Dec (_, a1_) as d1_), t1_), I.Lam
                             ((I.Dec (_, a2_) as d2_), u2_))
                              -> let e_ = genExp (d + 1, t1_, u2_)
                                   in (I.Lam (d1_, e_))
                          | (d, T, u_)
                              -> begin
               ...
               and genSpine =
                 function 
                          | (d, nil_, nil_) -> I.nil_
                          | (d, I.App (T, s1_), I.App (u_, s2_))
                              -> let e_ = genExp (d, T, u_)
                                   in let s'_ = genSpine (d, s1_, s2_)
                                        in (I.App (e_, s'_))
                          | (d, nil_, I.App (_, _)) -> raise DifferentSpines
                          | (d, I.App (_, _), nil_) -> raise DifferentSpines
                          | (d, I.SClo (_, _), _) -> raise DifferentSpines
                          | (d, _, I.SClo (_, _)) -> raise DifferentSpines
                 in (Variant (dt, genExp (dt, T, u_)));;
        let rec compatible =
          function 
                   | ((d_t_, ((d1, I.Root (h1_, s1_)) as t_)),
                      (d_u_, ((d2, I.Root (h2_, s2_)) as u_)), ds_, rho_t,
                      rho_u) -> begin
                       if compHeads ((d_t_, h1_), (d_u_, h2_)) then
                       compatible' ((d_t_, T), (d_u_, u_), ds_, rho_t, rho_u)
                       else NotCompatible end
                   | ((d_t_, T), (d_u_, u_), ds_, rho_t, rho_u)
                       -> compatible'
                          ((d_t_, T), (d_u_, u_), ds_, rho_t, rho_u);;
        let rec compatibleCtx =
          function 
                   | (asub, (dsq_, gsq_, eqn_sq), []) -> None
                   | (asub, (dsq_, gsq_, eqn_sq),
                      ((_, delta'_, g'_, eqn', answRef', _, status')
                       :: gRlist_)) -> begin
                       if instanceCtx (asub, (dsq_, gsq_), (delta'_, g'_))
                       then (Some ((delta'_, g'_, eqn'), answRef', status'))
                       else
                       compatibleCtx (asub, (dsq_, gsq_, eqn_sq), gRlist_)
                       end;;
        let rec instanceSub ((d_t_, nsub_t), (dsq_, squery), asub) =
          let rho_u = nid ()
            in let d_r2_ = copy dsq_
                 in let ac = ref (function 
                                           | (asub : exSubsts) -> ())
                      in try begin
                               S.forall
                               squery
                         ...
                         with 
                              | Instance msg -> NoCompatibleSub;;
        let rec instChild =
          function 
                   | ((Leaf ((d_t_, nsub_t), gList_) as n_), (d_sq_, sq),
                      asub)
                       -> instanceSub ((d_t_, nsub_t), (d_sq_, sq), asub)
                   | ((Node ((d_t_, nsub_t), children'_) as n_), (d_sq_, sq),
                      asub)
                       -> instanceSub ((d_t_, nsub_t), (d_sq_, sq), asub);;
            ...
            function 
                     ...
                     | (g_r_, (x :: l_), (dsq_, sub_u), asub, iList_)
                         -> let asub' = S.copy asub
                              in begin
                                 match instChild (! x, (dsq_, sub_u), asub)
                                 with 
                                      | NoCompatibleSub
                                          -> findAllCands
                                             (g_r_, l_, (dsq_, sub_u), asub',
                                              iList_)
                                      | InstanceSub (asub, drho2_)
                                          -> findAllCands
                                             (g_r_, l_, (dsq_, sub_u), asub',
                                              ((x, drho2_, asub) :: iList_))
                                 end
            in findAllCands (g_r_, children, ds_, asub, []);;
        let rec solveEqn =
          function 
                   | ((trivial_, s), g_) -> true
        ...
        let rec retrieveInst (nref_, (dq_, sq), asub, gr_) =
          let rec retrieve' =
            function 
                     | ((Leaf ((d_, s), gRlistRef_) as n_), (dq_, sq),
                        asubst,
                        ((((dEVars_, dAVars_) as dAEVars_), g_r_, eqn, stage,
                          status) as gr'_))
                         -> let (dsq_, d_g_) = collectEVar (dq_, sq)
                              in begin
                                 match compatibleCtx
                                       (asubst, (d_g_, g_r_, eqn),
                                        ! gRlistRef_)
                                 with 
                                      | None
                                          -> raise
                                             ((Instance
                                               "Compatible path -- different ctx\n"))
                                      | Some
                                          ((d'_, g'_, eqn'), answRef',
                                           status')
                                          -> let dAEVars_ =
                                               compose (dEVars_, dAVars_)
                                               in let esub =
                                                    ctxToAVarSub
                                                    (g'_, dAEVars_,
                                                     (I.Shift 0))
                     ...
                     | ((Node ((d_, sub), children) as n_), (dq_, sq), asub,
                        ((dAEVars_, g_r_, eqn, stage, status) as gr_))
                         -> let instCand_ =
                              findAllInst (g_r_, children, (dq_, sq), asub)
                              in let rec checkCandidates =
                                   function 
                                            | []
                                                -> raise
                                                   ((Instance
                                                     "No compatible child\n"))
                                            | ((childRef_, drho2_, asub)
                                               :: iCands_)
                                                -> try retrieve'
                                                       (! childRef_, drho2_,
                                                        asub, gr_)
                                                   with 
                                                        | Instance msg
                                                            -> checkCandidates
                                                               iCands_
                                   in checkCandidates instCand_
            in (function 
                         | () -> (),
                retrieve' (! nref_, (dq_, sq), asub, gr_));;
        let rec compatibleSub ((d_t_, nsub_t), (dsq_, squery)) =
          let (sigma, rho_t, rho_u) = (nid (), nid (), nid ())
            in let dsigma_ = emptyCtx ()
                 in let d_r1_ = copy d_t_
                      in let d_r2_ = copy dsq_
                           in let choose =
                                ref (function 
                                              | (match_ : bool) -> ())
        ...
        let rec mkNode =
          function 
                   | (Node (_, children_), ((ds_, sigma) as dsigma_),
                      ((d1_, rho1) as drho1_),
                      (((evarl, l), dp, eqn, answRef, stage, status) as gr_),
                      ((d2_, rho2) as drho2_))
                       -> let (d_rho2_, d_g2_) = collectEVar (d2_, rho2)
                            in let gr'_ =
                                 ((evarl, l), d_g2_, dp, eqn, answRef, stage,
                                  status)
                                 in let (sizeSigma, sizeRho1, sizeRho2) =
                                      (S.size sigma, S.size rho1,
                                       S.size rho2)
                                      in (Node
                                          (dsigma_,
                                           [ref
                                            ((Leaf
                                              ((d_rho2_, rho2), ref [gr'_])));
                                            ref ((Node (drho1_, children_)))]))
                   | (Leaf (c, gRlist_), ((ds_, sigma) as dsigma_),
                      ((d1_, rho1) as drho1_),
                       ...
                       -> let (d_rho2_, d_g2_) = collectEVar (d2_, rho2)
                            in let gr2'_ =
                                 ...
                                 in (Node
                                     (dsigma_,
                                      [ref
                                       ((Leaf ((d_rho2_, rho2), ref [gr2'_])));
                                       ref ((Leaf (drho1_, gRlist_)))]));;
        let rec compChild =
          function 
                   | ((Leaf ((d_t_, nsub_t), gList_) as n_), (d_e_, nsub_e))
                       -> compatibleSub ((d_t_, nsub_t), (d_e_, nsub_e))
                   | ((Node ((d_t_, nsub_t), children'_) as n_),
                      (d_e_, nsub_e))
                       -> compatibleSub ((d_t_, nsub_t), (d_e_, nsub_e));;
        let rec findAllCandidates (g_r_, children, ds_) =
          let rec findAllCands =
            function 
                     | (g_r_, [], (dsq_, sub_u), vList_, sList_)
                         -> (vList_, sList_)
                     | (g_r_, (x :: l_), (dsq_, sub_u), vList_, sList_)
                         -> begin
                            match compChild (! x, (dsq_, sub_u))
                            with 
                                 | NoCompatibleSub
                                     -> findAllCands
                                        (g_r_, l_, (dsq_, sub_u), vList_,
                                         sList_)
                                 | SplitSub (dsigma_, drho1_, drho2_)
                                     -> findAllCands
                                        (g_r_, l_, (dsq_, sub_u), vList_,
                                         ((x, (dsigma_, drho1_, drho2_))
                                          :: sList_))
                                 | VariantSub (((d_r2_, rho2) as drho2_))
                                     -> findAllCands
                                        (g_r_, l_, (dsq_, sub_u),
                                         ((x, drho2_, I.id) :: vList_),
                                         sList_)
                            end
            in findAllCands (g_r_, children, ds_, [], []);;
        let rec divergingCtx (stage, g_, gRlistRef_) =
          let l = (I.ctxLength g_) + 3
            in List.exists
               (function 
        ...
        let rec eqTerm =
          function 
                   | (I.Root (h2_, s2_), ((I.Root (h_, s_) as t), rho1))
                       -> begin
                       if eqHeads (h2_, h_) then eqSpine (s2_, (s_, rho1))
                       else false end
                   | (t2_, (I.NVar n, rho1))
                       -> begin
                          match S.lookup rho1 n
                          with 
                               | None -> false
                               | Some (dt1, t1_)
                                   -> eqTerm (t2_, (t1_, nid ()))
                          end
                   | (I.Lam (d2_, t2_), (I.Lam (d_, T), rho1))
                       -> eqTerm (t2_, (T, rho1))
                   | (_, (_, _)) -> false
        and eqSpine =
          function 
                   | (nil_, (nil_, rho1)) -> true
                   | (I.App (t2_, s2_), (I.App (T, s_), rho1))
                       -> (eqTerm (t2_, (T, rho1))) &&
                            (eqSpine (s2_, (s_, rho1)));;
          ...
          (function 
                    | (n2, (dt2, t2))
                        -> S.exists
                           sigma
                           (function 
                                     | (_, (d, t)) -> eqTerm (t2, (t, rho1))));;
        let rec variantCtx =
          function 
                   | ((g_, eqn), []) -> None
                   | ((g_, eqn),
                      ((l', d_g_, g'_, eqn', answRef', _, status')
                       :: gRlist_)) -> begin
                       if (equalCtx' (g_, g'_)) && (equalEqn (eqn, eqn'))
                       then (Some (l', answRef', status')) else
                       variantCtx ((g_, eqn), gRlist_) end;;
        let rec insert (nref_, (dsq_, sq), gr_) =
          let rec insert' =
            function 
                     | ((Leaf (_, gRlistRef_) as n_), (dsq_, sq),
                        ((l, g_r_, eqn, answRef, stage, status) as gr_))
                         -> begin
                            match variantCtx ((g_r_, eqn), ! gRlistRef_)
                            with 
                                 | None
                                     -> let (d_nsub_, d_g_) =
                                          collectEVar (dsq_, sq)
                                          in let gr'_ =
                                               (l, d_g_, g_r_, eqn, answRef,
                                                stage, status)
                                               in (function 
                                                            | ()
                                                                -> begin
                                 ...
                                 | Some (_, answRef', status')
                                     -> (function 
                                                  | () -> (),
                                         (T.RepeatedEntry
                                          ((I.id, I.id), answRef', status')))
                            end
                     | ((Node ((d_, sub), children) as n_), (dsq_, sq),
                        ((l, g_r_, eqn, answRef, stage, status) as gr_))
                         -> let (variantCand_, splitCand_) =
                              findAllCandidates (g_r_, children, (dsq_, sq))
                              in let (d_nsub_, d_g_) = collectEVar (dsq_, sq)
                                   in let gr'_ =
                                        (l, d_g_, g_r_, eqn, answRef, stage,
                                         status)
                                        in let rec checkCandidates =
                                             function 
                                                      | ([], [])
                                             ...
                                             in checkCandidates
                                                (variantCand_, splitCand_)
            in insert' (! nref_, (dsq_, sq), gr_);;
        let rec answCheckVariant (s', answRef, o_) =
          let rec member =
            function 
                     | ((d_, sk), []) -> false
                     | ((d_, sk), (((d1_, s1), _) :: s_)) -> begin
                         if (equalSub (sk, s1)) && (equalCtx' (d_, d1_)) then
                         true else member ((d_, sk), s_) end
            in let (dEVars_, sk) = A.abstractAnswSub s' in begin
                 if member ((dEVars_, sk), T.solutions answRef) then
                 T.repeated else
                 begin
                   T.addSolution (((dEVars_, sk), o_), answRef);T.new_
                   end
                 end;;
        let rec reset () =
          begin
        ...
        let rec callCheck (a, dAVars_, dEVars_, g_, u_, eqn, status) =
          let (n, tree_) = Array.sub (indexArray, a)
            in let sq = S.new_ ()
                 in let dAEVars_ = compose (dEVars_, dAVars_)
                      in let dq_ = emptyCtx ()
                           in let n = I.ctxLength g_
                                in let _ =
                                     makeCtx (n + 1, dAEVars_, (dq_ : ctx))
                                     in let l = I.ctxLength dAEVars_
                                          in let _ = S.insert sq (1, (0, u_))
                                               in let gr_ =
                                                    ((l, n + 1), g_, eqn,
                                                     emptyAnswer (),
                                                     ! TableParam.stageCtr,
        ...
        let rec insertIntoTree
          (a, dAVars_, dEVars_, g_, u_, eqn, answRef, status) =
          let (n, tree_) = Array.sub (indexArray, a)
            in let sq = S.new_ ()
                 in let dAEVars_ = compose (dEVars_, dAVars_)
                      in let dq_ = emptyCtx ()
        ...
        let rec answCheck (s', answRef, o_) =
          answCheckVariant (s', answRef, o_);;
        let rec updateTable () =
          let rec update arg__1 arg__2 =
            ...
            with 
                 ...
                 | ((answRef :: aList_), flag_)
                     -> let l = length (T.solutions answRef) in begin
                          if l = (T.lookup answRef) then update aList_ flag_
                          else
                          begin
            ...
            in let flag_ = update (! answList) false
                 in let r = flag_ || (! added) in begin
                                                    added := false;r
                                                    end;;
        end;;
    ...
    let callCheck =
      function 
               | (dAVars_, dEVars_, g_, u_, eqn, status)
                   -> callCheck
                      (cidFromHead (I.targetHead u_), dAVars_, dEVars_, g_,
                       u_, eqn, status);;
    let insertIntoTree =
      function 
      ...
      let rec instanceCtx' =
        function 
                 ...
                 | ((g_, v_), I.Decl (g'_, (I.Dec (_, v'_) as d'_)), n)
                     ...
                     then (Some d'_) else instanceCtx' ((g_, v_), g'_, n + 1)
                     end
        in instanceCtx' ((g_, v_), g'_, 1);;
    end;;
