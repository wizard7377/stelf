  ...
  open! struct
    ...
    let rec ctxToEVarSub = function
      ...
      | IntSyn.Decl (g_, IntSyn.Dec (_, a_)), s ->
          ...
          let x_ = IntSyn.newEVar (IntSyn.null_, I.EClo (a_, s')) in
    ...
    and collectSpine = function
      | gss_, gl_, (nil_, _), k_, dupVars_, flag, d -> (k_, dupVars_)
    ...
    and collectEVarFapStr
      ...
      | None ->
          ...
          let k'_, dupVars'_ =
            collectExp
              ((I.null_, I.id), I.null_, (v'_, I.id), k_, dupVars_, false, d)
          in
          collectSub
            ( gss_,
              gl_,
    ...
    let rec collectCtx = function
      | gss_, C.DProg (null_, null_), (k_, dupVars_), d -> (k_, dupVars_)
      | ( gss_,
          ...
          if flag then begin
            ...
            | Body ->
                ...
                ( posEA',
                  ...
                  I.Root (Bv, I.nil_),
                  TableParam.Unify
                    (gl_, I.Root (bv'_, s_), I.Root (bv1_, I.nil_), eqn1) )
            | TypeLabel ->
          ...
          let posEA', vars'_, s_, eqn1 =
            abstractSub
              ( flag,
                ...
                eqn )
          in
          (posEA', vars'_, I.Root (Bv, s_), eqn1)
      end
    ...
    and abstractSpine = function
      | flag, gs_, posEA, vars_, gl_, total, depth, (nil_, _), eqn ->
          (posEA, vars_, I.nil_, eqn)
    ...
    let rec abstractCtx' = function
      | gs_, epos, vars_, total, depth, C.DProg (null_, null_), g'_, eqn ->
          (epos, vars_, g'_, eqn)
      | ( gs_,
          epos,
          vars_,
    ...
    let rec abstractCtx (gs_, epos, vars_, total, depth, dProg) =
      abstractCtx'
        (gs_, epos, vars_, total, depth, dProg, I.null_, TableParam.trivial_)
    ...
    let rec evarsToSub = function
      ...
      | ( I.Decl
          ...
          let x_ =
            lowerEVar1
              (e_, I.EVar (i_, I.null_, v'_, cnstr), Whnf.whnf (v'_, I.id))
          in
    ...
    let rec abstractEVarCtx ((C.DProg (g_, dPool) as dp), p, s) =
      ...
      in
      let k_, dupVars_ = collectCtx ((gs_, ss), dp, (I.null_, I.null_), d) in
      let k'_, dupVars'_ =
        collectExp ((gs_, ss), I.null_, (p, s), k_, dupVars_, true, d)
      in
      let epos = I.ctxLength k'_ in
      let apos = I.ctxLength dupVars'_ in
      let total = epos + apos in
      let epos', vars'_, g'_, eqn =
        abstractCtx ((gs_, ss), epos, I.null_, total, d, dp)
      in
      let posEA'', vars''_, u'_, eqn' =
        abstractExp
          ( true,
      ...
      let dEVars_ = makeEVarCtx ((gs_, ss), vars''_, I.null_, vars''_, 0) in
      let s' = avarsToSub (dupVars'_, I.id) in
      let s'' = evarsToSub (vars''_, s') in
      let g''_ = reverseCtx (g'_, I.null_) in
      begin if !TableParam.strengthen then
...
*)
  ...
  let abstractAnswSub = function
    | s ->
        let k_, _ =
          collectSub ((I.null_, I.id), I.null_, s, I.null_, I.null_, false, 0)
        in
        let epos = I.ctxLength k_ in
        let _, vars_, s' (*0 *) =
          abstractSub' (false, (I.null_, I.id), epos, I.null_, epos, s)
          (* total *)
        in
        let dEVars_ = makeEVarCtx ((I.null_, I.id), vars_, I.null_, vars_, 0) in
