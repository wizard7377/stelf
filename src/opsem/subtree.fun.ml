...
module MemoTable (MemoTable__0 : sig
  ...
  let rec ctxToEVarSub = function
    ...
    | IntSyn.Decl (g_, IntSyn.Dec (_, a_)), s ->
        ...
        let x_ = IntSyn.newEVar (IntSyn.null_, IntSyn.EClo (a_, s')) in
  ...
  open! struct
            ...
            if k > d && k' > d then
              ...
              | None, None -> begin
                  if k1 = k2 then
                    try
                      ...
                      I.Root (h1_, s'_)
                    with differentSpine_ -> genNVar ((rho_t, T), (rho_u, u_))
                  else genNVar ((rho_t, T), (rho_u, u_))
                end
              ...
              if k = k' then
                try
                  let s'_ = genSpine (d, s1_, s2_) in
                  I.Root (h1_, s'_)
                with DifferentSpines -> genNVar ((rho_t, T), (rho_u, u_))
              else genNVar ((rho_t, T), (rho_u, u_))
            end
          end
        | ( d,
    ...
    let rec callCheck (a, dAVars_, dEVars_, g_, u_, eqn, status) =
      ...
      | sf, T.DivergingEntry answRef -> begin
          sf ();
          begin
            added := true;
            begin
