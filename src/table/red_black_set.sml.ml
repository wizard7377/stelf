  ...
  *);;
    open!
      struct
        ...
        let rec last (Set (n, dict)) =
          (n, valOf (lookup ((Set (n, dict))) n));;
            ...
            in let rec bbZip =
                 function 
                          ...
                          | (LeftBlack (x, Red (y, c, d), z), a)
                              -> bbZip
                                 (leftRed_ (x, c, leftBlack_ (y, d, z)), a)
                          | (LeftRed (x, Red (y, c, d), z), a)
                              -> bbZip
                                 (leftRed_ (x, c, leftBlack_ (y, d, z)), a)
                          | (LeftBlack (x, Black (w, Red (y, c, d), e), z),
                             a)
                              -> bbZip
                                 (leftBlack_
                          ...
                          | (LeftRed (x, Black (w, Red (y, c, d), e), z), a)
                              -> bbZip
                                 (leftRed_
                                  (x, (Black (y, c, (Red (w, d, e)))), z), a)
                          | (LeftBlack (x, Black (y, c, Red (w, d, e)), z),
                             a)
                              -> (false,
                                  zip
                                  (z,
                          ...
                          | (RightBlack (Red (y, c, d), x, z), b)
                              -> bbZip
                                 (rightRed_ (d, x, rightBlack_ (c, y, z)), b)
                          | (RightRed (Red (y, c, d), x, z), b)
                              -> bbZip
                                 (rightRed_ (d, x, rightBlack_ (c, y, z)), b)
                          | (RightBlack (Black (y, Red (w, c, d), e), x, z),
                             b)
                              -> bbZip
                                 (rightBlack_
                          ...
                          | (RightRed (Black (y, Red (w, c, d), e), x, z), b)
                              -> bbZip
                                 (rightRed_
                                  ((Black (w, c, (Red (y, d, e)))), x, z), b)
                          | (RightBlack (Black (y, c, Red (w, d, e)), x, z),
                             b)
                              -> (false,
                                  zip
                                  (z,
                 ...
                 in let rec delMin =
                      function 
                               ...
                               | (Black (y, Empty, b), z)
                                   -> (y, bbZip (z, b))
                               | (Red (y, a, b), z)
                                   -> delMin (a, leftRed_ (y, b, z))
                               | (Black (y, a, b), z)
                                   -> delMin (a, leftBlack_ (y, b, z))
        ...
        let rec update f (Set (n, dict)) =
          ...
          and upd' (((k, datum) as entry1), left, right) =
            let left' = upd left
              in let datum' = f datum
                   in let right' = upd right in ((k, datum'), left', right')
            in (Set (n, upd dict));;
        ...
        let rec existsOpt (Set (n, dict)) f =
          let rec ap =
            function 
                     ...
                     | Red tree -> ap' tree
                     | Black tree -> ap' tree
          and ap' (((k, d) as entry), left, right) = begin
            if f d then begin
                          print "SUCCESS\n";(Some k)
                          end
            else
        ...
        let rec exists (Set (n, dict)) f =
          let rec ap =
            function 
                     ...
                     | Red tree -> ap' tree
                     | Black tree -> ap' tree
          and ap' (entry, left, right) = begin
            if f entry then true else begin
            if ap left then true else ap right end end in ap dict;;
          ...
          let rec incr =
            function 
                     ...
                     | (a1, t1, Two (a2, t2, a3, t3, r))
                         -> (One
                             (a1, t1, incr (a2, (Black (a3, t3, t2)), r)))
            in incr (a, Empty, l);;
        let rec linkAll t =
          let rec link =
            function 
          ...
          let rec ins =
            function 
                     ...
                     | ((Black (x, _, _), r), n, result)
                         -> ins (next r, n + 1, addItem (x, result))
            in let rec union' (t1, t2, n, result) =
                 begin
                 ...
                 with 
                      ...
                      | ((tree1, r1), (tree2, r2))
                          -> let ((x, d1) as e1) = getEntry tree1
                               in let ((y, d2) as e2) = getEntry tree2
                                    in begin
                                       match compare (x, y)
                                       with 
                                            | LESS
                             ...
                             -> begin
                                ...
                                with 
                                     ...
                                     | _
                                         -> let (n, result) =
                                              union'
                                              (start s1, start s2, 0, Zero)
                                              in (Set (n, linkAll result))
                                end
                    end;;
                                       ...
                                       | Equal
                                           -> intersect
                                              (r1, r2, n + 1,
                                               addItem (e1, result))
                                       | GREATER
                                           -> intersect (t1, r2, n, result)
                                  end
            end
            in let (n, result) = intersect (start s1, start s2, 0, Zero)
                 in (Set (n, linkAll result));;
        let rec difference (Set (_, s1), Set (_, s2)) =
          let rec ins =
            function 
                     ...
                     | ((Black (x, _, _), r), n, result)
                         -> ins (next r, n + 1, addItem (x, result))
            in let rec diff (t1, t2, n, result) =
                 begin
                 match (next t1, next t2)
                 with 
                      | ((Empty, _), _) -> (n, result)
                 ...
                 in let (n, result) = diff (start s1, start s2, 0, Zero)
                      in (Set (n, linkAll result));;
        let rec difference2 (Set (_, s1), Set (_, s2)) =
          let rec ins =
            function 
                     ...
                     | ((Black (x, _, _), r), n, result)
                         -> ins (next r, n + 1, addItem (x, result))
            in let rec diff (t1, t2, (n1, result1), (n2, result2)) =
                 begin
                 match (next t1, next t2)
                 with 
                      | ((Empty, _), t2)
                          -> ((n1, result1), ins (t2, n2, result2))
                      | (t1, (Empty, _))
                          -> (ins (t1, n1, result1), (n2, result2))
                      | ((tree1, r1), (tree2, r2))
                          -> let ((x, d1) as e1) = getEntry tree1
                               in let ((y, d2) as e2) = getEntry tree2
                                    in begin
                 ...
                 in let ((n1, result1), (n2, result2)) =
                      diff (start s1, start s2, (0, Zero), (0, Zero))
                      in ((Set (n1, linkAll result1)),
                          (Set (n2, linkAll result2)));;
        ...
        let rec splitSets f_ (Set (_, s1), Set (_, s2)) =
          let rec ins =
            ...
            in let rec split
                 ...
                 in let ((n, r), (n1, r1), (n2, r2)) =
                      split
                      (start s1, start s2, (0, Zero), (0, Zero), (0, Zero))
                      in ((Set (n, linkAll r)), (Set (n1, linkAll r1)),
                          (Set (n2, linkAll r2)));;
        end;;
  ...
  *);;
   ...
   *);;
    ...
    *);;
    ...
    (* ignore size hint *);;
    let rec copy s_ = let s'_ = (New_ ()) in begin
                                               s'_ := (! s_);s'_
                                               end;;
    let insert =
      function 
               | set -> function 
                                 | entry -> set := (insert (! set, entry));;
    let insertLast =
      function 
               | set
                   -> function 
                               | datum -> set := (insertLast (! set, datum));;
    let insertList =
      function 
               | set -> function 
                                 | list -> set := (insertList (! set, list));;
    let insertShadow =
      function 
               | set
                   -> function 
                               | entry
                                   -> set := (insertShadow (! set, entry));;
    let isEmpty = function 
                           | ordSet -> isEmpty (! ordSet);;
    let last = function 
                        | ordSet -> last (! ordSet);;
    let lookup = function 
                          | ordSet -> function 
                                               | key -> lookup (! ordSet) key;;
    let clear = function 
                         | ordSet -> ordSet := empty;;
    let App_ = function 
                        | ordSet -> function 
