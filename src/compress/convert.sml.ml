...
module Convert = struct
                   ...
                   let rec findn arg__0 arg__1 =
                     ...
                     with 
                          ...
                          | ((v :: tl), v') -> begin
                              if v = v' then 0 else 1 + (findn tl v') end
                     end;;
                   let rec findid ctx v =
                     try var_ (findn ctx v)
                     with 
                          | NotFound _ -> const_ (findn (! sigma) v);;
                   let rec modeconvert =
                     function 
                              | mMINUS -> minus_
                              | mPLUS -> plus_
                   ...
                   let rec modesofclass =
                     function 
                              | Kclass_ type_ -> []
                              | Kclass_ (KPi (m, _, k))
                                  -> (m :: modesofclass (kclass k))
                              | Tclass_ (TRoot _) -> []
                              | Tclass_ (TPi (m, _, a))
                                  -> (m :: modesofclass (tclass a));;
...
*);;
                   let rec spine_form =
                     function 
                              ...
                              | (g_, Parse.App (t, u))
                                  -> let (h, mopt, p, s) = spine_form (g_, t)
                                       in (h, mopt, p, s @ [u])
                              | (g_, Parse.Lam _)
                                  -> raise ((Convert "illegal redex"))
                              ...
                              | (g_, Parse.Id s)
                                  -> let n = findn (! sigma) s
                                       in (n,
                                           ...
                                           (List.nth (! sigmat, n)), [])
                              | (g_, Parse.App (t, u))
                                  -> let (n, m, s) = type_spine_form (g_, t)
                                       in (n, m, s @ [u])
                              | (g_, _) -> raise ((Convert "level mismatch"));;
                   let rec safezip (l1, l2) = begin
                     if (length l1) = (length l2) then ListPair.zip (l1, l2)
                     else raise ((Convert "wrong spine length")) end;;
                   ...
                   let rec eltconvert arg__2 arg__3 =
                     ...
                     with 
                          ...
                          | (g_, (Parse.Ascribe (t, a), plus_))
                              -> ascribe_
                                 (nconvert (g_, t), typeconvert (g_, a))
                          | (g_, (t, plus_)) -> aElt_ (aconvert (g_, t))
                          | (g_, (omit_, omit_)) -> omit_
                          | (g_, (_, omit_))
                              -> raise
                                 ((Convert
                                   "found term expected to be omitted"))
                     end
                   and aconvert (g_, t) =
                     begin
                     match convert (g_, t)
                     with 
                          ...
                          | NTerm _
                              -> raise
                                 ((Convert "required atomic, found normal"))
                     end
                   and nconvert (g_, t) =
                     begin
                     match convert (g_, t)
                     with 
                          ...
                          | ATerm _
                              -> raise
                                 ((Convert "required normal, found atomic"))
                     end
                   and convert =
                     function 
                              | (g_, Parse.Lam ((v, _), t))
                                  -> nTerm_ (lam_ (convert ((v :: g_), t)))
                              | (g_, t)
                                  -> let (h, mopt, p, s) = spine_form (g_, t)
                                       in let s' =
                                            map
                                            ...
                                            in begin
                                            if p then aTerm_ (aRoot_ (h, s'))
                                            else nTerm_ (nRoot_ (h, s')) end
                   and typeconvert =
                     function 
                              | (g_, Parse.Pi (m, (v, Some t), t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            typeconvert ((v :: g_), t')
                                            in tPi_ (modeconvert m, ct, ct')
                              | (g_, Parse.Pi (m, (_, None), _))
                                  -> raise
                                     ((Convert "can't handle implicit pi"))
                              | (g_, Parse.Arrow (t, t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            typeconvert (("" :: g_), t')
                                            in tPi_ (minus_, ct, ct')
                              | (g_, Parse.PlusArrow (t, t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            ...
                                            in tPi_ (plus_, ct, ct')
                              | (g_, a)
                                  -> let (n, m, s) = type_spine_form (g_, a)
                                       in let s' =
                                            map
                                            (eltconvert g_)
                                            (safezip (s, m))
                     ...
                     function 
                              | (g_, Parse.Pi (m, (v, Some t), t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            kindconvert ((v :: g_), t')
                                            in kPi_ (modeconvert m, ct, ct')
                              | (g_, Parse.Arrow (t, t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            kindconvert (("" :: g_), t')
                                            in kPi_ (minus_, ct, ct')
                              | (g_, Parse.PlusArrow (t, t'))
                                  -> let ct = typeconvert (g_, t)
                                       in let ct' =
                                            kindconvert (("" :: g_), t')
