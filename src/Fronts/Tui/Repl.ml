...
module Repl (M : REPL.S) : REPL.REPL = struct
  ...
  let rec read : (string -> response Lwt.t) -> int Lwt.t =
   fun f ->
    let open Lwt.Syntax in
    let* term' = term in
    if not (LTerm.is_a_tty !term') then
      (* Lambda-term's line editor needs a real terminal even just to query
         its size, and raises [LTerm.Not_a_tty] on every call otherwise —
         not [End_of_file], so it would otherwise hit the catch-all retry
         below and spin forever (e.g. under `dune cram`, where stdin/stdout
         are never a tty). Exit the same way Ctrl-D does instead. *)
      let* () = flush () in
      Lwt.return 0
    else
