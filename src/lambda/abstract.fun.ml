...
module Abstract(Abstract__0: sig
    ...
    open!
      struct
        ...
        let rec checkConstraints k_ =
          let constraints = collectConstraints k_
            in let _ =
                 begin
                 match constraints
                 with 
        ...
        let rec collectExpW =
          function 
                   ...
                   | (g_, (I.Root ((I.FVar (name, v_, s') as f_), s_), s),
                      k_) -> begin
                       if exists (eqFVar f_) k_ then
                       collectSpine (g_, (s_, s), k_) else
                       collectSpine
                       (g_, (s_, s),
                        (I.Decl
                         (collectExp (I.null_, (v_, I.id), k_),
                          (Fv (name, v_)))))
                       end
                   | (g_,
                      (I.Root
                       (I.Proj
                        ((I.LVar ({ contents = None}, sk, (l, t)) as l_), i),
                        s_),
                       s),
                      k_)
                       -> collectSpine
                          (g_, (s_, s),
                           collectBlock (g_, I.blockSub (l_, s), k_))
                   | (g_, (I.Root (_, s_), s), k_)
                       -> collectSpine (g_, (s_, s), k_)
                   | (g_, (I.Lam (d_, u_), s), k_)
                       -> collectExp
                   ...
                   | (g_, ((I.EVar (r, gx_, v_, cnstrs) as x_), s), k_)
                       ...
                       let v'_ = raiseType (gx_, v_)
                         in let k'_ = collectExp (I.null_, (v'_, I.id), k_)
                              in collectSub (g_, s, (I.Decl (k'_, (Ev x_))))
                       end
                   | (g_, (I.FgnExp csfe, s), k_)
                       -> I.FgnExpStd.fold
                          csfe
                          (function 
                                    | (u_, k_)
                                        -> collectExp (g_, (u_, s), k_))
                          k_
        ...
        and collectSpine =
          function 
                   ...
                   | (g_, (I.App (u_, s_), s), k_)
                       -> collectSpine
                          (g_, (s_, s), collectExp (g_, (u_, s), k_))
        and collectDec =
          function 
                   | (g_, (I.Dec (_, v_), s), k_)
                       -> collectExp (g_, (v_, s), k_)
                   | (g_, (I.BDec (_, (_, t)), s), k_)
                       -> collectSub (g_, I.comp (t, s), k_)
                   | (g_, (I.NDec _, s), k_) -> k_
        and collectSub =
          function 
                   | (g_, I.Shift _, k_) -> k_
        ...
        and collectBlock =
          function 
                   ...
                   | (g_, (I.LVar (_, sk, (l, t)) as l_), k_) -> begin
                       if exists (eqLVar l_) k_ then
                       collectSub (g_, I.comp (t, sk), k_) else
                       (I.Decl
                        (collectSub (g_, I.comp (t, sk), k_), (Lv l_)))
                       end;;
        let rec collectCtx =
          function 
                   | (g0_, null_, k_) -> (g0_, k_)
                   | (g0_, I.Decl (g_, d_), k_)
                       -> let (g0'_, k'_) = collectCtx (g0_, g_, k_)
                            in let k''_ = collectDec (g0'_, (d_, I.id), k'_)
                                 in ((I.Decl (g0_, d_)), k''_);;
        let rec collectCtxs =
          function 
                   | (g0_, [], k_) -> k_
                   | (g0_, (g_ :: gs_), k_)
                       -> let (g0'_, k'_) = collectCtx (g0_, g_, k_)
                            in collectCtxs (g0'_, gs_, k'_);;
        let rec abstractEVar =
          function 
                   | (I.Decl (k'_, Ev (I.EVar (r', _, _, _))), depth,
                      (I.EVar (r, _, _, _) as x_)) -> begin
                       if r = r' then (I.BVar (depth + 1)) else
                       abstractEVar (k'_, depth + 1, x_) end
                   | (I.Decl (k'_, _), depth, x_)
                       -> abstractEVar (k'_, depth + 1, x_);;
        let rec abstractFVar =
          function 
                   | (I.Decl (k'_, Fv (n', _)), depth,
                      (I.FVar (n, _, _) as f_)) -> begin
                       if n = n' then (I.BVar (depth + 1)) else
                       abstractFVar (k'_, depth + 1, f_) end
                   | (I.Decl (k'_, _), depth, f_)
                       -> abstractFVar (k'_, depth + 1, f_);;
        let rec abstractLVar =
          function 
                   | (I.Decl (k'_, Lv (I.LVar (r', _, _))), depth,
                      (I.LVar (r, _, _) as l_)) -> begin
                       if r = r' then (I.Bidx (depth + 1)) else
                       abstractLVar (k'_, depth + 1, l_) end
                   | (I.Decl (k'_, _), depth, l_)
                       -> abstractLVar (k'_, depth + 1, l_);;
        let rec abstractExpW =
          function 
                   ...
                   | (k_, depth, ((I.EVar _ as x_), s))
                       -> (I.Root
                           (abstractEVar (k_, depth, x_),
                            abstractSub (k_, depth, s, I.nil_)))
                   | (k_, depth, (I.FgnExp csfe, s))
                       -> I.FgnExpStd.Map.apply
                          csfe
                          (function 
                                    | u_ -> abstractExp (k_, depth, (u_, s)))
        and abstractExp (k_, depth, us_) =
        ...
        let rec abstractSOME =
          function 
                   ...
                   | (k_, I.Dot (I.Idx k, s))
                       -> (I.Dot ((I.Idx k), abstractSOME (k_, s)))
                   | (k_, I.Dot (I.Exp u_, s))
                       -> (I.Dot
                   ...
                   | (k_, I.Dot (I.Block ((I.LVar _ as l_)), s))
                       -> (I.Dot
                           ((I.Block (abstractLVar (k_, 0, l_))),
                            abstractSOME (k_, s)));;
        let rec abstractCtx =
          function 
                   | (k_, depth, null_) -> (I.null_, depth)
                   | (k_, depth, I.Decl (g_, d_))
                       -> let (g'_, depth') = abstractCtx (k_, depth, g_)
                            in let d'_ = abstractDec (k_, depth', (d_, I.id))
                                 in ((I.Decl (g'_, d'_)), depth' + 1);;
        let rec abstractCtxlist =
          function 
                   | (k_, depth, []) -> []
                   | (k_, depth, (g_ :: gs_))
                       -> let (g'_, depth') = abstractCtx (k_, depth, g_)
                            in let gs'_ = abstractCtxlist (k_, depth', gs_)
                                 in (g'_ :: gs'_);;
        let rec abstractKPi =
          function 
                   | (null_, v_) -> v_
                   | (I.Decl (k'_, Ev (I.EVar (_, gx_, vx_, _))), v_)
                       -> let v'_ = raiseType (gx_, vx_)
                            in let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                                 in abstractKPi
                                    (k'_,
                                     (I.Pi
                                      (((I.Dec (None, v''_)), I.maybe_), v_)))
                   | (I.Decl (k'_, Fv (name, v'_)), v_)
                       -> let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                            in abstractKPi
                               (k'_,
                                (I.Pi
                                 (((I.Dec ((Some name), v''_)), I.maybe_),
                                  v_)))
                   | (I.Decl (k'_, Lv (I.LVar (r, _, (l, t)))), v_)
                       -> let t' = abstractSOME (k'_, t)
                            in abstractKPi
                               (k'_,
                                (I.Pi
                                 (((I.BDec (None, (l, t'))), I.maybe_), v_)));;
                       ...
                       -> let v'_ = raiseType (gx_, vx_)
                            in abstractKLam
                               (k'_,
                                (I.Lam
                                 ((I.Dec
                                   (None, abstractExp (k'_, 0, (v'_, I.id)))),
                                  u_)))
          ...
          function 
                   ...
                   | I.Decl (k'_, Ev (I.EVar (_, gx_, vx_, _)))
                       -> let v'_ = raiseType (gx_, vx_)
                            in let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                                 in (I.Decl
                                     (abstractKCtx k'_, (I.Dec (None, v''_))))
                   ...
                   | I.Decl (k'_, Lv (I.LVar (r, _, (l, t))))
                       -> let t' = abstractSOME (k'_, t)
                            in (I.Decl
                                (abstractKCtx k'_, (I.BDec (None, (l, t')))));;
        let rec abstractDecImp v_ =
          let k_ = collectExp (I.null_, (v_, I.id), I.null_)
            in let _ = checkConstraints k_
                 in (I.ctxLength k_,
                     abstractKPi (k_, abstractExp (k_, 0, (v_, I.id))));;
        let rec abstractDef (u_, v_) =
          let k_ =
            collectExp
            (I.null_, (u_, I.id), collectExp (I.null_, (v_, I.id), I.null_))
            in let _ = checkConstraints k_
                 in (I.ctxLength k_,
                     (abstractKLam (k_, abstractExp (k_, 0, (u_, I.id))),
                      abstractKPi (k_, abstractExp (k_, 0, (v_, I.id)))));;
        let rec abstractSpineExt (s_, s) =
          let k_ = collectSpine (I.null_, (s_, s), I.null_)
            in let _ = checkConstraints k_
                 in let g_ = abstractKCtx k_
                      in let s_ = abstractSpine (k_, 0, (s_, s)) in (g_, s_);;
        let rec abstractCtxs gs_ =
          let k_ = collectCtxs (I.null_, gs_, I.null_)
            in let _ = checkConstraints k_
                 in (abstractKCtx k_, abstractCtxlist (k_, 0, gs_));;
        let rec closedDec (g_, (I.Dec (_, v_), s)) =
          begin
          match collectExp (g_, (v_, s), I.null_)
          with 
               | null_ -> true
               | _ -> false
          end;;
        let rec closedSub =
          function 
                   ...
                   | (g_, I.Dot (I.Exp u_, s))
                       -> begin
                          match collectExp (g_, (u_, I.id), I.null_)
                          with 
                               | null_ -> closedSub (g_, s)
                               | _ -> false
                          end;;
        ...
        let rec closedCtx =
          function 
                   | null_ -> true
                   | I.Decl (g_, d_)
                       -> (closedCtx g_) && (closedDec (g_, (d_, I.id)));;
        let rec closedFor =
          function 
                   | (psi_, true_) -> true
                   | (psi_, T.All ((d_, _), f_))
                       -> (closedDEC (psi_, d_)) &&
                            (closedFor ((I.Decl (psi_, d_)), f_))
                   | (psi_, T.Ex ((d_, _), f_))
        ...
        let rec closedCTX =
          function 
                   | null_ -> true
                   | I.Decl (psi_, d_)
                       -> (closedCTX psi_) && (closedDEC (psi_, d_));;
        let rec evarsToK =
          function 
                   | [] -> I.null_
        ...
        let rec collectPrg =
          function 
                   | (_, (T.EVar (psi_, r, f_, _, _, _) as p_), k_)
                       -> (I.Decl (k_, (Pv p_)))
                   | (psi_, unit_, k_) -> k_
                   | (psi_, T.PairExp (u_, p_), k_)
                       -> collectPrg
                          (psi_, p_,
                           collectExp (T.coerceCtx psi_, (u_, I.id), k_));;
        let rec abstractPVar =
          function 
                   | (I.Decl (k'_, Pv (T.EVar (_, r', _, _, _, _))), depth,
                      (T.EVar (_, r, _, _, _, _) as p_)) -> begin
                       if r = r' then (T.Var (depth + 1)) else
                       abstractPVar (k'_, depth + 1, p_) end
                   | (I.Decl (k'_, _), depth, p_)
                       -> abstractPVar (k'_, depth + 1, p_);;
        let rec abstractPrg =
          function 
                   | (k_, depth, (T.EVar _ as x_))
                       -> abstractPVar (k_, depth, x_)
                   | (k_, depth, unit_) -> T.unit_
                   | (k_, depth, T.PairExp (u_, p_))
                       -> (T.PairExp
                           (abstractExp (k_, depth, (u_, I.id)),
                            abstractPrg (k_, depth, p_)));;
        let rec collectTomegaSub =
          function 
                   | T.Shift 0 -> I.null_
                   | T.Dot (T.Exp u_, t)
                       -> collectExp
                          (I.null_, (u_, I.id), collectTomegaSub t)
                   | T.Dot (T.Block b_, t)
                       -> collectBlock (I.null_, b_, collectTomegaSub t)
                   | T.Dot (T.Prg p_, t)
                       -> collectPrg (I.null_, p_, collectTomegaSub t);;
        ...
        let rec abstractTC =
          function 
                   | (k_, depth, T.Abs (d_, tc_))
                       -> (T.Abs
                           (abstractDec (k_, depth, (d_, I.id)),
                            abstractTC (k_, depth, tc_)))
                   | (k_, depth, T.Conj (tc1_, tc2_))
                       -> (T.Conj
                           (abstractTC (k_, depth, tc1_),
                            abstractTC (k_, depth, tc2_)))
                   | (k_, depth, T.Base o_)
                       -> (T.Base (abstractOrder (k_, depth, o_)));;
        let rec abstractTCOpt =
          function 
                   | (k_, depth, None) -> None
                   | (k_, depth, Some tc_)
                       -> (Some (abstractTC (k_, depth, tc_)));;
        let rec abstractMetaDec =
          function 
                   | (k_, depth, T.UDec d_)
                       -> (T.UDec (abstractDec (k_, depth, (d_, I.id))))
                   | (k_, depth, T.PDec (xx, f_, tc1_, tc2_))
                       -> (T.PDec
                           (xx, abstractFor (k_, depth, f_), tc1_, tc2_))
        and abstractFor =
          function 
                   | (k_, depth, true_) -> T.true_
                   | (k_, depth, T.All ((md_, q_), f_))
                       -> (T.All
                           ((abstractMetaDec (k_, depth, md_), q_),
                   ...
                   | (k_, depth, T.And (f1_, f2_))
                       -> (T.And
                           (abstractFor (k_, depth, f1_),
                            abstractFor (k_, depth, f2_)))
                   | (k_, depth, T.World (w_, f_))
                       -> (T.World (w_, abstractFor (k_, depth, f_)));;
        let rec abstractPsi =
          function 
                   | null_ -> I.null_
                   | I.Decl (k'_, Ev (I.EVar (_, gx_, vx_, _)))
                       -> let v'_ = raiseType (gx_, vx_)
                            in let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                                 in (I.Decl
                                     (abstractPsi k'_,
                                      (T.UDec ((I.Dec (None, v''_))))))
                   | I.Decl (k'_, Fv (name, v'_))
                       -> let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                            in (I.Decl
                                (abstractPsi k'_,
                                 (T.UDec ((I.Dec ((Some name), v''_))))))
                   | I.Decl (k'_, Lv (I.LVar (r, _, (l, t))))
                       -> let t' = abstractSOME (k'_, t)
                            in (I.Decl
                                (abstractPsi k'_,
                                 (T.UDec ((I.BDec (None, (l, t')))))))
                   | I.Decl (k'_, Pv (T.EVar (gx_, _, fx_, tc1_, tc2_, _)))
                       -> let f'_ =
                            abstractFor (k'_, 0, T.forSub (fx_, T.id))
                            in let tc1'_ = abstractTCOpt (k'_, 0, tc1_)
                                 in let tc2'_ = abstractTCOpt (k'_, 0, tc2_)
                                      in (I.Decl
                                          (abstractPsi k'_,
                                           (T.PDec (None, f'_, tc1_, tc2_))));;
        let rec abstractTomegaSub t =
          let k_ = collectTomegaSub t
            in let t' = abstractTomegaSub' (k_, 0, t)
                 in let psi_ = abstractPsi k_ in (psi_, t')
        and abstractTomegaSub' =
          function 
        ...
        let rec abstractTomegaPrg p_ =
          let k_ = collectPrg (I.null_, p_, I.null_)
            in let p'_ = abstractPrg (k_, 0, p_)
                 in let psi_ = abstractPsi k_ in (psi_, p'_);;
        end;;
