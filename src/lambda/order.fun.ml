...
module Order(Order__0: sig module Table : TABLE end) : ORDER =
  struct
    ...
    open!
      struct
        ...
        let rec uninstall cid =
          begin
          match Table.lookup orderTable_ cid
          with 
               | None -> false
               | Some _ -> begin
                             Table.delete orderTable_ cid;true
                             end
          end;;
        let rec installROrder (cid, p_) =
          Table.insert redOrderTable_ (cid, p_);;
        let rec uninstallROrder cid =
          begin
          match Table.lookup redOrderTable_ cid
          with 
               | None -> false
               | Some _ -> begin
                             Table.delete redOrderTable_ cid;true
                             end
          end;;
          ...
          function 
                   ...
                   | ((a :: a1s), a2s) -> begin
                       if List.exists (function 
                                                | a' -> a = a') a2s
                       then closure (a1s, a2s) else
                       closure ((mutual a) @ a1s, (a :: a2s)) end;;
        end;;
    (* mutual a = a's
