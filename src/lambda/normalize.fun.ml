...
module Normalize (Normalize__0 : sig
  ...
  open! struct
    ...
    let rec normalizePrg = function
      ...
      | T.PairExp (u_, p'_), t ->
          T.PairExp (Whnf.normalize (u_, T.coerceSub t), normalizePrg (p'_, t))
      | T.PairBlock (b_, p'_), t ->
          T.PairBlock (I.blockSub (b_, T.coerceSub t), normalizePrg (p'_, t))
      | T.PairPrg (p1_, p2_), t ->
          T.PairPrg (normalizePrg (p1_, t), normalizePrg (p2_, t))
      | unit_, _ -> T.unit_
      | T.EVar (_, { contents = Some p_ }, _), t -> T.PClo (p_, t)
      | (T.EVar _ as p_), t -> T.PClo (p_, t)
      | T.PClo (p_, t), t' -> normalizePrg (p_, T.comp (t, t'))

    and normalizeSpine = function
      | nil_, t -> T.nil_
      | T.AppExp (u_, s_), t ->
          T.AppExp (Whnf.normalize (u_, T.coerceSub t), normalizeSpine (s_, t))
      | T.AppPrg (p_, s_), t ->
          T.AppPrg (normalizePrg (p_, t), normalizeSpine (s_, t))
      | T.AppBlock (b_, s_), t ->
          T.AppBlock (I.blockSub (b_, T.coerceSub t), normalizeSpine (s_, t))
      | T.SClo (s_, t1), t2 -> normalizeSpine (s_, T.comp (t1, t2))
