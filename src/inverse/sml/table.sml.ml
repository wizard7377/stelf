...
(* end *)
module ArrayTable : TABLE = struct
  module L = Lib
  module A = Array
  ...
  let rec clear { arr; used } =
    begin
      used := 0;
      A.modify (function _ -> None) arr
    end

  let rec insert ({ arr; used } as t) (n, v) =
    begin if n < 0 || n > A.length arr then raise Subscript
    else begin
      match A.sub (arr, n) with
      | None -> begin
          A.update (arr, n, Some v);
          begin
            begin if n > !used then used := n else ()
            end;
            t
          end
        end
  ...
  let rec app f { arr; used } =
    let used' = !used in
    let rec f' (i, x) =
      begin if i >= used' then raise Done
      else begin
        match x with Some n -> f n | None -> ()
      end
      end
    in
  ...
  let rec appi f { arr; used } =
    ...
    let rec f' (i, x) =
      begin if i >= used' then raise Done
        ...
        match x with Some n -> f (i, n) | None -> ()
      end
      end
    in
    try A.appi f' arr with Done -> ()
