...
module Lib : LIB = struct
  ...
  (*  Streams                                                                   *)
  (* -------------------------------------------------------------------------- *)
  type 'a stream = SNil | SCons of (unit -> 'a * 'a stream)
  ...
  let rec repeat f n x =
    begin if n = 0 then x else repeat f (n - 1) (f x)
    end
  ...
  let rec uptoby k m n =
    begin if m > n then [] else m :: uptoby k (m + k) n
    end
  ...
  let rec log n =
    let rec log n even =
      ...
      | n ->
          ...
          let even'' = even' && Int.( mod ) (n, 2) = 0 in
          (1 + ln, even'')
      end
    in
    log n true
  ...
  let rec string_ord ((s1 : string), s2) =
    begin if s1 < s2 then LESS
    else begin
      if s1 = s2 then Equal else GREATER
    end
    end
  ...
  let rec lex_ord o1 o2 ((x1, y1), (x2, y2)) =
    begin match o1 (x1, x2) with Equal -> o2 (y1, y2) | x -> x
    end

  let rec eq_ord (x, y) =
    begin if x = y then Equal else LESS
    end
  ...
  (* ---------------------------------------------------------------------- *)
  let rec assert_ b s =
    begin if b then () else raise (Fail ("Assertion Failure: " ^ s))
    end

  let warn = ref true

  let rec warning s =
    begin if !warn then TextIO.print (("Warning: " ^ s) ^ "\n") else ()
    end
  ...
  let rec unzip l =
    itlist
      (function h1, h2 -> ( function t1, t2 -> (h1 :: t1, h2 :: t2)))
      l ([], [])
  ...
  let rec unzip4 l =
    itlist
      (function
        | h1, h2, h3, h4 -> (
            function t1, t2, t3, t4 -> (h1 :: t1, h2 :: t2, h3 :: t3, h4 :: t4)))
      l ([], [], [], [])
  ...
  let rec replicate arg__38 arg__39 =
    ...
    | x, 0 -> []
    | x, n -> begin if n > 0 then x :: replicate x (n - 1) else [] end
    end
  ...
  let rec gen_list_eq ord l1 l2 =
    itlist2
      (function
        | x -> ( function y -> ( function z -> ord (x, y) = Equal && z)))
      l1 l2 true
  ...
  let rec partition arg__44 arg__45 =
    ...
    | p, h :: t ->
        ...
        begin if p h then (h :: l, r) else (l, h :: r)
        end
    end
  ...
  let rec uniq arg__48 arg__49 =
    ...
    | ord, x :: (y :: _ as t) ->
        let t' = uniq ord t in
        begin if ord (x, y) = Equal then t' else x :: t'
        end
    | _, l -> l
    end
  ...
  let rec split_at arg__50 arg__51 =
    ...
    | 0, l -> ([], l)
    | n, (x :: ys as xs) -> begin
        if n < 0 then raise (Fail "split_at: arg out of range")
        else
          let ps, qs = split_at (n - 1) ys in
          (x :: ps, qs)
      end
    end
  ...
  let rec list_slice n m l =
    let _, r = split_at n l in
    let l', _ = split_at m r in
    l'
  ...
  let rec find_index p =
    let rec ind arg__54 arg__55 =
      ...
      | n, [] -> None
      | n, h :: t -> begin if p h then Some n else ind (n + 1) t end
      end
    in
    ind 0
  ...
  let rec flatten l = itlist (curry (fun (x__op, y__op) -> x__op @ y__op)) l []
  ...
  let rec exn_index f l =
    let rec exn_index arg__62 arg__63 arg__64 =
      ...
      | f, [], n -> None
      | f, h :: t, n -> begin
          if can f h then exn_index f t (n + 1) else Some n
        end
      end
    in
    exn_index f l 0
  ...
  let rec insert x l =
    begin if mem x l then l else x :: l
    end
  ...
  let rec gen_pairwise_disjoint arg__68 arg__69 =
    ...
    | p, [] -> true
    | p, h :: t -> forall (gen_disjoint p h) t && gen_pairwise_disjoint p t
    end
  ...
  let rec diff arg__70 arg__71 =
    ...
    | [], l -> []
    | h :: t, l -> begin if mem h l then diff t l else h :: diff t l end
    end
  ...
  let rec assoc x l =
    begin match find (function p -> fst p = x) l with
    | Some (x, y) -> Some y
    | None -> None
    end

  let rec rev_assoc x l =
    begin match find (function p -> snd p = x) l with
    | Some (x, y) -> Some x
    | None -> None
    end
  ...
  (*  Strings                                                                  *)
  (* ------------------------------------------------------------------------- *)
  let rec char_max c1 c2 =
    begin if Char.ord c1 < Char.ord c2 then c1 else c2
    end

  let rec string_lt (x : string) y = x < y
  let rec collect l = itlist (curry (fun (x__op, y__op) -> x__op ^ y__op)) l ""
  ...
  let rec ends_with s e =
    try substring (s, size s - size e, size e) = e with _ -> false

  let rec starts_with s e = try substring (s, 0, size e) = e with _ -> false
  ...
  let rec strip_path c s =
    ...
    let m' = substring (s, n + 1, size s - n - 1) in
    (m, m')
  ...
  let rec rev_strip_path c s =
    ...
    let n =
      begin match no with Some x -> x | None -> raise (Fail "rev_strip_path")
      end
    in
    let m = substring (s, 0, n) in
    let m' = substring (s, n + 1, size s - n - 1) in
  ...
  let rec time f x =
    ...
    try
      ...
      let _ =
        print ("CPU time (user): " ^ Time.toString ((fun r -> r.usr) time))
      in
      res
    with e ->
      let time = Timer.checkCPUTimer timer in
  ...
  let rec read_file file =
    ...
    let s = TextIO.inputAll f in
    let _ = TextIO.closeIn f in
    s
  ...
  let rec all_dir_files dir =
    ...
    begin
      begin
        while !f <> None do
          begin
            fs (fun (x__op, y__op) -> x__op @::= y__op) the !f;
