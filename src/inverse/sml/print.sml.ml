...
open! struct
  ...
  let rec uni_to_layout = function
  ...
  let rec spine_to_list = function
    | nil_ -> []
  ...
  let rec head_to_layout arg__0 arg__1 =
    ...
    | sgn, Const c ->
        (fun (x__op, y__op) -> x__op $ y__op) (const_to_string sgn c)
    | sgn, BVar n -> (fun (x__op, y__op) -> x__op $ y__op) (Int.toString n)
    end

  let rec needs_parens_in_arg_pos = function
    | Uni _ -> false
    | Root (_, nil_) -> false
    | _ -> true

  let rec needs_sparens_in_arg_pos = function
    | nil_ -> false
    | App (e_, nil_) -> needs_parens_in_arg_pos e_
    | _ -> true

  let rec maybe_paren e_ l =
    begin if needs_parens_in_arg_pos e_ then paren l else l
    end

  let rec maybe_sparen s_ l =
    begin if needs_sparens_in_arg_pos s_ then paren l else l
    end
