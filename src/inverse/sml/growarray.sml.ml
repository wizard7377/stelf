...
module GrowArray : GROWARRAY =
  struct
    ...
    let rec sub { contents = (used, a)} n = begin
      if (n < used) && (n >= 0) then
      begin
      match A.sub (a, n) with 
                              | None -> raise Subscript
                              | Some z -> z
      end else raise Subscript end;;
    let rec length { contents = (l, _)} = l;;
    (* grow to accomodate at least n elements *);;
    let rec accomodate (({ contents = (l, a)} as r)) n = begin
      if (A.length a) >= (n + 1) then () else
      let rec nextpower x = begin
        if x >= (n + 1) then x else nextpower (x * 2) end
        in let ns = nextpower (A.length a)
             in let na =
                  A.tabulate
                  (ns,
                   function 
                            | i -> begin if i < l then A.sub (a, i) else None
                                end)
                  in r := (l, na)
      end;;
    let rec update r n x = begin
      if n < 0 then raise Subscript else
      let _ = accomodate r n
        in let (l, a) = ! r
             in begin
                  A.update (a, n, (Some x));begin
                  if n >= l then r := (n + 1, a) else () end
                  end
             (* also update 'used' *)
      end;;
