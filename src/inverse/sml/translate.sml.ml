...
module Translate : TRANSLATE = struct
  ...
  (*  Basic Translation                                                         *)
  (* -------------------------------------------------------------------------- *)
  let rec translate_uni = function kind_ -> S.Kind | type_ -> S.Type

  let rec translate_head = function
    | I.BVar i -> S.BVar i
    | I.Const c -> S.Const c
    | I.Def c -> S.Const c
    | _ -> raise (Translate "translate_head: bad case")

  let rec translate_depend = function
    | no_ -> S.No
    | maybe_ -> S.Maybe
    | _ -> raise (Fail "translate_depend: bad case")

  and translate_exp = function
  ...
  let rec can_translate = function
    | I.ConDec _ -> true
    | I.ConDef _ -> true
    | I.AbbrevDef _ -> true
    | _ -> false
  ...
  let rec translate_signat' () =
    ...
    let cds' =
      L.filter (function id, Dec_ -> can_translate Dec_) (L.zip ns cds)
    in
    map (function (c, e) as dec -> (c, translate_condec Dec_)) cds'
