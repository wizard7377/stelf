...
module ListSignat : SIGNAT =
  struct
    ...
    exception Signat of string ;;
    let rec empty () = [];;
    let rec insert sgn (((k, a) as p)) = begin
      if L.exists (function 
                            | (k', _) -> k = k') sgn
      then raise ((Signat "insert: signat contains key")) else (p :: sgn) end;;
...
module GrowarraySignat : SIGNAT =
  struct
    ...
    let rec insert ((sgn : 'a sgn)) (n, v) = begin
      if (G.length ((fun r -> r.arr) sgn)) > n then
      raise ((Signat "insert: signat contains key")) else
      begin
        G.update ((fun r -> r.arr) sgn) n v;
        begin
          begin
          if n > (! ((fun r -> r.size) sgn)) then
          ((fun r -> r.size) sgn) := n else () end;sgn
          end
