...
module Syntax : SYNTAX =
  struct
    ...
    and sub = | Dot of exp * sub 
              | Shift of int 
              | Comp of sub * sub ;;
    type nonrec __0 = { var: string option ; arg: exp ; depend: depend ;
      body: exp };;
    ...
    type nonrec __3 = { id: const ; name: string ; exp: exp ; def: exp ;
      height: int ; root: const ; uni: uni };;
    type nonrec def = __3;;
    type nonrec __4 = { id: const ; name: string ; exp: exp ; def: exp ;
      uni: uni };;
    ...
    module Signat = struct
                      ...
                      let rec insert (c, d) =
                        ignore (T.insert global_signat (c, d));;
      ...
      function 
               ...
               | (ctx, Root (Const a, s_))
                   -> let aty = exp (Sig.lookup a)
                        in let n = num_pi_quants aty
                             in let n' = card s_ in begin
                                  if n = n' then Base else
                                  raise ((Fail "skeleton: not eta expanded"))
                                  end
               | (ctx, Root (BVar i, s_))
                   -> let aty = L.ith (i - 1) ctx
                        in let n = skel_length aty
                             in let n' = card s_ in begin
                                  if n = n' then Base else
                                  raise ((Fail "skeleton: not eta expanded"))
                                  end
    ...
    *);;
    let rec shift_head =
      function 
               ...
               | (lev, (BVar n as var)) -> begin
                   if n >= lev then (BVar (n + 1)) else var end;;
    let rec shift_spine =
      function 
               | (lev, Nil) -> Nil
               | (lev, App (m_, s_))
               ...
               | (lev, SClo _)
                   -> raise
                      ((Fail
                        "shift_spine: shouldn't have closures during eta expansion"))
    and shift_exp =
      function 
      ...
      function 
               ...
               | (ctx, Pi { arg; body; depend; var}, Base)
                   -> let arg' = long_exp (ctx, arg, Base)
                        in let tau = skeleton (ctx, arg')
                             in let body' =
                                  long_exp ((tau :: ctx), body, Base)
                                  in (Pi
                                      {
                                      arg = arg';
                                      body = body';
               ...
               | (ctx, Lam { var; body}, Arrow (tau1, tau2))
                   -> let body' = long_exp ((tau1 :: ctx), body, tau2)
                        in (Lam { var; body = body'} )
               | (ctx, (Root ((Const a as con), s_) as expr), Base)
                   -> let tau = skeleton (ctx, exp (Sig.lookup a))
                        in (Root (con, long_spine (ctx, s_, tau)))
               | (ctx, (Root ((BVar i as var), s_) as exp), Base)
                   -> let tau = L.ith (i - 1) ctx
                        in (Root (var, long_spine (ctx, s_, tau)))
                        (* indices start at 1 *)
               | (ctx, Root ((Const c as con), s_),
                  (Arrow (tau1, tau2) as tau))
                   -> let s'_ = concat (shift_spine' s_, one)
                        in begin
                             changed := true;
                             long_exp
                             (ctx,
                              (Lam { var = None; body = (Root (con, s'_))} ),
                              tau)
               ...
               | (_, exp, skel)
                   -> raise
                      ((Fail_exp_skel ("long_exp: bad case", exp, skel)))
    and long_spine =
      function 
               | (ctx, Nil, Base) -> Nil
               | (ctx, App (m_, s_), Arrow (tau1, tau2))
                   -> let m'_ = long_exp (ctx, m_, tau1)
                        in let s'_ = long_spine (ctx, s_, tau2)
                             in (App (m'_, s'_))
    ...
    let rec eta_expand' =
      function 
               ...
               | (e1, e2)
                   -> let () = changed := false
                        in let skel = skeleton ([], e2)
                             in let e2' = long_exp ([], e1, skel) in e2'
