...
open!
  struct
    ...
    let rec test (depth, names) =
      let a = map (function 
                            | x -> valOf (Names.nameLookup x)) names
        in let name = foldr (fun (x__op, y__op) -> x__op ^ y__op) "" names
             in let _ = Names.varReset ()
                  in let f_ = RelFun.convertFor a
                       in let os_ = map select a
                            in let _ = Twelf.chatter := 5
                                 in let _ = MTPi.reset ()
                                      in let _ =
                                           MTPi.init
                                           (depth,
