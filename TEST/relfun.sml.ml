  ...
  let a =
    ...
    in let name = foldr (fun (x__op, y__op) -> x__op ^ y__op) "" names
         in let _ = Names.varReset IntSyn.null_
              in let ss_ = map Worldify.worldify a
                   in let s_ =
                        foldr (fun (x__op, y__op) -> x__op @ y__op) [] ss_
                        in let _ = printS s_
                             in let _ = TextIO.print "[convertPrg "
                                  in let p_ = Converter.convertPrg a
                                       in let _ =
                                            TextIO.print "convertFor... "
                                            in let f_ =
                                                 Converter.convertFor a
                                                 in let _ =
                                                      TextIO.print
                                                      "installPrg... "
                                                      in let _ =
                                                           Converter.installPrg
                                                           a
                                                           in let _ =
                                                                TextIO.print
                                                                "printing... "
                                                                in let _ =
                                                                    ...
                                                                    TextIO.print
                                                                    "]" in p_
                                                                    (*      val _ = (FunTypeCheck.check (P, F); Twelf.OK)   *)(*      val LD = F.LemmaDec (names, P, F) *)(*      val _ = TextIO.print (FunPrint.lemmaDecToString LD) *)(*      FunNames.installName (name, F.lemmaAdd LD) *);;
let rec print names =
  let p_ = test names
    in begin
