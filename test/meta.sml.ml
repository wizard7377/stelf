...
open! struct
  ...
  let rec test names =
    ...
    let p_ = RelFun.convertPro a in
    let f_ = RelFun.convertFor a in
    let _ =
      begin
        FunTypeCheck.check (p_, f_);
        Twelf.ok_
      end
    in
    let ld_ = F.LemmaDec (names, p_, f_) in
    let _ = TextIO.print (FunPrint.lemmaDecToString ld_) in
    FunNames.installName (name, F.lemmaAdd ld_)
end
