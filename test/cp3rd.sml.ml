...
open! struct
    ...
    all_
      ( prim_ (I.Dec (Some "E", exp)),
        ex_
          ( I.Dec (Some "E'", exp'),
            ex_
              ( I.Dec (Some "D", I.Root (cp, I.App (two, I.App (one, I.nil_)))),
                true_ ) ) )
  ...
  let rec printCtx = function
    | null_ -> ()
    | I.Decl (g_, Prim d_) -> begin
        printCtx g_;
        TextIO.print (Print.decToString (makectx g_, d_) ^ "\n")
      end
    | I.Decl (g_, _) -> begin
        printCtx g_;
        TextIO.print "BLOCK\n"
                           ...
                           ( I.Root (I.BVar 5, I.App (two, I.nil_)),
                             inx_
                               ( I.Root
                                   ( I.BVar 4,
                                     I.App
                                       (three, I.App (two, I.App (one, I.nil_)))
                                   ),
                                 unit_ ) ) ) );
                   ( I.Decl
                       ( I.null_,
...
let _ =
  try check (cpt_proof, cpt_theorem) with
