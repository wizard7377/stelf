...
let test_rbt_insert () =
  let t = RBT.new_ 4 in
  RBT.insert t (1, "a");
  Alcotest.(check (option string)) "lookup after insert" (Some "a") (RBT.lookup t 1)
...
let test_rbt_shadow () =
  ...
  Alcotest.(check (option (pair int string)))
    "shadowed entry returned" (Some (1, "a")) shadowed;
  Alcotest.(check (option string)) "new value stored" (Some "b") (RBT.lookup t 1)
...
let test_rbt_sorted_order () =
  ...
  Alcotest.(check bool) "app collects each inserted key" true
...
let test_ht_basic () =
  let t = HT.new_ 8 in
  HT.insert t ("foo", 42);
  Alcotest.(check (option int)) "lookup after insert" (Some 42) (HT.lookup t "foo")
...
let test_ht_shadow () =
  ...
  Alcotest.(check (option (pair string int)))
    "insertShadow returns old binding" (Some ("k", 1)) shadowed;
...
let test_queue_empty () =
  Alcotest.(check bool) "delete on empty returns None"
    true (Q.delete Q.empty = None)
  ...
  | Some (x1, q') ->
    ...
    | Some (x2, q'') ->
      ...
      | Some (x3, _) ->
        ...
        Alcotest.(check (list int)) "FIFO order" [3; 2; 1] [x1; x2; x3]
...
let test_queue_insert_front () =
  ...
  | None -> Alcotest.fail "queue unexpectedly empty"
  | Some (x, _) ->
    Alcotest.(check int) "insertFront element dequeued first" 1 x
...
let test_ring_next_advances () =
  ...
  Alcotest.(check int) "next advances to second element" 2 (R.current (R.next r))
...
let test_ring_previous () =
  ...
  Alcotest.(check int) "previous from first wraps to last" 3 (R.current (R.previous r))
...
let rbt_suite =
  ...
  , [ Alcotest.test_case "insert and lookup" `Quick test_rbt_insert
    ...
    ; Alcotest.test_case "insertShadow returns old binding" `Quick test_rbt_shadow
    ; Alcotest.test_case "app visits all entries" `Quick test_rbt_app_count
    ; Alcotest.test_case "keys are unique after inserts" `Quick test_rbt_sorted_order
...
let ht_suite =
  ...
  , [ Alcotest.test_case "basic insert and lookup" `Quick test_ht_basic
    ...
    ; Alcotest.test_case "insertShadow returns old binding" `Quick test_ht_shadow
