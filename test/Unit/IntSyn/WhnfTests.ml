...
let test_isId_shift1 () =
  Alcotest.(check bool)
    "Shift 1 is not identity (weakening)" false (isId (Shift 1))
...
let test_whnf_uni_type () =
  ...
  Alcotest.(check bool)
    "whnf(Uni Type) returns Uni Type" true
    (fst result = Uni Type)

let test_whnf_uni_kind () =
  let result = whnf (Uni Kind, id_sub) in
  Alcotest.(check bool)
    "whnf(Uni Kind) returns Uni Kind" true
    (fst result = Uni Kind)
...
let test_whnf_lam_is_value () =
  ...
  Alcotest.(check bool)
    "whnf(Lam) is a value (not reduced)" true
...
let test_normalize_uni () =
  ...
  Alcotest.(check exp_testable)
    "normalize(Uni Type) = Uni Type" (Uni Type) result

let test_normalize_lam () =
  let lam = lam_ (Uni Type) (bvar 1) in
  let result = normalize (lam, id_sub) in
  Alcotest.(check bool)
    "normalize(Lam) preserves Lam structure" true
...
let test_normalize_pi () =
  ...
  Alcotest.(check bool)
    "normalize(Pi) preserves Pi structure" true
  ...
  [
    ...
    ( "Whnf.isPatSub",
      [
        ...
        Alcotest.test_case "Dot(Exp, _) not pattern" `Quick
          test_isPatSub_exp_front;
