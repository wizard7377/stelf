...
let test_closed_ctx_with_type () =
  ...
  Alcotest.(check bool)
    "Context with Uni Type decl is closed" true (closedCtx ctx)

let test_not_closed_ctx_with_evar () =
  let evar = EVar (ref None, null_ctx, Uni Type, ref []) in
  let ctx = Decl (null_ctx, Dec (None, evar)) in
  Alcotest.(check bool)
    "Context with EVar decl is not closed" false (closedCtx ctx)
  ...
  [
    ( "Abstract.closedExp",
      [
        ...
        Alcotest.test_case "instantiated EVar is closed" `Quick
          test_closed_instantiated_evar;
      ] );
    ( "Abstract.closedCtx",
      [
        Alcotest.test_case "Null ctx is closed" `Quick test_closed_ctx_null;
        Alcotest.test_case "ctx with type decl is closed" `Quick
          test_closed_ctx_with_type;
        Alcotest.test_case "ctx with EVar decl is not closed" `Quick
          test_not_closed_ctx_with_evar;
      ] );
    ( "Abstract.collectEVars",
      [
        ...
        Alcotest.test_case "same EVar deduplicated" `Quick
          test_collect_dedup_evar;
      ] );
  ]
