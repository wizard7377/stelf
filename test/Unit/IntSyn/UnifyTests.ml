...
let test_unify_bvar_clash () =
  ...
  check_unify_fails "BVar 1 ≢ BVar 2" null_ctx
    (Root (BVar 1, Nil), id_sub) (Root (BVar 2, Nil), id_sub)
...
let test_unify_evar_value () =
  ...
  | Some v -> Alcotest.(check bool) "EVar assigned to Uni Type" true (v = Uni Type)
...
let test_unify_two_evars () =
  ...
  Alcotest.(check bool) "at least one EVar is assigned" true
    (!r1 <> None || !r2 <> None)
...
let test_unify_backtrack_evar () =
  ...
  U.mark ();
  U.unify (null_ctx, (x, id_sub), (Uni Type, id_sub));
  let () = Alcotest.(check bool) "EVar assigned before unwind" true (!r <> None) in
  U.unwind ();
...
let test_unify_backtrack_two_evars () =
  ...
  U.unwind ();
  Alcotest.(check bool) "both EVars reset after unwind" true
    (!r1 = None && !r2 = None)
...
let test_unifiable_true () =
  ...
  Alcotest.(check bool) "EVar unifiable with Uni Type" true
    (U.unifiable (null_ctx, (x, id_sub), (Uni Type, id_sub)))

let test_unifiable_false () =
  (* BVar with different indices are not unifiable *)
  U.reset ();
  Alcotest.(check bool) "BVar 1 not unifiable with BVar 2" false
    (U.unifiable (null_ctx, (Root (BVar 1, Nil), id_sub), (Root (BVar 2, Nil), id_sub)))
...
let suites =
  ...
  ; ( "Unify (EVar instantiation)"
    , [ Alcotest.test_case "EVar is assigned" `Quick test_unify_evar_assigns
      ; Alcotest.test_case "EVar assigned correct value" `Quick test_unify_evar_value
      ; Alcotest.test_case "two EVars unified" `Quick test_unify_two_evars
      ] )
  ; ( "Unify (backtracking)"
    , [ Alcotest.test_case "unwind resets one EVar" `Quick test_unify_backtrack_evar
      ; Alcotest.test_case "unwind resets two EVars" `Quick test_unify_backtrack_two_evars
      ; Alcotest.test_case "pre-mark assignments survive unwind"
          `Quick test_unify_backtrack_preserves_earlier
      ] )
  ; ( "Unify.unifiable"
    , [ Alcotest.test_case "EVar unifiable with type" `Quick test_unifiable_true
      ; Alcotest.test_case "BVar 1 and BVar 2 not unifiable" `Quick test_unifiable_false
      ] )
  ]
