...
%term length_store_cons {S ref_store} {N nat} %if (length_store (store_cons T S) (s N)) %<- (length_store S N)
...
%term find_in_store_yes {T ref_tp} find_in_store (lbl z) (store_cons T S) T
%term find_in_store_no {N nat} {S ref_store} {T ref_tp} %if (find_in_store (lbl (s N)) (store_cons T' S) T) %<- (find_in_store (lbl N) S T)
...
%term length_heap_cons {H ref_heap} {N nat} %if (length_heap (heap_cons E H) (s N)) %<- (length_heap H N)
%worlds () (length_heap _ _)
%total H (length_heap H _)

%sort find_in_heap {_ label} {_ ref_heap} {_ ref_exp}
%mode find_in_heap %in %in %out
%term find_in_heap_yes {E ref_exp} find_in_heap (lbl z) (heap_cons E H) E
%term find_in_heap_no {N nat} {H ref_heap} {E ref_exp} %if (find_in_heap (lbl (s N)) (heap_cons E' H) E) %<- (find_in_heap (lbl N) H E)
