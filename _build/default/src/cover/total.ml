  ...
  struct
    ...
    let rec total cid =
      begin match lookup cid with 
                                  | None -> false
                                  | Some _ -> true end(* call only on constants *);;
    ...
    let rec checkDynOrder =
      function 
               ...
               | (g_, vs_, n, occ)
                   -> checkDynOrderW (g_, Whnf.whnf vs_, n, occ)(* n > 0 *)(* Sun Jan  5 12:17:06 2003 -fp *)(* Functional calculus now checks this *)(* raise Error' (occ, ""Output coverage for clauses of order >= 3 not yet implemented"") *)
    and checkDynOrderW =
      function 
               | (g_, (I.Root _, s), n, occ) -> ()
               | (g_, (I.Pi (((I.Dec (_, v1_) as d1_), no_), v2_), s), n,
                  occ)
                   -> begin
                        checkDynOrder (g_, (v1_, s), n - 1, P.label occ);
                        checkDynOrder
                        ((I.Decl (g_, d1_)), (v2_, I.dot1 s), n, P.body occ)
                        end
               | (g_, (I.Pi ((d1_, maybe_), v2_), s), n, occ)
                   -> checkDynOrder
                      ((I.Decl (g_, d1_)), (v2_, I.dot1 s), n, P.body occ)(* static (= dependent) assumption --- consider only body *)(* dynamic (= non-dependent) assumption --- calculate dynamic order of V1 *)(* atomic subgoal *);;
    ...
    *);;
    let rec checkClause (g_, vs_, occ) =
      checkClauseW (g_, Whnf.whnf vs_, occ)
    and checkClauseW =
      function 
               | (g_, (I.Pi ((d1_, maybe_), v2_), s), occ)
                   -> let d1'_ = N.decEName (g_, I.decSub (d1_, s))
                        in checkClause
                           ((I.Decl (g_, d1'_)), (v2_, I.dot1 s), P.body occ)
               | (g_, (I.Pi (((I.Dec (_, v1_) as d1_), no_), v2_), s), occ)
                   -> let _ =
                        checkClause
                        ((I.Decl (g_, d1_)), (v2_, I.dot1 s), P.body occ)
                        in checkGoal (g_, (v1_, s), P.label occ)
               | (g_, (I.Root _, s), occ) -> ()(* clause head *)(* subgoal *)(* quantifier *)
    ...
    and checkGoalW (g_, (v_, s), occ) =
      let a = I.targetFam v_
        in let _ = begin
             ...
             in let _ = checkDynOrderW (g_, (v_, s), 2, occ)
                  in try Cover.checkOut (g_, (v_, s))
                     with 
                          | Cover.Error msg
                              -> raise
                                 ((Error'
    ...
    *);;
    let rec checkDefinite =
      function 
               | (a, mnil_) -> ()
               | (a, M.Mapp (M.Marg (plus_, _), ms'))
                   -> checkDefinite (a, ms')
               | (a, M.Mapp (M.Marg (minus_, _), ms'))
                   -> checkDefinite (a, ms')
                         ...
                         (begin
                          ...
                          with 
                               ...
                               | Some x
                                   -> (" but argument " ^ x) ^
                                        {| is indefinite (*)|}
                          end))(* Fri Apr  5 19:25:54 2002 -fp *)(* Note: filename and location are missing in this error message *);;
    (* checkOutCover [c1,...,cn] = ()
      ...
      function 
               ...
               | (I.Const c :: cs)
                   -> begin
                        ...
                        print ((N.qidToString (N.constQid c)) ^ " ") else ()
                        end;
                        begin
                          begin
                          ...
                          begin
                            try checkClause
                                (I.null_, (I.constType c, I.id), P.top)
                            with 
                                 | Error' (occ, msg) -> error (c, occ, msg);
                            checkOutCover cs
                            end
               ...
               | (I.Def d :: cs)
                   -> begin
                        begin
                        if (! Global.chatter) >= 4 then
                        print ((N.qidToString (N.constQid d)) ^ " ") else ()
                        end;
                        begin
                          begin
                          if (! Global.chatter) >= 6 then print "\n" else ()
                          ...
                          begin
                            try checkClause
                                (I.null_, (I.constType d, I.id), P.top)
                            with 
    ...
    let rec checkFam a =
      let _ = Cover.checkNoDef a
        in let _ =
             ...
             in let _ =
                  try begin
                        Timers.time Timers.terminate Reduces.checkFam a;begin
                        if (! Global.chatter) >= 4 then
                        print
                                                          ...
                                                          | Cover.Error msg
                                                              -> raise
                                                                 ((Cover.Error
                                                                   msg))
                                                     in ()
                                                     (* Ensuring that there is no bad interaction with type-level definitions *)(* a cannot be a type-level definition *)(* Checking termination *)(* Checking input coverage *)(* by termination invariant, there must be consistent mode for a *)(* must be defined and well-moded *)(* all arguments must be either input or output *)(* Checking output coverage *)(* all variables in output args must be free *);;
    end;;
(* functor Total *);;
