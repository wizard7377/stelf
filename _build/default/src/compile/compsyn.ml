...
module type COMPSYN = sig
                      ...
                      type comDec_ =
                        ...
                        | BDec of
                          (resGoal_ * IntSyn.sub_ * IntSyn.head_) list 
                        | PDec 
                      ...
                      type conDec_ = | SClause of resGoal_ 
                                     | Void 
...
module CompSyn(CompSyn__0: sig
  ...
  *);;
    ...
    type conDecDirect_ = | HeadGoals of compHead_ * conjunction_ 
                         | Null ;;
    (* Compiled constant declaration     *);;
    ...
    open!
      struct
        ...
        let sProgArray =
          (Array.array (maxCid + 1, Void) : conDec_ Array.array);;
    ...
    let rec sProgReset () = Array.modify (function 
                                                   | _ -> Void) sProgArray;;
    let detTableInsert = Table.insert detTable;;
    let rec detTableCheck cid =
      begin
      match Table.lookup detTable cid
      with 
  ...
  *);;
    let rec goalSub =
      function 
               ...
               | (Impl (d, a_, ha_, g), s)
                   -> (Impl
                       (resGoalSub (d, s), (IntSyn.EClo (a_, s)), ha_,
                        goalSub (g, IntSyn.dot1 s)))
               | (All (d_, g), s)
                   -> (All
                       (IntSyn.decSub (d_, s), goalSub (g, IntSyn.dot1 s)))
    and resGoalSub =
      function 
               ...
               | (Exists (d_, r), s)
                   -> (Exists
                       (IntSyn.decSub (d_, s), resGoalSub (r, IntSyn.dot1 s)));;
    (* resGoalSub (r, s) = r'
