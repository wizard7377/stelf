...
module type CPRINT = sig
                     ...
                     val
                       goalToString : string ->
                                      (IntSyn.dctx * CompSyn.goal_) ->
                                      string
                     val
                       clauseToString : string ->
                                        (IntSyn.dctx * CompSyn.resGoal_) ->
                                        string
                     val sProgToString : unit -> string
                     val dProgToString : CompSyn.dProg_ -> string
                     val
                       subgoalsToString : string ->
                                          (IntSyn.dctx * CompSyn.conjunction_) ->
                                          string
                     end;;
(* signature CPRINT *);;
  ...
  struct
      ...
      with 
           ...
           | (t, (g_, Impl (p, a_, _, g)))
               -> (((((t ^ "ASSUME  ") ^ (Print.expToString (g_, a_))) ^ "\n")
                     ...
                     (goalToString
                      t
                      ((IntSyn.Decl (g_, (IntSyn.Dec (None, a_)))), g)))
                    ^ "\n"
           | (t, (g_, All (d_, g)))
               -> let d'_ = Names.decLUName (g_, d_)
                    in ((((t ^ "ALL     ") ^
                            (Formatter.makestring_fmt
    ...
    and auxToString arg__3 arg__4 =
      ...
      with 
           ...
           | (t, (g_, UnifyEq (g'_, p1, n_, ga)))
               -> (((((t ^ "UNIFYEqn  ") ^
                        (Print.expToString (compose (g'_, g_), p1)))
                       ^ " = ")
    ...
    and clauseToString arg__5 arg__6 =
      ...
      with 
           ...
           | (t, (g_, Assign (p, ga)))
               -> (((t ^ "ASSIGN  ") ^
                      (try Print.expToString (g_, p) with 
                                                          | _ -> "<exc>"))
                     ^ "\n")
                    ^ (auxToString t (g_, ga))
           | (t, (g_, And (r, a_, g)))
               -> (clauseToString
                   t
                   ((IntSyn.Decl (g_, (IntSyn.Dec (None, a_)))), r)) ^
                    (goalToString t (g_, g))
           | (t, (g_, In (r, a_, g)))
               -> let d_ = Names.decEName (g_, (IntSyn.Dec (None, a_)))
                    in (((((clauseToString t ((IntSyn.Decl (g_, d_)), r)) ^ t)
                            ^ "META    ")
      ...
      with 
           ...
           | (t, (g_, Conjunct (goal_, a_, sg_)))
               -> ((t ^
                      (goalToString
                       t
                       ((IntSyn.Decl (g_, (IntSyn.Dec (None, a_)))), goal_)))
                     ^ " and ")
      ...
      function 
               | (c, SClause r)
                   -> let _ = Names.varReset IntSyn.null_
                        in let name = IntSyn.conDecName (IntSyn.sgnLookup c)
                             in let l = String.size name
                                  in (name ^
                                        (begin if l > 6 then ":\n" else ":"
                                         end))
                                       ^
    ...
    let rec sProgToString () =
      let (size, _) = IntSyn.sgnSize ()
        in let rec ts cid = begin
             if cid < size then
    ...
    let rec dProgToString =
      function 
               | DProg (null_, null_) -> ""
               | DProg
                   (IntSyn.Decl (g_, IntSyn.Dec (Some x, _)), IntSyn.Decl
                    (dPool, CompSyn.Dec (r, _, _)))
                   -> ((((dProgToString ((DProg (g_, dPool)))) ^
                        ...
                        ^ (clauseToString "\t" (g_, r))
               | DProg
                   (IntSyn.Decl (g_, IntSyn.Dec (Some x, a_)), IntSyn.Decl
                    (dPool, parameter_))
                   -> ((((dProgToString ((DProg (g_, dPool)))) ^
                           "\nParameter ")
                          ^ x)
