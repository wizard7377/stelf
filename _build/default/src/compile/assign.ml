...
module type ASSIGN = sig
     ...
     returns a list of residual equations that have been postponed *)
                     ...
                     val
                       assignable : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                                    IntSyn.cnstr_
                                    list
                                    option
                     ...
                     val
                       instance : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                                  bool
                     val
                       firstConstArg : (IntSyn.exp_ * IntSyn.sub_) ->
                                       IntSyn.cid
                                       option
                     end;;
(* signature ASSIGN *);;
...
module Assign(Assign__0: sig
  ...
  struct
    ...
    open!
      struct
        ...
        let rec assignExpW =
          function 
                       ...
                       -> begin
                          ...
                          with 
                               | (Const c1, Const c2) -> begin
                                   if c1 = c2 then
                                   assignSpine
                                   (g_, (s1_, s1), (s2_, s2), cnstr) else
                                   raise ((Assignment "Constant clash")) end
                               | (BVar k1, BVar k2) -> begin
                                   if k1 = k2 then
                                   assignSpine
                                   (g_, (s1_, s1), (s2_, s2), cnstr) else
                                   raise
                                   ((Assignment "Bound variable clash")) end
                               ...
                               | (Def d1, Def d2) -> begin
                                   ...
                                   (g_, Whnf.expandDef us1_,
                                    Whnf.expandDef us2_, cnstr)
                                   end
                               | (Def d1, _)
                                   -> assignExp
                                      (g_, Whnf.expandDef us1_, us2_, cnstr)
                               | (_, Def d2)
                                   -> assignExp
                                      (g_, us1_, Whnf.expandDef us2_, cnstr)
                               | (FgnConst (cs1, ConDec (n1, _, _, _, _, _)),
                                  FgnConst (cs2, ConDec (n2, _, _, _, _, _)))
                                   ...
                                   raise
                                   ((Assignment "Foreign Constant clash"))
                                   end
                               | (FgnConst
                   ...
                   | (g_, (Lam (d1_, u1_), s1), (Lam (d2_, u2_), s2), cnstr)
                       -> assignExp
                          (decl_ (g_, decSub (d1_, s1)), (u1_, dot1 s1),
                           (u2_, dot1 s2), cnstr)
                   | (g_, (u1_, s1), (Lam (d2_, u2_), s2), cnstr)
                       -> assignExp
                          (decl_ (g_, decSub (d2_, s2)),
                           (redex_
                            (eClo_ (u1_, shift),
                             app_ (root_ (bVar_ 1, nil_), nil_)),
                            dot1 s1),
                           (u2_, dot1 s2), cnstr)
                   | (g_, (Pi (((Dec (_, v1_) as d1_), _), u1_), s1),
                      (Pi (((Dec (_, v2_) as d2_), _), u2_), s2), cnstr)
                       -> let cnstr' =
                            assignExp (g_, (v1_, s1), (v2_, s2), cnstr)
                            in assignExp
                               (decl_ (g_, decSub (d1_, s1)), (u1_, dot1 s1),
                                (u2_, dot1 s2), cnstr')
                   | (g_, ((u_, s1) as us1_),
                      ((EVar (r2, _, _, _), s2) as us2_), cnstr)
                       -> begin
                            r2 := ((Some (eClo_ us1_)));cnstr
                            end
                   ...
                   | (g_, ((EVar (r, _, v_, cnstr_), s) as us1_), us2_,
                      cnstr) -> (eqn_ (g_, eClo_ us1_, eClo_ us2_) :: cnstr)
                   | (g_, ((EClo (u_, s'), s) as us1_), us2_, cnstr)
                       -> assignExp (g_, (u_, comp (s', s)), us2_, cnstr)
                   | (g_, ((FgnExp (_, fe), _) as us1_), us2_, cnstr)
                       -> (eqn_ (g_, eClo_ us1_, eClo_ us2_) :: cnstr)
                   | (g_, us1_, ((FgnExp (_, fe), _) as us2_), cnstr)
                       -> (eqn_ (g_, eClo_ us1_, eClo_ us2_) :: cnstr)
        and assignSpine =
          function 
                   | (g_, (nil_, _), (nil_, _), cnstr) -> cnstr
                   ...
                   | (g_, (App (u1_, s1_), s1), (App (u2_, s2_), s2), cnstr)
                       -> let cnstr' =
                            assignExp (g_, (u1_, s1), (u2_, s2), cnstr)
                            in assignSpine (g_, (s1_, s1), (s2_, s2), cnstr')
        ...
        let rec solveCnstr =
          function 
                   ...
                   | (Eqn (g_, u1_, u2_) :: cnstr_)
                       -> (Unify.unifiable (g_, (u1_, id), (u2_, id))) &&
                            (solveCnstr cnstr_);;
        let rec printSub =
          function 
        ...
        let rec unifyW =
          function 
                   | (g_, ((AVar (({ contents = None} as r)) as xs1_), s),
                      us2_) -> r := ((Some (eClo_ us2_)))
  ...
  *);;
    ...
    let rec instance (g_, us1_, us2_) =
      ...
      with 
           | Match.Match msg -> false;;
    (*
    fun assign(G, Us1, Us2) = assignExp(G, Us1, Us2, [])
    *);;
    let rec assignable (g_, us1_, uts2_) =
      try (Some (assignExp (g_, us1_, uts2_, [])))
      with 
           | Assignment msg -> None;;
    let rec firstConstArg
      ((IntSyn.Root ((IntSyn.Const c as h), s_) as a_), s) =
      let i = IntSyn.conDecImp (IntSyn.sgnLookup c)
        in let rec constExp (u_, s) = constExpW (Whnf.whnf (u_, s))
           and constExpW =
             function 
                      | (IntSyn.Lam (d_, u_), s) -> constExp (u_, s)
                      | (IntSyn.Root ((IntSyn.Const cid as h_), s_), s)
                          -> (Some cid)
                      | (_, _) -> None
             in let rec ithElem =
                  function 
                           | (k, (IntSyn.App (u_, s_), s)) -> begin
                               if k = i then constExp (u_, s) else
                               ithElem (k + 1, (s_, s)) end
                           | (k, (nil_, s)) -> None
                  in ithElem (0, (s_, s))
