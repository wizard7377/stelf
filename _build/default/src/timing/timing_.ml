  ...
  struct
    ...
    *);;
    let rec checkCPUAndGCTimer timer =
      let { usr; sys} = Compat.Timer.checkCPUTimer timer
        in let gc = Compat.Timer.checkGCTime timer in { usr; sys; gc} ;;
    let rec time (_, counters) ((f : 'a -> 'b)) ((x : 'a)) =
      let realTimer = Timer.startRealTimer ()
        in let cPUTimer_ = Timer.startCPUTimer ()
             in let result = try (Value (f x)) with 
                                                    | exn -> (Exception exn)
                  in let evalCPUTime = checkCPUAndGCTimer cPUTimer_
                       in let evalRealTime = Timer.checkRealTimer realTimer
                            in let (cPUTime_, realTime) = ! counters
                                 in let _ =
                                      counters :=
                                        (plus (cPUTime_, evalCPUTime),
                                         (Time.( + )
                                          (realTime, evalRealTime)))
                                      in begin
                                         match result
                                         with 
                                              | Value v -> v
    ...
    *);;
    ...
    let rec timesToString
      (name, (({ usr = t1; sys = t2; gc = t3} as cPUTime_), realTime)) =
    ...
    let rec toString (name, { contents = (cPUTime_, realTime)}) =
    ...
    let rec sumToString (name, centers) =
      let rec sumup =
        function 
                 | ([], (cPUTime_, realTime))
                     -> timesToString (name, (cPUTime_, realTime))
                 | (((_, { contents = (c_, r_)}) :: centers),
                    (cPUTime_, realTime))
                     -> sumup
                        (centers,
                         (plus (cPUTime_, c_), (Time.( + ) (realTime, r_))))
        in sumup (centers, (zero, Time.zeroTime));;
    end;;
(* local ... *);;
(* structure Timing *);;
