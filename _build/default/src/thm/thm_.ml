      ...
      struct
        ...
        let rec unique (((a, p_), r), a_) =
          let rec unique' =
            function 
                     | (I.Uni _, [], a_) -> a_
                     | (I.Pi (_, v_), (None :: p_), a_)
                         -> unique' (v_, p_, a_)
                     | (I.Pi (_, v_), (Some x :: p_), a_)
                         -> begin
                              List.app
                              (function 
                                        | x' -> begin
            ...
            in let rec skip =
                 function 
                          ...
                          | (k, I.Pi (_, v_), p_, a_)
                              -> skip (k - 1, v_, p_, a_)
                 in skip (I.constImp a, I.constType a, p_, a_);;
        ...
        let rec wfCallpats (l0_, c0_, r) =
          let rec makestring =
            ...
            in let rec exists' =
                 function 
                          | (x, [], _) -> false
                          | (x, (None :: l_), M.Mapp (_, mS))
                              -> exists' (x, l_, mS)
                          | (x, (Some y :: l_), M.Mapp
                             (M.Marg (mode, _), mS)) -> begin
                              if x = y then
                              begin
                              match mode
                              with 
                                   ...
                                   | _
                                       -> error
                                          (r,
                                           ((("Expected " ^ x) ^ " to have ")
                                              ^ (M.modeToString M.Plus))
                                             ^ " mode")
                              end else exists' (x, l_, mS) end
                 in let rec skip =
                      function 
                      ...
                      in let rec delete =
                           function 
                                    | (x, (((a, p_) as aP) :: c_)) -> begin
                                        ...
                                        skip
                                        (I.constImp a, x, p_,
                                         valOf (ModeTable.modeLookup a))
                                        then c_ else (aP :: delete (x, c_))
                                        end
                                    | (x, [])
                                        -> error
                                           (r,
                                            ("Variable " ^ x) ^
                                              " does not occur as argument")
                           in let rec wfCallpats' =
                                function 
                                         | ([], []) -> ()
                                         | ((x :: l_), c_)
                                             -> wfCallpats'
          ...
          and wfOrders =
            ...
            in let rec allModed =
                 ...
                 in begin
                      allModed c_;
                      begin
                        uniqueCallpats (c_, rs);wfOrder o_
                        end
        ...
        let rec argPos =
          function 
                   ...
                   | (x, (Some x' :: l_), n) -> begin
                       if x = x' then (Some n) else argPos (x, l_, n + 1) end;;
        let rec locate ((x :: vars), params, imp) =
          begin
          match argPos (x, params, imp + 1)
          with 
        ...
        and argOrderL =
          function 
                   | ([], p_, n) -> []
                   | ((o_ :: l_), p_, n)
                       -> (argOrder (o_, p_, n) :: argOrderL (l_, p_, n));;
        let rec argOrderMutual =
          function 
                   | ([], k, a_) -> a_
                   | ((p_ :: l_), k, a_)
                       -> argOrderMutual (l_, k, k (p_, a_));;
                            ...
                            (thmsLE,
                             ...
                             ((aP :: thmsLT),
                              function 
                                       | ((a, _), l_) -> (O.LT (a, l_)),
                              O.empty_))
                            in let o'_ = argOrder (o_, p_, I.constImp a)
                                 in let s'_ =
                                      O.install (a, (O.TDec (o'_, m'_)))
                                      in installOrder
                                         (o_, thmsLE, (aP :: thmsLT));;
        let rec installDecl (o_, L.Callpats l_) =
          begin
            installOrder (o_, l_, []);map (function 
                                                    | (a, _) -> a) l_
            end;;
        let rec installTerminates (L.TDecl T, rrs) =
          begin
            wf (T, rrs);installDecl T
            end;;
        ...
        let rec argROrder =
          function 
                   | (L.Varg l_, p_, n) -> (O.Arg (locate (l_, p_, n)))
                   | (L.Simul l_, p_, n)
                       -> (O.Simul (argROrderL (l_, p_, n)))
                   | (L.Lex l_, p_, n) -> (O.Lex (argROrderL (l_, p_, n)))
        and argROrderL =
          function 
                   | ([], p_, n) -> []
                   | ((o_ :: l_), p_, n)
                       -> (argROrder (o_, p_, n) :: argROrderL (l_, p_, n));;
        ...
        let rec installPredicate =
          function 
                   | (_, [], _) -> ()
                   | (L.RedOrder (pred_, o1_, o2_),
                      (((a, p_) as aP) :: thmsLE), thmsLT)
                       -> let m'_ =
                            argOrderMutual
                            (thmsLE,
                             function 
                             ...
                             ((aP :: thmsLT),
                              function 
                                       | ((a, _), l_) -> (O.LT (a, l_)),
                              O.empty_))
                            in let o1'_ = argROrder (o1_, p_, I.constImp a)
                                 in let o2'_ =
                                      argROrder (o2_, p_, I.constImp a)
                                      in let pr =
                                           argPredicate (pred_, o1'_, o2'_)
                                           in let s''_ =
                                                O.installROrder
                                                (a, (O.RDec (pr, m'_)))
                                                in installPredicate
        ...
        let rec installRDecl (r_, L.Callpats l_) =
          begin
            installPredicate (r_, l_, []);map (function 
                                                        | (a, _) -> a) l_
            end;;
          ...
          let rec makestring =
            ...
            in let rec exists' =
                 function 
                          ...
                          | (x, (Some y :: l_)) -> begin
                              if x = y then true else exists' (x, l_) end
                 in let rec delete =
                      function 
                               | (x, (((a, p_) as aP) :: c_)) -> begin
                                   if exists' (x, p_) then c_ else
                                   (aP :: delete (x, c_)) end
                               | (x, [])
                                   -> error
                                      (r,
        ...
        let rec wfred ((L.RedOrder (pred_, o_, o'_), L.Callpats c_), (r, rs))
          ...
          and wfOrders =
            ...
            in begin
                 uniqueCallpats (c_, rs);begin
                 if (wfOrder o_) = (wfOrder o'_) then () else
                 error
                 (r,
                  ("Reduction Order (" ^
                     ((P.ROrderToString ((L.RedOrder (pred_, o_, o'_))))))
                    ^ ") requires both orders to be of the same type.")
                 end
                 end;;
        let rec installReduces (L.RDecl (r_, c_), rrs) =
          begin
            wfred ((r_, c_), rrs);installRDecl (r_, c_)
            end;;
        let rec uninstallReduces cid = O.uninstallROrder cid;;
        let rec installTabled (L.TabledDecl cid) =
          TabledSyn.installTabled cid;;
