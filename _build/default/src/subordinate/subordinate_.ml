...
module Subordinate(Subordinate__0: sig
  ...
  struct
    ...
    open!
      struct
        ...
        let memoClear = function 
                                 | () -> MemoTable.clear memoTable;;
        let memoCounter = ref 0;;
        let rec appReachable f b =
          let rec rch (b, visited) = begin
            if IntSet.member (b, visited) then visited else
            begin
              f b;IntSet.foldl rch (IntSet.insert (b, visited)) (adjNodes b)
              end
            end in begin
                     rch (b, IntSet.empty);()
                     end;;
        ...
        let rec reach (b, a, visited) =
          let rec rch (b, visited) = begin
            if IntSet.member (b, visited) then visited else
            let adj = adjNodes b in begin
              if IntSet.member (a, adj) then raise Reachable else
              IntSet.foldl rch (IntSet.insert (b, visited)) adj end
            end in rch (b, visited);;
            ...
            (function 
                      | ()
                          -> ((begin
                               ...
                               end) ^ (Names.qidToString (Names.constQid a)))
                               ^ "\n")
            in fInsert (a, frozen);;
        let rec checkFreeze (c, a) = begin
          if fGet a then
          raise
          ((Error
        ...
        let rec freeze l_ =
          let _ = freezeList := IntSet.empty
            in let l'_ = map expandFamilyAbbrevs l_
                 in let _ =
                      List.app
                      (function 
                                | a
                                    -> appReachable
                                       ...
                                       a)
                      l'_
                      in let cids = IntSet.foldl ( :: ) [] (! freezeList)
                           in cids;;
        let rec frozen l_ =
          let l'_ = map expandFamilyAbbrevs l_
        ...
        let rec computeBelow (a, b) =
          try begin
                reachable (b, a);
                begin
                  memoInsert ((b, a), (false, ! memoCounter));false
                  end
          ...
          with 
               | Reachable
                   -> begin
                        memoInsert ((b, a), (true, ! memoCounter));true
                        end;;
        let rec below (a, b) =
          begin
          match memoLookup (b, a)
          with 
        ...
        let rec addSubord (a, b) = begin
          if below (a, b) then () else begin
          if fGet b then
          raise
          ((Error
            ((("Freezing violation: " ^
          ...
          function 
                   | b -> begin
                       if List.exists (function 
                                                | a -> belowEq (a, b)) a's
                       then aboveList := ((b :: ! aboveList)) else () end;;
        let rec thaw a's =
          let a's' = map expandFamilyAbbrevs a's
            in let _ = aboveList := []
                 in let _ =
                      Table.app
                      (function 
                                | (b, _) -> addIfBelowEq a's' b)
                      soGraph
                      in let _ =
                           List.app
                           (function 
                                     | b -> fSet (b, false))
                           (! aboveList) in ! aboveList;;
        ...
        let rec insertNewDef (b, a) =
          begin
          match Table.lookup defGraph a
          with 
               | None
                   -> Table.insert
                      defGraph
                      (a, IntSet.insert (b, IntSet.empty))
        ...
        let rec reset () =
          begin
            ...
            begin
              Table.clear fTable;
              begin
                MemoTable.clear memoTable;Table.clear defGraph
                end
        ...
        and installTypeN' =
          function 
                   | (I.Pi (((I.Dec (_, v1_) as d_), _), v2_), a)
                       -> begin
                            addSubord (I.targetFam v1_, a);
                            begin
                              installTypeN v1_;installTypeN' (v2_, a)
                              end
                            
                            end
                   | ((I.Root (I.Def _, _) as v_), a)
                       -> let v'_ =
                            Whnf.normalize (Whnf.expandDef (v_, I.id))
                            in installTypeN' (v'_, a)
                   | (I.Root _, _) -> ()
        and installTypeN v_ = installTypeN' (v_, I.targetFam v_);;
        let rec installKindN =
          function 
                   | (I.Uni l_, a) -> ()
                   | (I.Pi ((I.Dec (_, v1_), p_), v2_), a)
                       -> begin
                            addSubord (I.targetFam v1_, a);
                            begin
                              installTypeN v1_;installKindN (v2_, a)
                              end
                            
                            end;;
        let rec install c =
          let v_ = I.constType c
            in begin
               match I.targetFamOpt v_
               with 
                    | None -> begin
                                insertNewFam c;installKindN (v_, c)
                                end
                    | Some a
                        -> begin
                             begin
                             match IntSyn.sgnLookup c
                             with 
                                  | IntSyn.ConDec _ -> checkFreeze (c, a)
                                  | IntSyn.SkoDec _ -> checkFreeze (c, a)
                                  | _ -> ()
                             end;installTypeN' (v_, a)
                             end
               end;;
        ...
        let rec installSome =
          function 
                   ...
                   | I.Decl (g_, d_)
                       -> begin
                            installSome g_;installDec d_
                            end;;
        let rec installBlock b =
          let I.BlockDec (_, _, g_, ds_) = I.sgnLookup b
            in begin
                 installSome g_;List.app (function 
                                                   | d_ -> installDec d_) ds_
                 end;;
        let rec checkBelow (a, b) = begin
          ...
          function 
                   | (I.Pi (((I.Dec (_, v1_) as d_), _), v2_), a)
                       -> begin
                            ...
                            begin
                              respectsTypeN v1_;respectsTypeN' (v2_, a)
                              end
                   ...
                   | ((I.Root (I.Def _, _) as v_), a)
                       -> let v'_ =
                            Whnf.normalize (Whnf.expandDef (v_, I.id))
                            in respectsTypeN' (v'_, a)
                   | (I.Root _, _) -> ()
        and respectsTypeN v_ = respectsTypeN' (v_, I.targetFam v_);;
        let rec respects (g_, (v_, s)) =
          respectsTypeN (Whnf.normalize (v_, s));;
          ...
          function 
                   ...
                   | (I.Decl (g'_, (I.Dec (name, v_) as d_)), a)
                       -> let w' = weaken (g'_, a) in begin
                            if belowEq (I.targetFam v_, a) then I.dot1 w'
                            else I.comp (w', I.shift) end;;
        open!
          struct
            let declared = ref 0;;
            let defined = ref 0;;
        ...
        let rec analyze =
          function 
                   ...
                   | I.ConDef (_, _, _, _, _, l_, ancestors)
                       -> begin
        ...
        let rec showDef () =
          let _ = reset ()
            in let _ = I.sgnApp (function 
                                          | c -> analyze (I.sgnLookup c))
                 in let _ =
                      print
                      (("Declared: " ^ (Int.toString (! declared))) ^ "\n")
                      in let _ =
                           print
                           (("Defined : " ^ (Int.toString (! defined))) ^
                              "\n")
                           in let _ =
                                print
                                (("Abbrevs : " ^ (Int.toString (! abbrev))) ^
                                   "\n")
                                in let _ =
                                     print
                                     (("Other   : " ^
                                     ...
                                     in let _ =
                                          print
                                          (("Max definition height: " ^
                                              (Int.toString (! maxHeight)))
                                             ^ "\n")
                                          in let _ =
                                               ArraySlice.appi
                                               (function 
