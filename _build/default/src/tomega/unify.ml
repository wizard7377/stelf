    ...
    open!
      struct
        ...
        and unifyForN =
          function 
                   | (psi_, true_, true_) -> ()
                   | (psi_, T.Ex ((d1_, _), f1_), T.Ex ((d2_, _), f2_))
                       -> begin
                            unifyDec (psi_, (T.UDec d1_), (T.UDec d2_));
                            unifyFor
                            ((I.Decl (psi_, (T.UDec d1_))), f1_, f2_)
                            end
                   | (psi_, T.All ((d1_, _), f1_), T.All ((d2_, _), f2_))
                       -> begin
        ...
        and unifyDec =
          function 
                   | (psi_, T.UDec d1_, T.UDec d2_) -> begin
                       ...
                       else raise ((Unify "Declaration mismatch")) end
                   | (psi_, T.PDec (_, f1_), T.PDec (_, f2_))
