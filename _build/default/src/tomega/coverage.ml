    ...
    open!
      struct
        ...
        let rec chatter chlev f = begin
          if (! Global.chatter) >= chlev then print ("[coverage] " ^ (f ()))
          else () end;;
        let rec purifyFor =
          function 
        ...
        let rec purifyCtx =
          function 
                   ...
                   | (T.Dot (T.Prg p_, t), I.Decl
                      (psi_, T.PDec (_, T.All _, _, _)))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in (t', psi'_, (T.Dot (T.undef_, s')))
                   | (T.Dot (T.Prg (T.Var _), t), I.Decl
                      (psi_, T.PDec (_, _, _, _)))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in (t', psi'_, (T.Dot (T.undef_, s')))
                   | (T.Dot (T.Prg (T.Const _), t), I.Decl
                      (psi_, T.PDec (_, _, _, _)))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in (t', psi'_, (T.Dot (T.undef_, s')))
                   | (T.Dot (T.Prg (T.PairPrg (_, _)), t), I.Decl
                      (psi_, T.PDec (_, _, _, _)))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in (t', psi'_, (T.Dot (T.undef_, s')))
                   | (T.Dot (T.Prg p_, t), I.Decl
                      (psi_, T.PDec (_, f_, _, _)))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in let (t'', psi''_, s'') =
                                 purifyFor
                                 ((p_, t'), (psi'_, T.forSub (f_, s')), s')
                                 in (t'', psi''_, (T.Dot (T.undef_, s'')))
                   | (T.Dot (f_, t), I.Decl (psi_, T.UDec d_))
                       -> let (t', psi'_, s') = purifyCtx (t, psi_)
                            in ((T.Dot (f_, t')),
                                (I.Decl
                                 (psi'_,
                                  (T.UDec (I.decSub (d_, T.coerceSub s'))))),
                                T.dot1 s');;
        let rec purify (psi0_, t, psi_) =
          let (t', psi'_, s') = purifyCtx (t, psi_)
            in let _ = TomegaTypeCheck.checkSub (psi0_, t', psi'_)
                 in (psi0_, t', psi'_);;
        let rec coverageCheckPrg =
          function 
                   | (w_, psi_, T.Lam (d_, p_))
                       -> coverageCheckPrg (w_, (I.Decl (psi_, d_)), p_)
                   | (w_, psi_, T.New p_) -> coverageCheckPrg (w_, psi_, p_)
                   | (w_, psi_, T.PairExp (u_, p_))
                       -> coverageCheckPrg (w_, psi_, p_)
                   | (w_, psi_, T.PairBlock (b_, p_))
                       -> coverageCheckPrg (w_, psi_, p_)
                   | (w_, psi_, T.PairPrg (p1_, p2_))
                       -> begin
                   ...
                   | (w_, psi_, (T.Let (d_, p1_, p2_) as p_))
                       -> begin
                            coverageCheckPrg (w_, psi_, p1_);
                            coverageCheckPrg (w_, (I.Decl (psi_, d_)), p2_)
                            end
                   | (w_, psi_, T.Redex (p_, s_))
                       -> coverageCheckSpine (w_, psi_, s_)
        and coverageCheckSpine =
          function 
                   | (w_, psi_, nil_) -> ()
                   | (w_, psi_, T.AppExp (u_, s_))
                       -> coverageCheckSpine (w_, psi_, s_)
                   | (w_, psi_, T.AppBlock (b_, s_))
                       -> coverageCheckSpine (w_, psi_, s_)
                   | (w_, psi_, T.AppPrg (p_, s_))
                       -> begin
                            coverageCheckPrg (w_, psi_, p_);
                            coverageCheckSpine (w_, psi_, s_)
                            end
        and coverageCheckCases =
          function 
                   | (w_, psi_, [], []) -> ()
                   | (w_, psi_, [], cs_)
                       -> let _ =
                            ...
                            (function 
                                      | ()
                                          -> (Int.toString (List.length cs_))
                                               ^ " cases to be checked\n")
                            in let (((_, _, psi'_) :: _) as cs'_) =
                                 map purify cs_
                                 in let cs''_ =
                                      map
                                      (function 
                                                | (psi0_, t, _)
                                                    -> (T.coerceCtx psi0_,
                                                        T.coerceSub t))
                                      cs'_
                                      in Cover.coverageCheckCases
                                         (w_, cs''_, T.coerceCtx psi'_)
                   | (w_, psi_, ((psi'_, t, p_) :: omega_), cs_)
                       -> begin
                            coverageCheckPrg (w_, psi'_, p_);
                            coverageCheckCases
                            (w_, psi_, omega_, ((psi'_, t, psi_) :: cs_))
                            end;;
        end;;
    (* chatter chlev f = ()
