...
module type TOMEGAABSTRACT = sig
                             ...
                             val
                               raiseFor : (IntSyn.dec_
                                           IntSyn.ctx_ *
                                           (Tomega.for_ * IntSyn.sub_)) ->
                                          Tomega.for_
                             val
                               raisePrg : (IntSyn.dec_
                                           IntSyn.ctx_ *
                                           Tomega.prg_ *
                                           Tomega.for_) ->
                                          Tomega.prg_
...
module TomegaAbstract(TomegaAbstract__0: sig
  ...
  struct
      ...
      struct
        ...
        let rec shiftCtx =
          function 
                   | (null_, t) -> (I.null_, t)
                   | (I.Decl (g_, d_), t)
                       -> let (g'_, t') = shiftCtx (g_, t)
                            in ((I.Decl (g'_, I.decSub (d_, t'))), I.dot1 t');;
        let rec dotn =
          function 
                   | (t, 0) -> t
          ...
          function 
                   ...
                   | (I.Dot (I.Idx _, t), l, (n, s_))
                       -> let t' = I.comp (t, I.invShift)
                            in strengthenToSpine
                               (t', l - 1,
                                (n + 1,
                                 (I.App ((I.Root ((I.BVar n), I.nil_)), s_))))
                   | (I.Dot (undef_, t), l, (n, s_))
                       -> strengthenToSpine (t, l - 1, (n + 1, s_))
                   | (I.Shift k, l, (n, s_))
                       -> strengthenToSpine
                          ((I.Dot ((I.Idx (k + 1)), (I.Shift (k + 1)))), l,
                           (n, s_));;
        let rec raiseFor =
          function 
                   | (b'_, (true_, t)) -> T.true_
                   | (b'_, (T.And (f1_, f2_), t))
                       -> let f1'_ = raiseFor (b'_, (f1_, t))
                            in let f2'_ = raiseFor (b'_, (f2_, t))
                                 in (T.And (f1'_, f2'_))
                   | (b'_, (T.Ex ((I.Dec (x, v_), q_), f_), t))
                       -> let w = S.weaken (b'_, I.targetFam v_)
                            in let iw = Whnf.invert w
                                 in let b''_ = Whnf.strengthen (iw, b'_)
                                      in let v'_ =
                                           A.raiseType
                                           (b''_,
                                            (I.EClo (v_, I.comp (t, iw))))
                                           in let (b'''_, _) =
                                                shiftCtx (b'_, I.shift)
                                                in let t'' =
                                                     dotn
                                                     (I.shift,
                                                      I.ctxLength b'_)
        ...
        let rec raisePrg =
          function 
                   ...
                   | (g_, (T.PairPrg (p1_, p2_), t), T.And (f1_, f2_))
                       -> let p1'_ = raisePrg (g_, (p1_, t), f1_)
                            in let p2'_ = raisePrg (g_, (p2_, t), f2_)
                                 in (T.PairPrg (p1'_, p2'_))
                   | (g_, (T.PairExp (u_, p_), t), T.Ex
                      ((I.Dec (_, v_), _), f_))
                       -> let w = S.weaken (g_, I.targetFam v_)
                            in let iw = Whnf.invert w
                                 in let g'_ = Whnf.strengthen (iw, g_)
                                      in let u'_ =
                                           A.raiseTerm
                                           (g'_,
                                            (I.EClo (u_, I.comp (t, iw))))
                                           in let p'_ =
                                                raisePrg (g_, (p_, t), f_)
                                                in (T.PairExp (u'_, p'_));;
        let rec raiseP (g_, p_, f_) =
          let (g'_, s) = T.deblockify g_
            in let f'_ = T.forSub (f_, s)
                 in let p''_ = raisePrg (g'_, (p_, T.coerceSub s), f'_)
                      in p''_;;
        let rec raiseF (g_, (f_, t)) =
          let (g'_, s) = T.deblockify g_
            in let f'_ = raiseFor (g'_, (f_, I.comp (t, T.coerceSub s)))
                 in f'_;;
        end;;
    (* dotn (t, n) = t'
    ...
    *);;
    ...
    let raisePrg = function 
                            | (g_, p_, f_) -> raisePrg (g_, (p_, I.id), f_);;
    let raiseP = raiseP;;
    let raiseFor = raiseFor;;
    let raiseF = raiseF;;
    end;;
(* functor TomegaAbstract *);;
# 1 "src/tomega/abstract.sml.ml"
