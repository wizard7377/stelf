  ...
  struct
    ...
    open!
      struct
          ...
          function 
                   ...
                   | ((C.Impl (r, a_, ha_, g), s), C.DProg (g_, dPool), sc)
                       -> let (I.Dec (Some x, _) as d'_) =
                            ...
                            in let _ = T.signal (g_, (T.IntroHyp (ha_, d'_)))
                                 in solve'
                                    ((g, I.dot1 s),
                                     (C.DProg
                                      ((I.Decl (g_, d'_)),
                   ...
                   | ((C.All (d_, g), s), C.DProg (g_, dPool), sc)
                       -> let (I.Dec (Some x, v_) as d'_) =
                            N.decUName (g_, I.decSub (d_, s))
                            in let ha_ = I.targetHead v_
                                 in let _ =
                                      T.signal (g_, (T.IntroParm (ha_, d'_)))
                                      in solve'
                                         ((g, I.dot1 s),
                                          (C.DProg
                                           ((I.Decl (g_, d'_)),
        ...
        and rSolve =
          function 
                   | (ps', (C.Eq q_, s), C.DProg (g_, dPool), hcHa_, sc)
                       -> begin
                            T.signal
                            (g_,
                             (T.Unify
                              (hcHa_, (I.EClo (q_, s)), (I.EClo ps'))));
                            begin
                            match Unify.unifiable' (g_, (q_, s), ps')
                            with 
                                 | None
                                     -> begin
                                          T.signal (g_, (T.Resolved hcHa_));
                                          begin
                                            sc I.nil_;true
                                            end
                                 ...
                                 | Some msg
                                     -> begin
                                          T.signal
                                          (g_, (T.FailUnify (hcHa_, msg)));
                                          false
                                          end
                            end
                            end
                   | (ps', (C.Assign (q_, eqns), s),
                      (C.DProg (g_, dPool) as dp), hcHa_, sc)
                       -> begin
                          match Assign.assignable (g_, ps', (q_, s))
                          with 
                               | Some cnstr
                                   -> aSolve
                                      ((eqns, s), dp, hcHa_, cnstr,
                                       function 
                                                | () -> sc I.nil_)
                               | None -> false
                          end
                   | (ps', (C.And (r, a_, g), s),
                      (C.DProg (g_, dPool) as dp), hcHa_, sc)
                       -> let x_ = I.newEVar (g_, (I.EClo (a_, s)))
                            in rSolve
                               (ps', (r, (I.Dot ((I.Exp x_), s))), dp, hcHa_,
                                function 
                                         | s_
                                             -> begin
                                                  T.signal
        ...
        and aSolve =
          function 
                   | ((trivial_, s), (C.DProg (g_, dPool) as dp), hcHa_,
                      cnstr, sc) -> begin
                   ...
                   | ((C.UnifyEq (g'_, e1, n_, eqns), s),
                      (C.DProg (g_, dPool) as dp), hcHa_, cnstr, sc)
        ...
        and matchAtom
          ...
          let tag = T.tagGoal ()
            in let _ = T.signal (g_, (T.SolveGoal (tag, ha_, (I.EClo ps'))))
                 in let deterministic = C.detTableCheck (cidFromHead ha_)
