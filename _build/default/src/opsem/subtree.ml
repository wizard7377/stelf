...
module MemoTable(MemoTable__0: sig
  ...
  struct
      ...
      let rec memb =
        function 
                 ...
                 | (x, (((y, e_) :: l_) as h_)) -> begin
                     if x = y then (Some (y, e_)) else memb (x, l_) end
        in memb (x, ! l_);;
    let rec insertList (e_, l_) = begin
                                    l_ := ((e_ :: ! l_));l_
                                    end;;
    (* ctxToEVarSub D = s
    ...
    *);;
    ...
    type retrieval_ = | Variant of IntSyn.exp_ 
                      | NotCompatible ;;
    type compSub_ =
      | SplitSub of
        (ctx * normalSubsts) *
      ...
      | VariantSub of ctx * (normalSubsts(* rho2 *)(* normalSubsts * *)) 
      | NoCompatibleSub ;;
    (* Index array
    ...
    open!
      struct
        ...
        let rec compatible' ((d_t_, T), (d_u_, u_), ds_, rho_t, rho_u) =
          let rec genNVar ((rho_t, T), (rho_u, u_)) =
            ...
            in let rec genRoot =
                 function 
                          | (depth, (I.Root ((I.Const k as h1_), s1_) as t_),
                             (I.Root (I.Const k', s2_) as u_)) -> begin
                              if k = k' then
                              let s'_ = genSpine (depth, s1_, s2_)
                                in (I.Root (h1_, s'_))
                              else genNVar ((rho_t, T), (rho_u, u_)) end
                          ...
                          | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
                             (I.Root (I.BVar k', s2_) as u_)) -> begin
                              ...
                              with 
                                   | DifferentSpines
                                       -> genNVar ((rho_t, T), (rho_u, u_))
                              else genNVar ((rho_t, T), (rho_u, u_)) end end
                          | (d, (I.Root ((I.BVar k as h1_), s1_) as t_),
               ...
               and genSpine =
                 ...
                 in let e_ = genExp (0, T, u_) in (Variant e_);;
        let rec compatible =
          function 
                   | ((d_t_, (I.Root (h1_, s1_) as t_)),
                      (d_u_, (I.Root (h2_, s2_) as u_)), ds_, rho_t, rho_u)
                       -> begin
        ...
        let rec findAllCandidates (g_r_, children, ds_) =
          let rec findAllCands =
            function 
                     | (g_r_, [], (d_u_, sub_u), vList_, sList_)
                         -> (vList_, sList_)
                     | (g_r_, (x :: l_), (d_u_, sub_u), vList_, sList_)
                         -> begin
                            match compChild (! x, (d_u_, sub_u))
                            with 
                                 | NoCompatibleSub
                                     -> findAllCands
                                        (g_r_, l_, (d_u_, sub_u), vList_,
                                         sList_)
          ...
          (function 
                    | (n2, t2)
                        -> S.exists
                           sigma
                           (function 
                                     | (_, t) -> eqTerm (t2, (t, rho1))));;
        let rec insert (nref_, (d_u_, nsub_u), gr_) =
          let rec insert' =
            function 
                     | ((Leaf ((d_, _), gRlistRef_) as n_), (d_u_, nsub_u),
                        (((evarl, l), g_r_, eqn, answRef, stage, status) as
                          gr_))
                         -> begin
                            match compatibleCtx ((g_r_, eqn), ! gRlistRef_)
                            with 
                                 | None -> begin
                                     if
                                     (! TableParam.divHeuristic) &&
                                       (divergingCtx
                                        (stage, g_r_, gRlistRef_))
                                     then
                                     (function 
                                               | ()
                                                   -> begin
                                 ...
                                 | Some
                                     ((evarl', glength_), answRef', status')
                                     -> (function 
                                                  | () -> (),
                                         (T.RepeatedEntry
                                          ((I.id, I.id), answRef', status')))
                            end
                     | ((Node ((d_, sub), children) as n_), (d_u_, nsub_u),
                        ((l, g_r_, eqn, answRef, stage, status) as gr_))
                         -> let (variantCand_, splitCand_) =
                              findAllCandidates
                              (g_r_, children, (d_u_, nsub_u))
            ...
            in insert' (! nref_, (d_u_, nsub_u), gr_);;
                 ...
                 in let dAEVars_ = compose (dEVars_, dAVars_)
                      in let d_ = emptyCtx ()
                           in let n = I.ctxLength g_
                                in let _ =
                                     makeCtx (n + 1, dAEVars_, (d_ : ctx))
                                     in let l = I.ctxLength dAEVars_
                                          in let _ =
                                               S.insert nsub_goal (1, u_)
                                               in let result =
                                                    insert
                                                    (tree_, (d_, nsub_goal),
                                                     ((l, n + 1), g_, eqn,
      ...
      let rec memberCtx' =
        function 
                 ...
                 | ((g_, v_), I.Decl (g'_, (I.Dec (_, v'_) as d'_)), n)
                     -> begin
                     if Conv.conv ((v_, I.id), (v'_, (I.Shift n))) then
                     (Some d'_) else memberCtx' ((g_, v_), g'_, n + 1) end
        in memberCtx' ((g_, v_), g'_, 1);;
    end;;
(*! sharing Print.IntSyn = IntSyn'!*);;
(* local *);;
(* functor MemoTable *);;
# 1 "src/opsem/subtree.sml.ml"
