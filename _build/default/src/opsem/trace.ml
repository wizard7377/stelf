...
module type TRACE = sig
                    ...
                    type event_ =
                      ...
                      | Subgoal of
                        (IntSyn.head_ * IntSyn.head_) * (unit -> int) 
                      | SolveGoal of goalTag * IntSyn.head_ * IntSyn.exp_ 
                      | SucceedGoal of
                        goalTag * (IntSyn.head_ * IntSyn.head_) * IntSyn.exp_ 
                      | CommitGoal of
                        goalTag * (IntSyn.head_ * IntSyn.head_) * IntSyn.exp_ 
                      | RetryGoal of
                        goalTag * (IntSyn.head_ * IntSyn.head_) * IntSyn.exp_ 
  ...
  struct
      ...
      function 
               ...
               | I.Decl (g_, d_)
                   -> begin
                        printCtx g_;
                        begin
                          newline ();print (decToString (g_, d_))
                          end
                        
                        end;;
    let rec evarsToString xnames_ =
      let inst = P.evarInstToString xnames_
        in let constrOpt = P.evarCnstrsToStringOpt xnames_
             in begin
                match constrOpt
                with 
                     | None -> inst
    ...
    let rec varsToEVarInst =
      function 
               ...
               | (name :: names)
                   -> begin
                      match N.getEVarOpt name
                      with 
                           | None
                               -> begin
                                    print
                                    (("Trace warning: ignoring unknown variable "
                                        ^ name)
                                       ^ "\n");
    ...
    let rec setDetail =
      function 
               | None
                   -> print "Trace warning: detail is not a valid integer\n"
               | Some n -> begin
                   if 0 <= n then detail := n else
    ...
    let rec setEVarInst xs_ =
      currentEVarInst :=
        (List.map (function 
                            | x_ -> (x_, N.evarName (I.null_, x_))) xs_);;
    ...
    let rec breakAction g_ =
      let _ = print " "
        in let line = Compat.inputLine97 TextIO.stdIn
             in begin
                match String.sub (line, 0)
                with 
                     | '\n' -> ()
                     | 'n' -> breakTSpec := All
                     | 'r' -> breakTSpec := None
                     | 's'
                         -> setWatchForTag
                            (Int.fromString (String.extract (line, 1, None)))
                     | 't'
                         -> begin
                              traceTSpec := All;
                              begin
                                print "% Now tracing all";breakAction g_
                                end
                     ...
                     | 'u'
                         -> begin
                              traceTSpec := None;
                              begin
                                print "% Now tracing none";breakAction g_
                                end
                     ...
                     | 'v' -> begin
                                printVarstring line;breakAction g_
                                end
                     | '?' -> begin
                                printHelp ();breakAction g_
                                end
                     | _
                         -> begin
    ...
    let rec init () =
      begin
        initTrace (! traceSpec);begin
                                  initBreak (! breakSpec);initTag ()
                                  end
        end;;
    type event_ =
      | IntroHyp of IntSyn.head_ * IntSyn.dec_ 
    ...
    let rec eventToString =
      function 
               ...
               | (g_, DischargeHyp (_, I.Dec (Some x, _)))
                   -> "% Discharging hypothesis " ^ x
               | (g_, IntroParm (_, d_))
                   -> "% Introducing parameter\n" ^ (decToString (g_, d_))
               | (g_, DischargeParm (_, I.Dec (Some x, _)))
               ...
               | (g_, FailUnify ((hc_, ha_), msg))
                   -> (("% Unification failed with clause " ^
                          (headToString (g_, hc_)))
                         ^ ":\n")
                        ^ msg;;
    ...
    let rec monitorEvent =
      function 
               ...
               | (cids, SucceedGoal (_, (hc_, ha_), _))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, CommitGoal (_, (hc_, ha_), _))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, RetryGoal (_, (hc_, ha_), _))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, FailGoal (_, h_, _)) -> monitorHead (cids, h_)
               | (cids, Resolved (hc_, ha_))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, Subgoal ((hc_, ha_), _))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, Unify ((hc_, ha_), _, _))
                   -> monitorHeads (cids, (hc_, ha_))
               | (cids, FailUnify ((hc_, ha_), _))
                   -> monitorHeads (cids, (hc_, ha_));;
               ...
               | (g_, Unify (_, q_, p_))
                   -> setEVarInst
                      (Abstract.collectEVars
                       (g_, (p_, I.id),
                        Abstract.collectEVars (g_, (q_, I.id), [])))
    ...
    let rec skipping () =
      begin match ! watchForTag with 
                                     | None -> false
                                     | Some _ -> true end;;
    let rec signal (g_, e) = begin
      if monitorDetail e then begin
      if skipping () then begin
      if watchFor e then begin
                           watchForTag := None;signal (g_, e)
                           end
      else begin
             monitorTrace (! traceTSpec, g_, e);()
             end
      end else begin
      if monitorBreak (! breakTSpec, g_, e) then () else
      begin
        monitorTrace (! traceTSpec, g_, e);()
        end
      end (* stops, continues after input *) end else () end
