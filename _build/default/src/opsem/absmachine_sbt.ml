# 1 "src/opsem/absmachine_sbt.sig.ml"
open! Basis;;
(* Abstract Machine *);;
  ...
  struct
    ...
    open!
      struct
        ...
        let rec invShiftN (n, s) = begin
          if n = 0 then I.comp (I.invShift, s) else
          I.comp (I.invShift, invShiftN (n - 1, s)) end;;
        let rec raiseType =
          function 
                   | (null_, v_) -> v_
                   | (I.Decl (g_, d_), v_)
        ...
        let rec ctxToEVarSub =
          function 
                   ...
                   | (gglobal_, I.Decl (g_, I.Dec (_, a_)), s)
                       -> let s' = ctxToEVarSub (gglobal_, g_, s)
                            in let x_ =
                                 I.newEVar (gglobal_, (I.EClo (a_, s')))
                                 in (I.Dot ((I.Exp x_), s'))
                   | (gglobal_, I.Decl (g_, I.ADec (_, d)), s)
                       -> let x_ = I.newAVar ()
                            in (I.Dot
                                ((I.Exp ((I.EClo (x_, (I.Shift (- d)))))),
                                 ctxToEVarSub (gglobal_, g_, s)));;
        let rec solve' =
          function 
        ...
        and matchSig
          ...
          let rec mSig =
            ...
            in mSig (Index.lookup (cidFromHead ha_))
        and matchIndexSig
          (((I.Root (ha_, s_), s) as ps'), (C.DProg (g_, dPool) as dp), sc) =
          SubTree.matchSig
          (cidFromHead ha_, g_, ps',
           function 
                    | ((conjGoals_, s), clauseName)
                        -> sSolve
                           ((conjGoals_, s), dp,
                            function 
                                     | s_ -> sc (((C.Pc clauseName) :: s_))))
        and matchAtom
          (((I.Root (ha_, s_), s) as ps'), (C.DProg (g_, dPool) as dp), sc) =
          let rec matchDProg =
            function 
...
*);;
  ...
  *);;
    ...
    let rec solve args =
      begin
      match ! CompSyn.optimize
      with 
           | no_ -> begin
                      mSig := matchSig;solve' args
                      end
           | linearHeads_ -> begin
                               mSig := matchSig;solve' args
                               end
           | indexing_ -> begin
                            mSig := matchIndexSig;solve' args
                            end
      end;;
    end;;
...
# 1 "src/opsem/absmachine_sbt.sml.ml"
