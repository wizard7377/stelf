    ...
    *);;
    ...
    let rec ctxToAVarSub =
      function 
               ...
               | (I.Decl (g_, I.Dec (_, a_)), s)
                   -> let x_ = I.newEVar (I.null_, a_)
                        in (I.Dot ((I.Exp x_), ctxToAVarSub (g_, s)))
 ...
 *);;
    let rec solveEqn =
      function 
               ...
               | ((T.Unify (g'_, e1, n_, eqns), s), g_)
                   -> let g''_ = append (g'_, g_)
                        in let s' = shift (g''_, s)
                             in (Assign.unifiable (g''_, (n_, s'), (e1, s')))
                                  && (solveEqn ((eqns, s), g_))
                             (* G, G' |- s' : D, G, G' *)(* . |- s : D *)(* D, G, G' |- e1 and D, G, G' |- N and D, G |- eqns *);;
    let rec unifySub' (g_, s1, s2) =
      try begin
            Unify.unifySub (g_, s1, s2);true
            end
      with 
           | Unify.Unify msg -> false;;
    let rec unify (g_, us_, us'_) =
      try begin
            Unify.unify (g_, us_, us'_);true
            end
      with 
           | Unify.Unify msg -> false;;
    let rec getHypGoal =
      function 
               | (DProg, (C.Atom p, s)) -> (DProg, (p, s))
               | (C.DProg (g_, dPool), (C.Impl (r, a_, ha_, g), s))
                   -> let d'_ = (IntSyn.Dec (None, (I.EClo (a_, s))))
                        in begin
    ...
    let rec updateGlobalTable (goal, flag) =
      let ((C.DProg (g_, dPool) as dProg), (p, s)) =
        getHypGoal ((C.DProg (I.null_, I.null_)), (goal, I.id))
        in let (g'_, dAVars_, dEVars_, u'_, eqn', s') =
             A.abstractEVarCtx (dProg, p, s)
             in let _ = begin
                  if solveEqn ((eqn', s'), g'_) then () else
                  print "\nresidual equation not solvable!\n" end
                  in let status = begin
                       if flag then TableParam.complete_ else
                       TableParam.incomplete_ end in begin
                       if TabledSyn.keepTable (IntSyn.targetFam u'_) then
                       begin
                       match MT.callCheck
                             (dAVars_, dEVars_, g'_, u'_, eqn', status)
                       with 
                            | T.RepeatedEntry (_, answRef, _)
                                -> TableParam.globalTable :=
                                     (((dAVars_, dEVars_, g'_, u'_, eqn',
                                        answRef, status)
                                       :: ! TableParam.globalTable))
                            | _
                                -> raise
                                   ((Error
                                     "Top level goal should always in the table\n"))
                       end else () end;;
    ...
    let rec fillTable () =
      let rec insert =
        function 
                 | [] -> ()
                 | ((dAVars_, dEVars_, g'_, u'_, eqn', answRef, status) :: T)
                     -> begin
                        match MT.insertIntoTree
                              (dAVars_, dEVars_, g'_, u'_, eqn', answRef,
                               status)
                        with 
                             | T.NewEntry _ -> insert T
                             | _ -> ()
                        end
        in insert (! TableParam.globalTable);;
    (*------------------------------------------------------------------------------------------*);;
    ...
    let rec retrieve' =
      function 
               ...
               | ((g_, u_, s), (esub, asub), (((d'_, s1), o1_) :: a_), sc)
                   -> let s1' =
                        ctxToEVarSub
                        (d'_, (I.Shift (I.ctxLength d'_)))
                        (* I.id *)
                        in let scomp = I.comp (s1, s1')
                             in let ss = shift (g_, s)
                                  in let ss1 = shift (g_, scomp)
                                       in let a = I.comp (asub, s)
                                            in let ass = shift (g_, a)
                                                 in let easub =
                                                      I.comp (asub, esub)
                                                      in begin
                                                           CSManager.trail
    ...
    let rec retrieveV =
      function 
               ...
               | ((g_, u_, s), (((dEVars_, s1), o1_) :: a_), sc)
                   -> let s1' =
                        ctxToEVarSub
                        (dEVars_, (I.Shift (I.ctxLength dEVars_)))
                        (* I.id *)
                        in let scomp = I.comp (s1, s1')
                             in let ss = shift (g_, s)
                                  in let ss1 = shift (g_, scomp)
   ...
   *);;
    let rec retrieve (k, (g_, u_, s), (asub, answRef), sc) =
      let lkp = T.lookup answRef
        in let asw' = List.take (rev (T.solutions answRef), T.lookup answRef)
             in let answ' = List.drop (asw', ! k)
                  in begin
                       k := lkp;retrieveSW ((g_, u_, s), asub, answ', sc)
                       end;;
    (* ---------------------------------------------------------------------- *);;
    (* solve ((g, s), dp, sc) => ()
     Invariants:
    ...
    let rec solve =
      function 
               | ((C.Atom p, s), (C.DProg (g_, dPool) as dp), sc) -> begin
                     ...
                     in let _ = begin
                          ...
                          in begin
                             ...
                             with 
                                  ...
                                  | T.RepeatedEntry
                                      (asub, answRef, incomplete_) -> begin
                  ...
                  *)
                                  | T.RepeatedEntry
                                      (asub, answRef, complete_) -> begin
                                      if T.noAnswers answRef then () else
                                      retrieve
                                      (ref 0, (g'_, u'_, s'),
                                       (asub, answRef), sc)
                                      end
    ...
    and rSolve =
      function 
               ...
               | (ps', (C.Assign (q_, eqns), s), (C.DProg (g_, dPool) as dp),
                  sc)
                   -> begin
               ...
               | (ps', (C.And (r, a_, g), s), (C.DProg (g_, dPool) as dp),
                  sc)
                   -> let x_ = I.newEVar (g_, (I.EClo (a_, s)))
                        in rSolve
                   ...
                   -> let x_ = I.newEVar (g_, (I.EClo (a_, s)))
                        in rSolve
                           (ps', (r, (I.Dot ((I.Exp x_), s))), dp,
                            function 
                                     | s_ -> sc s_)
               | (ps', (C.Axists (I.ADec (Some x_, d), r), s),
                  (C.DProg (g_, dPool) as dp), sc)
                   -> let x'_ = I.newAVar ()
                        in rSolve
                           (ps',
      ...
      function 
               ...
               | ((C.UnifyEq (g'_, e1, n_, eqns), s),
                  (C.DProg (g_, dPool) as dp), cnstr, sc)
                   -> let g''_ = append (g'_, g_)
                        in let s' = shift (g'_, s) in begin
                             if Assign.unifiable (g''_, (n_, s'), (e1, s'))
        ...
        function 
                 ...
                 | ((I.Const c as hc_) :: sgn')
                     -> let C.SClause r = C.sProgLookup (cidFromHead hc_)
                          ...
                          (* trail to undo EVar instantiations *)(* return indicates failure *)
        in let rec matchDProg =
             function 
                      | (null_, null_, _)
                          -> matchSig (Index.lookup (cidFromHead ha_))
                      | (I.Decl (g_, _), I.Decl (dPool', C.Dec (r, s, ha'_)),
                         k) -> begin
                          ...
                          else matchDProg (g_, dPool', k + 1) end
                              ...
                              | ()
                                  -> begin
                                     ...
                                     with 
                                          | Some u_
                                              -> begin
                                                   sc [(C.Csolver u_)];true
                                                   end
                                          | None -> false
                                     end)
                    in begin
                    if succeeded then matchConstraint (Solve_, try_ + 1) else
                    () end
                  in begin
                     match I.constStatus (cidFromHead ha_)
                     with 
                          | I.Constraint (cs, Solve_)
                              -> matchConstraint (Solve_, 0)
    ...
    let rec retrieval =
      function 
               | (Loop, (g'_, u'_, s'), sc, (asub, answRef), n) -> begin
                   if T.noAnswers answRef then () else
                   retrieve (n, (g'_, u'_, s'), (asub, answRef), sc) end
                   (* still  no answers available from previous stages *)(* NOTE: do not add the susp goal to suspGoal list
                because it is already in the suspGoal list *)(*  new answers available from previous stages
                          ...
                          (function 
                                    | ()
                                        -> begin
                                             Unify.resume trail;
                                             retrieval
                                             (susp_, s, sc, (asub, answRef),
                                              k)
                                             
                                             end);
                          resume goals_
                          end
        in let sg_ = rev (! suspGoals_) in begin
