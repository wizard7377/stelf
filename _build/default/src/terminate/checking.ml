...
module type CHECKING = sig
                       ...
                       type nonrec order = (IntSyn.eclo * IntSyn.eclo)
                         Order.order_
      ...
      struct
          ...
          function 
                   ...
                   | (g_, (o_ :: d'_))
                       -> ((atomicRCtxToString (g_, d'_)) ^ ", ") ^
                            (atomicPredToString (g_, o_));;
        let rec shiftO arg__1 arg__2 =
          begin
          match (arg__1, arg__2)
          with 
               | (R.Arg ((u_, us), (v_, vs)), f)
                   -> (R.Arg ((u_, f us), (v_, f vs)))
               | (R.Lex l_, f)
                   -> (R.Lex (map (function 
                                            | o_ -> shiftO o_ f) l_))
               | (R.Simul l_, f)
                   -> (R.Simul (map (function 
                                              | o_ -> shiftO o_ f) l_))
          end;;
        let rec shiftP arg__3 arg__4 =
          ...
          with 
               ...
               | (Pi ((I.Dec (x_, v_) as d_), p_), f)
                   -> (Pi (d_, shiftP p_ f))
          end;;
        let rec shiftRCtx rl_ f = map (function 
                                                | p -> shiftP p f) rl_;;
        ...
        let rec shiftACtx rl_ f = map (function 
                                                | p -> shiftArg p f) rl_;;
        let rec fmtOrder (g_, o_) =
          ...
          and fmtOrders =
            function 
                     ...
                     | (o_ :: []) -> [fmtOrder' o_]
                     | (o_ :: l_)
                         -> (fmtOrder' o_ :: F.break_ :: fmtOrders l_)
            in fmtOrder' o_;;
        let rec fmtComparison (g_, o_, comp, o'_) =
          (F.HOVbox0
           (1, 0, 1,
            [fmtOrder (g_, o_); F.break_; (F.String comp); F.break_;
             fmtOrder (g_, o'_)]));;
        ...
        let rec isExistential =
          function 
                   ...
                   | Exist -> true
                   | exist'_ -> true;;
        let rec isParameter (q_, x_) =
          isParameterW (q_, Whnf.whnf (x_, I.id))
        and isParameterW (q_, us_) =
          try isUniversal (I.ctxLookup (q_, Whnf.etaContract ((I.EClo us_))))
          with 
               | eta_ -> isFreeEVar us_
        and isFreeEVar =
          function 
                   ...
                   | (I.Lam (d_, u_), s)
                       -> isFreeEVar (Whnf.whnf (u_, I.dot1 s))
                   | _ -> false;;
        let rec isAtomic (gq_, us_) = isAtomicW (gq_, Whnf.whnf us_)
        and isAtomicW =
          function 
                   | (gq_, ((I.Root (I.Const c, s_) as x_), s))
                       -> isAtomicS (gq_, (s_, s))
                   | (gq_, ((I.Root (I.Def c, s_) as x_), s))
                       -> isAtomicS (gq_, (s_, s))
                   | (((g_, q_) as gq_), ((I.Root (I.BVar n, s_) as x_), s))
                       -> (isExistential (I.ctxLookup (q_, n))) ||
                            (isAtomicS (gq_, (s_, s)))
                   | (gq_, _) -> false
        and isAtomicS =
          function 
        ...
        let rec lookupEq =
          function 
                   ...
                   | (((g_, q_) as gq_), (Eq (usVs1_, usVs1'_) :: d_), usVs_,
                      usVs'_, sc)
                       -> (CSManager.trail
                           (function 
                                     | ()
        ...
        let rec lookupLt =
          function 
                   ...
                   | (((g_, q_) as gq_), (Less (usVs1_, usVs1'_) :: d_),
                      usVs_, usVs'_, sc)
                       -> (CSManager.trail
                           (function 
                                     | ()
        ...
        and transEq =
          function 
                   ...
                   | (((g_, q_) as gq_), (Eq (usVs1_, usVs1'_) :: d_), d'_,
                      usVs_, usVs'_, sc)
                       -> (CSManager.trail
                           (function 
                                     | ()
                            ...
                            ((CSManager.trail
                              (function 
                               ...
                               ||
                               (transEq
                                (gq_, d_, ((Eq (usVs1_, usVs1'_)) :: d'_),
                                 usVs_, usVs'_, sc)))
                   | (((g_, q_) as gq_), (Less (usVs1_, usVs1'_) :: d_), d'_,
                      usVs_, usVs'_, sc)
                       -> transEq (gq_, d_, d'_, usVs_, usVs'_, sc)
        and ltAtomic =
          function 
                           ...
                           (function 
                            ...
                            ((CSManager.trail
                              (function 
                               ...
                               ||
                               (transLt
                                (gq_, d_, ((Less (usVs1_, usVs1'_)) :: d'_),
                                 usVs_, usVs'_, sc)))
        and atomic =
          function 
        ...
        and ltInstLW =
          function 
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      ((u'_, s1'), (v'_, s2')), p'_, sc) -> begin
                       if
                       Subordinate.equiv (I.targetFam v'_, I.targetFam v1_)
                       then
                       let x_ = I.newEVar (g_, (I.EClo (v1_, s1)))
                         in let sc' =
                              function 
                                       | ()
                                           -> (isParameter (q_, x_)) &&
                       ...
                       if
                       Subordinate.below (I.targetFam v1_, I.targetFam v'_)
                       then
                       let x_ = I.newEVar (g_, (I.EClo (v1_, s1)))
                         in ltInstL
                            ((g_, q_), d_, d'_,
                   ...
                   | (gq_, d_, d'_, usVs_, usVs'_, p'_, sc)
                       -> leftInstantiate
                          (gq_, d_, ((Less (usVs_, usVs'_)) :: d'_), p'_, sc)
        ...
        and leInstLW =
          function 
                   ...
                   | (gq_, d_, d'_, usVs_, usVs'_, p_, sc)
                       -> leftInstantiate
                          (gq_, d_, ((Less (usVs_, usVs'_)) :: d'_), p_, sc)
        and eqInstL (gq_, d_, d'_, usVs_, usVs'_, p'_, sc) =
          eqInstLW
          (gq_, d_, d'_, Whnf.whnfEta usVs_, Whnf.whnfEta usVs'_, p'_, sc)
        and eqInstLW =
          function 
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      ((I.Lam (I.Dec (_, v1''_), u''_), s1''),
                       (I.Pi ((I.Dec (_, v2''_), _), v''_), s2'')),
                      p'_, sc)
                       -> let x_ = I.newEVar (g_, (I.EClo (v1''_, s1'')))
                            in eqInstL
                               (gq_, d_, d'_,
          ...
          function 
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      p'_, sc) -> begin
                       if eqCid (c, c') then
                       eqSpineIL
                       (gq_, d_, d'_, ((s_, s), (I.constType c, I.id)),
                        ((s'_, s'), (I.constType c', I.id)), p'_, sc)
                       else
                       begin
                         begin
                         if (! Global.chatter) > 4 then
                   ...
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      (((I.Root (I.Def c', s'_), s'), vs'_) as usVs'_), p'_,
                      sc) -> begin
                       if eqCid (c, c') then
                       eqSpineIL
                       (gq_, d_, d'_, ((s_, s), (I.constType c, I.id)),
                   ...
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      (((I.Root (I.BVar n, s'_), s') as us'_), vs'_), p'_,
                      sc) -> begin
                       if isAtomic (gq_, us'_) then
                       leftInstantiate
                       (gq_, d_, ((Eq ((us'_, vs'_), (us_, vs_))) :: d'_),
                        p'_, sc)
                       else
                       begin
                         begin
                   ...
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      (((I.Root (I.Def c, s'_), s') as us'_), vs'_), p'_, sc)
                       -> begin
                       if isAtomic (gq_, us_) then
                       leftInstantiate
                       (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                        p'_, sc)
                       else
                       begin
                         begin
                   ...
                   | (((g_, q_) as gq_), d_, d'_,
                      (((I.Root (I.BVar n, s_), s) as us_), vs_),
                      (((I.Root (I.Const c, s'_), s') as us'_), vs'_), p'_,
                      sc) -> begin
                       if isAtomic (gq_, us_) then
                       leftInstantiate
                       (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                   ...
                   | (((g_, q_) as gq_), d_, d'_,
                      ...
                      sc) -> begin
                       if n = n' then
                       let I.Dec (_, v'_) = I.ctxDec (g_, n)
                         in eqSpineIL
                            (gq_, d_, d'_, ((s_, s), (v'_, I.id)),
                             ((s'_, s'), (v'_, I.id)), p'_, sc)
                       else
                       leftInstantiate
                       (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                        p'_, sc)
                       end
                   | (((g_, q_) as gq_), d_, d'_, usVs_, usVs'_, p'_, sc)
                       -> begin
                            begin
                            if (! Global.chatter) > 4 then
                            print
        ...
        and eqSpineIL (gq_, d_, d'_, (ss_, vs_), (ss'_, vs'_), p'_, sc) =
          eqSpineILW
          (gq_, d_, d'_, (ss_, Whnf.whnf vs_), (ss'_, Whnf.whnf vs'_), p'_,
           sc)
        and eqSpineILW =
          function 
                   | (gq_, d_, d'_, ((nil_, s), vs_), ((nil_, s'), vs'_),
                      p'_, sc) -> leftInstantiate (gq_, d_, d'_, p'_, sc)
                   | (gq_, d_, d'_, ((I.SClo (s_, s'), s''), vs_), ssVs'_,
                      p'_, sc)
                       -> eqSpineIL
                          (gq_, d_, d'_, ((s_, I.comp (s', s'')), vs_),
                           ssVs'_, p'_, sc)
                   | (gq_, d_, d'_, ssVs_, ((I.SClo (s'_, s'), s''), vs'_),
                      p'_, sc)
                       -> eqSpineIL
                          (gq_, d_, d'_, ssVs_,
                           ((s'_, I.comp (s', s'')), vs'_), p'_, sc)
                   | (gq_, d_, d'_,
                      ((I.App (u_, s_), s1),
                       (I.Pi ((I.Dec (_, v1_), _), v2_), s2)),
                      ((I.App (u'_, s'_), s1'),
                       (I.Pi ((I.Dec (_, v1'_), _), v2'_), s2')),
                      p'_, sc)
                       -> let d1_ =
                            ((Eq
                              (((u_, s1), (v1_, s2)),
                            ...
                            in eqSpineIL
                               (gq_, d1_, d'_,
                                ((s_, s1),
                                 (v2_,
                                  (I.Dot ((I.Exp ((I.EClo (u_, s1)))), s2)))),
                                ((s'_, s1'),
                                 (v2'_,
                                  (I.Dot
                                   ((I.Exp ((I.EClo (u'_, s1')))), s2')))),
                                p'_, sc)
        and rightDecompose =
          function 
        ...
        and ordLtR =
          function 
                   | (gq_, d'_, R.Arg usVs_, R.Arg usVs'_)
                       -> ltAtomicR
                          (gq_, d'_, usVs_, usVs'_, init, leftInstantiate)
                   | (gq_, d'_, R.Lex o_, R.Lex o'_)
                       -> ltLexR (gq_, d'_, o_, o'_)
                   | (gq_, d'_, R.Simul o_, R.Simul o'_)
                       -> ltSimulR (gq_, d'_, o_, o'_)
        and ordLeR =
          function 
                   | (gq_, d'_, R.Arg usVs_, R.Arg usVs'_)
                       -> leAtomicR
                   ...
                   | (gq_, d'_, R.Lex o_, R.Lex o'_)
                       -> (ltLexR (gq_, d'_, o_, o'_)) ||
                            (ordEqsR (gq_, d'_, o_, o'_))
                   | (gq_, d'_, R.Simul o_, R.Simul o'_)
                       -> leSimulR (gq_, d'_, o_, o'_)
        and ordEqR =
          function 
                   | (gq_, d'_, R.Arg usVs_, R.Arg usVs'_)
                       -> (conv (usVs_, usVs'_)) ||
        ...
        and ordEqsR =
          function 
                   ...
                   | (gq_, d'_, (o_ :: l_), (o'_ :: l'_))
                       -> (ordEqR (gq_, d'_, o_, o'_)) &&
                            (ordEqsR (gq_, d'_, l_, l'_))
        and ltLexR =
          function 
                   | (gq_, d'_, [], []) -> false
                   | (gq_, d'_, (o_ :: l_), (o'_ :: l'_))
        ...
        and ltSimulR =
          function 
                   ...
                   | (gq_, d_, (o_ :: l_), (o'_ :: l'_))
                       -> ((ordLtR (gq_, d_, o_, o'_)) &&
                             (leSimulR (gq_, d_, l_, l'_)))
                            ||
                            ((ordEqR (gq_, d_, o_, o'_)) &&
                               (ltSimulR (gq_, d_, l_, l'_)))
        and leSimulR =
          function 
                   | (gq_, d_, [], []) -> true
                   | (gq_, d_, (o_ :: l_), (o'_ :: l'_))
                       -> (ordLeR (gq_, d_, o_, o'_)) &&
                            (leSimulR (gq_, d_, l_, l'_))
        and ltAtomicR (gq_, d_, usVs_, usVs'_, sc, k) =
          ltAtomicRW (gq_, d_, Whnf.whnfEta usVs_, usVs'_, sc, k)
        and ltAtomicRW =
          function 
                   | (gq_, d_, ((us_, ((I.Root _, s') as vs_)) as usVs_),
                      usVs'_, sc, k) -> ltR (gq_, d_, usVs_, usVs'_, sc, k)
                   | (((g_, q_) as gq_), d_,
                      ((I.Lam (_, u_), s1), (I.Pi ((Dec, _), v_), s2)),
                      ((u'_, s1'), (v'_, s2')), sc, k)
                       -> let usVs'_ =
                            ((u'_, I.comp (s1', I.shift)),
                             (v'_, I.comp (s2', I.shift)))
                            in let usVs_ = ((u_, I.dot1 s1), (v_, I.dot1 s2))
                                 in let d'_ =
                                      shiftACtx
                                      d_
                                      (function 
        ...
        and eqAtomicR (((g_, q_) as gq_), d_, usVs_, usVs'_, sc, k) =
          eqAtomicRW
          (gq_, d_, Whnf.whnfEta usVs_, Whnf.whnfEta usVs'_, sc, k)
        and eqAtomicRW =
          function 
                   | (((g_, q_) as gq_), d_,
                      ((I.Lam (_, u_), s1), (I.Pi ((Dec, _), v_), s2)),
                      ...
                      sc, k)
                       -> eqAtomicR
                          (((I.Decl
                             (g_, N.decLUName (g_, I.decSub (Dec, s2)))),
                            (I.Decl (q_, All))),
                           shiftACtx d_ (function 
                                                  | s -> I.comp (s, I.shift)),
                           ((u_, I.dot1 s1'), (v_, I.dot1 s2')),
                           ((u'_, I.dot1 s1'), (v'_, I.dot1 s2')), sc, k)
                   | (gq_, d_, (us_, ((I.Root _, s2) as vs_)),
                      (us'_, ((I.Root _, s2') as vs'_)), sc, k)
        ...
        and ltRW =
          function 
                   | (gq_, d_, (us_, vs_),
                      (((I.Root (I.Const c, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       ...
                       ltSpineR
                       (gq_, d_, (us_, vs_),
                        ((s'_, s'), (I.constType c, I.id)), sc, k)
                       end
                   | (gq_, d_, (us_, vs_),
                      (((I.Root (I.Def c, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       k (gq_, d_, [], (Less ((us_, vs_), (us'_, vs'_))), sc)
                       else
                   ...
                   | (((g_, q_) as gq_), d_, (us_, vs_),
                      (((I.Root (I.BVar n, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       k (gq_, d_, [], (Less ((us_, vs_), (us'_, vs'_))), sc)
                       else
                       let I.Dec (_, v'_) = I.ctxDec (g_, n)
                         in ltSpineR
                            (gq_, d_, (us_, vs_), ((s'_, s'), (v'_, I.id)),
                             sc, k)
                       end
                   | (gq_, d_, _, ((I.EVar _, _), _), _, _) -> false
                   | (((g_, q_) as gq_), d_, ((u_, s1), (v_, s2)),
                      ((I.Lam (I.Dec (_, v1'_), u'_), s1'),
                       (I.Pi ((I.Dec (_, v2'_), _), v'_), s2')),
                      sc, k) -> begin
                       if
                       Subordinate.equiv (I.targetFam v_, I.targetFam v1'_)
                       then
                       let x_ = I.newEVar (g_, (I.EClo (v1'_, s1')))
                         in let sc' =
                              function 
                       ...
                       if
                       Subordinate.below (I.targetFam v1'_, I.targetFam v_)
                       then
                       let x_ = I.newEVar (g_, (I.EClo (v1'_, s1')))
                         in ltR
                            (gq_, d_, ((u_, s1), (v_, s2)),
        ...
        and ltSpineRW =
          function 
                   ...
                   | (gq_, d_, (us_, vs_), ((I.SClo (s_, s'), s''), vs'_),
                      sc, k)
                       -> ltSpineR
                          (gq_, d_, (us_, vs_),
                           ((s_, I.comp (s', s'')), vs'_), sc, k)
                   | (gq_, d_, (us_, vs_),
                      ((I.App (u'_, s'_), s1'),
                       (I.Pi ((I.Dec (_, v1'_), _), v2'_), s2')),
                      sc, k)
                       -> (leAtomicR
                           (gq_, d_, (us_, vs_), ((u'_, s1'), (v1'_, s2')),
                            sc, k)) ||
                            (ltSpineR
                             (gq_, d_, (us_, vs_),
                              ((s'_, s1'),
                               (v2'_,
                                (I.Dot ((I.Exp ((I.EClo (u'_, s1')))), s2')))),
                              sc, k))
        ...
        and eqR (((g_, q_) as gq_), d_, usVs_, usVs'_, sc, k) =
          (CSManager.trail
           (function 
                     | () -> (eq (g_, usVs_, usVs'_)) && (sc ())))
            || (eqR' (gq_, d_, usVs_, usVs'_, sc, k))
        and eqR' =
          function 
                   | (gq_, d_,
                      (us_, ((I.Pi ((I.Dec (_, v2'_), _), v'_), s2') as vs_)),
                      (us'_, ((I.Root _, s2'') as vs'_)), sc, k) -> false
                   | (gq_, d_, (us_, ((I.Root _, s2') as vs_)),
                      (us'_,
                       ((I.Pi ((I.Dec (_, v2''_), _), v''_), s2'') as vs'_)),
                      sc, k) -> false
                   | (gq_, d_, (((I.Root (I.Const c, s_), s), vs_) as usVs_),
                      (((I.Root (I.Const c', s'_), s'), vs'_) as usVs'_), sc,
                      k) -> begin
                       if eqCid (c, c') then
                       eqSpineR
                       (gq_, d_, ((s_, s), (I.constType c, I.id)),
                        ((s'_, s'), (I.constType c', I.id)), sc, k)
                       else false end
                   | (gq_, d_, (((I.Root (I.Const c, s_), s) as us_), vs_),
                      (((I.Root (I.BVar n, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       k (gq_, d_, [], (Eq ((us'_, vs'_), (us_, vs_))), sc)
                       else false end
                   | (gq_, d_, (((I.Root (I.BVar n, s_), s) as us_), vs_),
                      (((I.Root (I.Const c, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us_) then
                       k (gq_, d_, [], (Eq ((us_, vs_), (us'_, vs'_))), sc)
                       else false end
                   | (gq_, d_, (((I.Root (I.Def c, s_), s), vs_) as usVs_),
                      (((I.Root (I.Def c', s'_), s'), vs'_) as usVs'_), sc,
                      k) -> begin
                       if eqCid (c, c') then
                       eqSpineR
                       (gq_, d_, ((s_, s), (I.constType c, I.id)),
                   ...
                   | (gq_, d_, (((I.Root (I.Def c, s_), s) as us_), vs_),
                      (((I.Root (I.BVar n, s'_), s') as us'_), vs'_), sc, k)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       k (gq_, d_, [], (Eq ((us'_, vs'_), (us_, vs_))), sc)
                   ...
                   | (((g_, q_) as gq_), d_,
                      ...
                      (((I.Root (I.BVar n', s'_), s') as us'_), vs'_), sc, k)
                       ...
                       let I.Dec (_, v'_) = I.ctxDec (g_, n)
                         in eqSpineR
                            (gq_, d_, ((s_, s), (v'_, I.id)),
                             ((s'_, s'), (v'_, I.id)), sc, k)
                       else
                       k (gq_, d_, [], (Eq ((us_, vs_), (us'_, vs'_))), sc)
                       end
                   | (gq_, d_, usVs_, usVs'_, sc, k)
                       -> k (gq_, d_, [], (Eq (usVs_, usVs'_)), sc)
        and eqSpineR (gq_, d_, (ss_, vs_), (ss'_, vs'_), sc, k) =
          eqSpineRW
          (gq_, d_, (ss_, Whnf.whnf vs_), (ss'_, Whnf.whnf vs'_), sc, k)
        and eqSpineRW =
          function 
                   | (gq_, d_, ((nil_, s), vs_), ((nil_, s'), vs'_), sc, k)
                       -> true
                   | (gq_, d_, ((I.SClo (s_, s'), s''), vs_), ssVs'_, sc, k)
                       -> eqSpineR
                          (gq_, d_, ((s_, I.comp (s', s'')), vs_), ssVs'_,
                           sc, k)
                   | (gq_, d_, ssVs_, ((I.SClo (s'_, s'), s''), vs'_), sc, k)
                       -> eqSpineR
                          (gq_, d_, ssVs_, ((s'_, I.comp (s', s'')), vs'_),
                           sc, k)
                   | (gq_, d_,
                      ((I.App (u_, s_), s1),
                       (I.Pi ((I.Dec (_, v1_), _), v2_), s2)),
        ...
        let rec leftDecompose =
          function 
                   ...
                   | (gq_, (Leq (R.Lex o_, R.Lex o'_) :: d_), d'_, p_)
                       -> (leftDecompose
                           (gq_, ((Less ((R.Lex o_), (R.Lex o'_))) :: d_),
                            d'_, p_)) &&
                            (leftDecompose
                             (gq_, ((Eq ((R.Lex o_), (R.Lex o'_))) :: d_),
                              d'_, p_))
                   ...
                   | (gq_, (Eq (R.Simul o_, R.Simul o'_) :: d_), d'_, p_)
                       -> eqsL (gq_, d_, d'_, o_, o'_, p_)
                   | (((g_, q_) as gq_), (Pi (Dec, o_) :: d_), d'_, p_)
                       -> begin
                            begin
                            ...
                            begin
                              ...
                              print
                              (F.makestring_fmt
                               (fmtPredicate (g_, (Pi (Dec, o_)))))
                              
                              end
                            else () end;leftDecompose (gq_, d_, d'_, p_)
                            end
        and ltLexL =
          function 
                   | (gq_, d_, d'_, [], [], p_) -> true
                   | (gq_, d_, d'_, (o_ :: l_), (o'_ :: l'_), p_)
                       -> (leftDecompose
                           (gq_, ((Less (o_, o'_)) :: d_), d'_, p_)) &&
                            (ltLexL
                             (gq_, ((Eq (o_, o'_)) :: d_), d'_, l_, l'_, p_))
        and eqsL =
          function 
                   | (gq_, d_, d'_, [], [], p_) -> true
                   | (gq_, d_, d'_, (o_ :: l_), (o'_ :: l'_), p_)
                       -> (leftDecompose
                           (gq_, ((Eq (o_, o'_)) :: d_), d'_, p_)) &&
        ...
        and ltSimulL =
          function 
                   ...
                   | (gq_, d_, d'_, (o_ :: l_), (o'_ :: l'_), p_)
                       -> (leSimulL
                           (gq_, ((Less (o_, o'_)) :: d_), d'_, l_, l'_, p_))
                            ||
                            (ltSimulL
                             (gq_, ((Eq (o_, o'_)) :: d_), d'_, l_, l'_, p_))
        and leSimulL =
                       ...
                       -> let d1_ =
                            ...
                            in let d1'_ =
                                 ...
                                 (function 
                                           | s -> I.comp (s, I.shift))
                                 in let usVs_ =
                                      ((u_, I.comp (s1, I.shift)),
                                       (v_, I.comp (s2, I.shift)))
                                      in let usVs'_ =
                                           ((u'_, I.dot1 s1'),
                                            (v'_, I.dot1 s2'))
                                           in let p'_ =
        ...
        and leAtomicLW =
          function 
                   | (gq_, d_, d'_, usVs_,
                      (us'_, ((I.Root (h_, s_), s') as vs'_)), p_)
        ...
        and eqAtomicL (gq_, d_, d'_, usVs_, usVs'_, p_) =
          eqAtomicLW
          (gq_, d_, d'_, Whnf.whnfEta usVs_, Whnf.whnfEta usVs'_, p_)
        and eqAtomicLW =
          function 
                   | (gq_, d_, d'_, (us_, ((I.Root _, s) as vs_)),
                      (us'_, ((I.Root _, s') as vs'_)), p_)
                       -> eqL (gq_, d_, d'_, (us_, vs_), (us'_, vs'_), p_)
                   | (gq_, d_, d'_, (us_, ((I.Root _, s) as vs_)),
                      (us'_, ((I.Pi _, s') as vs'_)), p_) -> true
                   | (gq_, d_, d'_, (us_, ((I.Pi _, s) as vs_)),
                      (us'_, ((I.Root _, s') as vs'_)), p_) -> true
                   | (gq_, d_, d'_, (us_, ((I.Pi _, s) as vs_)),
                      (us'_, ((I.Pi _, s') as vs'_)), p_)
                       -> leftDecompose
                          (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                           p_)
        ...
        and ltLW =
          function 
                   | (((g_, q_) as gq_), d_, d'_, usVs_,
                      (((I.Root (I.BVar n, s'_), s') as us'_), vs'_), p_)
                       -> begin
                       if isAtomic (gq_, us'_) then
                       leftDecompose
                       (gq_, d_, ((Less (usVs_, (us'_, vs'_))) :: d'_), p_)
                       else
                       let I.Dec (_, v'_) = I.ctxDec (g_, n)
                         in ltSpineL
                            (gq_, d_, d'_, usVs_, ((s'_, s'), (v'_, I.id)),
                             p_)
                       end
                   | (gq_, d_, d'_, usVs_,
                      ((I.Root (I.Const c, s'_), s'), vs'_), p_)
                       -> ltSpineL
                          (gq_, d_, d'_, usVs_,
                           ((s'_, s'), (I.constType c, I.id)), p_)
                   | (gq_, d_, d'_, usVs_,
                      ((I.Root (I.Def c, s'_), s'), vs'_), p_)
                       -> ltSpineL
                          (gq_, d_, d'_, usVs_,
                           ((s'_, s'), (I.constType c, I.id)), p_)
        ...
        and ltSpineLW =
          function 
                   ...
                   | (gq_, d_, d'_, usVs_, ((I.SClo (s_, s'), s''), vs'_),
                      p_)
                       -> ltSpineL
                          (gq_, d_, d'_, usVs_,
                           ((s_, I.comp (s', s'')), vs'_), p_)
                   | (gq_, d_, d'_, usVs_,
                      ((I.App (u'_, s'_), s1'),
                       (I.Pi ((I.Dec (_, v1'_), _), v2'_), s2')),
                      p_)
                       -> (leAtomicL
                           (gq_, d_, d'_, usVs_, ((u'_, s1'), (v1'_, s2')),
                            p_)) &&
                            (ltSpineL
                             (gq_, d_, d'_, usVs_,
                              ((s'_, s1'),
        ...
        and eqLW =
          function 
                   ...
                   | (gq_, d_, d'_,
                      (us_, ((I.Pi ((I.Dec (_, v2'_), _), v'_), s2') as vs_)),
                      (us'_, ((I.Root _, s2'') as vs'_)), p_) -> true
                   | (gq_, d_, d'_, (us_, ((I.Root _, s2') as vs_)),
                      (us'_,
                       ((I.Pi ((I.Dec (_, v2''_), _), v''_), s2'') as vs'_)),
                      p_) -> true
                   | (gq_, d_, d'_,
                      (((I.Root (I.Const c, s_), s), vs_) as usVs_),
                      (((I.Root (I.Const c', s'_), s'), vs'_) as usVs'_), p_)
                       -> begin
                       if eqCid (c, c') then
                       eqSpineL
                       (gq_, d_, d'_, ((s_, s), (I.constType c, I.id)),
                   ...
                   | (gq_, d_, d'_,
                      (((I.Root (I.BVar n, s_), s) as us_), vs_),
                      (((I.Root (I.Const c, s'_), s') as us'_), vs'_), p_)
                       -> begin
                       if isAtomic (gq_, us_) then
                       leftDecompose
                       (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                   ...
                   | (gq_, d_, d'_,
                      (((I.Root (I.Def c, s_), s), vs_) as usVs_),
                      (((I.Root (I.Def c', s'_), s'), vs'_) as usVs'_), p_)
                       -> begin
                       if eqCid (c, c') then
                       eqSpineL
                       (gq_, d_, d'_, ((s_, s), (I.constType c, I.id)),
                   ...
                   | (gq_, d_, d'_,
                      (((I.Root (I.BVar n, s_), s) as us_), vs_),
                      (((I.Root (I.Def c, s'_), s') as us'_), vs'_), p_)
                       -> begin
                       if isAtomic (gq_, us_) then
                       leftDecompose
                       (gq_, d_, ((Eq ((us_, vs_), (us'_, vs'_))) :: d'_),
                   ...
                   | (gq_, d_, d'_, usVs_, usVs'_, p_)
                       -> leftDecompose
                          (gq_, d_, ((Eq (usVs_, usVs'_)) :: d'_), p_)
        and eqSpineL (gq_, d_, d'_, (ss_, vs_), (ss'_, vs'_), p_) =
          eqSpineLW
          (gq_, d_, d'_, (ss_, Whnf.whnf vs_), (ss'_, Whnf.whnf vs'_), p_)
        and eqSpineLW =
          function 
                   | (gq_, d_, d'_, ((nil_, s), vs_), ((nil_, s'), vs'_), p_)
                       -> leftDecompose (gq_, d_, d'_, p_)
                   | (gq_, d_, d'_, ((I.SClo (s_, s'), s''), vs_), ssVs'_,
                      p_)
                       -> eqSpineL
                                ...
                                ((s'_, s1'),
                                 (v2'_,
                                  (I.Dot
                                   ((I.Exp ((I.EClo (u'_, s1')))), s2')))),
                                p_);;
        let rec deduce (g_, q_, d_, p_) =
          leftDecompose ((g_, q_), d_, [], p_);;
        end;;
