  ...
  struct
    ...
    open!
      struct
        ...
        let rec rlistToString (g_, rl_) =
          rlistToString' (Names.ctxName g_, rl_);;
        let rec orderToString (g_, p_) =
          F.makestring_fmt (fmtPredicate (Names.ctxName g_, p_));;
        let rec select (c, (s_, s)) =
          let so_ = R.selLookup c
            in let vid_ = (I.constType c, I.id)
                    ...
                    and select''W =
                      ...
                      in let rec select' =
                           function 
                                    ...
                                    | R.Simul l_
                                        -> (R.Simul (map select' l_))
                           in select' (R.selLookup c);;
                 ...
                 in let rec select' =
                      ...
                      in let rec selectP =
                           function 
                                    | R.Less (o1_, o2_)
                                        -> (C.Less
                                            (select' o1_, select' o2_))
                                    | R.Leq (o1_, o2_)
                                        -> (C.Leq (select' o1_, select' o2_))
                                    | R.Eq (o1_, o2_)
                                        -> (C.Eq (select' o1_, select' o2_))
                           in try (Some (selectP (R.selLookupROrder c)))
                              with 
                                   | R.Error s -> None;;
          ...
          function 
                   ...
                   | (g_, q_, (I.Pi ((d_, maybe_), v_), s), occ)
                       -> let o_ =
                            getROrder
                            ((I.Decl
                              (g_, N.decLUName (g_, I.decSub (d_, s)))),
                             (I.Decl (q_, C.all_)), (v_, I.dot1 s),
                             P.body occ)
                            in begin
                               match o_
                               with 
                                    ...
                                    | Some o'_
                                        -> (Some
                                            (abstractRO
                                             (g_, I.decSub (d_, s), o'_)))
                               end
                   | (g_, q_, (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), s),
                      occ)
                       -> let o_ =
                            getROrder
                            (g_, q_, (v2_, I.comp (I.invShift, s)),
                             P.body occ)
                            in begin
                               match o_
                               with 
        ...
        and checkGoalW =
          function 
                   | (g0_, q0_, rl_,
                      (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), s), vs'_,
                      occ)
                       -> begin
                            checkClause
                            ((g0_, q0_, rl_), I.null_, I.null_, (v1_, s),
                             P.label occ);
                            checkGoal
                            (g0_, q0_, rl_, (v2_, I.comp (I.invShift, s)),
                             vs'_, P.body occ)
                            
                            end
                   | (g0_, q0_, rl_, (I.Pi ((d_, maybe_), v_), s), (v'_, s'),
                      occ)
                       -> checkGoal
                          ((I.Decl
                            (g0_, N.decLUName (g0_, I.decSub (d_, s)))),
                           (I.Decl (q0_, C.all_)),
                           C.shiftRCtx
                           rl_
                           (function 
                                     | s -> I.comp (s, I.shift)),
                           (v_, I.dot1 s), (v'_, I.comp (s', I.shift)),
                           P.body occ)
                   | (g0_, q0_, rl_, ((I.Root (I.Const a, s_), s) as vs_),
                      ((I.Root (I.Const a', s'_), s') as vs'_), occ)
                       -> let rec lookup =
                            function 
                                     | (empty_, f) -> R.empty_
                                     | ((R.Le (a, a's') as a's), f) -> begin
                                         if f a then a's else
                                         lookup (a's', f) end
                                     | ((R.Lt (a, a's') as a's), f) -> begin
                                         if f a then a's else
                                         lookup (a's', f) end
                            in let p_ = selectOcc (a, (s_, s), occ)
                                 in let p'_ = select (a', (s'_, s'))
                                      in let a's = Order.mutLookup a
                                           in begin
                                              match lookup
                                                    (a's,
                                                     function 
                   ...
                   | (g0_, q0_, rl_, ((I.Root (I.Def a, s_), s) as vs_),
                      vs'_, occ)
                       -> raise
                          ((Error'
                            (occ,
                             (("Reduction checking for defined type families not yet available:\n"
                               ...
                               ^ ".")))
                   | (g0_, q0_, rl_, vs_,
                      ((I.Root (I.Def a', s'_), s') as vs'_), occ)
                       -> raise
                          ((Error'
                            (occ,
                             (("Reduction checking for defined type families not yet available:\n"
        ...
        and checkSubgoals =
          function 
                   | (g0_, q0_, rl_, vs_, n,
                      (I.Decl (g_, (I.Dec (_, v'_) as d_)), I.Decl
                       (q_, C.And occ)))
                       -> let _ =
                            checkGoal
                            (g0_, q0_, rl_, (v'_, (I.Shift (n + 1))), vs_,
                             occ)
                            in let ro_ =
                                 getROrder
                                 (g0_, q0_, (v'_, (I.Shift (n + 1))), occ)
                                 in let rl'_ =
                                      begin
                                      match ro_
                                      with 
                                      ...
                                      in checkSubgoals
                                         (g0_, q0_, rl'_, vs_, n + 1,
                                          (g_, q_))
                   | (g0_, q0_, rl_, vs_, n,
                      (I.Decl (g_, d_), I.Decl (q_, exist_)))
                       -> checkSubgoals (g0_, q0_, rl_, vs_, n + 1, (g_, q_))
                   | (g0_, q0_, rl_, vs_, n,
                      (I.Decl (g_, d_), I.Decl (q_, all_)))
                       -> checkSubgoals (g0_, q0_, rl_, vs_, n + 1, (g_, q_))
                   ...
                   | (gqr_, g_, q_, (I.Pi ((d_, maybe_), v_), s), occ)
                       -> checkClause
                          (gqr_,
                           ...
                           (I.Decl (q_, C.exist_)), (v_, I.dot1 s),
                           P.body occ)
                   | (gqr_, g_, q_,
                      (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), s), occ)
                       -> checkClause
                          (gqr_, (I.Decl (g_, I.decSub (d_, s))),
                           (I.Decl (q_, (C.And (P.label occ)))),
                           (v2_, I.dot1 s), P.body occ)
                   | (((g0_, q0_, rl_) as gqr_), g_, q_,
                      ((I.Root (I.Const a, s_), s) as vs_), occ)
                       -> let n = I.ctxLength g_
                            in let rl'_ =
                                 C.shiftRCtx
                                 rl_
                                 (function 
                                           | s -> I.comp (s, (I.Shift n)))
                                 in checkSubgoals
                                    (concat (g0_, g_), concat (q0_, q_),
                                     rl'_, vs_, 0, (g_, q_))
                   ...
                   | (g_, q_, rl_,
                      (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), s), occ)
                       -> begin
                            ...
                            checkRGoal
                            (g_, q_, rl_, (v2_, I.comp (I.invShift, s)),
                             P.body occ)
        ...
        and checkRImpW =
          function 
                   | (g_, q_, rl_, (I.Pi ((d'_, maybe_), v'_), s'), (v_, s),
                      occ)
                       -> checkRImp
                   ...
                   | (g_, q_, rl_,
                      ...
                      (v_, s), occ)
                       -> let rl'_ =
                            begin
                            match getROrder (g_, q_, (v1_, s'), occ)
                            with 
                                 | None -> rl_
                                 | Some o_ -> (o_ :: rl_)
                            end
                            in checkRImp
                               (g_, q_, rl'_, (v2_, I.comp (I.invShift, s')),
                                (v_, s), occ)
                   | (g_, q_, rl_, ((I.Root (I.Const a, s_), s) as vs'_),
                      vs_, occ) -> checkRGoal (g_, q_, rl_, vs_, occ)
                   | (g_, q_, rl_, ((I.Root (I.Def a, s_), s) as vs'_), vs_,
                      occ)
                       -> raise
                          ((Error'
                            (occ,
        ...
        and checkRClauseW =
          function 
                   ...
                   | (g_, q_, rl_,
                      (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), s), occ)
                       -> let g'_ = (I.Decl (g_, I.decSub (d_, s)))
                            in let q'_ = (I.Decl (q_, C.exist_))
                                 in let rl'_ =
                                      C.shiftRCtx
                                      rl_
                                      (function 
                   ...
                   | (g_, q_, rl_, ((I.Root (I.Const a, s_), s) as vs_), occ)
                       -> let ro_ =
                            ...
                            in let _ = begin
                                 ...
                                 if C.deduce (g_, q_, rl_, ro_) then () else
                                 raise
                                 ((Error'
                                   (occ,
                                    (("Reduction violation:\n" ^
        ...
        let rec checkFamReduction a =
          let rec checkFam' =
                     ...
                     | (I.Const b :: bs)
                         -> begin
                              ...
                              begin
                                ...
                                if (! Global.chatter) > 4 then
                                begin
                                  N.varReset IntSyn.null_;print "\n"
                                  end
                                else () end;
                                begin
                                  try checkRClause
                                      (I.null_, I.null_, [],
                                       (I.constType b, I.id), P.top)
                                  with 
                                       | Error' (occ, msg)
                                           -> error (b, occ, msg)
            ...
            in let _ = begin
                 if (! Global.chatter) > 3 then
                 print
                 (("Reduction checking family " ^
                     (N.qidToString (N.constQid a)))
                    ^ ":\n")
                 else () end in checkFam' (Index.lookup a);;
        let rec checkFam a =
          let rec checkFam' =
            function 
                     | []
            ...
            in let _ = begin
                 if (! Global.chatter) > 3 then
                 print
                 (("Termination checking family " ^
                     (N.qidToString (N.constQid a)))
                    ^ "\n")
                 else () end in checkFam' (Index.lookup a);;
        let rec reset () = begin
                             R.reset ();R.resetROrder ()
                             end;;
        end;;
    (*--------------------------------------------------------------------*);;
    (* Printing *);;
