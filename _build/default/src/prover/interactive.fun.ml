      ...
      struct
        ...
        let rec normalize () =
          ...
          with 
               | (S.State (w_, psi_, p_, f_) :: rest_)
                   -> focus_ :=
                        (((S.State (w_, psi_, T.derefPrg p_, f_)) :: rest_))
               | _ -> ()
          end;;
        let rec reset () = begin
                             initFocus ();menu_ := None
                             end;;
            ...
            in let nsolved = 0
                 in begin
                      ...
                      begin
                        print
                        (("Number of goals : " ^
                            (Int.toString (nopen + nsolved)))
                           ^ "\n");
                        begin
                          print
                          (("     open goals : " ^ (Int.toString nopen)) ^
                             "\n");
                          print
                          (("   solved goals : " ^ (Int.toString nsolved)) ^
                             "\n")
        ...
        let rec printmenu () =
          ...
          with 
                            ...
                            begin
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  begin
                                    print
                                    (TomegaPrint.prgToString (psi_, p_));
                                    begin
                                      print "\n-----------------------";
                                      begin
                                        print (menuToString ());
               ...
               | (S.StateLF ((I.EVar (r, g_, v_, cs_) as x_)) :: r_)
                   -> begin
                        print "\n=======================";
                        begin
                          print "\n=== THEOREM PROVER ====\n";
        ...
        let rec menu () =
          ...
          with 
               ...
               | (S.State (w_, psi_, p_, f_) :: _)
                   -> let xs_ = S.collectT p_
                        in let f1_ =
                             map
                             (function 
                                       | T.EVar (psi_, r, f_, tc_, tCs_, x_)
                                           -> begin
                                                Names.varReset I.null_;
                                                (S.Focus
                                                 ((T.EVar
                                                   (TomegaPrint.nameCtx psi_,
                                                    r, f_, tc_, tCs_, x_)),
                                                  w_))
                                                
                                                end)
                             xs_
                             in let ys_ = S.collectLF p_
                                  in let f2_ =
                                       map
                                       (function 
                                                 | y_ -> (S.FocusLF y_))
                                       ys_
                                       in let rec splitMenu =
                                            function 
                                                     | [] -> []
                                                     | (operators :: l)
                                                         -> (map
               ...
               | (S.StateLF y_ :: _)
                   -> let ys_ =
                        Abstract.collectEVars (I.null_, (y_, I.id), [])
                        in let f2_ =
                             map (function 
                                           | y_ -> (S.FocusLF y_)) ys_
                             in let fill =
        ...
        let rec select k =
          let rec select' =
            function 
                     ...
                     | (1, (Split o_ :: _))
                         -> Timers.time Timers.splitting Split.apply o_
                     | (1, (Introduce o_ :: _)) -> Introduce.apply o_
                     | (1, (Elim o_ :: _)) -> Elim.apply o_
                     | (1, (Fill o_ :: _))
                         -> Timers.time Timers.filling Fill.apply o_
            ...
            in let cL =
                 ...
                 (function 
                           | x
                               -> valOf
                                  (Names.constLookup
                                   (valOf (Names.stringToQid x))))
                 names
                 in let f_ = convertFor cL
                      in let ws_ = map W.lookup cL
                           in let rec select c =
                                try Order.selLookup c
                                with 
                                     | _ -> (Order.Lex [])
                                in let tc_ =
                                     Tomega.transformTC
                                     (I.null_, f_, map select cL)
                                     in let (w_ :: _) = ws_
                                          in let _ =
                                               focus_ := [S.init (f_, w_)]
                                               in let p_ =
                                                    ...
                                                    with 
                                                         | []
                                                             -> abort
                                                                "Initialization of proof goal failed\n"
                                                         | (S.State
                                                            (w_, psi_, p_,
                                                             f_)
                                                            :: _) -> p_
               ...
               | (S.State (w_, psi_, p_, f_) :: _)
                   -> let rec findIEVar =
                        function 
                                 | []
                                     -> raise
                                        ((Error ("cannot focus on " ^ n)))
                                 | (y_ :: ys_) -> begin
                                     if
                                     (Names.evarName (T.coerceCtx psi_, y_))
                                       = n
                                     then
                                     begin
                                       focus_ :=
                                     ...
                                     else findIEVar ys_ end
                        in let rec findTEVar =
                             function 
                                      | [] -> findIEVar (S.collectLF p_)
                                      | ((T.EVar (psi_, r, f_, tc_, tCs_, y_)
                                           as x_)
                                         :: xs_) -> begin
                                          if
                                          (Names.evarName
                                           (T.coerceCtx psi_, y_)) = n
                                          then
                                          begin
                                            focus_ :=
                                              (((S.State
                                          ...
                                          else findTEVar xs_ end
                             in findTEVar (S.collectT p_)
               | (S.StateLF u_ :: _)
                   -> begin
                      match Names.getEVarOpt n
                      with 
        ...
        let rec return () =
          ...
          with 
               | (s_ :: []) -> begin
                   if S.close s_ then print "[Q.E.D.]\n" else () end
               | (s_ :: rest_)
                   -> begin
                        focus_ := rest_;
                        begin
