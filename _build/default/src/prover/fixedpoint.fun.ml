...
module FixedPoint(FixedPoint__0: sig module State' : STATE end) : FIXEDPOINT
  =
  struct
    ...
    open!
      struct
        ...
        let rec expand (S.Focus (T.EVar (psi_, r, f_, _, tCs_, _), w_), o_) =
          let I.NDec x = Names.decName (T.coerceCtx psi_, (I.NDec None))
            in let d_ = (T.PDec (x, f_, None, None))
                 in let x_ =
                      T.newEVar
                      ((I.Decl (psi_, d_)), T.forSub (f_, (T.Shift 1)))
                      in (r, (T.Rec (d_, x_)));;
