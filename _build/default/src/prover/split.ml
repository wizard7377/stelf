    ...
    open!
      struct
          ...
          let w = weaken (g_, I.targetFam v_)
            in let iw = Whnf.invert w
                 in let g'_ = Whnf.strengthen (iw, g_)
                      in let x'_ = I.newEVar (g'_, (I.EClo (v_, iw)))
        ...
        and instEVarsW =
          function 
                   ...
                   | ((I.Pi ((I.Dec (xOpt, v1_), _), v2_), s), p, xsRev_)
                       -> let x1_ = I.newEVar (I.null_, (I.EClo (v1_, s)))
                            in instEVars
                   ...
                   | I.Decl (g'_, (I.Dec (_, v_) as d_))
                       -> let s = createEVarSub g'_
                            in let v'_ = (I.EClo (v_, s))
                                 in let x_ = I.newEVar (I.null_, v'_)
                                      in (I.Dot ((I.Exp x_), s));;
        ...
        and lowerSplitW (g_, ((I.Root (I.Const a, _), s) as vs_), w_, sc) =
          let _ = constCases (g_, vs_, Index.lookup a, sc)
            in let _ = paramCases (g_, vs_, I.ctxLength g_, sc)
                 in let _ = worldCases (g_, vs_, w_, sc) in ();;
        let rec splitEVar ((I.EVar (_, gx_, v_, _) as x_), w_, sc) =
          lowerSplit
          (I.null_, (v_, I.id), w_,
           function 
                    | u_ -> begin
                        if Unify.unifiable (I.null_, (x_, I.id), (u_, I.id))
                        then sc () else () end);;
        let rec createSub =
          function 
                   | null_ -> T.id
                   | I.Decl (psi_, T.UDec (I.Dec (xOpt, v1_)))
                       -> let t' = createSub psi_
                            in let x_ =
                                 I.newEVar
                                 (I.null_,
                                  (I.EClo (Whnf.whnf (v1_, T.coerceSub t'))))
                                 in (T.Dot ((T.Exp x_), t'))
                   | I.Decl (psi_, T.UDec (I.BDec (_, (l, s))))
                       -> let t' = createSub psi_
                            in let l_ =
                                 I.newLVar
                                 ((I.Shift 0),
                                  (l, I.comp (s, T.coerceSub t')))
                                 in (T.Dot ((T.Block l_), t'))
                   | I.Decl (psi_, T.PDec (_, f_, tc1_, tc2_))
                       -> let t' = createSub psi_
                            in let y_ =
                                 T.newEVarTC
                                 (I.null_, (T.FClo (f_, t')), tc1_, tc2_)
                                 in (T.Dot ((T.Prg y_), t'));;
        let rec mkCases =
          function 
                   | ([], f_) -> []
                   | (((psi_, t) :: cs), f_)
                       -> let x_ = T.newEVar (psi_, (T.FClo (f_, t)))
                            in ((psi_, t, x_) :: mkCases (cs, f_));;
        let rec split (S.Focus (T.EVar (psi_, r, f_, None, None, _), w_)) =
          let rec splitXs arg__1 arg__2 =
            ...
            with 
                 ...
                 | ((g_, i), ((x_ :: xs_), f_, w_, sc))
                     -> let _ = begin
                          ...
                          print
                          (("Split " ^ (Print.expToString (I.null_, x_))) ^
                             ".\n")
                          else () end
                          in let os_ = splitXs (g_, i + 1) (xs_, f_, w_, sc)
                               in let _ = resetCases ()
                                    in let s = Print.expToString (g_, x_)
                                         in let os'_ =
                                              try begin
                                                    splitEVar (x_, w_, sc);
                                                    ((Split
            ...
            in let t = createSub psi_
                 in let xs_ = State.collectLFSub t
                      in let rec init () =
                           addCase (Abstract.abstractTomegaSub t)
                           in let g_ = T.coerceCtx psi_
                                in let os_ =
                                     splitXs (g_, 1) (xs_, f_, w_, init)
                                     in os_;;
        let rec expand
          ((S.Focus (T.EVar (psi_, r, f_, None, None, _), w_) as s_)) = begin
          if Abstract.closedCTX psi_ then split s_ else [] end;;
