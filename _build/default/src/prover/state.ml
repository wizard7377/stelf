...
(* State definition for Proof Search *);;
(* Author: Carsten Schuermann *);;
module State(State__0: sig module Formatter : FORMATTER end) : STATE =
  struct
...
*);;
      ...
      struct
        ...
        let rec findPrg =
          function 
                   ...
                   | T.LetPairExp (d1_, d2_, p1_, p2_)
                       -> (findPrg p1_) @ (findPrg p2_)
                   | T.LetUnit (p1_, p2_) -> (findPrg p1_) @ (findPrg p2_)
                   | (T.EVar (_, { contents = None}, _, _, _, _) as x_)
                       -> [x_]
        ...
        let rec findExp arg__1 arg__2 =
          ...
          with 
               ...
               | ((psi_, T.Case (T.Cases c_)), k_)
                   -> findExpCases (psi_, c_) k_
               | ((psi_, T.PClo (p_, t)), k_)
          ...
          with 
               ...
               | ((psi_, T.AppExp (m_, s_)), k_)
                   -> findExpSpine
                      ...
                      (Abstract.collectEVars
                       (T.coerceCtx psi_, (m_, I.id), k_))
               | ((psi_, T.AppBlock (_, s_)), k_)
                   -> findExpSpine (psi_, s_) k_
          end
        ...
        let rec init (f_, w_) =
          let x_ = T.newEVar (I.null_, f_) in (State (w_, I.null_, x_, f_));;
        let rec close (State (w_, _, p_, _)) =
          begin
          match (findPrg p_, findExp (I.null_, p_) [])
          with 
               | ([], []) -> true
               | _ -> false
          end;;
    ...
    *);;
    ...
    let collectLF = function 
                             | p_ -> findExp (I.null_, p_) [];;
    let collectLFSub = function 
                                | s -> findExpSub (I.null_, s) [];;
    end;;
