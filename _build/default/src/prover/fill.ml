                      ...
                      function 
                               ...
                               | ((I.Const c :: l_), fs_)
                                   -> matchSig
                                      (l_,
                                       try_
                                       ((I.constType c, I.id), fs_,
                                        (FillWithConst (y_, c))))
                      in matchCtx
                         (g_, 0,
                          matchSig (Index.lookup (I.targetFam v_), []));;
        let rec apply =
          function 
                                                           ...
                                                           | s_
                                                               -> k
                                                                  ((I.App
                                                                    (x_, s_))))
                                     | ((I.EClo (v_, t), s), k)
                                         -> doit ((v_, I.comp (t, s)), k)
                            in let I.Dec (_, w_) = I.ctxDec (g_, n)
                                 in doit
                                    ((w_, I.id),
                                     function 
                                              | s_
        ...
        let rec menu =
          function 
                   | FillWithBVar ((I.EVar (_, g_, _, _) as x_), n)
                       -> begin
                          match I.ctxLookup (Names.ctxName g_, n)
                          with 
                               | I.Dec (Some x, _)
                                   -> (("Fill " ^ (Names.evarName (g_, x_)))
                                         ^ " with variable ")
                                        ^ x
                          end
