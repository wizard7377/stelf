...
module Names(Names__0: sig
  ...
  *);;
    ...
    *);;
        ...
        let rec toString =
          function 
                   | Infix (Strength p, Left)
                       -> "%infix left " ^ (Int.toString p)
                   | Infix (Strength p, Right)
                       -> "%infix right " ^ (Int.toString p)
                   | Infix (Strength p, None)
                       -> "%infix none " ^ (Int.toString p)
  ...
  *);;
    let rec checkAtomic =
      function 
               ...
               | (name, _, _) -> false(* raise Error (""Constant "" ^ name ^ "" takes too many explicit arguments for given fixity"") *)(* allow extraneous arguments, Sat Oct 23 14:18:27 1999 -fp *);;
    (* checkArgNumber (c, n) = ()
  ...
  *);;
    let rec checkArgNumber =
      function 
               | (IntSyn.ConDec (name, _, i, _, v_, l_), n)
                   -> checkAtomic (name, v_, i + n)
               | (IntSyn.SkoDec (name, _, i, v_, l_), n)
                   -> checkAtomic (name, v_, i + n)
  ...
  *);;
    let rec checkFixity =
      function 
               | (name, _, 0) -> ()
               | (name, cid, n) -> begin
                   if checkArgNumber (IntSyn.sgnLookup cid, n) then () else
                   raise
                   ((Error
                     (("Constant " ^ name) ^
  ...
  *);;
      ...
      function 
               ...
               | ((id :: ids) as l) -> begin
                   if List.exists (function 
                                            | s -> s = "") l
                   then None else (Some ((Qid (rev ids, id)))) end;;
    let rec stringToQid name =
      validateQualName (rev (String.fields (function 
                                                     | c -> c = '.') name));;
    ...
    let rec insertConst ((structTable, constTable), cid) =
      let Condec_ = IntSyn.sgnLookup cid
        in let id = IntSyn.conDecName Condec_
             in begin
                match StringTree.insertShadow constTable (id, cid)
                with 
                     | None -> ()
                     | Some _
                         -> raise
    ...
    let rec insertStruct ((structTable, constTable), mid) =
      let strdec = IntSyn.sgnStructLookup mid
        in let id = IntSyn.strDecName strdec
             in begin
                match StringTree.insertShadow structTable (id, mid)
                with 
                     | None -> ()
                     | Some _
                         -> raise
    ...
    open!
      struct
        let rec fromTo f (from, to_) = begin
          if from >= to_ then () else
          begin
            f from;fromTo f (from + 1, to_)
            end
          end;;
        ...
        let rec structShadowClear () =
          Array.modify (function 
                                 | _ -> None) structShadowArray;;
        let componentsArray : namespace Array.array =
          Array.array (maxMid + 1, dummyNamespace);;
        let rec componentsClear () =
          Array.modify (function 
                                 | _ -> dummyNamespace) componentsArray;;
        let topStructNamespace : IntSyn.mid HashTable.table_ =
          HashTable.new_ 4096;;
    ...
    *);;
    let rec installConstName cid =
      let Condec_ = IntSyn.sgnLookup cid
        in let id = IntSyn.conDecName Condec_
             in begin
                match topInsert (id, cid)
                with 
                     | None -> ()
                     | Some (_, cid')
                         -> Array.update (shadowArray, cid, (Some cid'))
                end;;
    let rec uninstallConst cid =
      let Condec_ = IntSyn.sgnLookup cid
        in let id = IntSyn.conDecName Condec_
             in begin
                  begin
                  match Array.sub (shadowArray, cid)
                  with 
                       | None -> begin
                           if (topLookup id) = ((Some cid)) then topDelete id
                           else () end
                       | Some cid'
                           -> begin
                                topInsert (id, cid');
    ...
    let rec installStructName mid =
      let strdec = IntSyn.sgnStructLookup mid
        in let id = IntSyn.strDecName strdec
             in begin
                match topStructInsert (id, mid)
                with 
                     | None -> ()
                     | Some (_, mid')
                         -> Array.update
                            (structShadowArray, mid, (Some mid'))
                end;;
    let rec uninstallStruct mid =
      let strdec = IntSyn.sgnStructLookup mid
        in let id = IntSyn.strDecName strdec
             in begin
                  begin
                  match Array.sub (structShadowArray, mid)
                  with 
                       | None -> begin
                           if (topStructLookup id) = ((Some mid)) then
                           topStructDelete id else () end
                       | Some mid'
                           -> begin
                                topStructInsert (id, mid');
                                Array.update (structShadowArray, mid, None)
                                end
                  end;Array.update (componentsArray, mid, dummyNamespace)
                  end;;
    let rec resetFrom (mark, markStruct) =
      let (limit, limitStruct) = IntSyn.sgnSize ()
        in let rec ct f (i, j) = begin
    ...
    *);;
    ...
    let rec structComps mid =
      (fun (r, _) -> r) (Array.sub (componentsArray, mid));;
    let rec constComps mid =
      (fun (_, r) -> r) (Array.sub (componentsArray, mid));;
    let rec findStruct =
      function 
               | (structTable, (id :: []))
               ...
               | (structTable, (id :: ids))
                   -> begin
                      match StringTree.lookup structTable id
                      with 
                           | None -> None
    ...
    let rec findTopStruct =
      function 
               ...
               | (id :: ids)
                   -> begin
                      match HashTable.lookup topStructNamespace id
                      with 
                           | None -> None
                           | Some mid -> findStruct (structComps mid, ids)
                      end;;
    ...
    let rec constLookupIn =
      function 
               ...
               | ((structTable, constTable), Qid (ids, id))
                   -> begin
                      match findStruct (structTable, ids)
                      with 
                           | None -> None
                           | Some mid
                               -> StringTree.lookup (constComps mid) id
                      end;;
               ...
               | ((structTable, constTable), Qid (ids, id))
                   -> begin
                      ...
                      with 
                           | None -> None
                           | Some mid
                               -> StringTree.lookup (structComps mid) id
                      end;;
    let rec constUndefIn =
      function 
               | ((structTable, constTable), Qid ([], id))
                   -> begin
                      match StringTree.lookup constTable id
                      with 
                           | None -> (Some ((Qid ([], id))))
                           | Some _ -> None
                      end
               | ((structTable, constTable), Qid (ids, id))
                   -> begin
                      match findUndefStruct (structTable, ids, [])
                      with 
                           | (Some _ as opt) -> opt
                           | None
                               -> begin
    ...
    *);;
    ...
    let rec constUndef =
      function 
               | Qid ([], id)
                   -> begin
                      match HashTable.lookup topNamespace id
                      with 
                           | None -> (Some ((Qid ([], id))))
                           | Some _ -> None
                      end
    ...
    let rec structPath (mid, ids) =
      let strdec = IntSyn.sgnStructLookup mid
        in let ids' = (IntSyn.strDecName strdec :: ids)
             in begin
                match IntSyn.strDecParent strdec
                with 
                     | None -> ids'
    ...
    let rec maybeShadow =
      function 
               | (qid, false) -> qid
               | (Qid ([], id), true) -> (Qid ([], ("%" ^ id) ^ "%"))
               | (Qid ((id :: ids), name), true)
                   -> (Qid ((("%" ^ id) ^ "%" :: ids), name));;
    let rec conDecQid Condec_ =
      let id = IntSyn.conDecName Condec_
        in begin
           match IntSyn.conDecParent Condec_
           with 
                | None -> (Qid ([], id))
    ...
    *);;
    let rec constQid cid =
      let Condec_ = IntSyn.sgnLookup cid
        in let qid = conDecQid Condec_
             in maybeShadow (qid, (constLookup qid) <> ((Some cid)));;
      ...
      let strdec = IntSyn.sgnStructLookup mid
        in let id = IntSyn.strDecName strdec
             in let qid =
                  ...
                  in maybeShadow (qid, (structLookup qid) <> ((Some mid)));;
      ...
      let l_ = IntSyn.constUni cid
        in let _ =
             ...
             with 
                  ...
                  | kind_ -> ()
             end in Array.update (namePrefArray, cid, (Some (ePref, uPref)));;
    ...
    *);;
    let rec namePrefOf (role, v_) =
      namePrefOf' (role, IntSyn.targetHeadOpt v_);;
    (* local ... *);;
    (******************);;
    (* Variable Names *);;
    (******************);;
  ...
  *);;
    open!
      struct
        ...
        let varContext : IntSyn.dctx ref = ref IntSyn.null_;;
          ...
          let rec evlk =
            function 
                     ...
                     | (r, ((IntSyn.EVar (r', _, _, _), name) :: l)) -> begin
                         if r = r' then (Some name) else evlk (r, l) end
        ...
        let rec evarCnstr' =
          function 
                   | ([], acc) -> acc
                   | ((((IntSyn.EVar ({ contents = None}, _, _, cnstrs),
                         name) as xn_)
                       :: l),
                      acc)
                       -> begin
                          match Constraints.simplify (! cnstrs)
                          with 
                               | [] -> evarCnstr' (l, acc)
        ...
        let rec nextIndex' =
          function 
                   | (name, None) -> begin
                                       indexInsert (name, 1);1
                                       end
                   | (name, Some i)
                       -> begin
                            indexInsert (name, i + 1);i + 1
                            end;;
    ...
    *);;
    ...
    let rec addEVar (x_, name) =
      begin
        evarInsert (x_, name);varInsert (name, evar_ x_)
        end;;
    let rec getEVarOpt name =
      begin
      match varLookup name with 
                                | None -> None
                                | Some (Evar x_) -> (Some x_)
      end;;
      ...
      let rec cdfd =
        function 
                 ...
                 | IntSyn.Decl (g'_, IntSyn.BDec (Some name', _))
                     -> (name = name') || (cdfd g'_)
                 | IntSyn.Decl (g'_, IntSyn.NDec (Some name'))
                     -> (name = name') || (cdfd g'_)
    ...
    *);;
    let rec tryNextName (g_, base) =
      let name = base ^ (Int.toString (nextIndex base)) in begin
        if
        (varDefined name) || ((conDefined name) || (ctxDefined (g_, name)))
        then tryNextName (g_, base) else name end;;
    let rec findNameLocal (g_, base, i) =
      let name = base ^ (begin if i = 0 then "" else Int.toString i end)
        in begin
        if
        (varDefined name) || ((conDefined name) || (ctxDefined (g_, name)))
        then findNameLocal (g_, base, i + 1) else name end;;
    let rec findName =
      function 
    ...
    let rec newEVarName =
      function 
               | (g_, (IntSyn.EVar (r, _, v_, cnstr_) as x_))
                   -> let name = tryNextName (g_, namePrefOf (Exist, v_))
                        in begin
                             evarInsert (x_, name);name
                             end
                        (* use name preferences below *)
               | (g_, (IntSyn.AVar r as x_))
                   -> let name = tryNextName (g_, namePrefOf' (Exist, None))
                        in begin
                             evarInsert (x_, name);name
                             end
                        (* use name preferences below *);;
    ...
    *);;
    let rec evarName (g_, x_) =
      ...
      with 
           | None
               -> let name = newEVarName (g_, x_)
                    in begin
                         varInsert (name, evar_ x_);name
                         end
           | Some name -> name
      end;;
    (* bvarName (G, k) = name
    ...
    let rec decName' arg__1 arg__2 =
      ...
      with 
           | (role, (g_, IntSyn.Dec (None, v_)))
               -> let name =
                    findName (g_, namePrefOf (role, v_), extent role)
                    in (IntSyn.Dec ((Some name), v_))
           | (role, (g_, (IntSyn.Dec (Some name, v_) as d_))) -> begin
               if
               (varDefined name) ||
                 ((conDefined name) || (ctxDefined (g_, name)))
               then (IntSyn.Dec ((Some (tryNextName (g_, baseOf name))), v_))
               else d_ end
           | (role, (g_, (IntSyn.BDec (None, ((cid, t) as b)) as d_)))
               -> let name =
                    findName
                    (g_, "#" ^ (IntSyn.conDecName (IntSyn.sgnLookup cid)),
                     Local)
                    in (IntSyn.BDec ((Some name), b))
           ...
           | (role, (g_, IntSyn.ADec (None, d)))
               -> let name =
                    findName (g_, namePrefOf' (role, None), extent role)
                    in (IntSyn.ADec ((Some name), d))
           ...
           | (role, (g_, (IntSyn.NDec None as d_)))
               -> let name = findName (g_, "@x", Local)
                    in let _ = print name in (IntSyn.NDec ((Some name)))
      ...
      end(*      IntSyn.ADec(SOME(name), d) *)(* use #l as base name preference for label l *);;
    ...
    *);;
    let rec ctxName =
      function 
               ...
               | IntSyn.Decl (g_, d_)
                   -> let g'_ = ctxName g_
                        in (IntSyn.Decl (g'_, decName (g'_, d_)));;
    (* ctxLUName G = G'
    ...
    *);;
    let rec ctxLUName =
      function 
               | null_ -> IntSyn.null_
               | IntSyn.Decl (g_, d_)
                   -> let g'_ = ctxLUName g_
                        in (IntSyn.Decl (g'_, decLUName (g'_, d_)));;
    (* pisEName' (G, i, V) = V'
    ...
    let rec pisEName' =
      function 
               ...
               | (g_, i, IntSyn.Pi ((d_, maybe_), v_))
                   -> let d'_ = decEName (g_, d_)
                        in (IntSyn.Pi
                            ((d'_, IntSyn.maybe_),
                             pisEName' ((IntSyn.Decl (g_, d'_)), i - 1, v_)))(* i > 0 *);;
    (* | pisEName' (G, i, V) = V *);;
    let rec pisEName (i, v_) = pisEName' (IntSyn.null_, i, v_);;
    ...
    *);;
    let rec defEName' =
      function 
               | (g_, 0, uv_) -> uv_
               | (g_, i,
                  (IntSyn.Lam (d_, u_), IntSyn.Pi (((_, p_)(* = D *)), v_)))
                   -> let d'_ = decEName (g_, d_)
                        in let (u'_, v'_) =
                             defEName'
                             ((IntSyn.Decl (g_, d'_)), i - 1, (u_, v_))
                             in ((IntSyn.Lam (d'_, u'_)),
                                 (IntSyn.Pi ((d'_, p_), v'_)))(* i > 0 *);;
    (* | defEName' (G, i, (U, V)) = (U, V) *);;
    let rec defEName (imp, uv_) = defEName' (IntSyn.null_, imp, uv_);;
    let rec nameConDec' =
      function 
               | IntSyn.ConDec (name, parent, imp, status, v_, l_)
                   -> (IntSyn.ConDec
                       (name, parent, imp, status, pisEName (imp, v_), l_))
               | IntSyn.ConDef (name, parent, imp, u_, v_, l_, Anc)
                   -> let (u'_, v'_) = defEName (imp, (u_, v_))
                        in (IntSyn.ConDef
                            (name, parent, imp, u'_, v'_, l_, Anc))
               | IntSyn.AbbrevDef (name, parent, imp, u_, v_, l_)
                   -> let (u'_, v'_) = defEName (imp, (u_, v_))
                        in (IntSyn.AbbrevDef
                            (name, parent, imp, u'_, v'_, l_))
               | skodec -> skodec;;
    (* fix ??? *);;
    ...
    let rec nameConDec conDec =
      begin
        varReset IntSyn.null_;nameConDec' conDec
        end
      (* declaration is always closed *);;
    let rec skonstName name = tryNextName (IntSyn.null_, name);;
