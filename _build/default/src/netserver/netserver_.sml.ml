...
module NetServer(NetServer__0: sig
  ...
  struct
    ...
    type nonrec __1 = { init: unit -> unit ; reset: unit -> unit ;
      recv: server -> string -> unit ; send: server -> string -> unit ;
      done_: unit -> unit };;
    ...
    let rec loop f = begin
                       f ();loop f
                       end;;
    let rec vec2str v =
      String.implode
      (map
       (fun x -> Char.chr (Word8.toInt x))
       (Word8Vector.foldr ( :: ) [] v));;
    let rec str2vec l =
    ...
    let rec fileText fname =
      let s = TextIO.openIn fname
        in let txt = TextIO.inputAll s in let _ = TextIO.closeIn s in txt;;
    let rec fileData fname =
      let s = BinIO.openIn fname
        in let data = BinIO.inputAll s
             in let _ = BinIO.closeIn s in vec2str data;;
    exception Eof ;;
    exception Quit ;;
    ...
    let rec parseCmd s =
      let (c, a) = SS.position " " (Compat.Substring.full s)
        in (SS.string c, SS.string (SS.dropl Char.isSpace a));;
    ...
    let rec serveExample e = begin
      ...
      with 
           | e -> raise ((Error (("Exception " ^ (exnName e)) ^ " raised.")))
      else raise ((Error ("Unknown example " ^ (quote e)))) end;;
    (* Natural numbers *);;
    let rec getNat =
      function 
               | (t :: [])
    ...
    let rec exec' arg__3 arg__4 =
      ...
      with 
           ...
           | (conn, ("set", args))
               -> begin
                    setParm (String.tokens Char.isSpace args);Twelf.ok_
                    end
    ...
    let rec stripcr s =
      Substring.string
      (Substring.dropr (function 
                                 | x -> x = 'r') (Compat.Substring.full s));;
    let rec flashProto () =
      let buf : string ref = ref ""
        in let rec isnull = function 
                                     | '\000' -> true
                                     | _ -> false
             in let rec recv ((u : server)) s =
                  let _ = buf := ((! buf) ^ s)
                    in let (rem :: cmds) = rev (String.fields isnull (! buf))
                         in let _ = (App_ ((fun r -> r.exec) u, rev cmds))
                              in buf := rem
                  in let rec send ((u : server)) s =
                       (fun r -> r.send) u (s ^ "\000")
                       in {
                          init
                          = function 
                                     | ()
    ...
    let rec humanProto () =
      let buf : string ref = ref ""
        in let rec isnewl = function 
                                     | '\n' -> true
                                     | 'r' -> false
                                     | _ -> false
             in let rec recv ((u : server)) s =
                  let _ = buf := ((! buf) ^ s)
                    in let (rem :: cmds) = rev (String.fields isnewl (! buf))
                         in let _ =
                              (App_
                               ((fun r -> r.exec) u, map stripcr (rev cmds)))
                              in buf := rem
                  in let rec send ((u : server)) s =
                       (fun r -> r.send) u ("> " ^ s)
                       in {
    ...
    let rec httpProto dir =
      let ibuf : string ref = ref ""
        in let obuf : string ref = ref ""
             in let parsingHeaders = ref true
                  in let contentLength = ref 0
                       in let method_ : string ref = ref ""
                            in let url : string ref = ref ""
                                 in let headers : string list ref = ref []
                                      in let rec isnewl =
                                           function 
                                                    | '\n' -> true
                                                    | _ -> false
                                           in let rec handlePostRequest
                                                ((u : server)) =
                                                let shouldQuit =
                                                  try begin
                                                        (fun r -> r.exec)
    ...
    let rec protoServer ((proto : protocol)) portNum =
      let sock = INetSock.TCP.socket ()
        in let _ = S.Ctl.setREUSEADDR (sock, true)
             in let _ = S.bind (sock, INetSock.any portNum)
                  in let _ = S.listen (sock, maxConnections)
                       in let rec read_one conn u () =
                            let v = S.recvVec (conn, 1024) in begin
                              if (Word8Vector.length v) = 0 then raise Eof
                              else (fun r -> r.recv) proto u (vec2str v) end
                              (* arbitrary buffer size *)
                            in let rec accept_one () =
                                 let (conn, addr) = S.accept sock
                                   in let _ = (fun r -> r.reset) proto ()
                                        in let u =
                                             {
                                             send = send conn;
                                             exec = exec conn}
