...
module Conv(Conv__0: sig module Whnf : WHNF end) : CONV =
  struct
      ...
      struct
        ...
        and convSub =
          function 
                   ...
                   | (Shift n, (Dot _ as s2))
                       -> convSub ((Dot (idx_ (n + 1), (Shift (n + 1)))), s2)
                   | ((Dot _ as s1), Shift m)
                       -> convSub (s1, (Dot (idx_ (m + 1), (Shift (m + 1)))))
