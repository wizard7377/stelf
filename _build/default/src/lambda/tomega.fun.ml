...
module Tomega(Tomega__0: sig module Whnf : WHNF module Conv : CONV end) : TOMEGA
  =
  struct
    ...
    type tC_ =
      ...
      | Conj of tC_ * tC_ 
      | Base of
        ((IntSyn.exp_ * IntSyn.sub_) * (IntSyn.exp_ * IntSyn.sub_))
        Order.order_ ;;
      ...
      struct
        ...
        let rec lemmaDef lemma =
          begin match lemmaLookup lemma with 
                                             | ValDec (_, p_, _) -> p_ end;;
        let rec lemmaFor lemma =
          begin
          match lemmaLookup lemma
          with 
               | ForDec (_, f_) -> f_
               | ValDec (_, _, f_) -> f_
          end;;
          ...
          with 
               ...
               | (n, s)
                   -> begin
                      ...
                      with 
                           | ForDec (s', f_) -> begin
                               if s = s' then n else lemmaName' (n - 1) s end
        ...
        let rec revCoerceSub =
          function 
                   | I.Shift n -> (Shift n)
                   | I.Dot (ft_, t)
                       -> (Dot (revCoerceFront ft_, revCoerceSub t));;
        ...
        let rec orderSub =
          function 
                   | (O.Arg ((u_, s1), (v_, s2)), s)
                       -> (O.Arg
                           ((u_, I.comp (s1, s)), (v_, I.comp (s2, s))))
                   | (O.Lex os_, s)
                       -> (O.Lex
                           (map (function 
                                          | o_ -> orderSub (o_, s)) os_))
                   | (O.Simul os_, s)
                       -> (O.Simul
                           (map (function 
                                          | o_ -> orderSub (o_, s)) os_));;
        let rec tCSub_ =
          function 
                   ...
                   | (Conj (tc1_, tc2_), s)
                       -> (Conj (tCSub_ (tc1_, s), tCSub_ (tc2_, s)))
                   | (Abs (d_, tc_), s)
                       -> (Abs (I.decSub (d_, s), tCSub_ (tc_, I.dot1 s)));;
        let rec tCSubOpt_ =
          function 
                   | (None, s) -> None
        ...
        let rec normalizeTC' =
          function 
                   | O.Arg (us_, vs_)
                       -> (O.Arg
                           ((Whnf.normalize us_, I.id),
                            (Whnf.normalize vs_, I.id)))
        ...
        let rec normalizeTC =
          function 
                   ...
                   | Conj (tc1_, tc2_)
                       -> (Conj (normalizeTC tc1_, normalizeTC tc2_))
                   | Abs (d_, tc_)
                       -> (Abs
                           (Whnf.normalizeDec (d_, I.id), normalizeTC tc_));;
        ...
        and convTCs =
          function 
                   | ([], []) -> true
                   | ((o1_ :: l1_), (o2_ :: l2_))
                       -> (convTC' (o1_, o2_)) && (convTCs (l1_, l2_));;
        let rec convTC =
          function 
        ...
        let rec transformTC =
          function 
                   | (g_, All ((UDec d_, _), f_), os_)
                       -> (Abs
                           (d_, transformTC ((I.Decl (g_, d_)), f_, os_)))
                   | (g_, And (f1_, f2_), (o_ :: os_))
                       -> (Conj
                           (transformTC (g_, f1_, [o_]),
                            transformTC (g_, f2_, os_)))
        ...
        let rec weakenSub =
          function 
                   ...
                   | I.Decl (psi_, (PDec _ as d_))
                       -> comp (weakenSub psi_, shift);;
        let rec forSub =
          function 
                   | (All ((d_, q_), f_), t)
                   ...
                   | (Ex ((d_, q_), f_), t)
                       -> (Ex
                           ((I.decSub (d_, coerceSub t), q_),
                            forSub (f_, dot1 t)))
                   | (And (f1_, f2_), t)
                       -> (And (forSub (f1_, t), forSub (f2_, t)))
        ...
        and decSub =
          function 
                   | (PDec (x, f_, tc1_, None), t)
                       -> let s = coerceSub t
                            in (PDec
                                (x, forSub (f_, t), tCSubOpt_ (tc1_, s),
                                 None))
                     ...
                     | I.EClo (u_, t)
                         -> begin
                            ...
                            with 
                                 ...
                                 | Some i
                                     -> getFrontIndex
                                        (revCoerceFront (I.bvarSub (i, t)))
                            end
                     | (I.Lam (I.Dec (_, u1_), u2_) as u_)
                         -> try (Some (Whnf.etaContract u_))
                            with 
                                 | eta_ -> None
                     | _ -> None
        ...
        let rec coerceCtx =
          function 
                   ...
                   | I.Decl (psi_, PDec (x, _, _, _))
                       -> (I.Decl (coerceCtx psi_, (I.NDec x)));;
        let rec strengthenCtx psi_ =
          let w = weakenSub psi_
            in let s = invertSub w in (coerceCtx psi_, w, s);;
        let rec convFor =
          function 
                   | ((True, _), (True, _)) -> true
                   ...
                   | ((Ex ((d1_, _), f1_), t1), (Ex ((d2_, _), f2_), t2))
                       -> (Conv.convDec
                           ((d1_, coerceSub t1), (d2_, coerceSub t2))) &&
                            (convFor ((f1_, dot1 t1), (f2_, dot1 t2)))
                   | ((And (f1_, f1'_), t1), (And (f2_, f2'_), t2))
                       -> (convFor ((f1_, t1), (f2_, t2))) &&
                            (convFor ((f1'_, t1), (f2'_, t2)))
                   | _ -> false
        and convDec =
          function 
                   | ((UDec d1_, t1), (UDec d2_, t2))
                       -> Conv.convDec
                          ((d1_, coerceSub t1), (d2_, coerceSub t2))
                   | ((PDec (_, f1_, tc1_, tc1'_), t1),
                      (PDec (_, f2_, tc2_, tc2'_), t2))
                       -> begin
                            convFor ((f1_, t1), (f2_, t2));
                            begin
                              convTCOpt (tc1_, tc1'_);convTCOpt (tc2_, tc2'_)
                              end
                            
                            end;;
        let rec newEVar (psi_, f_) =
          (EVar
           (psi_, ref None, f_, None, None,
            I.newEVar (coerceCtx psi_, (I.Uni I.type_))));;
        let rec newEVarTC (psi_, f_, tc_, tc'_) =
          (EVar
           (psi_, ref None, f_, tc_, tc'_,
            I.newEVar (coerceCtx psi_, (I.Uni I.type_))));;
        let rec exists =
          function 
                   | (x, []) -> false
        ...
        let rec mkInst =
          function 
                   | 0 -> []
                   | n -> ((I.Root ((I.BVar n), I.nil_)) :: mkInst (n - 1));;
        let rec deblockify =
          function 
                   | null_ -> (I.null_, id)
                   | I.Decl (g_, I.BDec (x, (c, s)))
                       -> let (g'_, t') = deblockify g_
                            in let (_, l_) = I.constBlock c
                                 in let n = List.length l_
                                      in let g''_ =
                                           append
                                           (g'_,
                                            (l_, I.comp (s, coerceSub t')))
                                           in let t'' = comp (t', (Shift n))
                                                in let i_ =
                                                     (I.Inst (mkInst n))
                                                     in let t''' =
                                                          (Dot
        ...
        and append =
          function 
                   ...
                   | (g'_, ((d_ :: l_), s))
                       -> append
                          ((I.Decl (g'_, I.decSub (d_, s))), (l_, I.dot1 s));;
        let rec whnfFor =
          function 
        ...
        let rec normalizePrg =
          function 
                   ...
                   | (PairExp (u_, p'_), t)
                       -> (PairExp
                           (Whnf.normalize (u_, coerceSub t),
                            normalizePrg (p'_, t)))
                   | (PairBlock (b_, p'_), t)
                       -> (PairBlock
                           (I.blockSub (b_, coerceSub t),
                            normalizePrg (p'_, t)))
                   | (PairPrg (p1_, p2_), t)
                       -> (PairPrg
                           (normalizePrg (p1_, t), normalizePrg (p2_, t)))
                   | (Unit, _) -> Unit
                   | (EVar (_, { contents = Some p_}, _, _, _, _), t)
                       -> (PClo (p_, t))
                   | ((EVar _ as p_), t) -> (PClo (p_, t))
                   | (Lam (d_, p_), t)
                       -> (Lam
                           (normalizeDec (d_, t), normalizePrg (p_, dot1 t)))
                   | (Rec (d_, p_), t)
                       -> (Rec
                           (normalizeDec (d_, t), normalizePrg (p_, dot1 t)))
                   ...
                   | (PDec (name, f_, tc1_, None), t)
                       -> (PDec
                           (name, forSub (f_, t),
                            ...
                            None))
                   | (UDec d_, t)
                       -> (UDec (Whnf.normalizeDec (d_, coerceSub t)));;
        let rec normalizeSub =
          function 
                   | (Shift n as s) -> s
                   | Dot (Prg p_, s)
                       -> (Dot
                           ((Prg (normalizePrg (p_, id))), normalizeSub s))
                   | Dot (Exp e_, s)
                       -> (Dot
                           ((Exp (Whnf.normalize (e_, I.id))),
                            normalizeSub s))
        ...
        let rec derefPrg =
          function 
                   ...
                   | PairPrg (p1_, p2_)
                       -> (PairPrg (derefPrg p1_, derefPrg p2_))
                   | Unit -> Unit
                           ...
                           ((Cases
                             (flattenCases
                              (map
                               ...
                               cs_)))))
                   | Let (d_, p1_, p2_)
                       -> (Let (derefDec d_, derefPrg p1_, derefPrg p2_))
                   | LetPairExp (d1_, d2_, p1_, p2_)
                       -> (LetPairExp
                           (d1_, derefDec d2_, derefPrg p1_, derefPrg p2_))
                   | LetUnit (p1_, p2_)
                       -> (LetUnit (derefPrg p1_, derefPrg p2_))
        and flattenCases =
          function 
                   | ((psi_, s, Case (Cases l_)) :: cs_)
                       -> (map
                           (function 
                                     | (psi'_, s', p'_)
                                         -> (psi'_, comp (s, s'), p'_))
                           (flattenCases l_)) @ (flattenCases cs_)
                   | ((psi_, s, p_) :: cs_)
                       -> ((psi_, s, p_) :: flattenCases cs_)
                   | [] -> []
        and derefSpine =
          function 
                   | Nil -> Nil
