...
module type INTSYN = sig
                     ...
                     and status_ =
                       | Normal 
                       | Constraint of
                         csid * ((dec_ ctx_ * spine_ * int) -> exp_ option) 
                     ...
                     and conDec_ =
                       ...
                       | ConDef of
                         ...
                         exp_ *
                         uni_ *
                         (ancestor_(* d = M : A : type           *)(* a = A : K : kind  or       *))
...
(* Author: Frank Pfenning, Carsten Schuermann *);;
(* Modified: Roberto Virga *);;
module IntSyn(IntSyn__0: sig module Global : GLOBAL end) : INTSYN =
  struct
    type nonrec cid = int;;
    (* Constant identifier        *);;
  ...
  *);;
    ...
    and status_ =
      ...
      | Foreign of csid * (spine_ -> exp_) 
    and fgnUnify_ = | Succeed of fgnUnifyResidual_ list 
                    | Fail 
    and fgnUnifyResidual_ =
      | Assign of dec_ ctx_ * exp_ * exp_ * sub_ 
      | Delay of exp_ * cnstr_ ref 
  ...
  *);;
    ...
    open!
      struct
        ...
        let sgnArray =
          (Array.array (maxCid + 1, dummyEntry) : conDec_ Array.array);;
        let nextCid = ref 0;;
        let maxMid = Global.maxMid;;
        let sgnStructArray =
          (Array.array (maxMid + 1, (StrDec ("", None))) : strDec_
           Array.array);;
        let nextMid = ref 0;;
        end;;
    (* Invariants *);;
    ...
    let rec sgnClean i = begin
      if i >= (! nextCid) then () else
      begin
        Array.update (sgnArray, i, dummyEntry);sgnClean (i + 1)
        end
      end;;
    let rec sgnReset () =
      begin
    ...
    let rec sgnApp f =
      let rec sgnApp' cid = begin
        if cid = (! nextCid) then () else begin
                                            f cid;sgnApp' (cid + 1)
                                            end
        end in sgnApp' 0;;
    let rec sgnStructAdd strDec =
      let mid = ! nextMid in begin
        if mid > maxMid then
        raise
        ((Error
    ...
    let rec rename (cid, new_) =
      let newConDec =
        ...
        end in Array.update (sgnArray, cid, newConDec);;
    let rec constDef d =
      begin
      match sgnLookup d
      with 
   ...
   *);;
    let rec comp =
      function 
               ...
               | (Dot (ft_, s), s')
                   -> (Dot (frontSub (ft_, s'), comp (s, s')))(* Sat Feb 14 10:15:16 1998 -fp *)(* roughly 15% on standard suite for Twelf 1.1 *)(* next line is an optimization *)
    and bvarSub =
      function 
               | (1, Dot (ft_, s)) -> ft_
    ...
    and blockSub =
      function 
               ...
               | ((Inst uLs_ as l_), s')
                   -> (Inst (map (function 
                                           | u_ -> (EClo (u_, s'))) uLs_))(* comp(^k, s) = ^k' for some k' by invariant *)(* was:
        LVar (r, comp(sk, s), (l, comp (t, s)))
        July 22, 2010 -fp -cs
       *)(* Thu Dec  6 20:30:26 2001 -fp !!! *)(* where is this needed? *)(* Since always . |- t : Gsome, discard s *)(* --cs Sun Dec  1 11:25:41 2002 *)(* -fp Sun Dec  1 21:18:30 2002 *)
    and frontSub =
      function 
               | (Idx n, s) -> bvarSub (n, s)
  ...
  *);;
    let rec blockDec (g_, (Bidx k as v), i) =
      let BDec (_, (l, s)) = ctxDec (g_, k)
        in let (gsome_, lblock_) = conDecBlock (sgnLookup l)
             in let rec blockDec' =
                  function 
                           ...
                           | (t, (_ :: l_), n, j)
                               -> blockDec'
                                  ((Dot
                                    ((Exp ((Root ((Proj (v, j)), Nil)))), t)),
                                   l_, n - 1, j + 1)
                  in blockDec' (s, lblock_, i, 1) (* G |- s : Gsome *);;
    (* EVar related functions *);;
