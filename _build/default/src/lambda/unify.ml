...
module type UNIFY = sig
                    ...
                    val
                      addConstraint : (IntSyn.cnstr list ref * IntSyn.cnstr) ->
                                      unit
                    val solveConstraint : IntSyn.cnstr -> unit
                    val delay : (IntSyn.eclo * IntSyn.cnstr) -> unit
                    (* unification *)
                    val
                      intersection : (IntSyn.sub_ * IntSyn.sub_) ->
                                     IntSyn.sub_
                    exception Unify of string 
                    val
                      unify : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                              unit
                    (* raises Unify *)
                    val
                      unifyW : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                               unit
                    (* raises Unify *)
                    val
                      unifyBlock : (IntSyn.dctx *
                                    IntSyn.block_ *
                                    IntSyn.block_) ->
                                   unit
                    (* raises Unify *)
                    val
                      unifySub : (IntSyn.dctx * IntSyn.sub_ * IntSyn.sub_) ->
                                 unit
                    (* raises Unify *)
                    val
                      invertible : (IntSyn.dctx *
                                    IntSyn.eclo *
                                    IntSyn.sub_ *
                                    IntSyn.exp_
                                    option
                    ...
                    val
                      unifiable : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                                  bool
  ...
  struct
    ...
    open!
      struct
        ...
        type cAction_ = | BindCnstr of cnstr_ ref * cnstr_ ;;
        type fAction_ =
        ...
        let rec copy =
          function 
                   ...
                   | Solve (cnstr, cnstr_)
                       -> (FSolve (cnstr, cnstr_, ! cnstr));;
        let rec resetCnstr =
          function 
                   | [] -> []
                   | (BindCnstr (refC, cnstr_) :: l_)
                       -> begin
                            refC := cnstr_;(refC :: resetCnstr l_)
                            end;;
        let rec reset =
          function 
                   | BindExp (refU, u_)
                       -> begin
                            refU := u_;(Instantiate refU)
                            end
                   | BindBlock (refB, b_)
                       -> begin
                            refB := b_;(InstantiateBlock refB)
                            end
                   | BindAdd (cnstrs, cActions_)
                       -> begin
                            cnstrs := (resetCnstr cActions_);(Add cnstrs)
                            end
                   | FSolve (refCnstr, cnstr_, cnstr'_)
                       -> begin
                            refCnstr := cnstr'_;(Solve (refCnstr, cnstr_))
                            end;;
        ...
        let rec solveConstraint (({ contents = cnstr_} as cnstr)) =
          begin
            cnstr := Solved;Trail.log (globalTrail, (Solve (cnstr, cnstr_)))
            end;;
        let rec delayExpW =
          function 
                   | (((Uni l_ as u_), s1), _) -> ()
                   ...
                   | ((Root (h_, s_), s), cnstr)
                       -> begin
                            delayHead (h_, cnstr);delaySpine ((s_, s), cnstr)
                            end
                   | ((Lam (d_, u_), s), cnstr)
                       -> begin
                            delayDec ((d_, s), cnstr);
                   ...
                   | ((EVar (g_, r, v_, cnstrs), s), cnstr)
                       -> addConstraint (cnstrs, cnstr)
                   | ((FgnExp csfe, s), cnstr)
                       -> FgnExpStd.App.apply
                          csfe
                          (function 
                                    | u_ -> delayExp ((u_, s), cnstr))
        ...
        let rec intersection =
          function 
                   | (Dot (Idx k1, s1), Dot (Idx k2, s2)) -> begin
                       if k1 = k2 then dot1 (intersection (s1, s2)) else
                       comp (intersection (s1, s2), shift) end
                   | ((Dot _ as s1), Shift n2)
                       -> intersection
                          (s1, (Dot ((Idx (n2 + 1)), (Shift (n2 + 1)))))
                   | (Shift n1, (Dot _ as s2))
                       -> intersection
                          ((Dot ((Idx (n1 + 1)), (Shift (n1 + 1)))), s2)
                   | (Shift _, Shift _) -> id;;
        let rec weakenSub =
          function 
                   | (g_, Shift n, ss) -> begin
                       ...
                       weakenSub
                       (g_, (Dot ((Idx (n + 1)), (Shift (n + 1)))), ss) else
                       id end
                   | (g_, Dot (Idx n, s'), ss)
                       -> begin
                          match bvarSub (n, ss)
        ...
        and invertExpW =
          function 
                   ...
                   | (g_, ((EVar (r, gx_, v_, cnstrs) as x_), s), ss, rOccur)
                       -> begin
                       if rOccur = r then raise NotInvertible else begin
                       if Whnf.isPatSub s then
                       let w = weakenSub (g_, s, ss) in begin
                         if Whnf.isId w then (EClo (x_, comp (s, ss))) else
                         raise NotInvertible end
                       else (EClo (x_, invertSub (g_, s, ss, rOccur))) end
                       end
        ...
        and pruneExpW =
          function 
                   ...
                   | (g_, ((EVar (r, gx_, v_, cnstrs) as x_), s), ss, rOccur)
                       -> begin
                       if rOccur = r then
                       raise ((Unify "Variable occurrence")) else begin
                       if Whnf.isPatSub s then
                       let w = weakenSub (g_, s, ss) in begin
                         if Whnf.isId w then (EClo (x_, comp (s, ss))) else
                         let wi = Whnf.invert w
                           in let v'_ = pruneExp (gx_, (v_, id), wi, rOccur)
                                in let gy_ = pruneCtx (wi, gx_, rOccur)
                                     in let y_ = newEVar (gy_, v'_)
                                          in let yw_ = (EClo (y_, w))
                                               in let _ =
                                                    instantiateEVar
                                                    (r, yw_, ! cnstrs)
                                                    in (EClo
                       ...
                       with 
                            | NotInvertible
                                -> let gy_ = pruneCtx (ss, g_, rOccur)
                                     in let v'_ =
                                          pruneExp (g_, (v_, s), ss, rOccur)
                                          in let y_ = newEVar (gy_, v'_)
                                               in let _ =
                                                    addConstraint
                                                    (cnstrs,
        ...
        and pruneHead =
          function 
                   ...
                   | (g_, (Proj (LVar (r, sk, (l, t)), i) as h_), ss, rOccur)
                       -> begin
                            pruneSub (g_, t, id, rOccur);h_
                            end
                   | (g_, (Skonst _ as h_), ss, rOccur) -> h_
                   | (g_, (Def _ as h_), ss, rOccur) -> h_
                   | (g_, FVar (x, v_, s'), ss, rOccur)
                       -> begin
                            pruneExp (g_, (v_, id), id, rOccur);
                            (FVar (x, v_, comp (s', ss)))
                            end
                   | (g_, (FgnConst _ as h_), ss, rOccur) -> h_
        and pruneSub =
          ...
          function 
                   ...
                   | (Dot (Idx k, t), Decl (g_, d_), rOccur)
                       -> let t' = comp (t, invShift)
                            in let d'_ = pruneDec (g_, (d_, id), t', rOccur)
                                 in (Decl (pruneCtx (t', g_, rOccur), d'_))
                   | (Dot (Undef, t), Decl (g_, d), rOccur)
                       -> pruneCtx (t, g_, rOccur)
                   | (Shift n, g_, rOccur)
                       -> pruneCtx
        ...
        let rec unifyExpW =
          function 
                   | (g_, ((FgnExp csfe1, _) as us1_), us2_)
                       -> begin
                          match FgnExpStd.UnifyWith.apply
                                csfe1
                                (g_, (EClo us2_))
                          with 
                               | Succeed residualL
                                   -> let rec execResidual =
                                        function 
                                                 ...
                                                 | Delay (u_, cnstr)
                                                     -> delayExp
                                                        ((u_, id), cnstr)
                                        in List.app execResidual residualL
                               | Fail
                                   -> raise
                                      ((Unify "Foreign Expression Mismatch"))
                          end
                       ...
                       -> begin
                          ...
                          with 
                               | (BVar k1, BVar k2) -> begin
                                   if k1 = k2 then
                                   unifySpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Unify "Bound variable clash")) end
                               | (Const c1, Const c2) -> begin
                                   if c1 = c2 then
                                   unifySpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Unify "Constant clash")) end
                               | (Proj (b1, i1), Proj (b2, i2)) -> begin
                                   ...
                                   raise ((Unify "Global parameter clash"))
                                   end
                               | (Skonst c1, Skonst c2) -> begin
                                   if c1 = c2 then
                                   unifySpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Unify "Skolem constant clash"))
                                   end
                               | (FVar (n1, _, _), FVar (n2, _, _)) -> begin
                                   if n1 = n2 then
                                   unifySpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Unify "Free variable clash")) end
                               | (Def d1, Def d2) -> begin
                                   if d1 = d2 then
                                   unifySpine (g_, (s1_, s1), (s2_, s2)) else
                                   unifyDefDefW (g_, us1_, us2_) end
                               | (Def d1, Const c2)
                                   -> begin
                                      match defAncestor d1
                                      with 
                                           | Anc (_, _, None)
                                               -> unifyExpW
                                                  (g_, Whnf.expandDef us1_,
                                                   us2_)
                                           | Anc (_, _, Some c1) -> begin
                                               ...
                                               raise
                                               ((Unify "Constant clash")) end
                                      end
                               | (Const c1, Def d2)
                                   -> begin
                                      match defAncestor d2
                                      with 
                                           | Anc (_, _, None)
                                               -> unifyExpW
                                                  (g_, us1_,
                               ...
                               | (Def d1, BVar k2)
                                   -> raise ((Unify "Head mismatch"))
                               | (BVar k1, Def d2)
                                   -> raise ((Unify "Head mismatch"))
                               | (Def d1, _)
                                   -> unifyExpW
                                      (g_, Whnf.expandDef us1_, us2_)
                               | (_, Def d2)
                                   -> unifyExpW
                                      (g_, us1_, Whnf.expandDef us2_)
                               | (FgnConst (cs1, ConDec (n1, _, _, _, _, _)),
                                  FgnConst (cs2, ConDec (n2, _, _, _, _, _)))
                                   -> begin
                                   if (cs1 = cs2) && (n1 = n2) then () else
                                   raise ((Unify "Foreign Constant clash"))
                                   end
                               | (FgnConst
                                  (cs1, ConDef (n1, _, _, w1_, _, _, _)),
                                  FgnConst
                                  (cs2, ConDef (n2, _, _, v_, w2_, _, _)))
                                   -> begin
                                   if (cs1 = cs2) && (n1 = n2) then () else
                                   unifyExp (g_, (w1_, s1), (w2_, s2)) end
                               | (FgnConst
                                  (_, ConDef (_, _, _, w1_, _, _, _)), _)
                                   -> unifyExp (g_, (w1_, s1), us2_)
                               | (_, FgnConst
                                  (_, ConDef (_, _, _, w2_, _, _, _)))
                   ...
                   | (g_, ((Pi (_, _), _) as us1_),
                      ((Root (Def _, _), _) as us2_))
                       -> unifyExpW (g_, us1_, Whnf.expandDef us2_)
                   | (g_, ((Root (Def _, _), _) as us1_),
                      ((Pi (_, _), _) as us2_))
                   ...
                   | (g_,
                      ...
                      (((EVar (r2, g2_, v2_, cnstrs2) as u2_), s2) as us2_))
                       ...
                       let s' = intersection (s1, s2) in begin
                         if Whnf.isId s' then () else
                         let ss' = Whnf.invert s'
                           in let v1'_ = (EClo (v1_, ss'))
                                in let g1'_ = Whnf.strengthen (ss', g1_)
                                     in instantiateEVar
                                        (r1,
                                         (EClo (newEVar (g1'_, v1'_), s')),
                                         ! cnstrs1)
                         end
                       else
                       addConstraint
                       (cnstrs2, ref ((Eqn (g_, (EClo us2_), (EClo us1_)))))
                       end else
                       addConstraint
                       (cnstrs1, ref ((Eqn (g_, (EClo us1_), (EClo us2_)))))
                       end else begin
                       if Whnf.isPatSub s1 then
                       let ss1 = Whnf.invert s1
                         in let u2'_ = pruneExp (g_, us2_, ss1, r1)
                              in instantiateEVar (r1, u2'_, ! cnstrs1)
                       else begin
                       if Whnf.isPatSub s2 then
                       let ss2 = Whnf.invert s2
                         in let u1'_ = pruneExp (g_, us1_, ss2, r2)
                              in instantiateEVar (r2, u1'_, ! cnstrs2)
                       else
                       let cnstr = ref ((Eqn (g_, (EClo us1_), (EClo us2_))))
                         in addConstraint (cnstrs1, cnstr)
                       end end end
                   | (g_, ((EVar (r, gx_, v_, cnstrs), s) as us1_),
                      ((u2_, s2) as us2_)) -> begin
                       if Whnf.isPatSub s then
                       let ss = Whnf.invert s
                         in let u2'_ = pruneExp (g_, us2_, ss, r)
                              in instantiateEVar (r, u2'_, ! cnstrs)
                       else
                       addConstraint
                       (cnstrs, ref ((Eqn (g_, (EClo us1_), (EClo us2_)))))
                       end
                   | (g_, ((u1_, s1) as us1_),
                      ((EVar (r, gx_, v_, cnstrs), s) as us2_)) -> begin
                       if Whnf.isPatSub s then
                       let ss = Whnf.invert s
                         in let u1'_ = pruneExp (g_, us1_, ss, r)
                              in instantiateEVar (r, u1'_, ! cnstrs)
                       else
                       addConstraint
                       (cnstrs, ref ((Eqn (g_, (EClo us1_), (EClo us2_)))))
                       end
        ...
        and unifyDefDefW
          ...
          let Anc (_, h1, c1Opt) = defAncestor d1
            in let Anc (_, h2, c2Opt) = defAncestor d2
                 in let _ =
                      begin
                      match (c1Opt, c2Opt)
                      with 
                           | (Some c1, Some c2) -> begin
                               ...
                               raise
                               ((Unify
                                 "Irreconcilable defined constant clash"))
                               else () end
                           | _ -> ()
                      end
                      in begin
                         match Int.compare (h1, h2)
                         with 
                              | Equal
                                  -> unifyExpW
                                     (g_, Whnf.expandDef us1_,
        ...
        and unifySub =
          function 
                   ...
                   | (g_, Shift n, (Dot _ as s2))
                       -> unifySub
                          (g_, (Dot ((Idx (n + 1)), (Shift (n + 1)))), s2)
                   | (g_, (Dot _ as s1), Shift m)
                       -> unifySub
                          (g_, s1, (Dot ((Idx (m + 1)), (Shift (m + 1)))))
                   | (g_, Dot (ft1_, s1), Dot (ft2_, s2))
                       -> begin
                            begin
                            match (ft1_, ft2_)
                            with 
                                 | (Idx n1, Idx n2) -> begin
                                     if n1 <> n2 then
                                     raise
                                     ((Error "SOME variables mismatch")) else
                                     () end
                                 | (Exp u1_, Exp u2_)
                                     -> unifyExp (g_, (u1_, id), (u2_, id))
                                 | (Exp u1_, Idx n2)
                                     -> unifyExp
                                        (g_, (u1_, id),
                                         ((Root ((BVar n2), Nil)), id))
                                 | (Idx n1, Exp u2_)
                                     -> unifyExp
                                        (g_, ((Root ((BVar n1), Nil)), id),
                                         (u2_, id))
                            end;unifySub (g_, s1, s2)
                            end
        and unifyBlock =
          function 
        ...
        and unifyBlockW =
          function 
                   | (g_, LVar (r1, (Shift k1 as s1), (l1, t1)), LVar
                      (r2, (Shift k2 as s2), (l2, t2))) -> begin
                       if l1 <> l2 then raise ((Unify "Label clash")) else
                       begin
                       if r1 = r2 then () else
                       begin
                         unifySub (g_, comp (t1, s1), comp (t2, s2));begin
                         if k1 < k2 then
                         instantiateLVar
                         (r1, (LVar (r2, (Shift (k2 - k1)), (l2, t2)))) else
                         instantiateLVar
                         (r2, (LVar (r1, (Shift (k1 - k2)), (l1, t1)))) end
                         end
                       end end
                   | (g_, LVar (r1, s1, (l1, t1)), b2_)
                       -> instantiateLVar
                          (r1, blockSub (b2_, Whnf.invert s1))
                   | (g_, b1_, LVar (r2, s2, (l2, t2)))
                       -> instantiateLVar
                          (r2, blockSub (b1_, Whnf.invert s2))
                   | (g_, Bidx n1, Bidx n2) -> begin
                       if n1 <> n2 then raise ((Unify "Block index clash"))
                       else () end;;
        let rec unify1W (g_, us1_, us2_) =
          begin
            unifyExpW (g_, us1_, us2_);awakeCnstr (nextCnstr ())
            end
        and unify1 (g_, us1_, us2_) =
          begin
            unifyExp (g_, us1_, us2_);awakeCnstr (nextCnstr ())
            end
        and awakeCnstr =
          function 
                   | None -> ()
                   ...
                   | Some { contents = FgnCnstr csfc} -> begin
                       if FgnCnstrStd.Awake.apply csfc () then () else
                       raise ((Unify "Foreign constraint violated")) end;;
        let rec unifyW (g_, us1_, us2_) =
          begin
            resetAwakenCnstrs ();unify1W (g_, us1_, us2_)
            end;;
        let rec unify (g_, us1_, us2_) =
          begin
            resetAwakenCnstrs ();unify1 (g_, us1_, us2_)
            end;;
        end;;
    (* ? *);;
...
*);;
    ...
    let rec invertible (g_, us_, ss, rOccur) =
      try begin
            invertExp (g_, us_, ss, rOccur);true
            end
      with 
           | NotInvertible -> false;;
