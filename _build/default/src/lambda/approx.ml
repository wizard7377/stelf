...
module type APPROX = sig
                     ...
                     val
                       exactToApx : (IntSyn.exp_ * IntSyn.exp_) ->
                                    exp_ * exp_ * uni_
                     exception Ambiguous 
                     val apxToUni : uni_ -> IntSyn.uni_
                     val
                       apxToClass : (IntSyn.dctx * exp_ * uni_ * bool) ->
                                    IntSyn.exp_
                     val
                       apxToExact : (IntSyn.dctx * exp_ * IntSyn.eclo * bool) ->
                                    IntSyn.exp_
...
module Approx(Approx__0: sig module Whnf : WHNF end) : APPROX =
  struct
    ...
    let rec getReplacementName ((CVar r as u_), v_, l_, allowed) =
      begin
      match varLookupRef r
      with 
           | Some (_, name) -> name
           | None
               -> let _ = begin if allowed then () else raise Ambiguous end
                    in let pref =
                         begin
                         match whnfUni l_
                         with 
                         ...
                         in let rec try_ i =
                              ...
                              in try_ 1 (* others impossible by invariant *)
      end;;
    ...
    let rec expToApx =
      function 
               | I.Uni l_
                   -> let l'_ = uniToApx l_
                        in ((Uni l'_), (Uni (whnfUni ((Next l'_)))))
               | I.Pi ((I.Dec (_, v1_), _), v2_)
                   -> let ((v1'_, _)(* Type *)) = expToApx v1_
                        in let (v2'_, l'_) = expToApx v2_
                             in ((Arrow (v1'_, v2'_)), l'_)
    ...
    let rec classToApx v_ =
      let (v'_, l'_) = expToApx v_ in let Uni l''_ = whnf l'_ in (v'_, l''_);;
    (* exactToApx (U, V) = (U-, V-)
     if G |- U : V *);;
    let rec exactToApx (u_, v_) =
      let (v'_, l'_) = classToApx v_
        in begin
           match whnfUni l'_
           with 
                | Level 1 -> (Undefined, v'_, l'_)
                | _
                    -> let ((u'_, _)(* V' *)) = expToApx u_
                         in (u'_, v'_, l'_)
           end
        (* Type *)(* Kind/Hyperkind *);;
    (* constDefApx (d) = V-
      ...
      function 
               ...
               | ((g_, Const h_, l_, allowed)(* Type *))
                   -> (I.Root
                       (h_,
                        Whnf.newSpineVar
                        (g_, (I.conDecType (headConDec h_), I.id))))(* convert undetermined CVars to FVars *)(* also, does the name of the bound variable here matter? *)(* this is probably very bad -- it should be possible to infer
         more accurately which pis can be dependent *)
    and apxToClass (g_, v_, l_, allowed) =
               ...
               | (g_, u_, (I.Pi ((d_, _), v_), s), allowed)
                   -> let d'_ = I.decSub (d_, s)
                        in (I.Lam
                            (d'_,
                             apxToExact
                             ((I.Decl (g_, d'_)), u_, (v_, I.dot1 s),
                              allowed)))
               | (g_, u_, (I.Uni l_, s), allowed)
                   -> apxToClass (g_, u_, uniToApx l_, allowed)
               | (g_, u_, ((I.Root (I.FVar (name, _, _), _), s) as vs_),
                  allowed)
                   -> let ((v_, l_, _)(* Next L *)) =
                        findByReplacementName name
                        in let Uni l_ = whnf l_
                             in begin
                                match whnfUni l_
                                with 
                                     | Level 1
    ...
    let rec matchUniW =
      function 
               | (Level i1, Level i2) -> begin
                   if i1 = i2 then () else raise ((Unify "Level clash")) end
               | (Level i1, Next l2_) -> begin
                   if i1 > 1 then matchUniW ((Level (i1 - 1)), l2_) else
                   raise ((Unify "Level clash")) end
               | (Next l1_, Level i2) -> begin
                   if i2 > 1 then matchUniW (l1_, (Level (i2 - 1))) else
                   raise ((Unify "Level clash")) end
    ...
    let rec matchW =
      function 
                   ...
                   -> begin
                      ...
                      with 
                           | (I.Const c1, I.Const c2) -> begin
                               if c1 = c2 then () else
                               raise ((Unify "Type/kind constant clash")) end
                           | (I.Def d1, I.Def d2) -> begin
                               if d1 = d2 then () else
                               match_ (constDefApx d1, constDefApx d2) end
               ...
               | ((Arrow _ as v1_), Const (I.Def d2))
                   -> match_ (v1_, constDefApx d2)
               | (Const (I.Def d1), (Arrow _ as v2_))
                   -> match_ (constDefApx d1, v2_)
               | ((Arrow _ as v1_), Const (I.NSDef d2))
                   -> match_ (v1_, constDefApx d2)
