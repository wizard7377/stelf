...
module type MATCH = sig
                    ...
                    val
                      match_ : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                               unit
                    (* raises Unify *)
                    val
                      matchW : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                               unit
                    (* raises Unify *)
                    val
                      matchBlock : (IntSyn.dctx *
                                    IntSyn.block_ *
                                    IntSyn.block_) ->
                                   unit
                    (* raises Unify *)
                    val
                      matchSub : (IntSyn.dctx * IntSyn.sub_ * IntSyn.sub_) ->
                                 unit
                    (* raises Unify *)
                    (* instance (G, Us,Us') will instantiate EVars as an effect 
     checks if Us' is an instance of Us *)
                    val
                      instance : (IntSyn.dctx * IntSyn.eclo * IntSyn.eclo) ->
                                 bool
  ...
  struct
    ...
    open!
      struct
        ...
        and pruneExpW =
          function 
                   ...
                   | (g_, ((EVar (r, gx_, v_, cnstrs) as x_), s), ss, rOccur)
                       -> begin
                       if rOccur = r then
                       raise ((Match "Variable occurrence")) else begin
          ...
          function 
                   | (g_, BVar k, ss, rOccur)
                       -> begin
                          ...
                          with 
                               | Undef
                                   -> raise ((Match "Parameter dependency"))
                               | Idx k' -> (BVar k')
                          end
                   ...
                   | (g_, ((FgnExp csfe1, _) as us1_), us2_)
                       -> begin
                          ...
                          with 
                               ...
                               | Fail
                                   -> raise
                                      ((Match "Foreign Expression Mismatch"))
                          end
                       ...
                       -> begin
                          ...
                          with 
                               | (BVar k1, BVar k2) -> begin
                                   if k1 = k2 then
                                   matchSpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Match "Bound variable clash")) end
                               | (Const c1, Const c2) -> begin
                                   if c1 = c2 then
                                   matchSpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Match "Constant clash")) end
                               | (Proj (b1, i1), Proj (b2, i2)) -> begin
                                   ...
                                   raise ((Match "Global parameter clash"))
                                   end
                               | (Skonst c1, Skonst c2) -> begin
                                   if c1 = c2 then
                                   matchSpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Match "Skolem constant clash"))
                                   end
                               | (FVar (n1, _, _), FVar (n2, _, _)) -> begin
                                   if n1 = n2 then
                                   matchSpine (g_, (s1_, s1), (s2_, s2)) else
                                   raise ((Match "Free variable clash")) end
                               | (Def d1, Def d2) -> begin
                                   if d1 = d2 then
                                   matchSpine (g_, (s1_, s1), (s2_, s2)) else
                                   matchDefDefW (g_, us1_, us2_) end
                               | (Def d1, Const c2)
                                   -> begin
                               ...
                               | (FgnConst (cs1, ConDec (n1, _, _, _, _, _)),
                                  FgnConst (cs2, ConDec (n2, _, _, _, _, _)))
                                   ...
                                   raise ((Match "Foreign Constant clash"))
                                   end
                               | (FgnConst
                      ...
                      ((u2_, s2) as us2_)) -> begin
                       ...
                       let ss = Whnf.invert s
                         in let u2'_ = pruneExp (g_, us2_, ss, r)
                              in Unify.instantiateEVar (r, u2'_, ! cnstrs)
                       else
                       Unify.addConstraint
        ...
        and matchBlockW =
          function 
                   | (g_, LVar (r1, Shift k1, (l1, t1)), LVar
                      (r2, Shift k2, (l2, t2))) -> begin
                       if l1 <> l2 then raise ((Match "Label clash")) else
                       begin
                       if r1 = r2 then () else
                       begin
                         matchSub (g_, t1, t2);
                         begin
                           begin if k1 <> k2 then raise bind_ else () end;
                           let ss = Whnf.invert ((Shift k1))
                             in let t2' = pruneSub (g_, t2, ss, ref None)
                                  in Unify.instantiateLVar
                   ...
                   | (g_, Bidx n1, Bidx n2) -> begin
                       if n1 <> n2 then raise ((Match "Block index clash"))
                       else () end;;
        let rec match1W (g_, us1_, us2_) =
          begin
            matchExpW (g_, us1_, us2_);awakeCnstr (Unify.nextCnstr ())
            end
        and match1 (g_, us1_, us2_) =
          begin
            matchExp (g_, us1_, us2_);awakeCnstr (Unify.nextCnstr ())
            end
        and awakeCnstr =
          function 
                   | None -> ()
                   | Some { contents = Solved}
                   ...
                   | Some { contents = FgnCnstr csfc} -> begin
                       ...
                       raise ((Match "Foreign constraint violated")) end;;
        let rec matchW (g_, us1_, us2_) =
          begin
            Unify.resetAwakenCnstrs ();match1W (g_, us1_, us2_)
            end;;
        let rec match_ (g_, us1_, us2_) =
          begin
            Unify.resetAwakenCnstrs ();match1 (g_, us1_, us2_)
            end;;
        end;;
    (* weakenSub (G1, s, ss) = w'
...
*);;
    ...
    let rec instance (g_, us1_, us2_) =
      try begin
            match_ (g_, us1_, us2_);true
            end with 
                     | Match msg -> false;;
    let rec instance' (g_, us1_, us2_) =
      try begin
            match_ (g_, us1_, us2_);None
            end
      with 
           | Match msg -> (Some msg);;
    end;;
