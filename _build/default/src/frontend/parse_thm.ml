# 1 "src/frontend/parse_thm.sig.ml"
open! Basis;;
(* Parsing Theorems *);;
      ...
      struct
        ...
        let rec idToNat (r, name) =
          ...
          with 
               ...
               | L.NotDigit _
                   -> Parsing.error (r, "Identifier not a natural number");;
        let rec stripRParen =
          function 
        ...
        let rec decideRBrace =
          function 
                   | (r0, (orders, LS.Cons ((rbrace_, r), s')))
                       -> ((Some (E.lex (r0, orders))), LS.expose s')
                   | (r0, (order, LS.Cons ((t, r), _)))
                       -> Parsing.error
                          (P.join (r0, r),
                           "Expected `}', found " ^ (L.toString t));;
        let rec decideRBracket =
          function 
                   | (r0, (orders, LS.Cons ((rbracket_, r), s')))
        ...
        let rec parseIds =
          function 
                   | LS.Cons ((L.Id (upper_, id), r), s')
                       -> let (ids, f') = parseIds (LS.expose s')
                            in ((id :: ids), f')
                   | LS.Cons (((L.Id (_, id) as t), r), s')
                       -> Parsing.error
                          (r,
                           "Expecter upper case identifier, found " ^
                             (L.toString t))
                   | f -> ([], f);;
        let rec parseArgPat =
          function 
                   ...
                   | LS.Cons ((L.Id (_, id), r), s')
                       -> Parsing.error
                          (r, "Expected upper case identifier, found " ^ id)
          ...
          function 
                   | LS.Cons ((L.Id (_, id), r), s')
                       -> let (idOpts, (LS.Cons ((_, r'), _) as f')) =
                            ...
                            in ((id, idOpts, P.join (r, r')), f')
                   | LS.Cons ((t, r), s)
                       -> Parsing.error
                          (r,
                           "Expected call pattern, found token " ^
                             (L.toString t));;
          ...
          function 
                   | LS.Cons ((lparen_, r), s')
                       -> let (cpat, f') = parseCallPat (LS.expose s')
                            in let (cpats, f'') =
                                 parseCallPats (stripRParen f')
                                 in ((cpat :: cpats), f'')
                   | (LS.Cons ((dot_, r), s') as f) -> ([], f)
                   | LS.Cons ((t, r), s)
                       -> Parsing.error
                          (r,
                           "Expected call patterns, found token " ^
                             (L.toString t));;
        ...
        and parseOrder' =
          function 
                   ...
                   | (None, LS.Cons ((t, r), s'))
                       -> Parsing.error
                          (r, "Expected order, found " ^ (L.toString t));;
        let rec parseTDecl f =
          let (order, f') = parseOrder f
            in let (callpats, f'') = parseCallPats f'
                 in (E.tdecl (order, E.callpats callpats), f'');;
        let rec parseTerminates' (LS.Cons ((terminates_, r), s')) =
          parseTDecl (LS.expose s');;
        let rec parseTotal' (LS.Cons ((total_, r), s')) =
          parseTDecl (LS.expose s');;
        let rec parsePDecl =
          function 
                   | LS.Cons ((L.Id (_, id), r), s')
                       -> let depth = idToNat (r, id)
                            in let (t', f') = parseTDecl (LS.expose s')
                                 in (E.prove (depth, t'), f')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected theorem identifier, found " ^
                             (L.toString t));;
        let rec parseProve' (LS.Cons ((prove_, r), s')) =
        ...
        let rec parseEDecl =
          function 
                   | LS.Cons ((L.Id (_, id), r), s')
                       -> let depth = idToNat (r, id)
                            in let (t', f') = parseTDecl (LS.expose s')
                                 in (E.establish (depth, t'), f')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected theorem identifier, found " ^
                             (L.toString t));;
        let rec parseEstablish' (LS.Cons ((establish_, r), s')) =
          parseEDecl (LS.expose s');;
        let rec parseAssert' (LS.Cons ((assert_, r), s')) =
          let (callpats, f'') = parseCallPats (LS.expose s')
            in (E.assert_ (E.callpats callpats), f'');;
        let rec stripRBrace =
          function 
        ...
        and parseDecs' =
          function 
                   | (drs_, LS.Cons ((((lbrace_, r), s') as bs_)))
                       -> let (dr_, f') = parseDec (r, LS.expose s')
                            in parseDecs' (E.decl (drs_, dr_), f')
                   | drs_ -> drs_
        and parseDecs =
          function 
                   | LS.Cons ((((lbrace_, r), s') as bs_))
                       -> let (dr_, f') = parseDec (r, LS.expose s')
                            in parseDecs' (E.decl (E.null, dr_), f')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r, "Expected `{', found " ^ (L.toString t));;
        let rec parsePi =
          function 
                   | LS.Cons ((L.Id (_, "pi"), r), s')
                       -> parseDecs (LS.expose s')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r, "Expected `pi', found " ^ (L.toString t));;
        let rec parseSome =
          function 
                   | (gbs, LS.Cons ((L.Id (_, "some"), r), s'))
                       -> let (g1, f') = parseDecs (LS.expose s')
                            in let (g2, f'') = parsePi f'
                                 in parseSome' (((g1, g2) :: gbs), f'')
                   | (gbs, (LS.Cons ((L.Id (_, "pi"), r), s') as f))
                       -> let (g2, f') = parsePi f
                            in parseSome' (((E.null, g2) :: gbs), f')
                   | (gbs, (LS.Cons ((rparen_, r), s') as f)) -> (gbs, f)
                   | (gbs, LS.Cons ((t, r), s'))
                       -> Parsing.error
                          (r,
                           "Expected `some' or `pi', found " ^ (L.toString t))
        and parseSome' =
          function 
                   | (gbs, (LS.Cons ((rparen_, r), s') as f)) -> (gbs, f)
                   | (gbs, LS.Cons ((L.Id (_, "|"), r), s'))
                       -> parseSome (gbs, LS.expose s')
                   | (gbs, LS.Cons ((t, r), s'))
                       -> Parsing.error
                          (r, "Expected `)' or `|', found " ^ (L.toString t));;
        let rec stripParen (gbs, LS.Cons ((rparen_, r), s')) =
          (gbs, LS.expose s');;
        let rec parseGBs =
          function 
                   | LS.Cons ((lparen_, r), s')
                       -> stripParen (parseSome ([], LS.expose s'))
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r, "Expected `(', found " ^ (L.toString t));;
        ...
        and parseTrue =
          function 
                   | LS.Cons ((L.Id (_, "true"), r), s')
                       -> top (LS.expose s', r)
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r, "Expected `true', found " ^ (L.toString t))
        and parseExists =
          function 
                   ...
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected theorem identifier, found " ^
                             (L.toString t));;
        let rec parseTheoremDec' (LS.Cons ((theorem_, r), s')) =
        ...
        let rec parseRDecl f =
          let (oOut, f1) = parseOrder f
            in let (p, f2) = parsePredicate f1
                 in let (oIn, f3) = parseOrder f2
                      in let (callpats, f4) = parseCallPats f3
                           in (E.rdecl (p, oOut, oIn, E.callpats callpats),
                               f4);;
        let rec parseReduces' (LS.Cons ((reduces_, r), s')) =
          parseRDecl (LS.expose s');;
        let rec parseTabledDecl ((LS.Cons ((L.Id (_, id), r), s') as f)) =
          begin
          match LS.expose s'
          with 
               | (LS.Cons ((dot_, r'), s) as f) -> (E.tableddecl (id, r), f)
               | _ -> Parsing.error (r, "Expected .")
          end;;
        let rec parseTabled' (LS.Cons ((tabled_, r), s')) =
          parseTabledDecl (LS.expose s');;
        let rec parseKeepTableDecl ((LS.Cons ((L.Id (_, id), r), s') as f)) =
          begin
          match LS.expose s'
          with 
               | (LS.Cons ((dot_, r'), s) as f)
                   -> (E.keepTabledecl (id, r), f)
               | _ -> Parsing.error (r, "Expected .")
          end;;
        let rec parseKeepTable' (LS.Cons ((keeptable_, r), s')) =
          parseKeepTableDecl (LS.expose s');;
        let rec parseWDecl f =
          let (qids, f1) = ParseTerm.parseQualIds' f
            in let (callpats, f2) = parseCallPats f1
                 in (E.wdecl (qids, E.callpats callpats), f2);;
        let rec parseWorlds' (LS.Cons ((worlds_, r), s')) =
...
(* local ... in *);;
(* functor Parser *);;
# 1 "src/frontend/parse_thm.sml.ml"
