...
module type EXTMODES = sig
                       ...
                       module Full : sig
                                     type nonrec mterm
                                     val
                                       mroot : (ExtSyn.term * Paths.region) ->
                                               mterm
                                     val
                                       mpi : (mode * ExtSyn.dec * mterm) ->
                                             mterm
                                     val toModedec : mterm -> modedec
                                     end
                       end;;
      ...
      struct
        ...
        module Short = struct
                         ...
                         let rec mapp (((m, r1), name), (mS, r2)) =
                           ((M.Mapp ((M.Marg (m, name)), mS)),
                            P.join (r1, r2));;
                         let rec mroot (ids, id, r1, (mS, r2)) =
                           let r = P.join (r1, r2)
                             in let qid = (Names.Qid (ids, id))
                                  in begin
        ...
        module Full = struct
                        ...
                        let rec mroot (tm, r) (g, d_) =
                          let T.JWithCtx (g_, T.JOf ((v_, _), _, _)) =
                            ...
                            in let _ = T.checkErrors r
                                 in let rec convertSpine =
                                      function 
                                               | nil_ -> M.mnil_
                                               | I.App (u_, s_)
                                                   -> let k =
                                                        try Whnf.etaContract
                                                            u_
                                                        with 
                                                             | eta_
                                                                 -> error
                                      ...
                                      in let rec convertExp =
                                           function 
                                                    ...
                                                    | _
                                                        -> error
                                                           (r,
                                                            "Call pattern not an atomic type")
                                           in let (a, mS) =
                                                convertExp
                        ...
                        let rec toModedec t =
                          let _ = Names.varReset I.null_
                            in let t' = t (I.null_, I.null_) in t';;
                        end;;
