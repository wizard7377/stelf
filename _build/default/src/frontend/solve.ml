...
module type SOLVE = sig
                    ...
                    val
                      solve : (ExtQuery.define
                               list *
                               ExtQuery.solve *
                               Paths.location) ->
                              (IntSyn.conDec_ * Paths.occConDec option)
                              list
                    val
                      query : ((int option * int option * ExtQuery.query) *
                               Paths.location) ->
                              unit
...
module Solve(Solve__0: sig
  ...
  *);;
    ...
    let rec boundToString = function 
    ...
    let rec checkSolutions (expected, try_, solutions) = begin
      if boundEq (boundMin (expected, try_), (Some solutions)) then () else
      raise
      ((AbortQuery
    ...
    (* the expected number, given the bound on the number of tries *);;
    let rec checkStages (try_, stages) = begin
      if boundEq (try_, (Some stages)) then () else
      raise
      ((AbortQuery
  ...
  *);;
    let rec moreSolutions () =
      begin
        print "More? ";
        begin
        match String.sub (Compat.inputLine97 TextIO.stdIn, 0)
        with 
             | 'y' -> true
             | 'Y' -> true
             | ';' -> true
  ...
  *);;
    let rec solve' (defines, Solve_, Paths.Loc (fileName, r)) =
      let (a_, finish) =
        ...
        in let _ = begin
             if (! Global.chatter) >= 3 then Msg.message "%solve " else ()
             end
             in let _ = begin
                  ...
                  in let g =
                       ...
                       in begin
                            ...
                            try begin
                                  ...
                                  ((g, IntSyn.id),
                                   (CompSyn.DProg
                                    (IntSyn.null_, IntSyn.null_)),
                                   function 
                                            | skel_
                                                -> raise
                                                   ((SolutionSkel skel_)));
                                  raise
                                  ((AbortQuery "No solution to %solve found"))
                                  end
                            (* Call to solve raises Solution _ if there is a solution,
          returns () if there is none.  It could also not terminate
          *)
                                 ...
                                 | SolutionSkel skel_
                                     -> try begin
                                              ...
                                              try begin
                                                    ...
                                                    raise
                                                    ((AbortQuery
                                                      "Proof reconstruction for %solve failed"))
                                                    
                                                    end
                                              with 
                                                   | Solution m_ -> finish m_
    ...
    let rec query' ((expected, try_, quy), Paths.Loc (fileName, r)) =
      let (a_, optName, xs_) =
        ...
        in let _ = begin
             ...
             in let _ = begin
                  if (! Global.chatter) >= 4 then Msg.message " " else () end
                  in let _ = begin
                       if (! Global.chatter) >= 3 then
                       Msg.message
                       (("\n" ^
                       ...
                       in let g =
                            ...
                            Compile.compileGoal
                            (IntSyn.null_, a_)
                            in let solutions = ref 0
                                 in let rec scInit m_ =
                                      begin
                                      ...
                                      in let rec search () =
                                           ...
                                           in begin
                                                ...
                                                (* solve query if bound > 0 *)(* in case Done was raised *)(* check if number of solutions is correct *)
                                                else begin
                                                if (! Global.chatter) >= 3
                                                then
                                                Msg.message
                                                "Skipping query (bound = 0)\n"
                                                else begin
                                                if (! Global.chatter) >= 4
                                                then Msg.message "skipping"
                                                else () end end end;begin
                                                if (! Global.chatter) >= 3
                                                then
                                                Msg.message
                                           ...
                                           (* optName = SOME(X) or NONE, Xs = free variables in query excluding X *)(* times itself *)(* Problem: we cannot give an answer substitution for the variables
                                 ...
                                 in let rec scInit m_ =
                                      ...
                                      in let rec search () =
                                           ...
                                           in begin
                                                ...
                                                (* solve query if bound > 0 *)(* printing is timed into solving! *)(* in case Done was raised *)(* check if number of solutions is correct *)
                                                else begin
                                                if (! Global.chatter) >= 3
                                                then
                                                Msg.message
...
or  %querytabled <expected solutions> <max stages tried>  X : A
  ...
  solutions or if we have reached the maximal number of stages *);;
             ...
             in let _ = begin
                  ...
                  in let _ = begin
                       ...
                       in let g =
                            ...
                            (IntSyn.null_, a_)
                            in let solutions = ref 0
                                 in let status = ref false
                                      in let solExists = ref false
                                           in let stages = ref 1
                                                in let rec scInit o_ =
                                                     begin
                                                       solutions :=
                                                         ((! solutions) + 1);
                                                       begin
                         ...
                         *);
                                                          ...
                                                          in let _ =
                                                               ...
                                                               in let _ =
                                                                    ...
                                                                    then
                                                                    Msg.message
                                                                    "Tabled evaluation COMPLETE \n \n"
                                                                    else
                                                                    Msg.message
                                                                    "Tabled evaluation NOT COMPLETE \n \n"
                                                                    end;
                                                                    begin
                                                                    Msg.message
                                                                    "\n____________________________________________\n\n";
    ...
    let rec qLoop () =
      qLoops
      (begin
         CSManager.reset ();Parser.parseTerminalQ ("?- ", "   ")
         end)
      ...
      function 
               ...
               | S.Cons (Query_, s')
                   -> let (a_, optName, xs_) =
                        ...
                        (Query_, (Paths.Loc ("stdIn", (Paths.Reg (0, 0)))))
                        in let g =
                             Timers.time
                             Timers.compiling
                             Compile.compileGoal
                             ...
                             in let rec scInit m_ =
                                  ...
                                  in let _ = begin
                                       ...
                                       (* times itself *)(* Ignore s': parse one query at a time *)(* normal exit *);;
               ...
               | S.Cons (Query_, s')
                   -> let solExists = ref false
                        in let (a_, optName, xs_) =
                             ReconQuery.queryToQuery
                             (Query_,
                              (Paths.Loc ("stdIn", (Paths.Reg (0, 0)))))
                             in let g =
                                  Timers.time
                                  Timers.compiling
                                  Compile.compileGoal
                                  (IntSyn.null_, a_)
                                  in let _ = Tabled.reset ()
                                       in let rec scInit o_ =
                                            begin
                                              begin
                                              if (! Global.chatter) >= 1 then
                                              ...
                                              begin
                                                ...
                                                begin
                                                  ...
                                                  (* Question: should we collect constraints from M? *);
                                                  begin
                                                    solExists := true;begin
                                                    if moreSolutions () then
                                                    () else raise Done end
                                                    end
                                            ...
                                            in let rec loop () = begin
                                                 if Tabled.nextStage () then
                                                 loop () else raise Completed
                                                 end
                                                 (* table did not change,
                         * i.e. all solutions have been found
                         * we check for *all* solutions
                         *)
                                                 in let _ = begin
                                                      ...
                                                      (* times itself *)(* loops -- scinit will raise exception Done *)(* Ignore s': parse one query at a time *)(* normal exit *);;
    (* primary, secondary prompt *);;
    end;;
(* functor Solve *);;
