...
module Lexer(Lexer__0: sig module Stream' : STREAM end) : LEXER =
  struct
    module Stream = Stream';;
    (*! structure Paths = Paths' !*);;
    open! struct
  ...
  *);;
    let rec lex ((inputFun : int -> string)) =
      ...
      and left = ref 0
      and right = ref 0
        in let _ = P.resetLines ()
             in let eOFString_ = String.str '\004'
                  in let rec readNext () =
                       let nextLine = inputFun (! right)
                         in let nextSize = String.size nextLine in begin
                              if nextSize = 0 then
                              begin
                                s := eOFString_;
                                begin
                              ...
                              (* fake EOF character string *) else
                              begin
                                s := nextLine;
                                begin
                                  left := (! right);
                              ...
                              (* end of file? *)(* remember new line position *)
                       in let rec char i = begin
                            if i >= (! right) then
                            begin
                              readNext ();char i
                              end
                            else String.sub (! s, i - (! left)) end
                            in let rec string (i, j) =
                                 String.substring (! s, i - (! left), j - i)
                                 in let rec idToToken (idCase, P.Reg (i, j))
                                      =
                                      stringToToken
                                      (idCase, string (i, j), (P.Reg (i, j)))
                                      in let rec qidToToken (P.Reg (i, j)) =
                                           ((Id (Lower, string (i, j + 1))),
                                            (P.Reg (i, j + 1)))
                                           in let rec lexInitial =
                                                function 
                                                         | (':', i)
                                                             -> (Colon,
                                                                 (P.Reg
                                                         ...
                                                         | (c, i) -> begin
                                                             ...
                                                             end end(* lexQUID (i-1,i) *)
                                              and lexID
                                                (idCase, P.Reg (i, j)) =
                                                let rec lexID' j = begin
                                                  if isIdChar (char j) then
                                                  lexID' (j + 1) else
                                                  idToToken
                                                  (idCase, (P.Reg (i, j)))
                                                  end in lexID' j
                                              and lexQUID (P.Reg (i, j)) =
                                                ...
                                                error
                                                ((P.Reg (i, j + 1)),
                                                 "Whitespace in quoted identifier")
                                                else begin
                                                if isQuote (char j) then
                                                qidToToken ((P.Reg (i, j)))
                                                else
                                                lexQUID ((P.Reg (i, j + 1)))
                                                end end
                                                             ...
                                                             -> error
                                                                (r,
                                                                 ("Unknown keyword %"
                                                                    ^ s)
                                                                   ^
                                                                   " (single line comment starts with `%<whitespace>' or `%%')")(* -fp 08/09/02 *)(* -rv 11/27/01 *)(* -gaw 07/11/08 *)(* -ABP 4/4/03 *)(* -rv 8/27/01 *)(* -bp 20/11/04 *)(* -bp 20/11/01 *)(* -bp 6/5/99 *)(* -fp 3/18/01 *)(* -cs 6/3/01 *)(* -fp 8/17/03 *)
                                              and lexComment =
                                                function 
                                                         | ('\n', i)
                                                             -> lexInitial
                                              ...
                                              and lexDComment =
                                                function 
                                                         ...
                                                         | (c, l, i)
                                                             -> lexDComment
                                                                (char i, l,
                                                                 i + 1)(* recover: (EOF, (i-1,i-1)) *)(* pass comment beginning for error message? *)
                                                   ...
                                                   and lexContinueQualId' j =
                                                     ...
                                                     (Stream.Cons
                                                      ((Dot,
                                                        (P.Reg (j, j + 1))),
                                                       lexContinue (j + 1)))
                                                     end else lexContinue' j
                                                     end in lexContinue 0
                                                     (* local state maintained by the lexer *)(* current string (line) *)(* position of first character in s *)(* position after last character in s *)(* initialize line counter *)(* neither lexer nor parser should ever try to look beyond EOF *)(* readNext () = ()
      ...
      *)(* string (i,j) = substring at region including i, excluding j
         ...
         Effects: None
      *)(* The remaining functions do not access the state or *)(* stream directly, using only functions char and string *)(* Quote characters are part of the name *)(* Treat quoted identifiers as lowercase, since they no longer *)(* override infix state.  Quoted identifiers are now only used *)(* inside pragmas *)(* The main lexing functions take a character c and the next
    ...
    *)(* recover by ignoring: lexInitial (char(i), i+1) *)(* lexQUID is currently not used --- no quoted identifiers *)(* comments are now started by %<whitespace> *)(*
    ...
    let rec lexStream instream =
      lex (function 
                    | i -> Compat.inputLine97 instream);;
    let rec lexTerminal (prompt0, prompt1) =
      lex
      (function 
                | 0
                    -> begin
                         print prompt0;Compat.inputLine97 TextIO.stdIn
                         end
                | i
                    -> begin
                         print prompt1;Compat.inputLine97 TextIO.stdIn
                         end);;
    let rec toString' =
      function 
               | Dot -> "."
               ...
               | Include -> "%include"
               | Open -> "%open"
               | Use -> "%use"(* -fp 08/09/02 *)(* -rv 11/27/01. *)(*  -bp 04/11/03. *)(*  -bp 20/11/01. *)(*  -bp 6/5/99. *)(* -cs 6/3/01. *)(* -ABP 4/4/03 *)(* -rv 8/27/01 *);;
    let rec toString =
      function 
               | Id (_, s) -> ("identifier `" ^ s) ^ "'"
               | Eof -> "end of file or `%.'"
    ...
    let rec charToNat c =
      let digit = (Char.ord c) - (Char.ord '0') in begin
    ...
    let rec stringToNat s =
      let l = String.size s
        in let rec stn (i, n) = begin
             if i = l then n else
             stn (i + 1, (10 * n) + (charToNat (String.sub (s, i)))) end
             in stn (0, 0);;
  ...
  *);;
    let rec isUpper =
      function 
               | "" -> false
               | s
                   -> let c = String.sub (s, 0)
                        in (Char.isUpper c) || (c = '_');;
    end;;
(*! structure Paths' : PATHS !*);;
(* local ... *);;
(* functor Lexer *);;
