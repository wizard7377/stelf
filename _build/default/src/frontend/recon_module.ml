# 1 "src/frontend/recon_module.sig.ml"
open! Basis;;
...
# 1 "src/frontend/recon_module.fun.ml"
open! Basis;;
module ReconModule(ReconModule__0: sig
  ...
  struct
    ...
    type inst_ = | External of ExtSyn.term 
    ...
    let rec coninst ((ids, id, r1), tm, r2) (ns, eqns) =
      let qid = (Names.Qid (ids, id))
        in begin
           match Names.constLookupIn (ns, qid)
           with 
                | None
                    -> error
                       (r1,
    ...
    let rec addStructEqn (rEqns, r1, r2, ids, mid1, mid2) =
      let ns1 = Names.getComponents mid1
        in let ns2 = Names.getComponents mid2
             in let rec push eqn = rEqns := ((eqn :: ! rEqns))
                  in let rec doConst (name, cid1) =
                       begin
                       match Names.constLookupIn
                             (ns2, (Names.Qid ([], name)))
                       with 
                            | None
                                -> error
                                   (r1,
                                    "Instantiating structure lacks component "
                                      ^
                                      (Names.qidToString
                                       ((Names.Qid (rev ids, name)))))
                            | Some cid2 -> push (cid1, (Internal cid2), r2)
                       end
                       in let rec doStruct (name, mid1) =
                            begin
                            match Names.structLookupIn
                                  (ns2, (Names.Qid ([], name)))
                            with 
                                 | None
                                     -> error
    ...
    let rec strinst ((ids, id, r1), strexp, r3) (ns, eqns) =
      let qid = (Names.Qid (ids, id))
        in let mid1 =
             begin
             match Names.structLookupIn (ns, qid)
             with 
                  | None
                      -> error
                         (r1,
             ...
             in let (mid2, r2) = strexp ()
                  in let rEqns = ref eqns
                       in begin
    ...
    let rec sigid (id, r) None =
      begin
      match ModSyn.lookupSigDef id
      with 
           | None -> error (r, "Undefined signature " ^ id)
           | Some module_ -> (module_, [])
      end;;
    let rec wheresig (sigexp, instList) moduleOpt =
      let (module_, wherecls) = sigexp moduleOpt
        in let rec wherecl ns =
             foldr (function 
                             | (inst, eqns) -> inst (ns, eqns)) [] instList
             in (module_, wherecls @ [wherecl]);;
    ...
    let rec structdec (idOpt, sigexp) moduleOpt =
      let (module_, inst) = sigexp moduleOpt
        in (StructDec (idOpt, module_, inst));;
    ...
    let rec applyEqns wherecl namespace =
      let eqns = wherecl namespace
        in let table : eqnTable = IntTree.new_ 0
             in let rec add (cid, inst_, r) =
                  begin
                  match IntTree.lookup table cid
                  with 
                       | None -> IntTree.insert table (cid, ref [(inst_, r)])
                       | Some rl -> rl := (((inst_, r) :: ! rl))
                  end
                  in let _ = List.app add eqns
                       in let rec doInst =
                            function 
                                     | ((Internal cid, r), Condec_)
                                         -> try ModSyn.strictify
                                                (ExtSyn.internalInst
                                                 (Condec_,
                                                  ModSyn.abbrevify
                                                  (cid, IntSyn.sgnLookup cid),
                                                  r))
                                            with 
                                                 | ExtSyn.Error msg
                                                     -> raise
                                     ...
                                     | ((External tm, r), Condec_)
                                         -> ModSyn.strictify
                                            (ExtSyn.externalInst
                                             (Condec_, tm, r))
                            in let rec transformConDec (cid, Condec_) =
                                 begin
                                 match IntTree.lookup table cid
                                 with 
                                      | None -> Condec_
                                      | Some { contents = l}
                                          -> List.foldr doInst Condec_ l
                                 end in transformConDec;;
    let rec moduleWhere (module_, wherecl) =
      let (mark, markStruct) = IntSyn.sgnSize ()
        in let module' =
             ModSyn.instantiateModule (module_, applyEqns wherecl)
             in let _ = Names.resetFrom (mark, markStruct) in module'
                  (* val _ = IntSyn.resetFrom (mark, markStruct) *);;
    end;;
# 1 "src/frontend/recon_module.sml.ml"
