  ...
  struct
    ...
    open!
      struct
        ...
        let rec lex (r0, l_) =
          let rec lex' =
            function 
                     ...
                     | ((o_, r) :: l_)
                         -> let (os_, r') = lex' l_
                              in ((o_ :: os_), Paths.join (r, r'))
            in let (os_, r1) = lex' l_ in ((ThmSyn.Lex os_), r1);;
        let rec simul (r0, l_) =
          let rec simul' =
            function 
                     | [] -> ([], r0)
        ...
        let rec checkCallPat =
          function 
                   ...
                   | (I.ConDec (a, _, _, I.Constraint _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal constraint constant " ^ a) ^
                             " in call pattern")
                   | (I.ConDec (a, _, _, I.Foreign _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal foreign constant " ^ a) ^
                             " in call pattern")
                   | (I.ConDec (a, _, _, _, _, type_), p_, r)
                       -> error
                          (r,
                           ("Constant " ^ a) ^
                             " in call pattern not a type family")
                   | (I.ConDef (a, _, _, _, _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal defined constant " ^ a) ^
                             " in call pattern")
                   | (I.AbbrevDef (a, _, _, _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal abbreviation " ^ a) ^ " in call pattern")
                   | (I.BlockDec (a, _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal block identifier " ^ a) ^
                             " in call pattern")
                   | (I.SkoDec (a, _, _, _, _), p_, r)
                       -> error
                          (r,
                           ("Illegal Skolem constant " ^ a) ^
                             " in call pattern");;
        let rec callpats l_ =
          let rec callpats' =
            function 
                     | [] -> ([], [])
                     | ((name, p_, r) :: l_)
                         -> let (cps, rs) = callpats' l_
                              in let qid = (Names.Qid ([], name))
                                   in begin
                                      ...
                                      with 
                                           ...
                                           | Some cid
                                               -> begin
                                                    checkCallPat
                                                    (I.sgnLookup cid, p_, r);
                                                    (((cid, p_) :: cps),
                                                     (r :: rs))
                                                    
                                                    end
                                      end
            in let (cps, rs) = callpats' l_ in ((ThmSyn.Callpats cps), rs);;
        type nonrec tdecl =
          ThmSyn.tDecl_ * (Paths.region * Paths.region list);;
        let rec tdecl ((o_, r), (c_, rs)) =
          ((ThmSyn.TDecl (o_, c_)), (r, rs));;
        ...
        type nonrec prove =
          ThmSyn.pDecl_ * (Paths.region * Paths.region list);;
        ...
        let rec ctxBlockToString (g0_, (g1_, g2_)) =
          let _ = Names.varReset I.null_
            in let g0'_ = Names.ctxName g0_
                 in let g1'_ = Names.ctxLUName g1_
                      in let g2'_ = Names.ctxLUName g2_
                           in ((((Print.ctxToString (I.null_, g0'_)) ^ "\n")
                                  ^
                                  (begin
                                   match g1'_
                                   with 
        ...
        let rec abstractCtxPair (g1, g2) =
          let r =
            begin
            match (T.ctxRegion g1, T.ctxRegion g2)
            with 
                 | (Some r1, Some r2) -> Paths.join (r1, r2)
                 | (_, Some r2) -> r2
            end
            in let T.JWithCtx (g1_, T.JWithCtx (g2_, _)) =
                 T.recon (T.jwithctx (g1, T.jwithctx (g2, T.jnothing)))
                 in let (g0_, (g1'_ :: (g2'_ :: []))) =
                      try Abstract.abstractCtxs [g1_; g2_]
                      with 
                           | Constraints.Error c_
                               -> error
                                  (r,
                      ...
                      in let _ = checkFreevars (g0_, (g1'_, g2'_), r)
                           in (g1'_, g2'_);;
        let rec top (gBs_, g, m_, k) = (gBs_, g, m_, k);;
        let rec exists (g', t) (gBs_, g, m_, k) =
          t
          (gBs_, ctxAppend (g, g'),
           ctxAppend (m_, ctxMap (function 
                                           | _ -> M.minus_) g'),
           k);;
        ...
        let rec theoremToTheorem t =
          let (gbs, g, m_, k) = t ([], I.null_, I.null_, 0)
            in let _ = Names.varReset IntSyn.null_
                 in let gBs_ = List.map abstractCtxPair gbs
                      in let T.JWithCtx (g_, _) =
                           T.recon (T.jwithctx (g, T.jnothing))
                           in (L.ThDecl (gBs_, g_, m_, k));;
        let rec theoremDecToTheoremDec (name, t) = (name, theoremToTheorem t);;
        let rec abstractWDecl w_ = let w'_ = List.map Names.qid_ w_ in w'_;;
        type nonrec wdecl = ThmSyn.wDecl_ * Paths.region list;;
        let rec wdecl (w_, (cp, rs)) =
          ((ThmSyn.WDecl (abstractWDecl w_, cp)), rs);;
