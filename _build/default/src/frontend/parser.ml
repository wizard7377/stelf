...
module type PARSER = sig
                     ...
                     type fileParseResult =
                       ...
                       | FixDec of
                         (Names.qid_ * Paths.region) * Names.Fixity.fixity 
                       | NamePref of
                         (Names.qid_ * Paths.region) *
                       ...
                       | Querytabled of
                         int option * int option * ExtQuery.query 
                     ...
                     val
                       parseStream : TextIO.instream ->
                                     (fileParseResult * Paths.region)
                                     Stream.stream
                     val
                       parseTerminalQ : (string * string) ->
                                        ExtQuery.query
                                        Stream.stream
                     end;;
      ...
      struct
          ...
          function 
                   ...
                   | LS.Cons ((L.Id (_, name), r), s')
                       -> try ((Some (L.stringToNat name)), s')
                          with 
                               | Overflow
                                   -> Parsing.error (r, "Bound too large")
                               | L.NotDigit _
                                   -> Parsing.error
                                      (r,
                                       ("Bound `" ^ name) ^
                                         "' neither `*' nor a natural number")
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected bound `*' or natural number, found " ^
                             (L.toString t));;
        let rec recParse (s, recparser, theSigParser, sc) =
          ...
          (function 
                    | ()
                        -> recParse'
                           (LS.expose s, recparser, theSigParser, sc))
                                     ...
                                     -> Parsing.error
                                        (r,
                                         "Expected `}', found " ^
                                           (L.toString t))
                        in (Stream.Cons
                            ((BeginSubsig, r1), theSigParser (s', finish)))
        ...
        let rec parseStream (s, sc) =
          Stream.delay (function 
                                 | () -> parseStream' (LS.expose s, sc))
        and parseStream' =
          function 
                   | ((LS.Cons ((L.Id (idCase, name), r0), s') as f), sc)
                       -> parseConDec' (f, sc)
                   | ((LS.Cons ((abbrev_, r), s') as f), sc)
                       -> parseAbbrev' (f, sc)
                   | ((LS.Cons ((underscore_, r), s') as f), sc)
                       -> parseConDec' (f, sc)
                   | ((LS.Cons ((infix_, r), s') as f), sc)
                       -> parseFixity' (f, sc)
                   | ((LS.Cons ((prefix_, r), s') as f), sc)
                       -> parseFixity' (f, sc)
                   | ((LS.Cons ((postfix_, r), s') as f), sc)
                       -> parseFixity' (f, sc)
                   ...
                   | ((LS.Cons ((define_, r), s') as f), sc)
                       -> parseSolve' (f, sc)
                   | ((LS.Cons ((solve_, r), s') as f), sc)
                       -> parseSolve' (f, sc)
                   | (LS.Cons ((query_, r0), s'), sc)
                       -> let (expected, s1) = parseBound' (LS.expose s')
                            in let (try_, s2) = parseBound' (LS.expose s1)
                                 in let (query, (LS.Cons ((_, r'), _) as f3))
                                      = ParseQuery.parseQuery' (LS.expose s2)
                                      in let r = Paths.join (r0, r')
                                           in (Stream.Cons
                                               (((Query
                                                  (expected, try_, query)),
                                                 r),
                                      ...
                                      in let r = Paths.join (r0, r')
                                           in (Stream.Cons
                                               (((Querytabled
                                                 ...
                                                 r),
                                                parseStream (stripDot f3, sc)))
                   | ((LS.Cons ((mode_, r), s') as f), sc)
                       -> parseMode' (f, sc)
                   | ((LS.Cons ((unique_, r), s') as f), sc)
                       -> parseUnique' (f, sc)
                   | ((LS.Cons ((covers_, r), s') as f), sc)
                       -> parseCovers' (f, sc)
                   | ((LS.Cons ((total_, r), s') as f), sc)
                       -> parseTotal' (f, sc)
                   | ((LS.Cons ((terminates_, r), s') as f), sc)
                       -> parseTerminates' (f, sc)
                   | ((LS.Cons ((block_, r), s') as f), sc)
                       -> parseConDec' (f, sc)
                   | ((LS.Cons ((worlds_, r), s') as f), sc)
                       -> parseWorlds' (f, sc)
                   | ((LS.Cons ((reduces_, r), s') as f), sc)
                       -> parseReduces' (f, sc)
                   | ((LS.Cons ((tabled_, r), s') as f), sc)
                       -> parseTabled' (f, sc)
                   | ((LS.Cons ((keeptable_, r), s') as f), sc)
                       -> parseKeepTable' (f, sc)
                   | ((LS.Cons ((theorem_, r), s') as f), sc)
                       -> parseTheorem' (f, sc)
                   | ((LS.Cons ((prove_, r), s') as f), sc)
                       -> parseProve' (f, sc)
                   | ((LS.Cons ((establish_, r), s') as f), sc)
                       -> parseEstablish' (f, sc)
                   | ((LS.Cons ((assert_, r), s') as f), sc)
                       -> parseAssert' (f, sc)
                   | ((LS.Cons ((trustme_, r), s') as f), sc)
                       -> parseTrustMe' (f, sc)
                   | ((LS.Cons ((freeze_, r), s') as f), sc)
                       -> parseFreeze' (f, sc)
                   | ((LS.Cons ((subord_, r), s') as f), sc)
                       -> parseSubord' (f, sc)
                   | ((LS.Cons ((thaw_, r), s') as f), sc)
                       -> parseThaw' (f, sc)
                   | ((LS.Cons ((deterministic_, r), s') as f), sc)
                       -> parseDeterministic' (f, sc)
                   | ((LS.Cons ((compile_, r), s') as f), sc)
                       -> parseCompile' (f, sc)
                   | ((LS.Cons ((clause_, r), s') as f), sc)
                       -> parseClause' (f, sc)
                   | ((LS.Cons ((sig_, r), s') as f), sc)
                       -> parseSigDef' (f, sc)
                   | ((LS.Cons ((struct_, r), s') as f), sc)
                       -> parseStructDec' (f, sc)
                   | ((LS.Cons ((include_, r), s') as f), sc)
                       -> parseInclude' (f, sc)
                   | ((LS.Cons ((open_, r), s') as f), sc)
                       -> parseOpen' (f, sc)
                   | ((LS.Cons ((use_, r), s') as f), sc)
                       -> parseUse' (LS.expose s', sc)
        ...
        and parseConDec' ((LS.Cons ((_, r0), _) as f), sc) =
          let (conDec, (LS.Cons ((_, r'), _) as f')) =
            ParseConDec.parseConDec' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((ConDec conDec), r), parseStream (stripDot f', sc)))
        and parseAbbrev' ((LS.Cons ((_, r0), _) as f), sc) =
          let (conDec, (LS.Cons ((_, r'), _) as f')) =
            ParseConDec.parseAbbrev' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((AbbrevDec conDec), r), parseStream (stripDot f', sc)))
        and parseClause' ((LS.Cons ((_, r0), _) as f), sc) =
          let (conDec, (LS.Cons ((_, r'), _) as f')) =
            ParseConDec.parseClause' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((ClauseDec conDec), r), parseStream (stripDot f', sc)))
        and parseFixity' ((LS.Cons ((_, r0), _) as f), sc) =
          let (fdec, (LS.Cons ((_, r'), _) as f')) =
            ParseFixity.parseFixity' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((FixDec fdec), r), parseStream (stripDot f', sc)))
        and parseSolve' ((LS.Cons ((_, r0), _) as f), sc) =
          let (defnssolve, (LS.Cons ((_, r'), _) as f')) =
            ParseQuery.parseSolve' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((Solve defnssolve), r), parseStream (stripDot f', sc)))
        and parseMode' ((LS.Cons ((_, r0), _) as f), sc) =
          let (mdecs, (LS.Cons ((_, r'), _) as f')) = ParseMode.parseMode' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((ModeDec mdecs), r), parseStream (stripDot f', sc)))
        and parseUnique' ((LS.Cons ((_, r0), _) as f), sc) =
          let (mdecs, (LS.Cons ((_, r'), _) as f')) = ParseMode.parseMode' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((UniqueDec mdecs), r), parseStream (stripDot f', sc)))
        and parseCovers' ((LS.Cons ((_, r0), _) as f), sc) =
          let (mdecs, (LS.Cons ((_, r'), _) as f')) = ParseMode.parseMode' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((CoversDec mdecs), r), parseStream (stripDot f', sc)))
        and parseTotal' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseTotal' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((TotalDec ldec), r), parseStream (stripDot f', sc)))
        and parseTerminates' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) =
            ParseThm.parseTerminates' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((TerminatesDec ldec), r),
                      parseStream (stripDot f', sc)))
        and parseReduces' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseReduces' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((ReducesDec ldec), r), parseStream (stripDot f', sc)))
        and parseTabled' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseTabled' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((TabledDec ldec), r), parseStream (stripDot f', sc)))
        and parseKeepTable' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) =
            ParseThm.parseKeepTable' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((KeepTableDec ldec), r),
                      parseStream (stripDot f', sc)))
        and parseWorlds' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseWorlds' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((WorldDec ldec), r), parseStream (stripDot f', sc)))
        and parseTheorem' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) =
            ParseThm.parseTheoremDec' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((TheoremDec ldec), r), parseStream (stripDot f', sc)))
        and parseProve' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseProve' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((ProveDec ldec), r), parseStream (stripDot f', sc)))
        and parseEstablish' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) =
            ParseThm.parseEstablish' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((EstablishDec ldec), r),
                      parseStream (stripDot f', sc)))
        and parseAssert' ((LS.Cons ((_, r0), _) as f), sc) =
          let (ldec, (LS.Cons ((_, r'), _) as f')) = ParseThm.parseAssert' f
            in let r = Paths.join (r0, r')
                 in (Stream.Cons
                     (((AssertDec ldec), r), parseStream (stripDot f', sc)))
        and parseTrustMe' ((LS.Cons ((_, r0), s) as f), sc) =
          let rec parseNextDec' =
            function 
                     ...
                     | empty_
                         -> Parsing.error
                            (r0, "No declaration after `%trustme'")
            in parseNextDec' (parseStream' (LS.expose s, sc))
          ...
          let rec finish (sigdef, (LS.Cons ((_, r2), _) as f')) =
            (Stream.Cons
             (((SigDef sigdef), Paths.join (r1, r2)),
              parseStream (stripDot f', sc)))
            in recParse' (f, ParseModule.parseSigDef', parseStream, finish)
        ...
        and parseUse' =
          function 
                   | (LS.Cons ((L.Id (_, name), r0), s), sc)
                       -> let (LS.Cons ((_, r'), _) as f) = LS.expose s
                            in let r = Paths.join (r0, r')
                                 in (Stream.Cons
                                     (((Use name), r),
                                      parseStream (stripDot f, sc)))
                   | (LS.Cons ((_, r), _), sc)
                       -> Parsing.error
                          (r, "Constraint solver name expected");;
        let rec parseQ s =
          Stream.delay (function 
                                 | () -> parseQ' (LS.expose s))
        and parseQ' f =
          let (query, f') = ParseQuery.parseQuery' f
            in (Stream.Cons (query, parseQ (stripDot f')));;
        let rec parseTLStream instream =
          let rec finish =
            function 
                     | LS.Cons ((eof_, r), s) -> Stream.empty_
                     | LS.Cons ((rbrace_, r), s)
                         -> Parsing.error (r, "Unmatched `}'")
            in parseStream (L.lexStream instream, finish);;
        end;;
