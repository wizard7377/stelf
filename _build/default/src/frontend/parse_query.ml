# 1 "src/frontend/parse_query.sig.ml"
open! Basis;;
(* Parsing Queries *);;
...
module type PARSE_QUERY = sig
                          ...
                          val
                            parseSolve' : (ExtQuery.define
                                           list *
                                           ExtQuery.solve)
                            Parsing.parser
                          end;;
(* signature PARSE_QUERY *);;
# 1 "src/frontend/parse_query.fun.ml"
open! Parsing;;
open! Basis;;
(* Parsing Queries *);;
(* Author: Frank Pfenning *);;
      ...
      struct
        ...
        let rec parseQuery1 =
          function 
                   | (name, f, LS.Cons ((colon_, r), s'))
                       -> returnQuery
                          ((Some name), ParseTerm.parseTerm' (LS.expose s'))
                   | (name, f, _)
        ...
        let rec parseDefine4 (optName, optT, s) =
          let (tm', f') = ParseTerm.parseTerm' (LS.expose s)
            in (ExtQuery.define (optName, tm', optT), f');;
          ...
          function 
                   | (defns, nameOpt, LS.Cons ((colon_, r), s'), r0)
                       -> let (tm, (LS.Cons ((_, r), _) as f')) =
                            ...
                            in ((List.rev defns,
                                 ExtQuery.solve (nameOpt, tm, P.join (r0, r))),
                                f')
          ...
          function 
                   ...
                   | (defns, LS.Cons ((define_, r0), s'))
                       -> let (defn, f') = parseDefine1 (LS.expose s')
                            in parseSolve1 ((defn :: defns), f')
                   | (defns, LS.Cons ((t, r), s))
                       -> Parsing.error
                          (r,
                           "Expected %define or %solve, found " ^
                             (L.toString t))
...
(* local ... in *);;
(* functor ParseQuery *);;
# 1 "src/frontend/parse_query.sml.ml"
