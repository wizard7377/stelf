...
module type PARSE_MODE = sig
                         ...
                         val parseMode' : ExtModes.modedec list
                           Parsing.parser
                         end;;
  ...
  struct
    ...
    open!
      struct
        ...
        module E = ExtModes;;
        module P = Paths;;
        let rec extract (s, i) = begin
          if i = (String.size s) then None else
          (Some (String.extract (s, i, None))) end;;
        let rec splitModeId (r, id) =
          begin
          match String.sub (id, 0)
          with 
               | '*' -> (E.star r, extract (id, 1))
               | '-' -> begin
                   if ((String.size id) > 1) && ((String.sub (id, 1)) = '1')
                   then (E.minus1 r, extract (id, 2)) else
                   (E.minus r, extract (id, 1)) end
               | '+' -> (E.plus r, extract (id, 1))
               | _
                   -> Parsing.error
                      (r,
                       "Expected mode `+', `-', `*', or `-1'  found " ^ id)
          end;;
        let rec validateMArg =
          function 
                   | (r, ((mode, Some id) as mId)) -> begin
                       if L.isUpper id then mId else
                       Parsing.error
                       (r, "Expected free uppercase variable, found " ^ id)
                       end
                   | (r, (_, None))
                       -> Parsing.error
                          (r, "Missing variable following mode");;
        ...
        let rec stripRParen =
          function 
                   | LS.Cons ((rparen_, r), s') -> (LS.expose s', r)
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
        ...
        let rec parseShortSpine =
          function 
                   | (LS.Cons ((dot_, r), s') as f) -> (E.Short.mnil r, f)
                   | (LS.Cons ((rparen_, r), s') as f) -> (E.Short.mnil r, f)
                   | LS.Cons ((L.Id (_, id), r), s')
                       -> let mId = validateMArg (r, splitModeId (r, id))
                            in let (mS', f') = parseShortSpine (LS.expose s')
                                 in (E.Short.mapp (mId, mS'), f')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected mode or `.', found " ^ (L.toString t));;
        let rec parseFull =
          function 
                          ...
                          with 
                                                  ...
                                                  in let (f'', r') =
                                                       ...
                                                       in let dec =
                                                            ...
                                                            with 
                                                                 | None
                                                                    -> 
                                                                    ParseTerm.ExtSyn.dec0
                                                                    (x,
                                                                    P.join
                                                                    (r, r'))
                                                                 | Some y
                                                                    -> 
                                                                    ParseTerm.ExtSyn.dec
                                                                    (x, y,
                                                                    P.join
                                                                    (r, r'))
                                                            end
                               ...
                               | LS.Cons ts_
                                   -> let (t', (LS.Cons ((_, r), s') as f'))
                                        =
                                        ParseTerm.parseTerm'
                                        ((LS.Cons (t0, LS.cons ts_)))
                                        in (E.Full.mroot (t', P.join (r, r1)),
                                            f')
                          end
                   | (LS.Cons ((lparen_, r0), s'), r1)
                       -> let (t', f') = ParseTerm.parseTerm' (LS.expose s')
                            in let (f'', r') = stripRParen f'
                                 in (E.Full.mroot (t', P.join (r', r1)), f'')
                   | (LS.Cons ((t, r), s'), _)
                       -> Parsing.error
                          (r,
                           "Expected mode or identifier, found " ^
                             (L.toString t));;
        let rec parseMode2 =
          function 
                   | (lexid, LS.Cons ((((lbrace_, r), s') as bs_)), r1)
                       -> let (t', f') =
                            parseFull ((LS.Cons (lexid, LS.cons bs_)), r1)
                            in (E.Full.toModedec t', f')
                   | ((L.Id (_, name), r), f, _)
                       -> let (mS', f') = parseShortSpine f
                            in (E.Short.toModedec
                                (E.Short.mroot ([], name, r, mS')), f');;
        let rec parseModeParen =
          function 
                   ...
                   | (LS.Cons ((t, r), s'), _)
                       -> Parsing.error
                          (r, "Expected identifier, found " ^ (L.toString t));;
        let rec parseMode1 =
          function 
                   ...
                   | LS.Cons ((t, r), _)
                       -> Parsing.error
                          (r,
                           "Expected identifier or mode, found " ^
                             (L.toString t))
        and parseModeNext =
          function 
                   | (modedec, (LS.Cons ((dot_, _), s') as f))
                       -> ([modedec], f)
                   | (modedec, f)
        ...
        let rec parseMode' =
          function 
                   | LS.Cons ((mode_, r), s') -> parseMode1 (LS.expose s')
                   | LS.Cons ((unique_, r), s') -> parseMode1 (LS.expose s')
                   | LS.Cons ((covers_, r), s') -> parseMode1 (LS.expose s');;
        end;;
