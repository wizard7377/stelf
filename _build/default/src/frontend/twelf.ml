  ...
  struct
    open!
      struct
        ...
        type 'a result_ = | Value of 'a 
                          | Exception of exn ;;
        let rec withOpenIn fileName scope =
          let instream = TextIO.openIn fileName
            in let _ = fileOpenMsg fileName
                 in let result =
                      try (Value (scope instream))
                      with 
                           | exn -> (Exception exn)
                      in let _ = fileCloseMsg fileName
                           in let _ = TextIO.closeIn instream
                                in begin
                                   match result
                                   with 
                                        | Value x -> x
        ...
        let rec evarInstToString xs_ = begin
          if (! Global.chatter) >= 3 then Print.evarInstToString xs_ else ""
          end;;
        ...
        let rec abortIO =
          function 
                   | (fileName,
                      { cause = OS.SysErr (m, _); function_ = f; name = n})
                       -> begin
                            msg
                            (((("IO Error on file " ^ fileName) ^ ":\n") ^ m)
                               ^ "\n");
                            Abort
                            end
                   | (fileName, { function_ = f})
                       -> begin
                            msg
                            (((("IO Error on file " ^ fileName) ^
                                 " from function ")
        ...
        let rec handleExceptions chlev fileName ((f : 'a -> status_))
          ((x : 'a)) =
          try f x
          with 
               ...
               | IntSyn.Error msg
                   -> abort chlev (("Signature error: " ^ msg) ^ "\n")
               ...
               | CSManager.Error msg
                   -> abort
                      chlev
                      (("Constraint Solver Manager error: " ^ msg) ^ "\n")
               | exn
                   -> begin
                        abort 0 (UnknownExn.unknownExn exn);raise exn
                        end;;
        ...
        let rec installConst fromCS (cid, fileNameocOpt) =
          let _ = Origins.installOrigin (cid, fileNameocOpt)
            in let _ = Index.install fromCS ((IntSyn.Const cid))
                 in let _ = IndexSkolem.install fromCS ((IntSyn.Const cid))
                      in let _ =
                           Timers.time
                           Timers.compiling
                           Compile.install
        ...
        let rec installConDec fromCS
          ...
          let _ =
            ...
            Timers.modes
            ModeCheck.checkD
            (conDec, fileName, ocOpt)
            in let cid = IntSyn.sgnAdd conDec
                 in let _ =
                      try begin
                          match (fromCS, ! context)
                          with 
                               | (ordinary_, Some namespace)
                                   -> Names.insertConst (namespace, cid)
                               | (clause_, Some namespace)
                                   -> Names.insertConst (namespace, cid)
                               | _ -> ()
                          end
                      with 
                      ...
                      in let _ = Names.installConstName cid
                           in let _ =
                                     ...
                                     | Subordinate.Error msg
                                         -> raise
                                            ((Subordinate.Error
                                              (Paths.wrap (r, msg))))
                                in let _ =
                                     Origins.installLinesInfo
                                     (fileName, Paths.getLinesInfo ())
                                     in let _ = begin
                                          if (! Global.style) >= 1 then
                                          StyleCheck.checkConDec cid else ()
                                          end in cid;;
            ...
            in let _ =
                 ...
                 in let _ = Names.installConstName cid
                      in let _ =
                           ...
                           in let _ =
                                Origins.installLinesInfo
                                (fileName, Paths.getLinesInfo ()) in cid;;
        ...
        let rec installStrDec (strdec, module_, r, isDef) =
          let rec installAction (((cid, _) as data)) =
            begin
              installConst IntSyn.ordinary_ data;begin
              if (! Global.chatter) >= 4 then
              msg ((Print.conDecToString (IntSyn.sgnLookup cid)) ^ "\n") else
              () end
              end
            in let _ =
                 try ModSyn.installStruct
        ...
        let rec includeSig (module_, r, isDef) =
          let rec installAction (((cid, _) as data)) =
            ...
            in let _ =
                 try ModSyn.installSig
                     (module_, ! context, installAction, isDef)
                 with 
                      | Names.Error msg
                          -> raise ((Names.Error (Paths.wrap (r, msg))))
                 in ();;
        ...
        let rec invalidate uninstallFun cids msg =
          let uninstalledCids =
            List.filter (function 
                                  | a -> uninstallFun a) cids
            in let _ =
                 begin
                 match uninstalledCids
                 with 
                      | [] -> ()
                      | _
                          -> chmsg
        ...
        and install1WithSig =
          function 
                   ...
                   | (fileName, moduleOpt, (Parser.StructDec structdec, r))
                       -> begin
                          match ReconModule.structdecToStructDec
                                (structdec, moduleOpt)
                          with 
                               | ReconModule.StructDec
                   ...
                   | (fileName, moduleOpt, (Parser.Include sigexp, r))
                       -> let (module_, wherecls) =
                            ...
                            in let module' =
                                 ...
                                 module_
                                 wherecls
                                 in let _ = includeSig (module', r, false)
                                      in let _ = begin
                                           if (! Global.chatter) = 3 then
                                           msg "%include { ... }.\n" else ()
                                           end in ()
                   | (fileName, None, (Parser.Open strexp, r))
                       -> let mid = ReconModule.strexpToStrexp strexp
                            in let ns = Names.getComponents mid
                                 in let module_ =
                                      ModSyn.abstractModule (ns, (Some mid))
                                      in let _ =
                                           includeSig (module_, r, true)
                                           in let _ = begin
                                                if (! Global.chatter) = 3
                                                then
                                                msg
        ...
        let rec installSubsig (fileName, s) =
          let namespace = Names.newNamespace ()
            in let (mark, markStruct) = IntSyn.sgnSize ()
                 in let markSigDef = ModSyn.sigDefSize ()
                      in let oldContext = ! context
                           in let _ = context := ((Some namespace))
                                in let _ = begin
                                     if (! Global.chatter) >= 4 then
                                     msg "\n% begin subsignature\n" else ()
                                     end
                                     in let rec install s =
                                          install'
                                          (Timers.time
                                           Timers.parsing
                                           S.expose
                        ...
                        -> let _ = ReconTerm.resetErrors fileName
                             in let rec install s =
                                ...
                                and install' =
                                  ...
                                  in install (Parser.parseStream instream));;
        let rec loadString str =
          ...
          (function 
                    | ()
                        -> let _ = ReconTerm.resetErrors "string"
                             in let rec install s =
                                  install'
                                  (Timers.time Timers.parsing S.expose s)
                                and install' =
          ...
          ();;
        let rec sLoop () = begin if Solve.qLoop () then Ok else Abort end;;
        let rec topLoop () =
          begin
          match handleExceptions 0 "stdIn" sLoop ()
          with 
               | Abort -> topLoop ()
               | Ok -> ()
          end;;
        ...
        let rec readDecl () =
          ...
          (function 
                    | ()
                        -> let _ = ReconTerm.resetErrors "stdIn"
                             in let rec install s =
                                  install'
                                  (Timers.time Timers.parsing S.expose s)
                                and install' =
                                  ...
                                  in install
                                     (Parser.parseStream TextIO.stdIn))
          ();;
        let rec decl id =
          begin
          match Names.stringToQid id
          with 
               | None
                   -> begin
                        msg
                        (id ^ " is not a well-formed qualified identifier\n");
                        Abort
                        end
               | Some qid
                   -> begin
        ...
        module Config = struct
                          ...
                          let rec mkRel (prefix, path) =
                            OS.Path.mkCanonical
                            (begin
                             if OS.Path.isAbsolute path then path else
                             OS.Path.concat (prefix, path) end);;
                          let rec read config =
                              ...
                              let rec appendUniq' =
                                function 
                                         | (x :: l2) -> begin
                                             ...
                                             (function 
                                                       | y -> x = y)
                                             l1 then appendUniq' l2 else
                                             (x :: appendUniq' l2) end
                                         | [] -> List.rev l1
                                in List.rev (appendUniq' (List.rev l2))
                              in let rec isConfig item =
                                   let suffix_size =
                                     (String.size (! suffix)) + 1
                                     in let suffix_start =
                                          (String.size item) - suffix_size
                                          in (suffix_start >= 0) &&
                                               ((String.substring
                                                 (item, suffix_start,
                                                  suffix_size)) =
                                                  ("." ^ (! suffix)))
                                   in let rec fromUnixPath path =
                                        let vol = OS.Path.getVolume config
                                          in let isAbs =
                                               String.isPrefix "/" path
                                               in let arcs =
                                                    String.tokens
                                                    (function 
                                                              | c -> c = '/')
                                                    ...
                                                    in OS.Path.toString
                                                       { isAbs; vol; arcs} 
                                        in let rec read' (sources, configs)
                                             config =
                                             withOpenIn
                                             config
                                             (function 
                                                  ...
                                                  | m
                                                      -> mkRel
                                                         (d',
                                                          ModFile.fileName m))
                                        fs'
                                        in let rec redundant m =
                                             let n =
                                               mkRel (d, ModFile.fileName m)
                                               in List.exists
                                                  (function 
                                                            | n' -> n = n')
                                                  fns'
                          ...
                          let rec loadAbort =
                            function 
                                     | (mfile, Ok)
                                         -> let status =
                                              loadFile
                                              (ModFile.fileName mfile)
                                              in begin
                                                   begin
                                                   match status
                                                   with 
                                                        | Ok
                                                            -> ModFile.makeUnmodified
                                                               mfile
                                                        | _ -> ()
                                                   end;status
                                                   end
                            ...
                            let rec fromFirstModified =
                              function 
                                       ...
                                       | ((x :: xs) as sources) -> begin
                                           if ModFile.modified x then sources
                                           else fromFirstModified xs end
                              in let rec mkAbsolute p =
                                   Compat.OS.Path.mkAbsolute
                                   { path = p; relativeTo = pwdir} 
                                   in let sources' = begin
                                        if pwdir = (OS.FileSys.getDir ())
                                        then sources else
                                        List.map
                                        (ModFile.editName mkAbsolute)
                                        sources end
                                        in let sources'' =
                                             fromFirstModified sources'
                                             in List.foldl
                                                loadAbort
                                                Ok
                                                sources'';;
    ...
    *);;
    ...
    module Table : sig
      ...
      struct
        ...
        (* top () = () starts interactive query loop *);;
        let rec top () =
          let rec sLoopT () = begin if Solve.qLoopT () then ok_ else abort_
            end
            in let rec topLoopT () =
                 begin
                 match handleExceptions 0 "stdIn" sLoopT ()
                 with 
                      | abort_ -> topLoopT ()
                      | ok_ -> ()
                 end (* ""stdIn"" as fake fileName *) in topLoopT ();;
        end;;
    end;;
(* local *);;
(* functor Twelf *);;
# 1 "src/frontend/twelf.sml.ml"
