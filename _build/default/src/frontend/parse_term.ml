# 1 "src/frontend/parse_term.sig.ml"
open! Basis;;
...
module type PARSE_TERM = sig
                         ...
                         val parseQualId' : (string list * Parsing.lexResult)
                           Parsing.parser
                         val parseQualIds' : (string list * string) list
                           Parsing.parser
                         val parseFreeze' : (string list * string) list
                           Parsing.parser
                         val
                           parseSubord' : ((string list * string) *
                                           (string list * string))
                           list Parsing.parser
                         val parseThaw' : (string list * string) list
                           Parsing.parser
                         val parseDeterministic' : (string list * string)
                           list Parsing.parser
                         val parseCompile' : (string list * string) list
                           Parsing.parser
                         (* -ABP 4/4/03 *)
                         val parseTerm' : ExtSyn.term Parsing.parser
                         val parseDec' : (string option * ExtSyn.term option)
                           Parsing.parser
                         val parseCtx' : ExtSyn.dec list Parsing.parser
                         end;;
(* signature PARSE_TERM *);;
# 1 "src/frontend/parse_term.fun.ml"
open! Parsing;;
open! Basis;;
  ...
  struct
    ...
    open!
      struct
        ...
        let backArrowOp =
          (Infix ((FX.dec FX.minPrec, FX.left_), ExtSyn.backarrow));;
        let colonOp =
          (Infix ((FX.dec (FX.dec FX.minPrec), FX.left_), ExtSyn.hastype));;
        let rec infixOp (infixity, tm) =
          (Infix
           (infixity,
            function 
                     | (tm1, tm2) -> ExtSyn.app (ExtSyn.app (tm, tm1), tm2)));;
        let rec prefixOp (prec, tm) =
          (Prefix (prec, function 
                                  | tm1 -> ExtSyn.app (tm, tm1)));;
        let rec postfixOp (prec, tm) =
          (Postfix (prec, function 
                                   | tm1 -> ExtSyn.app (tm, tm1)));;
        let rec idToTerm =
          function 
                   | (lower_, ids, name, r) -> ExtSyn.lcid (ids, name, r)
                   | (upper_, ids, name, r) -> ExtSyn.ucid (ids, name, r)
                   | (quoted_, ids, name, r) -> ExtSyn.quid (ids, name, r);;
        let rec isQuoted = function 
                                    | quoted_ -> true
                                    | _ -> false;;
          ...
          struct
            let rec reduce =
              function 
                       | (Atom tm2 :: (Infix (_, con) :: (Atom tm1 :: p')))
                           -> ((Atom (con (tm1, tm2))) :: p')
                       | (Atom tm :: (Prefix (_, con) :: p'))
                           -> ((Atom (con tm)) :: p')
                       | (Postfix (_, con) :: (Atom tm :: p'))
                           -> ((Atom (con tm)) :: p');;
            let rec reduceRec =
              function 
                       | (Atom e :: []) -> e
            ...
            let rec reduceAll =
              function 
                       ...
                       | (r, (Infix _ :: p'))
                           -> Parsing.error
                              (r, "Incomplete infix expression")
                       | (r, (Prefix _ :: p'))
                           -> Parsing.error
                              (r, "Incomplete prefix expression")
            ...
            let rec shift =
              function 
                       ...
                       | (r, Infix _, (Prefix _ :: p'))
                           -> Parsing.error
                              (r, "Infix operator following prefix operator")
                       | (r, Infix _, [])
                       ...
                       | (r, Postfix _, (Infix _ :: p'))
                           -> Parsing.error
                              (r,
                               "Postfix operator following infix operator")
                       | (r, Postfix _, (Prefix _ :: p'))
                           -> Parsing.error
                              (r,
                               "Postfix operator following prefix operator")
                       | (r, Postfix _, [])
            ...
            let rec resolve =
              function 
                       | (r, (Infix ((prec, assoc), _) as opr),
                          ((Atom _ :: (Infix ((prec', assoc'), _) :: p')) as
                            p))
                           -> begin
                              match (FX.compare (prec, prec'), assoc, assoc')
                              with 
                                   | (GREATER, _, _) -> shift (r, opr, p)
                                   | (LESS, _, _)
                                       -> resolve (r, opr, reduce p)
                                   | (Equal, left_, left_)
                                       -> resolve (r, opr, reduce p)
                                   | (Equal, right_, right_)
                                       -> shift (r, opr, p)
        ...
        let rec parseQualId' ((LS.Cons (((L.Id (_, id) as t), r), s') as f))
          =
          begin
          match LS.expose s'
          with 
               | LS.Cons ((pathsep_, _), s'')
        ...
        let rec parseQualIds1 =
          function 
                   | (ls, (LS.Cons (((L.Id (_, id) as t), r0), s') as f))
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in let r = Paths.join (r0, r1)
                                 in let f'' = stripBar f'
                                      in parseQualIds1
                                         (((ids, name) :: ls), f'')
                   | (ls, LS.Cons ((rparen_, r), s')) -> (ls, LS.expose s')
                   | (ls, LS.Cons ((t, r), s))
                       -> Parsing.error
                          (r,
                           "Expected label, found token " ^ (L.toString t));;
        let rec parseQualIds' =
          function 
                   | LS.Cons ((lparen_, r), s')
                       -> parseQualIds1 ([], LS.expose s')
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected list of labels, found token " ^
                             (L.toString t));;
        let rec stripRParen =
          function 
                   | LS.Cons ((rparen_, r), s') -> LS.expose s'
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected closing `)', found " ^ (L.toString t));;
        let rec parseSubordPair2 =
          function 
                   | ((LS.Cons ((L.Id _, _), _) as f), qid)
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in ((qid, (ids, name)), stripRParen f')
                   | (LS.Cons ((t, r), s'), qid)
                       -> Parsing.error
                          (r,
        ...
        let rec parseSubordPair1 =
          function 
                   | (LS.Cons ((L.Id _, _), _) as f)
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in parseSubordPair2 (f', (ids, name))
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected identifier, found token " ^
        ...
        let rec parseSubord' =
          function 
                   | (LS.Cons ((lparen_, r), s'), qidpairs)
                       -> let (qidpair, f) = parseSubordPair1 (LS.expose s')
                            in parseSubord' (f, (qidpair :: qidpairs))
                   | ((LS.Cons ((dot_, _), _) as f), qidpairs)
                       -> (List.rev qidpairs, f)
                   | (LS.Cons ((t, r), s'), qidpairs)
                       -> Parsing.error
                          (r,
                           "Expected a pair of identifiers, found token " ^
                             (L.toString t));;
        let rec parseFreeze' =
          function 
                   | ((LS.Cons ((L.Id _, _), _) as f), qids)
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in parseFreeze' (f', ((ids, name) :: qids))
                   | ((LS.Cons ((dot_, _), _) as f), qids)
                       -> (List.rev qids, f)
                   | (LS.Cons ((t, r), s'), qids)
                       -> Parsing.error
                          (r,
        ...
        let rec parseCompile' =
          function 
                   | ((LS.Cons ((L.Id _, _), _) as f), qids)
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in parseCompile' (f', ((ids, name) :: qids))
                   | ((LS.Cons ((dot_, _), _) as f), qids)
                       -> (List.rev qids, f)
                   | (LS.Cons ((t, r), s'), qids)
                       -> Parsing.error
          ...
          function 
                   | ((LS.Cons ((L.Id _, r0), _) as f), p)
                       -> let ((ids, (L.Id (idCase, name), r1)), f') =
                            parseQualId' f
                            in let r = Paths.join (r0, r1)
                                 in let tm = idToTerm (idCase, ids, name, r)
                                      in begin
                                      if isQuoted idCase then
                                      parseExp' (f', P.shiftAtom (tm, p))
                                      else
                                      begin
                                      match Names.fixityLookup
                                            ((Names.Qid (ids, name)))
                                      with 
                                           | nonfix_
                                               -> parseExp'
                   ...
                   | (LS.Cons ((underscore_, r), s), p)
                       -> parseExp (s, P.shiftAtom (ExtSyn.omitted r, p))
                   | (LS.Cons ((type_, r), s), p)
                       -> parseExp (s, P.shiftAtom (ExtSyn.typ r, p))
                   | (LS.Cons ((colon_, r), s), p)
                       -> parseExp (s, P.resolve (r, colonOp, p))
                   | (LS.Cons ((backarrow_, r), s), p)
                       -> parseExp (s, P.resolve (r, backArrowOp, p))
                   | (LS.Cons ((arrow_, r), s), p)
                       -> parseExp (s, P.resolve (r, arrowOp, p))
                   | (LS.Cons ((lparen_, r), s), p)
                       -> decideRParen (r, parseExp (s, []), p)
                   | ((LS.Cons ((rparen_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | (LS.Cons ((lbrace_, r), s), p)
                       -> decideRBrace (r, parseDec s, p)
                   | ((LS.Cons ((rbrace_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | (LS.Cons ((lbracket_, r), s), p)
                       -> decideRBracket (r, parseDec s, p)
                   | ((LS.Cons ((rbracket_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | ((LS.Cons ((equal_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | ((LS.Cons ((dot_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | ((LS.Cons ((eof_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | ((LS.Cons ((solve_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
                   | ((LS.Cons ((define_, r), s) as f), p)
                       -> (P.reduceAll (r, p), f)
        ...
        and parseDec' =
          function 
                   | LS.Cons ((L.Id (quoted_, name), r), s')
                       -> Parsing.error
                          (r, "Illegal bound quoted identifier " ^ name)
                   | LS.Cons ((L.Id (idCase, name), r), s')
                       -> begin
                          match Names.fixityLookup ((Names.Qid ([], name)))
                          with 
                               | nonfix_
                                   -> parseDec1 ((Some name), LS.expose s')
                               | FX.Infix _
                                   -> Parsing.error
                                      (r,
                                       "Cannot bind infix identifier " ^ name)
                               | FX.Prefix _
                                   -> Parsing.error
                                      (r,
                                       "Cannot bind prefix identifier " ^
                                         name)
                               | FX.Postfix _
                                   -> Parsing.error
                                      (r,
                                       "Cannot bind postfix identifier " ^
                                         name)
                          end
                   ...
                   | LS.Cons ((eof_, r), s')
                       -> Parsing.error
                          (r, "Unexpected end of stream in declaration")
                   | LS.Cons ((t, r), s')
                       -> Parsing.error
                          (r,
                           "Expected variable name, found token " ^
                             (L.toString t))
        and parseDec1 =
          function 
                   | (x, LS.Cons ((colon_, r), s'))
                       -> let (tm, f'') = parseExp (s', [])
                            in ((x, (Some tm)), f'')
                   | (x, (LS.Cons ((rbrace_, _), _) as f)) -> ((x, None), f)
                   | (x, (LS.Cons ((rbracket_, _), _) as f))
                       -> ((x, None), f)
                   | (x, LS.Cons ((t, r), s'))
                       -> Parsing.error
                          (r,
        ...
        and decideRParen =
          function 
                   | (r0, (tm, LS.Cons ((rparen_, r), s)), p)
                       -> parseExp (s, P.shiftAtom (tm, p))
                   | (r0, (tm, LS.Cons ((_, r), s)), p)
                       -> Parsing.error
                          (Paths.join (r0, r), "Unmatched open parenthesis")
        and decideRBrace =
          function 
                   | (r0, ((x, yOpt), LS.Cons ((rbrace_, r), s)), p)
                       -> let dec =
                            begin
                            match yOpt
                            with 
                                 | None
                                     -> ExtSyn.dec0 (x, Paths.join (r0, r))
                                 | Some y
                                     -> ExtSyn.dec (x, y, Paths.join (r0, r))
                            end
                            in let (tm, f') = parseExp (s, [])
                                 in parseExp'
                                    (f',
                                     P.shiftAtom (ExtSyn.pi (dec, tm), p))
                   | (r0, (_, LS.Cons ((_, r), s)), p)
                       -> Parsing.error
                          (Paths.join (r0, r), "Unmatched open brace")
        and decideRBracket =
          function 
                   | (r0, ((x, yOpt), LS.Cons ((rbracket_, r), s)), p)
                       -> let dec =
                            begin
                            match yOpt
                            with 
                            ...
                            in let (tm, f') = parseExp (s, [])
                                 in parseExp'
                                    (f',
                                     P.shiftAtom (ExtSyn.lam (dec, tm), p))
                   | (r0, (dec, LS.Cons ((_, r), s)), p)
                       -> Parsing.error
                          (Paths.join (r0, r), "Unmatched open bracket");;
        let rec stripRBrace =
          function 
                   | LS.Cons ((rbrace_, r), s') -> (LS.expose s', r)
                   | LS.Cons ((t, r), _)
                       -> Parsing.error
                          (r, "Expected `}', found " ^ (L.toString t))
        and parseBracedDec (r, f) =
          let ((x, yOpt), f') = parseDec' f
            in let (f'', r2) = stripRBrace f'
                 in let d =
                      begin
                      match yOpt
                      with 
          ...
          function 
                   | (b, ds, LS.Cons ((((lbrace_, r), s') as bs_)))
                       -> let (d, f') = parseBracedDec (r, LS.expose s')
                            in parseCtx (false, (d :: ds), f')
                   | (b, ds, (LS.Cons ((t, r), s') as f)) -> begin
                       if b then
                       Parsing.error
                       (r, "Expected `{', found " ^ (L.toString t)) else
                       (ds, f) end;;
        end;;
    (* some shorthands *);;
    ...
    *);;
    ...
    let parseSubord' = function 
                                | f -> parseSubord' (f, []);;
    let parseFreeze' = function 
                                | f -> parseFreeze' (f, []);;
    let parseThaw' = function 
                              | f -> parseThaw' (f, []);;
    let parseDeterministic' = function 
                                       | f -> parseDeterministic' (f, []);;
    let parseCompile' = function 
                                 | f -> parseCompile' (f, []);;
    (* -ABP 4/4/03 *);;
    let parseTerm' = function 
                              | f -> parseExp' (f, []);;
    let parseDec' = parseDec';;
    let parseCtx' = function 
                             | f -> parseCtx (true, [], f);;
    end;;
(* local ... in *);;
(* functor ParseTerm *);;
# 1 "src/frontend/parse_term.sml.ml"
