  ...
  *);;
    ...
    let rec queryToQuery (Query_ (optName, tm), Paths.Loc (fileName, r)) =
      let _ = Names.varReset IntSyn.null_
        in let _ = T.resetErrors fileName
             in let T.JClass ((v_, oc), l_) =
                  Timers.time Timers.recon T.reconQuery (T.jclass tm)
                  in let _ = T.checkErrors r
                       in let _ =
                            begin
                            match l_
                            with 
                            ...
                            in let xs_ = Names.namedEVars ()
                                 in let _ = begin
                                      if freeVar (optName, xs_) then
                                      error
                                      (r,
                                       ("Proof term variable " ^
                                          (valOf optName))
                                         ^ " occurs in type")
                                      else () end in (v_, optName, xs_)
    ...
    let rec finishDefine
      ...
      let (i, (u'_, v'_)) =
        ...
        with 
             | Abstract.Error msg
                 -> raise
                    ((Abstract.Error (Paths.wrap (Paths.toRegion oc1, msg))))
        in let name =
             begin match optName with 
                                      | None -> "_"
                                      | Some name -> name end
             in let ocd = Paths.def (i, oc1, oc2Opt)
                  in let cd =
                       try begin
                             Strict.check ((u'_, v'_), (Some ocd));
                             (IntSyn.ConDef
                              (name, None, i, u'_, v'_, l_,
                               IntSyn.ancestor u'_))
        ...
        in let name =
             ...
             in let cd =
                  try begin
                        ...
                        (IntSyn.ConDef
                         (name, None, i, u'_, v'_, IntSyn.type_,
                          IntSyn.ancestor u'_))
        ...
        in let _ = T.resetErrors fileName
             in let rec mkd =
                  ...
                  in let rec mkj =
                       ...
                       in let T.JAnd (defines', T.JClass ((v_, _), l_)) =
                            Timers.time
                            Timers.recon
                            T.reconQuery
