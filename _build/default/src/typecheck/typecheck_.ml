...
(* Type Checking *);;
(* Author: Carsten Schuermann *);;
module TypeCheck(TypeCheck__0: sig
                               (*! structure IntSyn' : INTSYN !*)
                               module Conv : CONV
    ...
    open!
      struct
        ...
        let rec checkExp (g_, us_, vs_) =
          let us'_ = inferExp (g_, us_) in begin
            if Conv.conv (us'_, vs_) then () else
            raise ((Error "Type mismatch")) end
        and inferUni type_ = I.kind_
        and inferExpW =
          function 
                   ...
                   | (g_, (I.Root (c_, s_), s))
                       -> inferSpine
                          (g_, (s_, s), Whnf.whnf (inferCon (g_, c_), I.id))
                   | (g_, (I.Lam (d_, u_), s))
                       -> begin
                            checkDec (g_, (d_, s));
                            ((I.Pi
                              ((I.decSub (d_, s), I.maybe_),
                               (I.EClo
                                (inferExp
                   ...
                   | (g_, (I.FgnExp csfe, s))
                       -> inferExp
                          (g_, (I.FgnExpStd.ToInternal.apply csfe (), s))
                   ...
                   | (g_, (I.App (u_, s_), s1),
                      (I.Pi ((I.Dec (_, v1_), _), v2_), s2))
                       -> begin
                            ...
                            (g_, (s_, s1),
                             Whnf.whnf
                             (v2_, (I.Dot ((I.Exp ((I.EClo (u_, s1)))), s2))))
                            
                            end
                   | (g_, ((I.App _, _) as ss_),
                      ((I.Root (I.Def _, _), _) as vs_))
        ...
        and inferCon =
          function 
                   | (g_, I.BVar k')
                       -> let I.Dec (_, v_) = I.ctxDec (g_, k') in v_
                   | (g_, I.Proj (b_, i))
                       -> let I.Dec (_, v_) = I.blockDec (g_, b_, i) in v_
        ...
        and typeCheck (g_, (u_, v_)) =
          begin
            checkCtx g_;checkExp (g_, (u_, I.id), (v_, I.id))
            end
        and checkSub =
          function 
                   | (null_, I.Shift 0, null_) -> ()
                   | (I.Decl (g_, d_), I.Shift k, null_) -> begin
                       if k > 0 then
                       checkSub (g_, (I.Shift (k - 1)), I.null_) else
                       raise ((Error "Substitution not well-typed")) end
                   | (g'_, I.Shift k, g_)
                       -> checkSub
                          (g'_, (I.Dot ((I.Idx (k + 1)), (I.Shift (k + 1)))),
                           g_)
                   | (g'_, I.Dot (I.Idx k, s'), I.Decl (g_, I.Dec (_, v2_)))
                       -> let _ = checkSub (g'_, s', g_)
                   ...
                   | (g'_, I.Dot (I.Exp u_, s'), I.Decl (g_, I.Dec (_, v2_)))
                       -> let _ = checkSub (g'_, s', g_)
                            in let _ =
                                 typeCheck (g'_, (u_, (I.EClo (v2_, s'))))
                                 in ()
                   | (g'_, I.Dot (I.Idx w, t), I.Decl
                      (g_, I.BDec (_, (l, s))))
                       -> let _ = checkSub (g'_, t, g_)
                            in let I.BDec (_, (l', s')) = I.ctxDec (g'_, w)
                                 in begin
                                 if l <> l' then
                                 raise
                                 ((Error "Incompatible block labels found"))
                                 else begin
                                 if Conv.convSub (I.comp (s, t), s') then ()
                                 else
                                 raise
                                 ((Error
                                   "Substitution in block declaration not well-typed"))
                                 end end
                   | (g'_, I.Dot (I.Block (I.Inst i_), t), I.Decl
                      (g_, I.BDec (_, (l, s))))
                       -> let _ = checkSub (g'_, t, g_)
                            in let (g_, l_) = I.constBlock l
                                 in let _ =
                                      checkBlock
                                      (g'_, i_, (I.comp (s, t), l_)) in ()
                   | (g'_, (I.Dot (_, _) as s), null_)
                       -> raise
                          ((Error
                            (("Long substitution" ^ "\n") ^
                               (subToString (g'_, s)))))
        and checkBlock =
          function 
                   | (g_, [], (_, [])) -> ()
                   | (g_, (u_ :: i_), (t, (I.Dec (_, v_) :: l_)))
                       -> begin
                            checkExp (g_, (u_, I.id), (v_, t));
                            checkBlock
                            (g_, i_, ((I.Dot ((I.Exp u_), t)), l_))
                            end
        and checkDec =
          function 
                   | (g_, (I.Dec (_, v_), s))
                       -> checkExp (g_, (v_, s), ((I.Uni I.type_), I.id))
                   | (g_, (I.BDec (_, (c, t)), s))
                       -> let (gsome_, piDecs) = I.constBlock c
                            in checkSub (g_, I.comp (t, s), gsome_)
                   | (g_, (NDec, _)) -> ()
        and checkCtx =
          function 
                   | null_ -> ()
                   | I.Decl (g_, d_)
                       -> begin
                            checkCtx g_;checkDec (g_, (d_, I.id))
                            end;;
        let rec check (u_, v_) = checkExp (I.null_, (u_, I.id), (v_, I.id));;
        let rec infer u_ = (I.EClo (inferExp (I.null_, (u_, I.id))));;
        let rec infer' (g_, u_) = (I.EClo (inferExp (g_, (u_, I.id))));;
        let rec checkConv (u1_, u2_) = begin
          if Conv.conv ((u1_, I.id), (u2_, I.id)) then () else
          raise
          ((Error
            ((("Terms not equal\n  left: " ^
                 (Print.expToString (I.null_, u1_)))
                ^ "\n  right:")
               ^ (Print.expToString (I.null_, u2_)))))
          end;;
        end;;
