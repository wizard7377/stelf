...
module type STRICT = sig
                     ...
                     val
                       check : ((IntSyn.exp_ * IntSyn.exp_) *
                                Paths.occConDec
                                option) ->
                               unit
                     val
                       checkType : ((int * IntSyn.exp_) *
                                    Paths.occConDec
                                    option) ->
                                   unit
                     end;;
...
(* Author: Carsten Schuermann *);;
module Strict(Strict__0: sig module Whnf : WHNF end) : STRICT =
  struct
    (*! structure IntSyn = IntSyn' !*);;
    (*! structure Paths = Paths' !*);;
    exception Error of string ;;
      ...
      struct
                   ...
                   | (k, I.App (I.Root (I.BVar k', nil_), s_))
                       -> let rec indexDistinct =
                            function 
                                     ...
                                     | _ -> false
                            in (k' <= k) &&
                                 ((patSpine (k, s_)) && (indexDistinct s_))
                   | _ -> false;;
        let rec strictExp =
          function 
                   | (_, _, I.Uni _) -> false
                   ...
                   | (k, p, I.Root (h_, s_))
                       -> begin
                          ...
                          with 
                               | I.BVar k' -> begin
                                   if k' = p then patSpine (k, s_) else begin
                                   if k' <= k then strictSpine (k, p, s_)
                                   else false end end
        ...
        and strictSpine =
          function 
                   | (_, _, nil_) -> false
                   | (k, p, I.App (u_, s_))
                       -> (strictExp (k, p, u_)) || (strictSpine (k, p, s_))
        ...
        let rec occToString =
          function 
                   | (Some ocd, occ)
                       -> Paths.wrap (Paths.occToRegionDef1 ocd occ, "")
                   | (None, occ) -> "Error: ";;
          ...
          let rec strictArgParms =
            function 
                         ...
                         -> raise
                            ((Error
                              ((occToString (ocdOpt, occ)) ^
                                 "Head not rigid, use %abbrev")))
                     | (I.Root _, _, _) -> ()
                     | (I.Pi _, _, _) -> ()
                     | (I.FgnExp _, _, _) -> ()
                     | (I.Lam (d_, u'_), I.Pi (_, v'_), occ) -> begin
                         if strictArgParm (1, u'_) then
                         strictArgParms (u'_, v'_, Paths.body occ) else
                         raise
                         ((Error
                           ((((occToString (ocdOpt, occ)) ^
                     ...
                     | ((I.Lam _ as u_), (I.Root (I.Def _, _) as v_), occ)
                         -> strictArgParms
                            (u_, Whnf.normalize (Whnf.expandDef (v_, I.id)),
                             occ)
            in strictArgParms (u_, v_, Paths.top);;
        let rec occursInType ((i, v_), ocdOpt) =
          let rec oit =
            function 
                     ...
                     | ((i, I.Pi ((d_, p_), v_)), occ)
                         -> begin
                            match Abstract.piDepend ((d_, p_), v_)
                            with 
                                 | I.Pi ((d'_, maybe_), v_)
                                     -> oit ((i - 1, v_), Paths.body occ)
                                 | _
                                     -> raise
                                        ((Error
                                          ((((occToString (ocdOpt, occ)) ^
