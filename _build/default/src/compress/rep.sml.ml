...
module Rep = struct
               ...
               let rec cidSize cid =
                 ...
                 with 
                      | I.ConDec (_, _, _, _, _, type_)
                          -> S.size_tp (S.typeOf (Sgn.classifier cid))
                      | I.ConDec (_, _, _, _, _, kind_)
                          -> S.size_knd (S.kindOf (Sgn.classifier cid))
                      | I.ConDef (_, _, _, _, _, _, _)
                          -> defSize (Sgn.def cid)
                      | I.AbbrevDef (_, _, _, _, _, _)
                          -> defSize (Sgn.def cid)
                      | _ -> 0
                 end;;
               ...
               let rec sanityCheck cid =
                 try begin
                     ...
                     with 
                          ...
                          | I.ConDec (_, _, _, _, _, kind_)
                              -> Reductio.check_kind
                                 ([], Sgn.kindOf (Sgn.classifier cid))
                          | I.ConDef (_, _, _, _, _, type_, _)
                              -> let Sgn.Def_term y = Sgn.def cid
                                   in let Syntax.Tclass_ z =
                                        Sgn.classifier cid
                                        in Reductio.check ([], y, z)
                                        (*				     l := (y,z):: !l; *)
                          | I.ConDef (_, _, _, _, _, kind_, _)
                              -> let Sgn.Def_type y = Sgn.def cid
                                   in let Syntax.Kclass_ z =
                                        Sgn.classifier cid
                                        in Reductio.check_type
                                           Reductio.Con_lf
                                           (Syntax.explodeKind z, y)
                          | I.AbbrevDef (_, _, _, _, _, type_)
                              -> let Sgn.Def_term y = Sgn.def cid
                                   in let Syntax.Tclass_ z =
                                        Sgn.classifier cid
                                        in Reductio.check ([], y, z)
                                        (*				     l := (y,z):: !l; *)
                          | I.AbbrevDef (_, _, _, _, _, kind_)
                              -> let Sgn.Def_type y = Sgn.def cid
                                   in let Syntax.Kclass_ z =
                                        Sgn.classifier cid
                                        in Reductio.check_type
               ...
               let rec gen_graph n autoCompress =
                 let _ = autoCompress n
                   in let rec sanity n = begin
                        if n < 0 then true else
                        (sanity (n - 1)) &&
                          (begin
                           if sanityCheck n then true else
                           begin
                                        ...
                                        (function 
                                                  | (x, y)
                                                      -> (((Int.toString x) ^
                                                           ...
                                                           ^ "\n")
                                        pairs)
                                       in let f = TextIO.openOut "/tmp/graph"
                                            in let _ = TextIO.output (f, s)
                                                 in let _ = TextIO.closeOut f
                                                      in ();;
