...
module Sgn = struct
               ...
               type nonrec __0 = { name: string ; classifier: class_ ;
                 o_classifier: class_ ; def: def ; o_def: def ;
                 abbreviation: bool };;
               type nonrec sigent = __0;;
               let sgn_size = 14000;;
               (* XXX *);;
               let sigma : sigent option Array.array =
                 Array.array (sgn_size, None);;
               let all_modes : mode list option Array.array =
                 Array.array (sgn_size, None);;
               let all_ps : bool option Array.array =
                 Array.array (sgn_size, None);;
               ...
               let rec setter table (n, x) =
                 Array.update (table, n, (Some x));;
