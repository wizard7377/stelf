                  ...
                  let rec lower arg__0 arg__1 =
                    ...
                    with 
                         ...
                         | (acc, (TPi (m, a, b), (elt :: sp)))
                             -> let newacc =
                                  (TermDot (termof elt, subst_tp acc a, acc))
                                  in lower newacc (b, sp)
                         | (_, _)
                             -> raise ((Syntax "type mismatch in lowering"))
                    end(*
		...
		(aa, TermDot(termof elt, a, subst))
                  ...
                  and substNth =
                    function 
                             ...
                             | (ZeroDotShift s, n) -> begin
                                 if n = 0 then (SrVar_ 0) else
                                 begin
                                 match substNth (s, n - 1)
                                 with 
                                      | SrTerm_ (t, a)
                                          -> (SrTerm_
                                              (shift t, shift_tp 0 a))
                                      | SrVar_ n -> (SrVar_ (n + 1))
                                      | SrEVar_ (ev, sl)
                                          -> (SrEVar_
                                              (ev, ((Shift (0, 1)) :: sl)))
                             ...
                             | (TermDot (m, a, s), n) -> begin
                                 if n = 0 then (SrTerm_ (m, a)) else
                                 substNth (s, n - 1) end
                             | (EVarDot (ev, sl, s), n) -> begin
                                 if n = 0 then (SrEVar_ (ev, sl)) else
                                 substNth (s, n - 1) end
                             | (Shift (n, m), n') -> begin
                                 if n' >= n then (SrVar_ (n' + m)) else
                                 (SrVar_ n') end
                             | (VarOptDot (no, s), n') -> begin
                                 ...
                                 end else substNth (s, n' - 1) end
                             | (Compose [], n) -> (SrVar_ n)
                             | (Compose ((h :: tl)), n)
                                 -> subst_sr h (substNth ((Compose tl), n))
                  and subst_sr arg__2 arg__3 =
                    begin
                    match (arg__2, arg__3)
                    with 
                         | (s, SrTerm_ (t, a))
                             -> (SrTerm_ (subst_term s t, subst_tp s a))
                  ...
                  and subst_spinelt arg__4 arg__5 =
                    ...
                    with 
                         ...
                         | (s, Ascribe (t, a))
                             -> (Ascribe (subst_nterm s t, subst_tp s a))
                         | (s, Omit) -> Omit
                    end
                  ...
                  and subst_nterm arg__8 arg__9 =
                    ...
                    with 
                         | (s, Lam t)
                             -> (Lam (subst_term ((ZeroDotShift s)) t))
                         | (s, NRoot (h, sp))
                             -> (NRoot (h, subst_spine s sp))
                    end
                  and subst_aterm arg__10 arg__11 =
                    ...
                    with 
                         | (s, ARoot (Const n, sp))
                             -> (ATerm
                                 ((ARoot ((Const n), subst_spine s sp))))
                         | (s, ARoot (Var n, sp))
                  ...
                  and subst_aterm_plus arg__12 arg__13 =
                    ...
                    with 
                         ...
                         | (s, ARoot (Var n, sp))
                             -> reduce_plus
                                (substNth (s, n), subst_spine s sp)
                         | (s, ERoot ((({ contents = None}, _) as ev), sl))
                             -> (AElt ((ERoot (ev, subst_compose (s, sl)))))
                         | (s, (ERoot _ as t))
                             -> subst_spinelt s (eroot_elim_plus t)
                    end
                  and subst_tp arg__14 arg__15 =
                    ...
                    with 
                         ...
                         | (s, TPi (m, b, b'))
                             -> (TPi
                                 (m, subst_tp s b,
                                  subst_tp ((ZeroDotShift s)) b'))
                    end
                  and subst_knd arg__16 arg__17 =
                    begin
                    match (arg__16, arg__17)
                    with 
                         | (s, Type) -> Type
                         | (s, KPi (m, b, k))
                             -> (KPi
                                 (m, subst_tp s b,
                                  subst_knd ((ZeroDotShift s)) k))
                    end
                  and reduce =
                    function 
                             | (SrVar_ n, sp)
                                 -> (ATerm ((ARoot ((Var n), sp))))
                             | (SrTerm_ (NTerm (Lam n), TPi (_, a, b)),
                                (h :: sp))
                                 -> let s = (TermDot (termof h, a, Id))
                                      in let n' = subst_term s n
                                           in let b' = subst_tp s b
                                                in reduce
                                                   ((SrTerm_ (n', b')), sp)
                             | (SrTerm_ ((NTerm (NRoot (h, sp)) as t), a),
                                []) -> t
                             | (SrTerm_ ((ATerm (ARoot (h, sp)) as t), a),
                                []) -> t
                             | (SrTerm_
                                (ATerm
                                 ((ERoot (({ contents = Some _}, _), _) as t)),
                                 a),
                                [])
                                 -> reduce ((SrTerm_ (eroot_elim t, a)), [])
                             | (SrTerm_
                                (ATerm
                                 ((ERoot (({ contents = None}, _), _) as t)),
                                 a),
                                []) -> (ATerm t)
                             | (SrEVar_ ((x, a), sl), sp)
                                 -> let (a', subst) =
                                      lower (substs_comp sl) (a, sp)
                                      in (ATerm ((ERoot ((x, a'), subst))))
                             | _
                                 -> raise
                                    ((Syntax
                                      "simplified-type mismatch in reduction"))
                  and reduce_plus =
                    function 
                             | (SrVar_ n, sp)
                                 -> (AElt ((ARoot ((Var n), sp))))
                  ...
                  and tp_reduce (a, k, sp) =
                    let rec subst_tpfn arg__18 arg__19 =
                      ...
                      with 
                           | (s, TpfnLam_ a)
                               -> (TpfnLam_
                                   (subst_tpfn ((ZeroDotShift s)) a))
                           | (s, TpfnType_ a) -> (TpfnType_ (subst_tp s a))
                      end
                      in let rec tp_reduce' =
                           function 
                                    | (TpfnLam_ a, KPi (_, b, k), (h :: sp))
                                        -> let s =
                                             (TermDot (termof h, b, Id))
                                             in let a' = subst_tpfn s a
                                                  in let k' = subst_knd s k
                                                       in tp_reduce'
                                                          (a', k', sp)
                                    | (TpfnType_ a, Type, []) -> a
                                    | _
                                        -> raise
                                           ((Syntax
                                             "simplified-kind mismatch in type reduction"))
                           in let rec wrap =
                                function 
                                         | (a, KPi (_, b, k))
                                             -> (TpfnLam_ (wrap (a, k)))
                  ...
                  and subst_compose =
                    function 
                             ...
                             | (s, Compose ((h :: tl)))
                                 -> subst_compose
                                    (subst_compose (s, h), (Compose tl))
                             | (Compose ((h :: tl)), s)
                                 -> subst_compose
                                    (h, subst_compose ((Compose tl), s))
                             | (ZeroDotShift s, Shift (0, m))
                                 -> subst_compose
                                    (subst_compose ((Shift (0, 1)), s),
                                     (Shift (0, m - 1)))
                             ...
                             | (Shift (n, m'), (Shift (0, m) as t))
                                 -> subst_compose
                                    ((ZeroDotShift ((Shift (n - 1, m')))), t)
                             | (s, Shift (n, m))
                                 -> subst_compose
                                    (s, (ZeroDotShift ((Shift (n - 1, m)))))
                             | (s, ZeroDotShift s')
                                 -> composeNth
                                    (s, 0,
                                     subst_compose ((Shift (0, 1)), s'))
                             | (s, TermDot (t, a, s'))
                                 -> (TermDot
                                     (subst_term s t, subst_tp s a,
                                      subst_compose (s, s')))
                             | (s, EVarDot (ev, sl, s'))
                                 -> (EVarDot
                                     (ev, (s :: sl), subst_compose (s, s')))
                             | (s, VarOptDot (no, s'))
                                 -> begin
                                    match no
                                    with 
                                    ...
                                    end(* ZeroDotShift (Shift (n-1,m)) = Shift(n,m) but the former is 'smaller' *)
                  ...
                  and shift_aterm arg__22 arg__23 =
                    ...
                    with 
                         | (n, ARoot (Const n', sp))
                             -> (ARoot ((Const n'), shift_spine n sp))
                         | (n, ERoot (ev, sl))
                             -> (ERoot
                                 (ev, subst_compose ((Shift (n, 1)), sl)))
                         | (n, ARoot (Var n', sp))
                             -> let sp' = shift_spine n sp in begin
                                  if n' >= n then
                                  (ARoot ((Var (n' + 1)), sp')) else
                                  (ARoot ((Var n'), sp')) end
                    end
                  and shift_spinelt arg__24 arg__25 =
                    ...
                    with 
                         | (n, Elt (ATerm t))
                             -> (Elt ((ATerm (shift_aterm n t))))
                         | (n, Elt (NTerm t))
                             -> (Elt ((NTerm (shift_nterm n t))))
                         | (n, AElt t) -> (AElt (shift_aterm n t))
                         | (n, Ascribe (t, a))
                             -> (Ascribe (shift_nterm n t, shift_tp n a))
                         | (n, Omit) -> Omit
                    end
                  ...
                  and shift_tp arg__26 arg__27 =
                    ...
                    with 
                         | (n, TPi (m, a, b))
                             -> (TPi (m, shift_tp n a, shift_tp (n + 1) b))
                         | (n, TRoot (h, sp))
                             -> (TRoot (h, shift_spine n sp))
                    end
        ...
        greater than or equal to n. *);;
                  ...
                  let rec ntm_eroot_elim =
                    function 
                             ...
                             | x -> x;;
                  let rec ctxLookup (g_, n) =
                    subst_tp ((Shift (0, n + 1))) (List.nth (g_, n));;
                  ...
                  and size_spinelt =
                    function 
                             ...
                             | Ascribe (t, a)
                                 -> (size_term ((NTerm t))) + (size_tp a)
                             | Omit -> 0
                  and size_tp =
                    function 
                             | TRoot (_, s) -> 1 + (size_spine s)
                             | TPi (_, a, b)
                                 -> (1 + (size_tp a)) + (size_tp b)
                  and size_knd =
                    function 
                             | Type -> 1
                             | KPi (_, a, b)
                                 -> (1 + (size_tp a)) + (size_knd b);;
