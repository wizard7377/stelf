      ...
      struct
        ...
        let rec instEVars =
          function 
                   | (g_, (I.Pi ((I.Dec (_, v1_), _), v2_), s))
                       -> let x1_ = I.newEVar (g_, (I.EClo (v1_, s)))
                            in instEVars
                               (g_, (v2_, (I.Dot ((I.Exp x1_), s))))
        ...
        let rec unifiableSpines =
          function 
                   ...
                   | (g_, (I.App (u1_, s2_), s), (I.App (u1'_, s2'_), s'),
        ...
        let rec unifiableRoots
          (g_, (I.Root (I.Const a, s_), s), (I.Root (I.Const a', s'_), s'),
           ms)
        ...
        let rec checkNotUnifiableTypes (g_, vs_, vs'_, ms, (bx, by)) =
          begin
            ...
            (function 
                      | () -> begin
                          if unifiableRoots (g_, vs_, vs'_, ms) then
                          raise
                          ((Error
                            (((("Blocks " ^ (pName bx)) ^ " and ") ^
                                (pName by))
          ...
          let _ =
            ...
            (function 
                      | ()
                          -> ((("?- " ^ (cName cid)) ^ " ~ ") ^ (cName cid'))
                               ^ "\n")
            in let vs_ = instEVars (I.null_, (I.constType cid, I.id))
                 in let vs'_ = instEVars (I.null_, (I.constType cid', I.id))
                      in let _ =
                           CSManager.trail
                           (function 
                                     | () -> begin
        ...
        let rec checkDiffBlocksInternal =
          function 
                   | (g_, vs_, (t, []), (a, ms), bx) -> ()
                   | (g_, (v_, s),
                      (t, ((I.Dec (yOpt, v'_) as d_) :: piDecs)), (a, ms),
                      (b, xOpt))
                       -> let a' = I.targetFam v'_
                            in let _ = begin
                                 if a = a' then
        ...
        let rec checkUniqueBlockInternal' =
          function 
                   | (g_, (t, []), (a, ms), b) -> ()
                   | (g_, (t, ((I.Dec (xOpt, v_) as d_) :: piDecs)), 
                      (a, ms), b)
                       -> let a' = I.targetFam v_
                            in let _ = begin
                                 if a = a' then
                                 let (v'_, s) = instEVars (g_, (v_, t))
                                   in checkDiffBlocksInternal
                                      ((I.Decl (g_, d_)),
                                       (v'_, I.comp (s, I.shift)),
                                       (I.dot1 t, piDecs), (a, ms),
                                       (b, xOpt))
                                 else () end
                                 in checkUniqueBlockInternal'
                                    ((I.Decl (g_, d_)), (I.dot1 t, piDecs),
                                     (a, ms), b);;
        let rec checkUniqueBlockInternal ((gsome_, piDecs), (a, ms), b) =
          let t = createEVarSub (I.null_, gsome_)
            in checkUniqueBlockInternal' (I.null_, (t, piDecs), (a, ms), b);;
                            ...
                            (function 
                                      | ()
                                          -> ((("?- " ^ (pName bx)) ^ " ~ ")
                                               ...
                                               ^ "\n")
                            in let vs'_ =
                                 instEVars (g_, (I.constType cid, I.id))
                                 in let _ =
                                      CSManager.trail
                                      (function 
                                                | () -> begin
        ...
        let rec checkUniqueBlockBlocks =
          function 
                   | (g_, vs_, [], (a, ms), bx) -> ()
                   | (g_, vs_, (b :: bs), (a, ms), bx)
                       -> let (gsome_, piDecs) = I.constBlock b
                            in let t = createEVarSub (g_, gsome_)
                                 in let _ =
                                      checkUniqueBlockBlock
                                      (g_, vs_, (t, piDecs), (a, ms),
                                       (bx, b))
                                      in checkUniqueBlockBlocks
                                         (g_, vs_, bs, (a, ms), bx);;
        let rec checkUniqueBlock' =
          function 
                   | (g_, (t, []), bs, cs, (a, ms), b) -> ()
                   | (g_, (t, ((I.Dec (xOpt, v_) as d_) :: piDecs)), bs, cs,
                      (a, ms), b)
                       -> let a' = I.targetFam v_
                            in let _ = begin
                                 if a = a' then
        ...
        let rec checkUniqueBlock ((gsome_, piDecs), bs, cs, (a, ms), b) =
          let t = createEVarSub (I.null_, gsome_)
            in checkUniqueBlock' (I.null_, (t, piDecs), bs, cs, (a, ms), b);;
          ...
          function 
                   ...
                   | ((b :: bs), cs, (a, ms))
                       -> begin
                            ...
                            (I.constBlock b, (a, ms), b);
                            begin
                              checkUniqueBlock
                              (I.constBlock b, (b :: bs), cs, (a, ms), b);
                            ...
                            in let _ =
                                 ...
                                 in let _ =
                                      ...
                                      in let _ =
                                           ...
                                           (function 
                                                     | ()
                                                         -> ("Checking uniqueness modes for family "
                                                               ^ (cName a))
                                                              ^ "\n")
                                           in let _ =
                                                try UniqueCheck.checkMode
                                                    (a, ms)
                                                with 
                                                     | UniqueCheck.Error msg
                                                         -> raise
                                                            ((Error
