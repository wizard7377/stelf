  ...
  struct
    ...
    open!
      struct
        ...
        let rec insertState s_ = begin
          if Qed.subgoal s_ then solvedStates := ((s_ :: ! solvedStates))
          else openStates := ((s_ :: ! openStates)) end;;
        let rec cLToString =
          function 
                   | [] -> ""
        ...
        let rec install installConDec =
          let rec install' =
            function 
                     ...
                     | M.ConDec (e, s_)
                         -> begin
                              installConDec e;install' s_
                              end
            in let is_ = begin
                 if (List.length (! openStates)) > 0 then
                 raise ((Error "Theorem not proven")) else
                 makeSignature (! solvedStates) end
                 in begin
                      install' is_;begin
        ...
        let rec printState () =
          let rec print' =
            function 
                     ...
                     | (s_ :: l_)
                         -> begin
                              print (MetaPrint.stateToString s_);print' l_
                              end
            in begin
                 print "Open problems:\n";
                 begin
