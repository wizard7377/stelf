  ...
  struct
    ...
    open!
      struct
        ...
        let rec solve =
          function 
                   | ((C.Atom p, s), dp, sc, acck)
                       -> matchAtom ((p, s), dp, sc, acck)
                   | ((C.Impl (r, a_, h_, g), s), C.DProg (g_, dPool), sc,
                      acck)
          ...
          function 
                   | (ps', (C.Eq q_, s), C.DProg (g_, dPool), sc,
                      ((acc, k) as acck)) -> begin
                       ...
                       sc (I.nil_, acck) else acc end
                   | (ps', (C.And (r, a_, g), s),
                      (C.DProg (g_, dPool) as dp), sc, acck)
        ...
        and matchAtom
          (((I.Root (ha_, _), _) as ps'), (C.DProg (g_, dPool) as dp), sc,
           (acc, k))
          =
                                               ...
                                               | ()
                                                   -> rSolve
                                                      (ps', (r, I.id), dp,
                                                       ...
                                                       (acc'', k - 1)))
                                     in matchSig' (sgn', acc''')
              in matchSig' (Index.lookup (cidFromHead ha_), acc')
                                                           ...
                                                           | (s_, acck')
                                                               -> sc
                                                                  ((I.Root
                                                                   ...
                                                                   acck'),
                                                  (acc', k - 1)))
                                in matchDProg (dPool', n + 1, acc'')
                              else matchDProg (dPool', n + 1, acc') end
                          | (I.Decl (dPool', parameter_), n, acc')
                              -> matchDProg (dPool', n + 1, acc')
                 in begin if k < 0 then acc else matchDProg (dPool, 1, acc)
                 end;;
        ...
        let rec selectEVar =
          function 
                   ...
                   | (((I.EVar (r, _, _, _) as x_) :: ge_), vs_, acc)
                       ...
                       selectEVar (ge_, vs_, (x_ :: acc)) else
                       selectEVar (ge_, vs_, acc) end;;
        ...
        let rec deepen f p_ =
          let rec deepen' (level, acc) = begin
            if level > (! MetaGlobal.maxFill) then acc else
            begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                end;raise ((Error "No object found"))
                end
