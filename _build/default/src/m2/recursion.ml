      ...
      struct
        ...
        let rec vectorToString (g_, o_) =
            ...
            function 
                     | Order.Arg (us_, vs_)
                         -> [(F.String
                             ...
                             (F.String ":");
                             (F.String
                              (Print.expToString (g_, (I.EClo vs_))))]
                     | Order.Lex l_
          ...
          and fmtOrders =
            function 
                     ...
                     | (o_ :: l_)
                         -> (fmtOrder o_) @
                              (((F.String " ") :: fmtOrders l_))
            in F.makestring_fmt ((F.HVbox (fmtOrder o_)));;
        let rec vector (c, (s_, s)) =
          let vid_ = (I.constType c, I.id)
            in let rec select' (n, (ss'_, vs''_)) =
                 select'W (n, (ss'_, Whnf.whnf vs''_))
               and select'W =
                 function 
                          | (1,
                             ((I.App (u'_, s'_), s'),
                              (I.Pi ((I.Dec (_, v''_), _), _), s'')))
                              -> ((u'_, s'), (v''_, s''))
                          | (n, ((I.SClo (s'_, s1'), s2'), vs''_))
                              -> select'W
                                 (n, ((s'_, I.comp (s1', s2')), vs''_))
                          | (n,
                             ((I.App (u'_, s'_), s'),
                              (I.Pi ((I.Dec (_, v1''_), _), v2''_), s'')))
                              -> select'
                                 (n - 1,
                                  ((s'_, s'),
                                   (v2''_,
                                    (I.Dot
                                     ((I.Exp ((I.EClo (u'_, s')))), s'')))))
                 in let rec select =
                      function 
                               | O.Arg n
                                   -> (O.Arg (select' (n, ((s_, s), vid_))))
                               | O.Lex l_ -> (O.Lex (map select l_))
                               | O.Simul l_ -> (O.Simul (map select l_))
                      in select (O.selLookup c);;
        let rec set_parameter (g_, (I.EVar (r, _, v_, _) as x_), k, sc, ops)
          ...
          let rec set_parameter' =
            function 
                     | (0, ops') -> ops'
                     | (k', ops')
                         -> let (I.Dec (_, v'_) as d'_) = I.ctxDec (g_, k')
                              in let ops'' =
                                   CSManager.trail
                                   (function 
                                             | () -> begin
                                                   ...
                                                   (Unify.unifiable
                                                    (g_, (x_, I.id),
                                                     ((I.Root
                                                       ((I.BVar k'), I.nil_)),
                                                      I.id)))
                                                 then sc ops' else ops' end)
                                   in set_parameter' (k' - 1, ops'')
            in set_parameter' (k, ops);;
        ...
        and ltinitW =
          function 
                   ...
                   | (g_, k, ((I.Lam (d1_, u_), s1), (I.Pi (d2_, v_), s2)),
                      ((u'_, s1'), (v'_, s2')), sc, ops)
                       -> ltinit
                          ((I.Decl (g_, I.decSub (d1_, s1))), k + 1,
        ...
        and ltW =
          function 
                   | (g_, k, (us_, vs_),
                      ((I.Root (I.Const c, s'_), s'), vs'_), sc, ops)
                       -> ltSpine
                          (g_, k, (us_, vs_),
                           ((s'_, s'), (I.constType c, I.id)), sc, ops)
                   | (g_, k, (us_, vs_),
                      ((I.Root (I.BVar n, s'_), s'), vs'_), sc, ops) -> begin
                       if n <= k then
                       let I.Dec (_, v'_) = I.ctxDec (g_, n)
                         in ltSpine
                            (g_, k, (us_, vs_), ((s'_, s'), (v'_, I.id)), sc,
                             ops)
                       else ops end
        ...
        and ltSpineW =
          function 
                   ...
                   | (g_, k, (us_, vs_), ((I.SClo (s_, s'), s''), vs'_), sc,
                      ops)
                       -> ltSpineW
                          (g_, k, (us_, vs_), ((s_, I.comp (s', s'')), vs'_),
                           sc, ops)
                   | (g_, k, (us_, vs_),
                      ((I.App (u'_, s'_), s1'),
        ...
        and le (g_, k, (us_, vs_), (us'_, vs'_), sc, ops) =
          let ops' = eq (g_, (us_, vs_), (us'_, vs'_), sc, ops)
            in leW (g_, k, (us_, vs_), Whnf.whnfEta (us'_, vs'_), sc, ops')
        and leW =
          function 
                   | (g_, k, ((u_, s1), (v_, s2)),
                      ((I.Lam ((I.Dec (_, v1'_) as d_), u'_), s1'),
        ...
        let rec ordlt =
          function 
                   ...
                   | (g_, O.Lex l_, O.Lex l'_, sc, ops)
                       -> ordltLex (g_, l_, l'_, sc, ops)
                   | (g_, O.Simul l_, O.Simul l'_, sc, ops)
                       -> ordltSimul (g_, l_, l'_, sc, ops)
        and ordltLex =
          function 
                   | (g_, [], [], sc, ops) -> ops
          ...
          function 
                   ...
                   | (g_, (o_ :: l_), (o'_ :: l'_), sc, ops)
                       -> ordeq
                          (g_, o_, o'_,
                           function 
                                    | ops' -> ordeqs (g_, l_, l'_, sc, ops'),
                           ops)
        and ordle (g_, o_, o'_, sc, ops) =
          let ops' =
            CSManager.trail (function 
                                      | () -> ordeq (g_, o_, o'_, sc, ops))
            in ordlt (g_, o_, o'_, sc, ops');;
        let rec createEVars =
          function 
        ...
        let rec lemma (s_, t, ops) =
          let M.State (name, gm_, v_) = Lemma.apply (s_, t)
            in let (M.Prefix (g'_, m'_, b'_), s') = createEVars gm_
                 in let
                      (g''_,
                       ((I.Root (I.Const a1, s1_), s1),
                        (I.Root (I.Const a2, s2_), s2)))
                      = select (g'_, (v_, s'))
                      in (g''_, vector (a1, (s1_, s1)),
                          vector (a2, (s2_, s2)),
                          function 
                                   | ops'
        ...
        let rec expandLazy' =
          function 
                   ...
                   | (s_, O.Lt (t, l_), ops)
                       -> expandLazy' (s_, l_, ordlt (lemma (s_, t, ops)));;
        let rec recursionDepth v_ =
          let rec recursionDepth' =
            function 
                     | (I.Root _, n) -> n
                     | (I.Pi (_, v_), n) -> recursionDepth' (v_, n + 1)
            in recursionDepth' (v_, 0);;
        let rec expandLazy ((M.State (_, _, v_) as s_)) = begin
          if (recursionDepth v_) > (! MetaGlobal.maxRecurse) then [] else
          expandLazy' (s_, O.mutLookup (I.targetFam v_), []) end;;
        ...
        and inputConvSpine =
          function 
                   ...
                   | (mS, ((I.SClo (s1_, s1'), s1), vs1_), (ss2_, vs2_))
                       -> inputConvSpine
                          (mS, ((s1_, I.comp (s1', s1)), vs1_), (ss2_, vs2_))
                   | (mS, (ss1_, vs1_), ((I.SClo (s2_, s2'), s2), vs2_))
                       -> inputConvSpine
                          (mS, (ss1_, vs1_), ((s2_, I.comp (s2', s2)), vs2_))
                   | (ModeSyn.Mapp (ModeSyn.Marg (minus_, _), mS),
                      ((I.App (u1_, s1_), s1),
                       (I.Pi ((I.Dec (_, v1_), _), w1_), t1)),
                      ((I.App (u2_, s2_), s2),
                       (I.Pi ((I.Dec (_, v2_), _), w2_), t2)))
                       -> (Conv.conv ((v1_, t1), (v2_, t2))) &&
                            (inputConvSpine
                             (mS,
                              ((s1_, s1),
                               (w1_,
                                (I.Dot ((I.Exp ((I.EClo (u1_, s1)))), t1)))),
                              ((s2_, s2),
                               (w2_,
                                (I.Dot ((I.Exp ((I.EClo (u1_, s1)))), t2))))))
                   | (ModeSyn.Mapp (ModeSyn.Marg (plus_, _), mS),
                      ...
                      ((I.App (u2_, s2_), s2),
                       ...
                       -> inputConvSpine
                          (mS,
                           ((s1_, s1),
                            (w1_, (I.Dot ((I.Exp ((I.EClo (u1_, s1)))), t1)))),
                           ((s2_, s2),
                            (w2_, (I.Dot ((I.Exp ((I.EClo (u2_, s2)))), t2)))));;
                            ...
                            function 
                                     ...
                                     | (((v1_, s1) as vs1_),
                                        (I.Pi ((d2_, _), v2_), s2))
                                         -> (compDec (vs1_, (d2_, s2))) ||
                                              (compExp
                                               ((v1_, I.comp (s1, I.shift)),
                                                (v2_, I.dot1 s2)))
                          and compDec (vs1_, (I.Dec (_, v2_), s2)) =
                            inputConv (vs1_, (v2_, s2))
                            in let rec check (M.State (name, gm_, v_)) =
                                 checkW (Whnf.whnf (v_, I.id))
                               and checkW (I.Pi ((d_, _), v_), s) =
                                 checkDec
                                 ((d_, I.comp (s, I.shift)), (v_, I.dot1 s))
                               and checkDec ((I.Dec (_, v1_), s1), vs2_) =
                                 compExp ((v1_, s1), vs2_) in begin
                                 if check s'_ then removeDuplicates ops else
                                 (s'_ :: removeDuplicates ops) end;;
        let rec fillOps =
          function 
                   | [] -> []
                   | (s'_ :: ops)
                       -> let rec fillOps' =
                            ...
                            in let (fillop, _) = Filling.expand s'_
                                 in (fillOps' fillop) @ (fillOps ops);;
        let rec expandEager s_ = removeDuplicates (fillOps (expandLazy s_));;
        let rec apply s_ = s_;;
        let rec menu
          ((M.State
             (name, M.Prefix (g'_, m'_, b'_), I.Pi ((I.Dec (_, v_), _), _))
             as s_))
