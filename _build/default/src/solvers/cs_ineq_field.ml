  ...
  struct
    ...
    open!
      struct
        ...
        let rec gt (u_, v_) =
          (Root ((Const (! gtID)), (App (u_, (App (v_, Nil))))));;
        let rec geq (u_, v_) =
          (Root ((Const (! geqID)), (App (u_, (App (v_, Nil))))));;
        let rec gt0 u_ = gt (u_, (Constant_ zero));;
        let rec geq0 u_ = geq (u_, (Constant_ zero));;
        ...
        let rec gtGeq (u_, v_, w_) =
          (Root
           ((Const (! gtGeqID)), (App (u_, (App (v_, (App (w_, Nil))))))));;
        let rec geq00 () = (Root ((Const (! geq00ID)), Nil));;
        let rec gtNConDec d =
          (ConDec
           (((toString d) ^ ">") ^ (toString zero), None, 0, Normal,
            gt0 ((Constant_ d)), Type));;
        let rec gtNExp d = (Root ((FgnConst (! myID, gtNConDec d)), Nil));;
        let rec geqN0 d = begin
          if d = zero then geq00 () else
          gtGeq ((Constant_ d), (Constant_ zero), gtNExp d) end;;
        let rec parseGtN string =
          let suffix = ">" ^ (toString zero)
            in let stringLen = String.size string
                 in let suffixLen = String.size suffix
                      in let numLen = (Int.( - ) (stringLen, suffixLen))
                           in begin
                           if
                           ((Int.( > ) (stringLen, suffixLen))) &&
                             ((String.substring (string, numLen, suffixLen))
                                = suffix)
                           then
                           begin
                           match fromString
                           ...
                           with 
                                | Some d -> begin
                                    if d > zero then (Some (gtNConDec d))
                                    else None end
                                | None -> None
                           end else None end;;
        type position_ = | Row of int 
                         | Col of int ;;
        type owner_ =
        ...
        type nonrec label =
          [%record_type
            type __record = { owner: owner_ ; tag: int ref ;
              restr: restriction_ option ref ; dead: bool ref };;];;
        type operation_ =
          | Insert of position_ 
          | Pivot of int * int 
          | Kill of position_ 
        ...
        type nonrec tableau =
          [%record_type
            type __record = { rlabels: label Array.array ;
              clabels: label Array.array ; consts: number Array.array ;
            ...
            let t = (Real.( * ) (a, ! seed))
              in begin
                   seed :=
                     ((Real.( - )
                       (t,
                        (Real.( * ) (m, Real.fromInt (Real.floor (t / m)))))));
                   ((Real.( - ) (! seed, 1.0))) / ((Real.( - ) (m, 1.0)))
                   end
            in (Int.( + )
                (min,
                 Real.floor ((Real.( * ) (nextrand (), Real.fromInt size)))));;
        let tableau =
          let l =
            {
                 ...
                 dead = ref (dead old)}
                  in begin
                     ...
                     with 
                          | Row i
                              -> Array.update
                                 ((fun r -> r.rlabels) tableau, i, New_)
                          | Col j
                              -> Array.update
                                 ((fun r -> r.clabels) tableau, j, New_)
                     end;;
          ...
          function 
                   | Sum (m, (Mon (n, _) :: monL))
                       -> begin
                            ...
                            begin
                              print " ? + ";displaySum ((Sum (m, monL)))
                              end
                            
                            end
                   | Sum (m, [])
                       -> begin
                            print (OrderedField.toString m);print " >= 0\n"
                            end;;
        let rec display () =
          let rec printLabel (col, (l : label)) =
            begin
              ...
              begin
                begin
                match (fun r -> r.owner) l
                with 
                     | Var _ -> print "V"
                     | Exp _ -> print "E"
                end;
                begin
                  begin if restricted l then print ">" else print "*" end;
                  begin if dead l then print "#" else print "" end
                  end
                
                end
              
              end
            in let rec printRow (row, (l : label)) =
                 let rec printCol (col, (d : number)) =
                   begin
                     print "\t";print (toString d)
                     end
                   in let vec =
                        Array2.row
        ...
        let rec findMon mon =
          (let exception Found of int 
           in let rec find (i, (l : label)) =
                begin
                match (fun r -> r.owner) l
                with 
                     | Var (g_, mon') -> begin
                         if compatibleMon (mon, mon') then raise ((Found i))
                         else () end
                     | _ -> ()
                end
                in try begin
        ...
        let rec findTag t =
          (let exception Found of int 
           in let rec find (i, (l : label)) = begin
                if ((fun r -> r.tag) l) = t then raise ((Found i)) else ()
                end
                in try begin
                         Array.app
                         find
        ...
        let rec isConstant row =
          ...
          (function 
                    | (j, l, rest)
                        -> ((dead l) || ((coeff (row, j)) = zero)) && rest)
          true
          ((fun r -> r.clabels) tableau, 0, nCols ());;
        let rec isSubsumed row =
          let constRow = const row
            in let rec isSubsumedByRow () =
                 let candidates =
                   Array.foldl
                   (function 
                             | (i, (l : label), rest) -> begin
                                 if
                                 (i <> row) &&
                                   ((not (dead l)) && ((const i) = constRow))
                                 then (i :: rest) else rest end)
                   []
                   ((fun r -> r.rlabels) tableau, 0, nRows ())
                   in let rec filter =
                        function 
                                 ...
                                 | (j, (l : label), candidates) -> begin
                                     ...
                                     (function 
                                               | i
                                                   -> (coeff (i, j)) =
                                                        (coeff (row, j)))
                                     candidates else candidates end
                        in begin
                           match Array.foldl
                                 filter
                                 candidates
                 ...
                 in let rec isSubsumedByCol () = begin
                      ...
                      let nonNull =
                        ...
                        (function 
                                  | (j, (l : label), rest) -> begin
                                      if not (dead l) then
                                      let value = coeff (row, j) in begin
                                        if value <> zero then
                                        ((j, value) :: rest) else rest end
                                      else rest end)
                        []
                        ((fun r -> r.clabels) tableau, 0, nCols ())
                        in begin
                           ...
                           with 
                                | ((j, value) :: []) -> begin
                                    if value = one then (Some j) else None
                                    end
                                | _ -> None
                           end
                      else None end
                      in begin
                         match isSubsumedByRow ()
                         with 
                              | Some i -> (Some ((Row i)))
                              | None
                                  -> begin
                                     match isSubsumedByCol ()
                                     with 
                                          | Some j -> (Some ((Col j)))
                                          | None -> None
                                     end
        ...
        let rec findPivot row =
          let rec compareScore =
            function 
                     ...
                     | (None, Some d') -> GREATER
                     | (None, None) -> Equal
            in let rec findPivotCol
                 (j, (l : label), ((score, champs) as result)) =
                 let value = coeff (row, j)
                   in let rec findPivotRow sgn
                        (i, (l : label), ((score, champs) as result)) =
                        let value = coeff (i, j) in begin
                          if
                          (not (dead l)) &&
                            ((i <> row) &&
                               ((restricted l) &&
                          ...
                          then
                          let score' =
                            (Some (abs ((const i) * (inverse value))))
                            in begin
                               match compareScore (score, score')
                               with 
                                    | GREATER -> (score', [(i, j)])
                 ...
                 in begin
                    ...
                    with 
                         ...
                         | (_, champs)
                             -> (Some
                                 (List.nth
                                  (champs, rand (0, List.length champs))))
                    end;;
        let rec pivot (row, col) =
          let pCoeffInverse = inverse (coeff (row, col))
            in let pRowVector =
                 Array2.row ((fun r -> r.coeffs) tableau, row, (0, nCols ()))
                 in let rec pRow j = Vector.sub (pRowVector, j)
                      in let pColVector =
                           Array2.column
                           ((fun r -> r.coeffs) tableau, col, (0, nRows ()))
                           in let rec pCol i = Vector.sub (pColVector, i)
                                in let pConst = const row
                                     in let pRLabel = rlabel row
                                          in let pCLabel = clabel col
                                               in begin
                                                    Array.modify
                                                    (function 
                                                              | (i, value)
            ...
            with 
                 | Some (i, j) -> begin
                     ...
                     begin
                       ...
                       begin
                         pivot (i, j);maximizeRow row
                         end
                       
                       end
                     else (Unbounded j) end
                 | None -> (Maximized value)
            end else Positive end;;
        let rec delayMon (Mon (n, usL_), cnstr) =
          List.app (function 
                             | us_ -> Unify.delay (us_, cnstr)) usL_;;
        let rec unifyRestr (Restr (g_, proof, strict), proof') = begin
          if Unify.unifiable (g_, (proof, id), (proof', id)) then () else
          raise Error end;;
        let rec unifySum (g_, sum, d) = begin
          if Unify.unifiable (g_, (toExp sum, id), ((Constant_ d), id)) then
        ...
        let rec decomposeSum (g_, Sum (m, monL)) =
          let rec monToWPos ((Mon (n, usL_) as mon)) =
                               ...
                               dead = ref false}
                                in begin
                                     ...
                                     begin
                                       delayMon
                                       (mon,
                                        ref (makeCnstr ((fun r -> r.tag) l)));
                                       begin
                                         Array.update
                                         ((fun r -> r.clabels) tableau, New_,
                                          l);
                                         (n, (Col New_))
                                         end
            ...
            end in (m, List.map monToWPos monL)
        and insertDecomp (((d, wposL) as decomp), owner) =
          let New_ = incrNRows ()
            in let rec insertWPos (d, pos) =
                 begin
                 match pos
                 with 
                      | Row row
                          -> begin
                               incrArray2Row
                               ((fun r -> r.coeffs) tableau, New_,
                                (0, nCols ()),
                                function 
                                         | j -> d * (coeff (row, j)));
                               incrArray
                               ((fun r -> r.consts) tableau, New_,
                                d * (const row))
                               
                               end
                      | Col col
                          -> incrArray2
                             ((fun r -> r.coeffs) tableau, New_, col, d)
                 end
                 in begin
        ...
        and insert (g_, us_) =
          let sum = fromExp us_
            in insertDecomp (decomposeSum (g_, sum), (Exp (g_, sum)))
        and minimize row =
          let rec killColumn (j, (l : label)) = begin
            if (not (dead l)) && ((coeff (row, j)) <> zero) then
            begin
              Trail.log ((fun r -> r.trail) tableau, (Kill ((Col j))));
              begin
                ((fun r -> r.dead)
                 (Array.sub ((fun r -> r.clabels) tableau, j))) := true;
                begin
                  begin
                  match restriction l
                  with 
                  ...
                  with 
                       | (Var _ as owner)
                           -> unifySum
                              (ownerContext owner, ownerSum owner, zero)
                       | _ -> ()
                  end
                  end
            ...
            in let rec killRow (i, (l : label)) = begin
                 if not (dead l) then begin
                 if isConstant i then
                 begin
                   Trail.log ((fun r -> r.trail) tableau, (Kill ((Row i))));
                   begin
                     ((fun r -> r.dead)
                      (Array.sub ((fun r -> r.rlabels) tableau, i))) := true;
                     begin
                       begin
                       match restriction l
                       with 
                       ...
                       with 
                            | (Var _ as owner)
                                -> unifySum
                                   (ownerContext owner, ownerSum owner,
                                    const i)
                            | _ -> ()
                       end
                       end
                          ...
                          -> let l' = (Label_ pos')
                               in begin
                                    ...
                                    begin
                                      ((fun r -> r.dead)
                                       (Array.sub
                                        ((fun r -> r.rlabels) tableau, i)))
                                        := true;
                                      begin
                                      match (restriction l, restriction l')
                                      with 
                                           | (Some restr, Some
                                              (Restr (_, proof', _)))
        ...
        and restrict =
          function 
                   | ((Col col as pos), restr)
                       -> let l = (Label_ pos) in begin
                            if dead l then unifyRestr (restr, geq00 ()) else
                            begin
                            match restriction l
                            with 
                                 | Some (Restr (_, proof', _))
                                     -> unifyRestr (restr, proof')
                                 | None
                                     -> let nonNull =
                                          Array.foldl
                   ...
                   | ((Row row as pos), restr)
                       -> let l = (Label_ pos) in begin
                            if dead l then
                            unifyRestr (restr, geqN0 (const row)) else
                            begin
                            match restriction l
                            with 
                                 | Some (Restr (_, proof', _))
        ...
        and insertEqual (g_, pos, sum) =
          let (m, wposL) = decomposeSum (g_, sum)
            in let decomp' = (m, ((- one, pos) :: wposL))
                 in let pos' =
                      insertDecomp (decomp', (Exp (g_, (Sum (zero, [])))))
                      in let decomp'' = unaryMinusDecomp decomp'
                           in let tag'' =
                                (fun r -> r.tag)
                                ((Label_
                                  (insertDecomp
                                   (decomp'', (Exp (g_, (Sum (zero, []))))))))
                                in begin
                                     restrict
                                     (pos', (Restr (g_, geq00 (), false)));
                                     begin
                                     match findTag tag''
                                     with 
                                          | Some pos''
        ...
        and update (g_, pos, sum) =
          let l = (Label_ pos)
            in begin
                 ...
                 begin
                   setOwnership (pos, (Exp (g_, sum)), ref 0);begin
                   if dead l then
                   begin
                   match pos
                   with 
                        | Row row -> begin
                            if isConstant row then
                            unifySum (g_, sum, const row) else
                            begin
                            match isSubsumed row
                            with 
        ...
        and restrictions pos =
          let rec member (x, l) = List.exists (function 
                                                        | y -> x = y) l
            in let rec test l = (restricted l) && (not (dead l))
                 in let rec reachable =
                      function 
                               | (((Row row as pos) :: candidates), tried,
                                  closure) -> begin
                                   if member (pos, tried) then
                                   ...
                                   let new_candidates =
                                     ...
                                     []
                                     ((fun r -> r.clabels) tableau, 0,
                                      nCols ())
                                     in let closure' = begin
                                          if test ((Label_ pos)) then
                                          (pos :: closure) else closure end
                                          in reachable
                                             (new_candidates @ candidates,
                                              (pos :: tried), closure')
                                   end
                               | (((Col col as pos) :: candidates), tried,
                                  closure) -> begin
                                   if member (pos, tried) then
                                   reachable (candidates, tried, closure)
                                   else
                               ...
                               | ([], _, closure) -> closure
                      in let rec restrExp pos =
                           let l = (Label_ pos)
                             in let owner = (fun r -> r.owner) l
                                  in let g_ = ownerContext owner
                                       in let u_ = toExp (ownerSum owner)
                                            in begin
                                               match restriction
                                                     ((Label_ pos))
                                               with 
                           ...
                           in List.map restrExp (reachable ([pos], [], []))
        ...
        let rec awake tag () =
          try begin
              ...
              with 
                   | Some pos
                       -> let owner = (fun r -> r.owner) ((Label_ pos))
                            in let g_ = ownerContext owner
                                 in let sum = normalize (ownerSum owner)
                                      in begin
                                           update (g_, pos, sum);true
                                           end
                   | None -> true
              end
          with 
          ...
          function 
                   | Insert (Row row)
                       -> begin
                            ...
                            begin
                              clearArray2Row
                              ((fun r -> r.coeffs) tableau, row,
                               (0, nCols ()));
                              begin
                                Array.update
                                ((fun r -> r.consts) tableau, row, zero);
                                decrNRows ()
                                end
                   ...
                   | Kill pos -> ((fun r -> r.dead) ((Label_ pos))) := false
                   | Restrict pos
                       -> ((fun r -> r.restr) ((Label_ pos))) := None
        ...
        let rec isConstantExp u_ =
          begin
          match fromExp (u_, id) with 
                                      | Sum (m, []) -> (Some m)
                                      | _ -> None
          end;;
        let rec isZeroExp u_ =
          begin
          match isConstantExp u_ with 
                                      | Some d -> d = zero
                                      | None -> false
          end;;
        let rec solveGt =
          function 
                   | (g_, s_, 0)
                       -> let rec solveGt0 w_ =
                            begin
                            match isConstantExp w_
                            with 
                                 | Some d -> begin
                                     if d > zero then gtNExp d else
                                     raise Error end
                                 | None
                                     -> let proof = newEVar (g_, gt0 w_)
                                          in let _ =
                                               restrict
                                               (insert (g_, (w_, id)),
                                                (Restr
                            ...
                            in let u1_ = (EClo (fst (s_, id)))
                                 in let u2_ = (EClo (snd (s_, id)))
                                      in try begin
                                         if isZeroExp u2_ then
                                         (Some (solveGt0 u1_)) else
                                         let w_ = minus (u1_, u2_)
                                           in let proof = solveGt0 w_
                                                in (Some
                                                    (gtAdd
                                                     (w_, (Constant_ zero),
                                                      u2_, proof)))
          ...
          function 
                   | (g_, s_, 0)
                       -> let rec solveGeq0 w_ =
                            ...
                            with 
                                 | Some d -> begin
                                     if d >= zero then geqN0 d else
                                     raise Error end
                                 | None
                                     -> let proof = newEVar (g_, geq0 w_)
                                          in let _ =
                                               restrict
                                               (insert (g_, (w_, id)),
                                                (Restr (g_, proof, false)))
                                               in proof
                            end
                            in let u1_ = (EClo (fst (s_, id)))
                                 in let u2_ = (EClo (snd (s_, id)))
                                      in try begin
                                         if isZeroExp u2_ then
                                         (Some (solveGeq0 u1_)) else
                                         let w_ = minus (u1_, u2_)
                                           in let proof = solveGeq0 w_
                                                in (Some
                                                    (geqAdd
                                                     (w_, (Constant_ zero),
                                                      u2_, proof)))
                                                  ...
                                                  | Error -> None
                   | (g_, s_, n) -> None;;
        let rec pi (name, u_, v_) =
          (Pi (((Dec ((Some name), u_)), Maybe), v_));;
                ...
                (installF
                 ((ConDec
                   (">", None, 0, (Constraint (! myID, solveGt)),
                    ...
                    Kind)),
                  (Some ((FX.Infix (FX.minPrec, FX.none_)))),
                  [(MS.Mapp
