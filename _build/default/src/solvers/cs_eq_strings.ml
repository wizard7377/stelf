    ...
    open!
      struct
        ...
        let rec fromString string =
          let len = String.size string in begin
            if
            ((String.sub (string, 0)) = '"') &&
              ((String.sub (string, len - 1)) = '"')
            then (Some (String.substring (string, 1, len - 2))) else None end;;
        ...
        let rec toExp =
          function 
                   ...
                   | Concat ((a_ :: al_))
                       -> concatExp
                          (toExp ((Concat [a_])), toExp ((Concat al_)));;
        let rec catConcat =
          function 
                   ...
                   | (Concat al1_, Concat al2_)
                       -> begin
                          match (List.rev al1_, al2_)
                          with 
                               | ((String str1 :: revAL1'),
                                  (String str2 :: al2'_))
                                   -> (Concat
                                       ((List.rev revAL1') @
                                          (((String (str1 ^ str2)) :: al2'_))))
        ...
        let rec fromExpW =
          function 
                   ...
                   | ((Root (FgnConst (cs, conDec), _), _) as us_) -> begin
                       ...
                       with 
                            | Some str -> begin
                                if str = "" then (Concat []) else
                                (Concat [(String str)]) end
                       end else (Concat [(Exp us_)]) end
                   | us_ -> (Concat [(Exp us_)])
        ...
        and normalize =
          function 
                   ...
                   | Concat ((a_ :: al_))
                       -> catConcat
                          (normalize ((Concat [a_])),
                           normalize ((Concat al_)));;
        let rec mapConcat (f, Concat al_) =
          let rec mapConcat' =
            function 
                     | [] -> []
                     | (Exp us_ :: al_)
                         -> ((Exp (f ((EClo us_)), id)) :: mapConcat' al_)
                     | (String str :: al_)
                         -> ((String str) :: mapConcat' al_)
            in (Concat (mapConcat' al_));;
        let rec appConcat (f, Concat al_) =
          let rec appAtom =
            function 
                     | Exp us_ -> f ((EClo us_))
                     | String _ -> ()
            in List.app appAtom al_;;
        ...
        let rec index (str1, str2) =
          let max = (String.size str2) - (String.size str1)
            in let rec index' i = begin
                 if i <= max then begin
                 if String.isPrefix str1 (String.extract (str2, i, None))
                 then (i :: index' (i + 1)) else index' (i + 1) end else []
                 end in index' 0;;
        let rec split (str1, str2) =
          let len = String.size str1
            in let rec split' i =
                 ...
                 in List.map split' (index (str1, str2));;
        ...
        and unifyRigid (g_, Concat al1_, Concat al2_) =
          let rec unifyRigid' =
            function 
                     ...
                     | ((String str1 :: al1_), (String str2 :: al2_))
                         -> begin
                         if str1 = str2 then unifyRigid' (al1_, al2_) else
                         Failure end
                     | ((Exp ((EVar (r, _, _, _) as u1_), s) :: al1_),
                        (Exp ((Root (FVar _, _) as u2_), _) :: al2_))
                         -> let ss = Whnf.invert s in begin
                              if Unify.invertible (g_, (u2_, id), ss, r) then
                              begin
                              match unifyRigid' (al1_, al2_)
                              with 
                                   | MultAssign l
                                       -> (MultAssign
                                           (((g_, u1_, u2_, ss) :: l)))
                                   | Failure -> Failure
                              end else Failure end
                     | ((Exp ((Root (FVar _, _) as u1_), _) :: al1_),
                        (Exp ((EVar (r, _, _, _) as u2_), s) :: al2_))
                         -> let ss = Whnf.invert s in begin
                              if Unify.invertible (g_, (u1_, id), ss, r) then
                              begin
                              match unifyRigid' (al1_, al2_)
                              with 
                                   | MultAssign l
                     ...
                     | ((Exp (((Root (FVar _, _), _) as us1_)) :: al1_),
                        (Exp (((Root (FVar _, _), _) as us2_)) :: al2_))
                         -> begin
                         if sameExpW (us1_, us2_) then
                         unifyRigid' (al1_, al2_) else Failure end
        ...
        let rec unifyString =
          function 
                   | (g_, Concat ((String prefix :: al_)), str, cnstr)
                       ...
                       let suffix =
                         String.extract (str, String.size prefix, None)
                         in unifyString (g_, (Concat al_), suffix, cnstr)
                       else Failure end
                   | (g_, Concat al_, str, cnstr)
                       -> let rec unifyString' =
                            function 
                                     ...
                                     | ([], candidates)
                                         -> ((MultDelay ([], cnstr)), [])
                                     | ((Exp us1_ :: (Exp us2_ :: al_)), _)
                                         -> ((MultDelay
                                              ([(EClo us1_); (EClo us2_)],
                                               cnstr)),
                                             [])
                                     | ((Exp ((EVar (r, _, _, _) as u_), s)
                                         :: al_),
                                        candidates) -> begin
                                         ...
                                         -> let rec successors
                                              ...
                                              in let candidates' =
                                                   ...
                                                   in unifyString'
                                                      (al_, candidates')
                            in begin
                               match unifyString' (al_, [(Decomp (str, []))])
                               with 
                                    | (result, []) -> result
                                    | (result, ("" :: [])) -> result
        ...
        let rec unifyConcat
          (g_, (Concat al1_ as concat1), (Concat al2_ as concat2)) =
          let u1_ = toFgn concat1
            in let u2_ = toFgn concat2
                 in let cnstr = ref ((Eqn (g_, u1_, u2_)))
                      in begin
                         match (al1_, al2_)
                         with 
                              | ([], []) -> (MultAssign [])
                              ...
                              | ((String str1 :: []), (String str2 :: []))
                                  -> begin
                                  if str1 = str2 then (MultAssign []) else
                                  Failure end
                              | ((Exp ((EVar (r, _, _, _) as u_), s) :: []),
                                 _) -> begin
                                  if Whnf.isPatSub s then
                                  let ss = Whnf.invert s in begin
                                    if
                                    Unify.invertible (g_, (u2_, id), ss, r)
                                    then (MultAssign [(g_, u_, u2_, ss)])
                                    else (MultDelay ([u1_; u2_], cnstr)) end
                                  else (MultDelay ([u1_; u2_], cnstr)) end
                              | (_,
                                 (Exp ((EVar (r, _, _, _) as u_), s) :: []))
                                  -> begin
                                  ...
                                  let ss = Whnf.invert s in begin
                                    if
                                    Unify.invertible (g_, (u1_, id), ss, r)
                                    then (MultAssign [(g_, u_, u1_, ss)])
                                    else (MultDelay ([u1_; u2_], cnstr)) end
                                  else (MultDelay ([u1_; u2_], cnstr)) end
                              | ((String str :: []), _)
                                  -> unifyString (g_, concat2, str, cnstr)
                              | (_, (String str :: []))
                                  -> unifyString (g_, concat1, str, cnstr)
                              | _
                                  -> begin
                                     match unifyRigid (g_, concat1, concat2)
                                     with 
                                          | (MultAssign _ as result)
                                              -> result
                                          | Failure -> begin
                                              if
                                              sameConcat (concat1, concat2)
                                              then (MultAssign []) else
                                              (MultDelay ([u1_; u2_], cnstr))
                                              end
                                     end
                         end
        and toFgn =
        ...
        let rec unifyWith arg__11 arg__12 =
          ...
          with 
               | (MyIntsynRep concat, (g_, u2_))
                   -> toFgnUnify
                      (unifyConcat (g_, normalize concat, fromExp (u2_, id)))
               | (fe, _) -> raise ((UnexpectedFgnExp fe))
          end;;
        ...
        let rec makeFgn (arity, opExp) s_ =
          let rec makeParams =
            function 
                     ...
                     | n
                         -> (App
                             ((Root ((BVar n), Nil)), makeParams (n - 1)))
            in let rec makeLam arg__13 arg__14 =
                 begin
                 ...
                 with 
                      ...
                      | (e_, n)
                          -> (Lam
                              ((Dec (None, string ())), makeLam e_ (n - 1)))
                 end
                 in let rec expand =
                      function 
                               | ((Nil, s), arity)
                  ...
                  (installF
                   ((ConDec
                     ("++", None, 0,
                      ...
                      Type)),
                    (Some ((FX.Infix (FX.maxPrec, FX.right_)))), []));
                begin
                  installFgnExpOps ();()
                  end
...
(* functor CSEqStrings *);;
# 1 "src/solvers/cs_eq_strings.sml.ml"
