...
open! Basis;;
module CSIntWord(CSIntWord__0: sig
  ...
  struct
    ...
    open!
      struct
        ...
        let max =
          (W.( >> ) (W.notb zero, Word.fromInt (W.wordSize - wordSize')));;
        let rec numCheck d = (W.( <= ) (d, max));;
        let rec plusCheck (d1, d2) =
          let d3 = (W.( + ) (d1, d2))
            in ((W.( >= ) (d3, d1))) &&
                 (((W.( >= ) (d3, d2))) && ((W.( <= ) (d3, max))));;
        let rec timesCheck (d1, d2) = begin
          if (d1 = zero) || (d2 = zero) then true else
          let d3 = W.div (W.div (max, d1), d2) in (W.( > ) (d3, zero)) end;;
        ...
        let rec plusExp (u_, v_, w_) =
          (Root
           ((Const (! plusID)), (App (u_, (App (v_, (App (w_, Nil))))))));;
        let rec timesExp (u_, v_, w_) =
          (Root
           ((Const (! timesID)), (App (u_, (App (v_, (App (w_, Nil))))))));;
        let rec quotExp (u_, v_, w_) =
          (Root
           ((Const (! quotID)), (App (u_, (App (v_, (App (w_, Nil))))))));;
        ...
        let rec numberConDec d =
          (ConDec (W.fmt StringCvt.dec_ d, None, 0, Normal, word (), Type));;
        let rec numberExp d =
        ...
        let rec scanNumber str =
          let rec check =
            ...
            begin
            match StringCvt.scanString (W.scan StringCvt.dec_) str
            with 
        ...
        let rec plusPfConDec (d1, d2) =
          let d3 = (W.( + ) (d1, d2))
            in (ConDec
                (((W.fmt StringCvt.dec_ d1) ^ "+") ^
                   (W.fmt StringCvt.dec_ d2),
                 None, 0, Normal,
        ...
        let rec parseBinopPf oper string =
          begin
          match (oper, scanBinopPf oper string)
          with 
        ...
        let rec parseAll string =
          begin
          match parseNumber string
          with 
               | Some conDec -> (Some conDec)
               | None
                   -> begin
                      match parsePlusPf string
                      with 
                           | Some conDec -> (Some conDec)
                           | None
                               -> begin
                                  match parseTimesPf string
                                  with 
                                       | Some conDec -> (Some conDec)
        ...
        let rec fromExpW =
          function 
                   ...
                   | ((Root (Def d, _), _) as us_)
                       -> fromExpW (Whnf.expandDef us_)
                   | us_ -> (Expr us_)
        ...
        and awakePlus (g_, proof, u1_, u2_, u3_) () =
          begin
          match solvePlus (g_, (App (u1_, (App (u2_, (App (u3_, Nil)))))), 0)
          with 
               | Some proof'
                   -> Unify.unifiable (g_, (proof, id), (proof', id))
               | None -> false
          end
        ...
        and solvePlus =
          function 
                   | (g_, s_, 0)
                       -> let us1_ = fst (s_, id)
                            in let us2_ = snd (s_, id)
                                 in let us3_ = trd (s_, id)
                                      in begin
                                         match (fromExp us1_, fromExp us2_,
                                                fromExp us3_)
                                         with 
        ...
        and awakeTimes (g_, proof, u1_, u2_, u3_) () =
          begin
          match solveTimes
                (g_, (App (u1_, (App (u2_, (App (u3_, Nil)))))), 0)
          with 
               | Some proof'
                   -> Unify.unifiable (g_, (proof, id), (proof', id))
               | None -> false
        ...
        and makeCnstrTimes (g_, proof, u1_, u2_, u3_) =
          (FgnCnstr
           (! myID, (MyFgnCnstrRepTimes (g_, proof, u1_, u2_, u3_))))
        and solveTimes =
          function 
                   | (g_, s_, 0)
                       -> let us1_ = fst (s_, id)
        ...
        and awakeQuot (g_, proof, u1_, u2_, u3_) () =
          begin
          match solveQuot (g_, (App (u1_, (App (u2_, (App (u3_, Nil)))))), 0)
          with 
               | Some proof'
                   -> Unify.unifiable (g_, (proof, id), (proof', id))
               | None -> false
                      ...
                      ((MS.Marg (MS.Plus, (Some "X"))),
                       (MS.Mapp
                        ((MS.Marg (MS.Plus, (Some "Y"))),
                         (MS.Mapp
                          ((MS.Marg (MS.Minus, (Some "Z"))), MS.Mnil))))));
                     (MS.Mapp
                      ((MS.Marg (MS.Plus, (Some "X"))),
                       (MS.Mapp
                       ...
                       (MS.Mapp
                        ((MS.Marg (MS.Plus, (Some "X"))),
                         (MS.Mapp
                          ((MS.Marg (MS.Minus, (Some "Y"))),
                           (MS.Mapp
                            ((MS.Marg (MS.Plus, (Some "Z"))), MS.Mnil))))));
                       (MS.Mapp
                        ((MS.Marg (MS.Minus, (Some "X"))),
                         (MS.Mapp
                          ((MS.Marg (MS.Plus, (Some "Y"))),
                               ...
                               ("Y", word (),
                                (Pi_
                                 ("Z", word (),
                                  (Pi_
                                   ("P",
                                    plusExp ((Bvar_ 3), (Bvar_ 2), (Bvar_ 1)),
                                    (Uni Type))))))))),
                            Kind)),
                          None,
                          [(MS.Mapp
                                  ...
                                  (Pi_
                                   ("Z", word (),
                                    (Pi_
                                     ("P",
                                      plusExp
                                      ((Bvar_ 3), (Bvar_ 2), (Bvar_ 1)),
                                      provePlusExp
                                      ((Bvar_ 4), (Bvar_ 3), (Bvar_ 2),
                                       (Bvar_ 1)))))))))),
                              Type)),
                            None, []));
                        begin
