# 1 "src/solvers/cs_manager.sig.ml"
open! Basis;;
...
# 1 "src/solvers/cs_manager.fun.ml"
open! Basis;;
(* Constraint Solver Manager *);;
(* Author: Roberto Virga *);;
module CSManager(CSManager__0: sig
  ...
  struct
    ...
    type nonrec fgnConDec = __0;;
    (* foreign constant declaration *);;
    type nonrec __1 = { name: string ; keywords: string ;
      needs: string list ; fgnConst: fgnConDec option ;
    ...
    open!
      struct
        ...
        let installFN =
          (ref (function 
                         | _ -> (-1)) : (sigEntry -> IntSyn.cid) ref);;
        let rec setInstallFN f = installFN := f;;
        let rec installSolver solver =
          let cs = ! nextCS
            in let _ = begin
                 if (! nextCS) > maxCS then
                 raise ((Error "too many constraint solvers")) else () end
                 in let _ =
                      Array.update
                      (csArray, cs, (Solver (solver, ref false)))
                      in let _ = nextCS := ((! nextCS) + 1) in cs;;
          ...
          begin
            ...
            (function 
                      | (cs, Solver (solver, active)) -> begin
                          if ! active then
                          begin
                            active := false;(fun r -> r.reset) solver ()
                            end
                          else () end)
            (ArraySlice.slice (csArray, 0, (Some (! nextCS))));
            begin
              activeKeywords := [];useSolver "Unify"
              end
            
            end
        and useSolver name =
          (let exception Found of IntSyn.csid 
           in let rec findSolver name =
                try begin
                      ArraySlice.appi
                      (function 
                                | (cs, Solver (solver, _)) -> begin
                                    if ((fun r -> r.name) solver) = name then
                                    raise ((Found cs)) else () end)
                      (ArraySlice.slice (csArray, 0, (Some (! nextCS))));None
                      end
                with 
                     | Found cs -> (Some cs)
                in begin
                   match findSolver name
                   with 
                        | Some cs
                            -> let Solver (solver, active) =
                                 Array.sub (csArray, cs) in begin
                                 if ! active then () else begin
                                 if
                                 List.exists
                                 (function 
                                           | s
                                 ...
                                 begin
                                   ...
                                   begin
                                     activeKeywords :=
                                       (((fun r -> r.keywords) solver
                                         :: ! activeKeywords));
                                     begin
                                       List.app
                                       useSolver
                                       ((fun r -> r.needs) solver);
                                       (fun r -> r.init)
                                       solver
                                       (cs, ! installFN)
                                       end
                        ...
                        | None
                            -> raise
                               ((Error (("solver " ^ name) ^ " not found")))
                   end);;
        let rec parse string =
          (let exception Parsed of IntSyn.csid * IntSyn.conDec_ 
           in let rec parse' (cs, (solver : solver)) =
                begin
                match (fun r -> r.fgnConst) solver
                with 
                     | None -> ()
                     | Some fgnConDec
                         -> begin
                            match (fun r -> r.parse) fgnConDec string
                            with 
                                 | None -> ()
                                 | Some conDec
                                     -> raise ((Parsed (cs, conDec)))
                            end
                end
                in try begin
                         ArraySlice.appi
                         (function 
                                   | (cs, Solver (solver, active)) -> begin
                                       if ! active then parse' (cs, solver)
                                       else () end)
                         (ArraySlice.slice (csArray, 0, (Some (! nextCS))));
                         None
                         end
                   with 
                        | Parsed info -> (Some info));;
        ...
        let rec reset () =
          ...
          (function 
                    | (_, Solver (solver, active)) -> begin
                        if ! active then
                        begin
                          markCount := 0;(fun r -> r.reset) solver ()
                          end
                        else () end)
          (ArraySlice.slice (csArray, 0, (Some (! nextCS))));;
        let rec mark () =
                         ...
                         -> begin
                              ...
                              (function 
                                        | (_, Solver (solver, active))
                                            -> begin
                                            if ! active then
                                            (fun r -> r.unwind) solver ()
                                            else () end)
                              (ArraySlice.slice
                               (csArray, 0, (Some (! nextCS))));
                              unwind' (k - 1)
                              end
            in unwind' ((! markCount) - targetCount);;
        let rec trail f =
          let current = ! markCount
            in let _ = mark () in let r = f () in let _ = unwind current in r;;
        end;;
    (* vacuous solver *);;
...
(*! structure ModeSyn : MODESYN !*);;
(* functor CSManager *);;
module CSManager = (CSManager)(struct
                                 module Global = Global;;
                                 (*! structure IntSyn = IntSyn !*);;
...
(*! structure ModeSyn = ModeSyn !*);;
# 1 "src/solvers/cs_manager.sml.ml"
