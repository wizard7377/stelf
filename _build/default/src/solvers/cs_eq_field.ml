# 1 "src/solvers/cs_eq_field.sig.ml"
open! Basis;;
...
module type CS_EQ_FIELD = sig
                          ...
                          and mon_ =
                            | Mon of
                              Field.number * (IntSyn.exp_ * IntSyn.sub_) mset 
                          (* Sum :                      *)
                          (* Sum ::= m + M1 + ...       *)
                          (* Monomials:                 *)
                          (* Mon ::= n * U1[s1] * ...   *)
                          ...
                          val
                            plus : (IntSyn.exp_ * IntSyn.exp_) -> IntSyn.exp_
                          val
                            minus : (IntSyn.exp_ * IntSyn.exp_) ->
                                    IntSyn.exp_
                          val
                            times : (IntSyn.exp_ * IntSyn.exp_) ->
                                    IntSyn.exp_
                          val constant : Field.number -> IntSyn.exp_
                          end;;
(* signature CS_EQ_FIELD *);;
# 1 "src/solvers/cs_eq_field.fun.ml"
open! Basis;;
      ...
      struct
        ...
        let rec unifySum (g_, sum1, sum2) =
          let rec invertMon =
            function 
                     | (g_, Mon
                        (n, (((EVar (r, _, _, _) as lhs_), s) :: [])), sum)
                         -> begin
                         if Whnf.isPatSub s then
                         let ss = Whnf.invert s
                           in let rhs_ =
                                toFgn
                                (timesSum ((Sum (- (inverse n), [])), sum))
                                in begin
...
(* local *);;
(* functor CSEqField *);;
# 1 "src/solvers/cs_eq_field.sml.ml"
