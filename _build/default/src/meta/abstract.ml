...
module type MTPABSTRACT = sig
                          ...
                          type approxFor_ =
                            | Head of
                              IntSyn.dctx * (FunSyn.for_ * IntSyn.sub_) * int 
                            | Block of
                              (IntSyn.dctx *
                               IntSyn.sub_ *
                               int *
                          ...
                          (*  | (t, G2), AF *)
                          val
                            weaken : (IntSyn.dctx * IntSyn.cid) ->
                                     IntSyn.sub_
                          val
                            raiseType : (IntSyn.dctx * IntSyn.exp_) ->
                                        IntSyn.exp_
                          val
                            abstractSub : (IntSyn.sub_ *
                                           StateSyn.tag_
                                           IntSyn.ctx_ *
                                           (IntSyn.dctx *
                                            StateSyn.tag_
...
module MTPAbstract(MTPAbstract__0: sig
  ...
  struct
    ...
    type approxFor_ =
      ...
      | Block of
        (IntSyn.dctx * IntSyn.sub_ * int * IntSyn.dec_ list) * approxFor_ ;;
    (* Approximat formula *);;
    (* AF ::= F [s] *);;
    (*      | (t, G2), AF *);;
    open!
      struct
        ...
        let rec restore =
          function 
                   | (0, gp_) -> (gp_, I.null_)
                   | (n, I.Decl (g_, d_))
                       -> let (gp'_, gx'_) = restore (n - 1, g_)
                            in (gp'_, (I.Decl (gx'_, d_)));;
        let rec concat =
          function 
                   | (gp_, null_) -> gp_
                   | (gp_, I.Decl (g_, d_))
                       -> (I.Decl (concat (gp_, g_), d_));;
        let rec collectExpW =
          function 
                   | (T, d, g_, (I.Uni l_, s), k_) -> k_
                   | (T, d, g_, (I.Pi ((d_, _), v_), s), k_)
                       -> collectExp
                          (T, d, (I.Decl (g_, I.decSub (d_, s))),
                           (v_, I.dot1 s),
                           collectDec (T, d, g_, (d_, s), k_))
                   | (T, d, g_, (I.Root (_, s_), s), k_)
                       -> collectSpine (S.decrease T, d, g_, (s_, s), k_)
                   | (T, d, g_, (I.Lam (d_, u_), s), k_)
                       -> collectExp
                          (T, d, (I.Decl (g_, I.decSub (d_, s))),
                           (u_, I.dot1 s),
                           collectDec (T, d, g_, (d_, s), k_))
                   | (T, d, g_, ((I.EVar (r, gdX_, v_, cnstrs) as x_), s),
                      k_) -> begin
                       if exists (eqEVar x_) k_ then
                       collectSub (T, d, g_, s, k_) else
                       let (gp_, gx_) =
                         restore ((I.ctxLength gdX_) - d, gdX_)
                         in let _ = checkEmpty (! cnstrs)
                              in let w = weaken (gx_, I.targetFam v_)
                                   in let iw = Whnf.invert w
                                        in let gx'_ =
                                             Whnf.strengthen (iw, gx_)
                                             in let
                                                  (I.EVar (r', _, _, _) as
                                                    x'_)
                                                  =
                          ...
                          (function 
                                    | (u_, k'_)
                                        -> collectExp
                                           (T, d, g_, (u_, s), k'_))
                          k_
        and collectExp (T, d, g_, us_, k_) =
          collectExpW (T, d, g_, Whnf.whnf us_, k_)
        and collectSpine =
          function 
                   | (T, d, g_, (nil_, _), k_) -> k_
                   | (T, d, g_, (I.SClo (s_, s'), s), k_)
                       -> collectSpine (T, d, g_, (s_, I.comp (s', s)), k_)
                   | (T, d, g_, (I.App (u_, s_), s), k_)
                       -> collectSpine
                          (T, d, g_, (s_, s),
                           collectExp (T, d, g_, (u_, s), k_))
        and collectDec (T, d, g_, (I.Dec (_, v_), s), k_) =
          collectExp (T, d, g_, (v_, s), k_)
        and collectSub =
          function 
                   | (T, d, g_, I.Shift _, k_) -> k_
                   | (T, d, g_, I.Dot (I.Idx _, s), k_)
                       -> collectSub (T, d, g_, s, k_)
                   | (T, d, g_, I.Dot (I.Exp u_, s), k_)
                       -> collectSub
                          (T, d, g_, s,
                           collectExp (T, d, g_, (u_, I.id), k_));;
        let rec abstractEVar =
          function 
                   | (I.Decl (k'_, Ev (r', _, _, d)), depth,
                      (I.EVar (r, _, _, _) as x_)) -> begin
                       if r = r' then ((I.BVar (depth + 1)), d) else
                       abstractEVar (k'_, depth + 1, x_) end
                   | (I.Decl (k'_, Bv _), depth, x_)
                       -> abstractEVar (k'_, depth + 1, x_);;
        let rec lookupBV (k_, i) =
          let rec lookupBV' =
            function 
                     | (I.Decl (k_, Ev (r, v_, _, _)), i, k)
                         -> lookupBV' (k_, i, k + 1)
        ...
        let rec abstractExpW =
          function 
                   ...
                   | (k_, depth, ((I.EVar (_, g_, _, _) as x_), s))
                       -> let (h_, d) = abstractEVar (k_, depth, x_)
                            in (I.Root
                                (h_,
                                 abstractSub
                                 ((I.ctxLength g_) - d, k_, depth, s, I.nil_)))
        ...
        let rec checkType v_ =
          ...
          with 
               ...
               | _
                   -> raise
                      ((Error "Typing ambiguous -- free type variable"))
          end;;
        let rec abstractCtx =
          function 
                   | null_ -> (I.null_, I.null_)
                   | I.Decl (k'_, Ev (_, v'_, (S.Lemma b as t_), _))
                       -> let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                            in let _ = checkType v''_
                                 in let (g'_, b'_) = abstractCtx k'_
                                      in let d'_ = (I.Dec (None, v''_))
                                           in ((I.Decl (g'_, d'_)),
                                               (I.Decl (b'_, T)))
                   | I.Decl (k'_, Ev (_, v'_, (none_ as t_), _))
                       -> let v''_ = abstractExp (k'_, 0, (v'_, I.id))
                            in let _ = checkType v''_
                                 in let (g'_, b'_) = abstractCtx k'_
                                      in let d'_ = (I.Dec (None, v''_))
                                           in ((I.Decl (g'_, d'_)),
                                               (I.Decl (b'_, S.none_)))
                   | I.Decl (k'_, Bv (d_, T))
                       -> let d'_ = abstractDec (k'_, 0, (d_, I.id))
                            in let (g'_, b'_) = abstractCtx k'_
                                 in ((I.Decl (g'_, d'_)), (I.Decl (b'_, T)));;
        let rec abstractGlobalSub =
          function 
                   | (k_, I.Shift _, null_) -> (I.Shift (I.ctxLength k_))
                   | (k_, I.Shift n, (I.Decl _ as b_))
                       -> abstractGlobalSub
                          (k_, (I.Dot ((I.Idx (n + 1)), (I.Shift (n + 1)))),
                           b_)
                   | (k_, I.Dot (I.Idx k, s'), I.Decl
                      (b_, (S.Parameter _ as t_)))
                       -> (I.Dot
                           ((I.Idx (lookupBV (k_, k))),
                            abstractGlobalSub (k_, s', b_)))
                   | (k_, I.Dot (I.Exp u_, s'), I.Decl
                      (b_, (S.Lemma _ as t_)))
                       -> (I.Dot
        ...
        let rec collectGlobalSub =
          function 
                   ...
                   | (g0_, s, (I.Decl (_, S.Parameter (Some l)) as b_),
                      collect)
                       -> let F.LabelDec (name, _, g2_) = F.labelLookup l
                            in skip (g0_, List.length g2_, s, b_, collect)
                   | (g0_, I.Dot (I.Exp u_, s), I.Decl (b_, T), collect)
                       -> collectGlobalSub
                          (g0_, s, b_,
                           function 
                                    | (d, k_)
        ...
        and skip =
          function 
                   | (g0_, 0, s, b_, collect)
                       -> collectGlobalSub (g0_, s, b_, collect)
                   | (I.Decl (g0_, d_), n, s, I.Decl
                      (b_, (S.Parameter _ as t_)), collect)
                       -> skip
        ...
        let rec abstractNew ((g0_, b0_), s, b_) =
          let cf = collectGlobalSub (g0_, s, b_, function 
                                                          | (_, k'_) -> k'_)
            in let k_ = cf (0, I.null_)
                 in (abstractCtx k_, abstractGlobalSub (k_, s, b_));;
        let rec abstractSubAll (t, b1_, (g0_, b0_), s, b_) =
          let rec skip'' =
            function 
                     | (k_, (null_, null_)) -> k_
                     | (k_, (I.Decl (g0_, d_), I.Decl (b0_, T)))
                         -> (I.Decl (skip'' (k_, (g0_, b0_)), (Bv (d_, T))))
            in let collect2 =
                 collectGlobalSub (g0_, s, b_, function 
                                                        | (_, k'_) -> k'_)
                 in let collect0 =
                      collectGlobalSub
                      (I.null_, t, b1_, function 
                                                 | (_, k'_) -> k'_)
                      in let k0_ = collect0 (0, I.null_)
                           in let k1_ = skip'' (k0_, (g0_, b0_))
                                in let d = I.ctxLength g0_
                                     in let k_ = collect2 (d, k1_)
                                          in (abstractCtx k_,
                                              abstractGlobalSub (k_, s, b_));;
        let rec abstractFor =
          function 
        ...
        let rec allClo =
          function 
                   | (null_, f_) -> f_
                   | (I.Decl (gx_, d_), f_)
                       -> allClo (gx_, (F.All ((F.Prim d_), f_)));;
        let rec convert =
          function 
                   | null_ -> I.null_
                   | I.Decl (g_, d_)
                       -> (I.Decl
                           (convert g_, (Bv (d_, (S.Parameter None)))));;
        let rec createEmptyB =
          function 
                   | 0 -> I.null_
                   | n -> (I.Decl (createEmptyB (n - 1), S.none_));;
        let rec lower =
          function 
                   | (_, 0) -> I.null_
        ...
        let rec split =
          function 
                   | (g_, 0) -> (g_, I.null_)
                   | (I.Decl (g_, d_), n)
                       -> let (g1_, g2_) = split (g_, n - 1)
                            in (g1_, (I.Decl (g2_, d_)));;
        let rec shift =
          function 
                   | null_ -> I.shift
        ...
        let rec ctxSub =
          function 
                   | ([], s) -> []
                   | ((d_ :: g_), s)
                       -> (I.decSub (d_, s) :: ctxSub (g_, I.dot1 s));;
        let rec weaken2 =
          function 
                   | (null_, a, i) -> (I.id, function 
                                                      | s_ -> s_)
                   | (I.Decl (g'_, (I.Dec (name, v_) as d_)), a, i)
                       -> let (w', s'_) = weaken2 (g'_, a, i + 1) in begin
                            if Subordinate.belowEq (I.targetFam v_, a) then
                            (I.dot1 w',
                             function 
                                      | s_
                                          -> (I.App
                                              ((I.Root ((I.BVar i), I.nil_)),
                                               s_)))
                            else (I.comp (w', I.shift), s'_) end;;
        let rec raiseType =
          function 
                   | (null_, v_) -> v_
                   | (I.Decl (g_, d_), v_)
                       -> raiseType
                          (g_,
                           Abstract.piDepend
                           ((Whnf.normalizeDec (d_, I.id), I.maybe_), v_));;
        let rec raiseFor =
          function 
                   | (k, gorig_, (true_ as f_), w, sc) -> f_
                   | (k, gorig_, F.Ex (I.Dec (name, v_), f_), w, sc)
                       -> let g_ =
                            F.listToCtx (ctxSub (F.ctxToList gorig_, w))
                            in let g = I.ctxLength g_
                                 in let s = sc (w, k)
                                      in let v'_ = (I.EClo (v_, s))
                                           in let (nw, s_) =
                                                weaken2
                                                (g_, I.targetFam v_, 1)
                                                in let iw = Whnf.invert nw
                                                     in let gw_ =
                                                          Whnf.strengthen
                                                          (iw, g_)
                                                          in let v''_ =
                                                               ...
                                                               in let v'''_ =
                                                                    ...
                                                                    v'''_)),
                                                                    f'_))
                   | (k, gorig_, F.All (F.Prim (I.Dec (name, v_)), f_), w,
                      sc)
                       -> let g_ =
                            F.listToCtx (ctxSub (F.ctxToList gorig_, w))
                            in let g = I.ctxLength g_
        ...
        let rec extend =
          function 
                   | (k_, []) -> k_
                   | (k_, (d_ :: l_))
                       -> extend ((I.Decl (k_, (Bv (d_, S.none_)))), l_);;
        let rec makeFor =
          function 
                   | (k_, w, Head (g_, (f_, s), d))
                       -> let cf =
                            ...
                            (g_, s, createEmptyB d,
                             function 
                                      | (_, k'_) -> k'_)
                            in let k = I.ctxLength k_
                                 in let k'_ = cf (I.ctxLength g_, k_)
                                      in let k' = I.ctxLength k'_
                                           in let (gk_, bk_) =
                                                abstractCtx k'_
                                                in let _ = begin
                   ...
                   | (k_, w, Block ((g_, t, d, g2_), af_))
                       -> let k = I.ctxLength k_
                            in let collect =
                                 collectGlobalSub
                                 (g_, t, createEmptyB d,
                                  function 
                                           | (_, k'_) -> k'_)
                                 in let k'_ = collect (I.ctxLength g_, k_)
                                      in let k' = I.ctxLength k'_
                                           in let k''_ = extend (k'_, g2_)
                                                in let w' =
                                                     F.dot1n
                                                     (F.listToCtx g2_,
                                                      I.comp
        ...
        let rec abstractApproxFor =
          function 
                   | (Head (g_, _, _) as af_)
                       -> let (_, f_) = makeFor (convert g_, I.id, af_) in f_
