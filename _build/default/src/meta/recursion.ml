      ...
      struct
        ...
        let rec closedCtx =
          function 
                   ...
                   | I.Decl (g_, d_) -> begin
                       ...
                       raise Domain else closedCtx g_ end;;
        let rec spine =
          function 
                   | 0 -> I.nil_
                   | n
        ...
        let rec appendCtx =
          function 
                   ...
                   | ((g1_, b1_), T, (d_ :: g2_))
                       -> appendCtx
                          (((I.Decl (g1_, d_)), (I.Decl (b1_, T))), T, g2_);;
        let rec createCtx =
          function 
                   | ((g_, b_), [], s)
                       -> ((g_, b_), s, function 
                                                 | af_ -> af_)
                   | ((g_, b_), (n :: ll), s)
                       -> let F.LabelDec (l, g1_, g2_) = F.labelLookup n
                            in let t = someEVars (g_, g1_, I.id)
                                 in let g2'_ = ctxSub (g2_, t)
                                      in let (g'_, b'_) =
                                           appendCtx
                                           ((g_, b_),
                                            (S.Parameter ((Some n))), g2'_)
                                           in let s' =
                                                I.comp
                                                (s,
                                                 (I.Shift (List.length g2'_)))
                                                in let (gb''_, s'', af'') =
                                                     createCtx
                                                     ((g'_, b'_), ll, s')
                                                     in (gb''_, s'',
        ...
        let rec createEVars =
          function 
                   ...
                   | (g_, I.Decl (g0_, I.Dec (_, v_)))
                       -> let s = createEVars (g_, g0_)
                            in (I.Dot
                                ((I.Exp (I.newEVar (g_, (I.EClo (v_, s))))),
                                 s));;
        let rec checkCtx =
          function 
                   | (g_, [], (v2_, s)) -> false
                   | (g_, ((I.Dec (_, v1_) as d_) :: g2_), (v2_, s))
                       -> (CSManager.trail
                           (function 
                            ...
                            (checkCtx
                             ((I.Decl (g_, d_)), g2_,
                              (v2_, I.comp (s, I.shift))));;
        let rec checkLabels ((g'_, b'_), (v_, s), ll, l) = begin
          if l < 0 then None else
          let F.LabelDec (name, g1_, g2_) = F.labelLookup l
            in let s = someEVars (g'_, g1_, I.id)
                 in let g2'_ = ctxSub (g2_, s)
                      in let t = someEVars (g'_, g1_, I.id)
                           in let g2'_ = ctxSub (g2_, t) in begin
                                if
                                (not
                                ...
                                then (Some l) else
                                checkLabels ((g'_, b'_), (v_, s), ll, l - 1)
                                end
          end;;
        let rec appendRL =
          function 
                   ...
                   | (((Lemma (n, f_) as l_) :: ds1_), ds2_)
                       -> let ds'_ = appendRL (ds1_, ds2_) in begin
                            if
                            List.exists
                            (function 
                                      | Lemma (n', f'_)
                                          -> (n = n') &&
                                               (F.convFor
                                                ((f_, I.id), (f'_, I.id))))
                            ds'_ then ds'_ else (l_ :: ds'_) end;;
        let rec recursion
          ((nih, gall_, fex_, oex_),
           (ncurrent, (g0_, b0_), ll, ocurrent_, h_, f_))
          =
          let ((g'_, b'_), s', af) = createCtx ((g0_, b0_), ll, I.id)
            in let t' = createEVars (g'_, gall_)
                 in let af_ =
                      af ((A.Head (g'_, (fex_, t'), I.ctxLength gall_)))
                      in let oex'_ = S.orderSub (oex_, t')
                           in let ocurrent'_ = S.orderSub (ocurrent_, s')
                                in let rec sc ds_ =
                                     let fnew_ = A.abstractApproxFor af_
                                       in begin
                                       if
                                       List.exists
                                       (function 
                                                     ...
                                                     -> (nih = nhist) &&
                                                          (F.convFor
                                                           ((fnew_, I.id),
                                                            (fhist_, I.id))))
                                       h_ then ds_ else
                                       ((Lemma (nih, fnew_)) :: ds_) end
                                     in let rec ac ((g'_, b'_), vs_, ds_) =
                                          begin
                                          match checkLabels
                                                ((g'_, b'_), vs_, ll,
                                                 (F.labelSize ()) - 1)
                                          ...
                                          if ncurrent < nih then
                                          ordle
                                          ((g'_, b'_), oex'_, ocurrent'_, sc,
                                           ac, [])
                                          else
                                          ordlt
                                          ((g'_, b'_), oex'_, ocurrent'_, sc,
        ...
        and set_parameter
          ...
          let rec set_parameter' =
            function 
                     ...
                     | ((I.Decl (g_, d_), I.Decl (b_, S.Parameter _)), k,
                        ds_)
                         -> let (I.Dec (_, v'_) as d'_) =
                              I.decSub (d_, (I.Shift k))
                              in let ds'_ =
                                   CSManager.trail
                                   (function 
                                             | () -> begin
        ...
        and ltW =
          function 
                   | (gb_, k, (us_, vs_),
                      ((I.Root (I.Const c, s'_), s'), vs'_), sc, ac, ds_)
                       -> ltSpine
                          (gb_, k, (us_, vs_),
                           ((s'_, s'), (I.constType c, I.id)), sc, ac, ds_)
                   | (((g_, b_) as gb_), k, (us_, vs_),
                      ((I.Root (I.BVar n, s'_), s'), vs'_), sc, ac, ds_)
                       -> begin
                          match I.ctxLookup (b_, n)
                          with 
                               | S.Parameter _
                                   -> let I.Dec (_, v'_) = I.ctxDec (g_, n)
                                        in ltSpine
                                           (gb_, k, (us_, vs_),
                                            ((s'_, s'), (v'_, I.id)), sc, ac,
                                            ds_)
                               | S.Lemma _ -> ds_
                          end
                   | (gb_, _, _, ((I.EVar _, _), _), _, _, ds_) -> ds_
                   | (((g_, b_) as gb_), k, ((u_, s1), (v_, s2)),
                      ((I.Lam ((I.Dec (_, v1'_) as d_), u'_), s1'),
                       (I.Pi ((I.Dec (_, v2'_), _), v'_), s2')),
                      sc, ac, ds_)
                       -> let ds'_ = ds_ in begin
                            if
                            Subordinate.equiv
                            (I.targetFam v_, I.targetFam v1'_) then
                            ...
                            if
                            Subordinate.below
                            (I.targetFam v1'_, I.targetFam v_) then
                            let x_ = I.newEVar (g_, (I.EClo (v1'_, s1')))
                              in lt
                                 (gb_, k, ((u_, s1), (v_, s2)),
                                  ((u'_, (I.Dot ((I.Exp x_), s1'))),
        ...
        and ltSpineW =
          function 
                   ...
                   | (gb_, k, (us_, vs_), ((I.SClo (s_, s'), s''), vs'_), sc,
                      ac, ds_)
                       -> ltSpineW
                          (gb_, k, (us_, vs_),
                           ((s_, I.comp (s', s'')), vs'_), sc, ac, ds_)
                   | (gb_, k, (us_, vs_),
                      ((I.App (u'_, s'_), s1'),
        ...
        and le (gb_, k, (us_, vs_), (us'_, vs'_), sc, ac, ds_) =
          let ds'_ = eq (gb_, (us_, vs_), (us'_, vs'_), sc, ac, ds_)
            in leW
               (gb_, k, (us_, vs_), Whnf.whnfEta (us'_, vs'_), sc, ac, ds'_)
        and leW =
          function 
                   | (((g_, b_) as gb_), k, ((u_, s1), (v_, s2)),
                      ((I.Lam ((I.Dec (_, v1'_) as d_), u'_), s1'),
                       (I.Pi ((I.Dec (_, v2'_), _), v'_), s2')),
                      sc, ac, ds_)
                       -> let ds'_ = ac (gb_, (v1'_, s1'), ds_) in begin
                            if
                            Subordinate.equiv
                            (I.targetFam v_, I.targetFam v1'_) then
                            let x_ = I.newEVar (g_, (I.EClo (v1'_, s1')))
                   ...
                   | (gb_, (o_ :: l_), (o'_ :: l'_), sc, ac, ds_)
                       -> let ds'_ =
                            ...
                            (function 
                                      | ()
                                          -> ordlt
                                             (gb_, o_, o'_, sc, ac, ds_))
                            in ordeq
                               (gb_, o_, o'_,
                                function 
                                         | ds''_
                                             -> ordltLex
                                                (gb_, l_, l'_, sc, ac, ds''_),
                                ac, ds'_)
        and ordltSimul =
          function 
                   | (gb_, [], [], sc, ac, ds_) -> ds_
        ...
        and ordeq =
          function 
                   | ((g_, b_), S.Arg (us_, vs_), S.Arg (us'_, vs'_), sc, ac,
                      ds_) -> begin
                       if
                       (Unify.unifiable (g_, vs_, vs'_)) &&
                         (Unify.unifiable (g_, us_, us'_))
        ...
        and ordle (gb_, o_, o'_, sc, ac, ds_) =
          let ds'_ =
            CSManager.trail
            (function 
                      | () -> ordeq (gb_, o_, o'_, sc, ac, ds_))
            in ordlt (gb_, o_, o'_, sc, ac, ds'_);;
        let rec skolem =
          function 
                   ...
                   | ((du, de), (g_, b_), w, F.Ex (I.Dec (name, v_), f_), sc)
                       -> let (s', v'_, f'_) = sc (w, de)
                            in let v1_ = (I.EClo (v_, s'))
                                 in let v2_ = Whnf.normalize (v'_ v1_, I.id)
                                      in let f1_ =
                                           (F.Ex
                                            ((I.Dec (name, v1_)), F.true_))
                                           in let f2_ = f'_ f1_
                                                in let _ = begin
        ...
        let rec updateState =
          function 
                   | (s_, ([], s)) -> s_
                   | ((S.State (n, (g_, b_), (ih_, oh_), d, o_, h_, f_) as
                        s_),
                      ((Lemma (n', frl'_) :: l_), s))
                       -> let ((g''_, b''_), s') =
                            skolem
                            ((0, 0), (g_, b_), I.id, F.forSub (frl'_, s),
                             function 
                                      | (s', _)
                                          -> (s', function 
                                                           | v'_ -> v'_,
        ...
        let rec selectFormula =
          function 
                   | (n,
                      (g0_, F.All (F.Prim ((I.Dec (_, v_) as d_)), f_), S.All
                       (_, o_)),
                      s_)
                       -> selectFormula (n, ((I.Decl (g0_, d_)), f_, o_), s_)
                   | (n, (g0_, F.And (f1_, f2_), S.And (o1_, o2_)), s_)
                       -> let (n', s'_) =
                            selectFormula (n, (g0_, f1_, o1_), s_)
                            in selectFormula (n, (g0_, f2_, o2_), s'_)
                   | (nih, (gall_, fex_, oex_),
                      (S.State
                        (ncurrent, (g0_, b0_), (_, _), _, ocurrent_, h_, f_)
                        as s_))
                       -> let ds_ =
                            recursion
                            ((nih, gall_, fex_, oex_),
                             (ncurrent, (g0_, b0_), [], ocurrent_, h_, f_))
                            in (nih + 1, updateState (s_, (ds_, I.id)));;
        let rec expand
          ((S.State (n, (g_, b_), (ih_, oh_), d, o_, h_, f_) as s_)) =
          let _ = begin
            if ! Global.doubleCheck then FunTypeCheck.isState s_ else () end
            in let (_, s'_) = selectFormula (1, (I.null_, ih_, oh_), s_)
                 in s'_;;
        let rec apply s_ =
          begin
            begin
