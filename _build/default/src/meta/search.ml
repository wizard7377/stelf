...
module type MTPSEARCH = sig
                        ...
                        val
                          searchEx : ((int * IntSyn.exp_ list * (int -> unit)) ->
      ...
      struct
        ...
        and occursInExpW =
          function 
                   ...
                   | (r, (I.Pi ((d_, _), v_), s))
                       -> (occursInDec (r, (d_, s))) ||
                            (occursInExp (r, (v_, I.dot1 s)))
                   | (r, (I.Root (_, s_), s)) -> occursInSpine (r, (s_, s))
                   | (r, (I.Lam (d_, v_), s))
                       -> (occursInDec (r, (d_, s))) ||
                            (occursInExp (r, (v_, I.dot1 s)))
                   | (r, (I.EVar (r', _, v'_, _), s))
                       -> (r = r') || (occursInExp (r, (v'_, s)))
                   | (r, (I.FgnExp csfe, s))
                       -> I.FgnExpStd.fold
                          csfe
                          (function 
                                    | (u_, b_)
                                        -> b_ || (occursInExp (r, (u_, s))))
                          false
        and occursInSpine =
          function 
                   | (_, (nil_, _)) -> false
                   | (r, (I.SClo (s_, s'), s))
                       -> occursInSpine (r, (s_, I.comp (s', s)))
                   | (r, (I.App (u_, s_), s))
                       -> (occursInExp (r, (u_, s))) ||
                            (occursInSpine (r, (s_, s)))
        and occursInDec (r, (I.Dec (_, v_), s)) = occursInExp (r, (v_, s));;
        let rec nonIndex =
          function 
                   | (_, []) -> true
                   | (r, (I.EVar (_, _, v_, _) :: ge_))
                       -> (not (occursInExp (r, (v_, I.id)))) &&
                            (nonIndex (r, ge_));;
        let rec selectEVar =
          function 
                   | [] -> []
                   | ((I.EVar (r, _, _, { contents = []}) as x_) :: ge_)
                       -> let xs_ = selectEVar ge_ in begin
                            if nonIndex (r, xs_) then xs_ @ [x_] else xs_ end
                   | ((I.EVar (r, _, _, cnstrs) as x_) :: ge_)
                       -> let xs_ = selectEVar ge_ in begin
                            if nonIndex (r, xs_) then (x_ :: xs_) else xs_
                            end;;
        let rec pruneCtx =
          function 
                   | (g_, 0) -> g_
        ...
        let rec solve =
          function 
                   | (max, depth, (C.Atom p, s), (C.DProg (g_, dPool) as dp),
                      sc) -> matchAtom (max, depth, (p, s), dp, sc)
                   | (max, depth, (C.Impl (r, a_, ha_, g), s), C.DProg
                      (g_, dPool), sc)
                       -> let d'_ = (I.Dec (None, (I.EClo (a_, s))))
                            in (Solve_
                                (max, depth + 1, (g, I.dot1 s),
                                 ...
                                 function 
                                          | m_ -> sc ((I.Lam (d'_, m_)))))
                   | (max, depth, (C.All (d_, g), s), C.DProg (g_, dPool),
                      sc)
        ...
        and rSolve =
          function 
                   | (max, depth, ps', (C.Eq q_, s), C.DProg (g_, dPool), sc)
                       -> begin
                       if Unify.unifiable (g_, ps', (q_, s)) then sc I.nil_
                       else () end
                   | (max, depth, ps', (C.Assign (q_, eqns), s),
                   ...
                   | (max, depth, ps', (C.And (r, a_, g), s),
                      (C.DProg (g_, dPool) as dp), sc)
                       -> let x_ = I.newEVar (g_, (I.EClo (a_, s)))
                            in rSolve
                               (max, depth, ps',
                                (r, (I.Dot ((I.Exp x_), s))), dp,
                                function 
                                         | s_
                                             -> (Solve_
                                                 (max, depth, (g, s), dp,
                                                  function 
                                                           | m_
                                                               -> sc
                                                                  ((I.App
                                                                    (m_, s_))))))
                   | (max, depth, ps', (C.In (r, a_, g), s),
                      (C.DProg (g_, dPool) as dp), sc)
                       -> let g0_ = pruneCtx (g_, depth)
                            in let dPool0 = pruneCtx (dPool, depth)
                                 in let w = (I.Shift depth)
                                      in let iw = Whnf.invert w
                                           in let s' = I.comp (s, iw)
                                                in let x_ =
                                                     I.newEVar
                                                     (g0_, (I.EClo (a_, s')))
                                                     in let x'_ =
                   ...
                   | (max, depth, ps', (C.Exists (I.Dec (_, a_), r), s),
                      (C.DProg (g_, dPool) as dp), sc)
                       -> let x_ = I.newEVar (g_, (I.EClo (a_, s)))
                            in rSolve
                               (max, depth, ps',
        ...
        and matchAtom =
          function 
                   | (0, _, _, _, _) -> ()
                   | (max, depth, ((I.Root (ha_, _), _) as ps'),
                      (C.DProg (g_, dPool) as dp), sc)
                       -> let rec matchSig' =
                            function 
                                     | [] -> ()
                                                              ...
                                                              -> rSolve
                                                                 (max - 1,
                                                                  ...
                                                                  function 
                                                                    ...
                                                                    ((I.BVar
                                                                    n), s_)))))
                                                in matchDProg (dPool', n + 1)
                                              else matchDProg (dPool', n + 1)
                                              end
                                          | (I.Decl (dPool', parameter_), n)
                                              -> matchDProg (dPool', n + 1)
                                 in matchDProg (dPool, 1)
        ...
        let rec deepen depth f p_ =
          let rec deepen' level = begin
            if level > depth then () else
            begin
              begin if (! Global.chatter) > 5 then print "#" else () end;
              begin
                f level p_;deepen' (level + 1)
                end
                        ...
                        | max
                            -> begin
                                 ...
                                 let ge'_ =
                                   foldr
                                   (function 
                                             | ((I.EVar (_, g_, _, _) as x_),
                                                l_)
                                                 -> Abstract.collectEVars
                                                    (g_, (x_, I.id), l_))
                                   []
                                   ge_
                                   in let gE' = List.length ge'_ in begin
                                        if gE' > 0 then begin
                                        if it > 0 then
                                        searchEx (it - 1, 1) (ge'_, sc) else
                                        () end else sc max end
