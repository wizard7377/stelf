      ...
      struct
        ...
        and createEVarSpineW =
          function 
                   ...
                   | (g_, (I.Pi (((I.Dec (_, v1_) as d_), _), v2_), s))
                       -> let x_ = I.newEVar (g_, (I.EClo (v1_, s)))
                            in let (s_, vs_) =
                                 createEVarSpine
        ...
        let rec createAtomConst (g_, h_) =
          let cid =
            begin match h_ with 
                                | I.Const cid -> cid
                                | I.Skonst cid -> cid end
            in let v_ = I.constType cid
          ...
          function 
                   ...
                   | (g_, (I.Dec (_, v_) :: l_), s)
                       -> someEVars
                          (g_, l_,
                           (I.Dot
                            ((I.Exp (I.newEVar (g_, (I.EClo (v_, s))))), s)));;
        let rec maxNumberParams a =
          let rec maxNumberParams' n = begin
            if n < 0 then 0 else
            let F.LabelDec (name, g1_, g2_) = F.labelLookup n
              in let m' =
                   foldr
                   (function 
                             | (I.Dec (_, v_), m) -> begin
                   ...
                   0
                   g2_ in (maxNumberParams' (n - 1)) + m'
            end in maxNumberParams' ((F.labelSize ()) - 1);;
        let rec maxNumberLocalParams =
          function 
                   | (I.Pi ((I.Dec (_, v1_), _), v2_), a)
                       -> let m = maxNumberLocalParams (v2_, a) in begin
                            if (I.targetFam v1_) = a then m + 1 else m end
                   | (I.Root _, a) -> 0;;
        ...
        let rec createTags =
          function 
                   ...
                   | (n, l)
                       -> (I.Decl
                           (createTags (n - 1, l), (S.Parameter ((Some l)))));;
        ...
        let rec metaCases (d, ops0) (c, g_, k, vs_, abstract) =
          let g = I.ctxLength g_
            in let rec select =
                 function 
                          | (0, ops) -> ops
                          | (d', ops)
                              -> let n = (g - d') + 1
                                   in let I.Dec (_, v_) = I.ctxDec (g_, n)
                                        in let ops' = begin
                                             if (I.targetFam v_) = c then
                                             let (u_, vs'_) =
                                               createAtomBVar (g_, n)
                                               in CSManager.trail
                                                  (function 
        ...
        let rec lowerSplitDest =
          function 
                   | (g_, k, ((I.Root (I.Const c, _) as v_), s'), abstract,
                      cases)
        ...
        let rec split (((I.Dec (_, v_) as d_), T), sc, abstract) =
          let rec split' (n, cases) = begin
            if n < 0 then
            let ((g'_, b'_), s', (g0_, b0_), _) = sc (I.null_, I.null_)
              in let rec abstract' u'_ =
                   let ((g''_, b''_), s'') =
                     MTPAbstract.abstractSub'
                     ((g'_, b'_), (I.Dot ((I.Exp u'_), s')),
                      (I.Decl (b0_, T)))
                     in let _ = begin
                          if ! Global.doubleCheck then
                          let psi''_ = aux (g''_, b''_)
                            in let _ =
                                 TypeCheck.typeCheckCtx (F.makectx psi''_)
                                 in let psi_ =
                                      aux
                                      ((I.Decl (g0_, d_)), (I.Decl (b0_, T)))
                                      in let _ =
                                           TypeCheck.typeCheckCtx
                                           (F.makectx psi_)
                                           in FunTypeCheck.checkSub
                                              (psi''_, s'', psi_)
                          else () end in abstract ((g''_, b''_), s'')
                   in lowerSplitDest
                      (g'_, 0, (v_, s'), abstract', constAndParamCases cases)
            else
            let F.LabelDec (name, g1_, g2_) = F.labelLookup n
              in let t = someEVars (I.null_, g1_, I.id)
                   in let b1_ = createLemmaTags (F.listToCtx g1_)
                        in let g2t_ = ctxSub (g2_, t)
                             in let length = List.length g2_
                                  in let b2_ = createTags (length, n)
                                       in let ((g'_, b'_), s', (g0_, b0_), p)
                                            =
                                            sc
                                            (Names.ctxName (F.listToCtx g2t_),
                                             b2_)
                                            in let rec abstract' u'_ = begin
                                                 if p then
                                                 raise
                                                 ((MTPAbstract.Error
                                                   "Cannot split right of parameters"))
                                                 else
                                                 let ((g''_, b''_), s'') =
                                                   MTPAbstract.abstractSub
                                                   (t, b1_, (g'_, b'_),
          ...
          function 
                   ...
                   | (k, I.Pi (dp_, v_))
                       -> (occursInDecP (k, dp_)) ||
                            (occursInExp (k + 1, v_))
                   | (k, I.Root (c_, s_))
                       -> (occursInCon (k, c_)) || (occursInSpine (k, s_))
                   | (k, I.Lam (d_, v_))
        ...
        let rec abstractInit
          ...
          begin
            begin
            if ! Global.doubleCheck then TypeCheck.typeCheckCtx g'_ else ()
            end;
            begin
              begin
              if ! Global.doubleCheck then
              FunTypeCheck.isFor (g'_, F.forSub (f_, s')) else () end;
              (S.State
               (n, (g'_, b'_), (ih_, oh_), d, S.orderSub (o_, s'),
                map (function 
                              | (i, f'_) -> (i, F.forSub (f'_, s'))) h_,
                F.forSub (f_, s')))
              
              end
            
            end;;
        let rec abstractCont ((d_, T), abstract) ((g_, b_), s) =
          abstract
          (((I.Decl (g_, Whnf.normalizeDec (d_, s))),
            (I.Decl (b_, S.normalizeTag (T, s)))),
           I.dot1 s);;
        ...
        let rec occursInOrder =
          function 
                   | (n, S.Arg (us_, vt_), k, sc)
                       -> let u'_ = Whnf.normalize us_ in begin
                            if occursInExp (k, u'_) then (Some n) else
                            sc (n + 1) end
                   | (n, S.Lex os_, k, sc) -> occursInOrders (n, os_, k, sc)
                   | (n, S.Simul os_, k, sc)
                       -> occursInOrders (n, os_, k, sc)
        and occursInOrders =
          function 
                   | (n, [], k, sc) -> sc n
                   | (n, (o_ :: os_), k, sc)
                       -> occursInOrder
                          (n, o_, k,
                           function 
                                    | n' -> occursInOrders (n', os_, k, sc));;
        let rec inductionInit o_ k =
          occursInOrder (0, o_, k, function 
                                            | n -> None);;
        let rec inductionCont induction k = induction (k + 1);;
        let rec expand' =
          function 
                   | (((null_, null_) as gb_), isIndex, abstract,
                      makeAddress, induction)
                       -> (function 
                                    | (gp_, bp_)
                                        -> ((gp_, bp_),
                                            (I.Shift (I.ctxLength gp_)), gb_,
                                            false),
                           [])
                   | (((I.Decl (g_, d_), I.Decl
                        (b_, (S.Lemma ((S.Splits _ as k_)) as t_))) as gb_),
                      isIndex, abstract, makeAddress, induction)
                       -> let (sc, ops) =
                            expand'
                            ((g_, b_), isIndexSucc (d_, isIndex),
                             abstractCont ((d_, T), abstract),
                             makeAddressCont makeAddress,
                             inductionCont induction)
                            in let I.Dec (xOpt, v_) = d_
                                 in let rec sc' (gp_, bp_) =
                       ...
                       -> let (sc, ops) =
                            ...
                            in let I.Dec (xOpt, v_) = d_
                                 in let rec sc' (gp_, bp_) =
                                      ...
                                      in (sc', ops)
                   | ((I.Decl (g_, d_), I.Decl
                       (b_, (S.Parameter (Some _) as t_))),
                      isIndex, abstract, makeAddress, induction)
                       -> let (sc, ops) =
                            expand'
                            ((g_, b_), isIndexSucc (d_, isIndex),
                             abstractErrorLeft, makeAddressCont makeAddress,
                             inductionCont induction)
                            in let I.Dec (xOpt, v_) = d_
                                 in let rec sc' (gp_, bp_) =
                                      let ((g'_, b'_), s', (g0_, b0_), _) =
                                        sc (gp_, bp_)
                                        in (((I.Decl
                                              (g'_,
                                               Names.decName
        ...
        let rec expand ((S.State (n, (g0_, b0_), _, _, o_, _, _) as s0_)) =
          let _ = begin
            if ! Global.doubleCheck then FunTypeCheck.isState s0_ else () end
            in let (_, ops) =
                 expand'
                 ((g0_, b0_), isIndexInit, abstractInit s0_,
                  makeAddressInit s0_, inductionInit o_)
                 in ops;;
        let rec index (Operator ((s_, index), sl_, { c = k})) = k;;
        let rec compare (Operator (_, _, i1_), Operator (_, _, i2_)) =
          H.compare (i1_, i2_);;
        let rec isInActive = function 
                                      | Active _ -> false
                                      | InActive -> true;;
          ...
          (function 
                    | Active s_
                        -> begin
                             begin
                             if ! Global.doubleCheck then
                             FunTypeCheck.isState s_ else () end;s_
                             end
                    | InActive
                        -> raise
                           ((Error "Not applicable: leftover constraints")))
          sl_;;
        let rec menu
          ((Operator
             ((S.State (n, (g_, b_), (ih_, oh_), d, o_, h_, f_), i), sl_, i_)
             as op_))
          =
          let rec active =
            function 
            ...
            in let rec inactive =
                 ...
                 in let rec casesToString =
                      function 
                               | 0 -> "zero cases"
                               | 1 -> "1 case"
                               | n -> (Int.toString n) ^ " cases"
                      in let rec flagToString =
                           function 
                                    | (_, 0) -> ""
                                    | (n, m)
                                        -> (((" [active: " ^ (Int.toString n))
                                               ^ " inactive: ")
                                              ^ (Int.toString m))
                                             ^ "]"
                           in (((("Splitting : " ^
                                    (Print.decToString (g_, I.ctxDec (g_, i))))
                                   ^ " ")
                                  ^ (H.indexToString i_))
                                 ^
                                 (flagToString
                                  (active (sl_, 0), inactive (sl_, 0))))
                                ^ "";;
        end;;
    (* recursive case *);;
    (* non-recursive case *);;
