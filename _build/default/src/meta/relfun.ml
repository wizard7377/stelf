  ...
  struct
    ...
    open!
      struct
          ...
          let v_ =
            ...
            in let mS =
                 ...
                 in let rec convertFor' =
                      function 
                               | (I.Pi ((d_, _), v_), M.Mapp
                                  (M.Marg (plus_, _), mS), w1, w2, n)
                                   -> let (f'_, f''_) =
                                        convertFor'
                                        (v_, mS, I.dot1 w1,
                                         (I.Dot ((I.Idx n), w2)), n - 1)
                                        in (function 
                                                     | f_
                                                         -> (F.All
                                                             ((F.Prim
                                                               (Weaken.strengthenDec
                                                                (d_, w1))),
                                                              f'_ f_)),
                                            f''_)
                               | (I.Pi ((d_, _), v_), M.Mapp
                                  (M.Marg (minus_, _), mS), w1, w2, n)
                                   -> let (f'_, f''_) =
                                        convertFor'
                                        (v_, mS, I.comp (w1, I.shift),
                                         I.dot1 w2, n + 1)
                                        in (f'_,
                                            (F.Ex (I.decSub (d_, w2), f''_)))
                               | (I.Uni type_, mnil_, _, _, _)
                                   -> (function 
                                                | f_ -> f_, F.true_)
                               | _
                                   -> raise
                      ...
                      in let rec shiftPlus mS =
                           let rec shiftPlus' =
                             function 
                                      | (mnil_, n) -> n
                                      | (M.Mapp (M.Marg (plus_, _), mS'), n)
                                          -> shiftPlus' (mS', n + 1)
                                      | (M.Mapp (M.Marg (minus_, _), mS'), n)
                                          -> shiftPlus' (mS', n)
                             in shiftPlus' (mS, 0)
                           in let n = shiftPlus mS
                                in let (f_, f'_) =
                                     convertFor'
                                     (v_, mS, I.id, (I.Shift n), n) in 
                                     f_ f'_;;
        let rec convertFor =
          function 
        ...
        let rec dot1inv w =
          Weaken.strengthenSub (I.comp (I.shift, w), I.shift);;
        let rec shiftinv w = Weaken.strengthenSub (w, I.shift);;
        let rec eqIdx = function 
                                 | (I.Idx n, I.Idx k) -> n = k
                                 | _ -> false;;
        let rec peel w = begin
          if eqIdx (I.bvarSub (1, w), (I.Idx 1)) then dot1inv w else
          shiftinv w end;;
        let rec peeln =
          function 
                   | (0, w) -> w
          ...
          let mS =
            ...
            in let v_ =
                 ...
                 with 
                      ...
                      | _
                          -> raise
                             ((Error "Type Constant declaration expected"))
                 end
                 in let rec abstract' =
                      function 
                               | ((_, mnil_), w) -> function 
                                                             | p -> p
                               | ((I.Pi ((d_, _), v2_), M.Mapp
                                   (M.Marg (plus_, _), mS)),
                                  w)
                                   -> let d'_ = Weaken.strengthenDec (d_, w)
                                        in let p_ =
                                             abstract' ((v2_, mS), I.dot1 w)
                                             in function 
                                                         | p
                                                             -> (F.Lam
                                                                 ((F.Prim
                               ...
                               | ((I.Pi (_, v2_), M.Mapp
                                  ...
                                  w)
                                   -> abstract'
                                      ((v2_, mS), I.comp (w, I.shift))
                      in abstract' ((v_, mS), I.id);;
          ...
          let mS =
            ...
            in let v_ =
                 ...
                 in let rec transformInit' =
                      ...
                      in transformInit'
                         ((s_, mS), v_,
                          (I.id, (I.Shift (F.lfctxLength psi_))));;
        let rec transformDec (ts_, (psi_, g0_), d, (a, s_), w1, w2, t0) =
          let mS =
            ...
            in let v_ =
                 ...
                 in let rec raiseExp (g_, u_, a) =
                      let rec raiseExp' =
                        function 
                                 | null_ -> (I.id, function 
                                                            | x -> x)
                                 | I.Decl (g_, (I.Dec (_, v_) as d_))
                                     -> let (w, k) = raiseExp' g_ in begin
                                          if
                                          Subordinate.belowEq
                                                    ...
                                                    | x
                                                        -> k
                                                           ((I.Lam
                                                             (Weaken.strengthenDec
                                                              (d_, w), x))))
                                          else (I.comp (w, I.shift), k) end
                        in let (w, k) = raiseExp' g_
                             in k (Weaken.strengthenExp (u_, w))
                      in let rec raiseType (g_, u_, a) =
                           let rec raiseType' =
                             function 
                                      | (null_, n)
                                          -> (I.id, function 
                             ...
                             in let (w, k, k') = raiseType' (g_, 2)
                                  in (k (Weaken.strengthenExp (u_, w)),
                                      (I.Root ((I.BVar 1), k' I.nil_)))
                           in let rec exchangeSub g0_ =
                                let g0 = I.ctxLength g0_
                                  in let rec exchangeSub' =
                                       function 
                                                | (0, s) -> s
                                                | (k, s)
                                                    -> exchangeSub'
                                ...
                                in let rec transformDec' =
                                     function 
                                              ...
                                              | (d,
                                                 ...
                                                 ((I.Dec (_, v1_), dp_), v2_),
                                                 (z1, z2), (w, t))
                                                  -> let g = I.ctxLength g0_
                                                       in let w1' =
                                                            peeln (g, w1)
                                                            in let (g1_, _) =
                                                                 Weaken.strengthenCtx
                                                                 (g0_, w1')
                                                                 in let
                                                                    (g2_, _)
                                                                    =
            ...
            in let rec transformConc' =
                 function 
                          ...
                          | (I.App (u_, s'_), M.Mapp
                             (M.Marg (minus_, _), mS'))
                              -> (F.Inx
                                  (Weaken.strengthenExp (u_, w),
                                   transformConc' (s'_, mS')))
                 in transformConc' (s_, mS);;
        let rec traverse (ts_, c) =
          let rec traverseNeg =
            function 
                         ...
                         -> begin
                            match traverseNeg
                                  (c'',
                                   (I.Decl
                                    (psi_,
                                     (F.Prim (Weaken.strengthenDec (d_, v))))),
                                   (v2_, I.dot1 v), l_)
                            with 
                                 | (Some (w', d', pq'_), l'_)
                                     -> ((Some (peel w', d', pq'_)), l'_)
                                 | (None, l'_) -> (None, l'_)
                            end
                     | (c'', psi_,
                        (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), v), l_)
                         -> begin
                            match traverseNeg
                                  (c'', psi_, (v2_, I.comp (v, I.shift)), l_)
                            with 
                                 | (Some (w', d', pq'_), l'_)
                                     -> traversePos
                                        (c'', psi_, I.null_,
                                         (Weaken.strengthenExp (v1_, v),
                                          I.id),
                                         (Some (w', d', pq'_)), l'_)
                                 | (None, l'_)
                                     -> traversePos
                     ...
                     | (c'', psi_, ((I.Root (I.Const c', s_) as v_), v), l_)
                         -> begin
                         if c = c' then
                         let s'_ = Weaken.strengthenSpine (s_, v)
                           in let (psi'_, w') =
                                strengthen
                                (psi_, (c', s'_),
                                 (I.Shift (F.lfctxLength psi_)), M.plus_)
                                in let (w'', s'') =
                                     transformInit (psi'_, (c', s'_), w')
                                     in ((Some
                                          (w', 1,
                                           (function 
                                                     | p -> (psi'_, s'', p),
          ...
          and traversePos =
            function 
                        ...
                        Some (w, d, pq_), l_)
                         -> begin
                            match traversePos
                                  (c'', psi_,
                                   (I.Decl
                                    (g_, Weaken.strengthenDec (d_, v))),
                                   (v2_, I.dot1 v),
                                   (Some (I.dot1 w, d, pq_)), l_)
                            with 
                                 | (Some (w', d', pq'_), l'_)
                                     -> ((Some (w', d', pq'_)), l'_)
                            end
                     | (c'', psi_, g_,
                        (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), v), Some
                        (w, d, pq_), l_)
                         -> begin
                            match traversePos
                                  (c'', psi_, g_, (v2_, I.comp (v, I.shift)),
                                   (Some (w, d, pq_)), l_)
                            with 
                                 | (Some (w', d', pq'_), l'_)
                                     -> begin
                     ...
                     | (c'', psi_, null_, (v_, v), Some (w1, d, (p_, q_)),
                        l_)
                         -> let I.Root (I.Const a', s_) =
                              Whnf.normalize
                              (Weaken.strengthenExp (v_, v), I.id)
                              in let (psi'_, w2) =
                                   strengthen (psi_, (a', s_), w1, M.minus_)
                                   in let _ = begin
                              ...
                              in let
                                   ...
                                   in let _ = begin
                                        ...
                                        in let g = I.ctxLength g_
                                             in let w1' = peeln (g, w1)
                                                  in let w2' = peeln (g, w2)
                                                       in let (g1_, _) =
                                                            Weaken.strengthenCtx
                                                            (g_, w1')
                                                            in let w3 =
                     ...
                     | (c'', psi_, g_,
                        (I.Pi (((I.Dec (_, v1_) as d_), no_), v2_), v), None,
                        l_)
                         -> begin
                            match traversePos
                                  (c'', psi_, g_, (v2_, I.comp (v, I.shift)),
            ...
            in let rec traverseSig' (c'', l_) = begin
                 if c'' = ((fun (r, _) -> r) (I.sgnSize ())) then l_ else
                 begin
                 match I.sgnLookup c''
                 with 
                      | I.ConDec (name, _, _, _, v_, type_)
                          -> begin
                             match traverseNeg (c'', I.null_, (v_, I.id), l_)
                             with 
          ...
          let rec convertOnePro a =
            let v_ =
              ...
              in let mS =
                   ...
                   in let p_ = abstract a
                        in p_ ((F.Case ((F.Opts (traverse (ts_, a))))))
            in let rec convertPro' =
                 function 
                          | []
                              -> raise
                                 ((Error "Cannot convert Empty program"))
                          | (a :: []) -> convertOnePro a
                          | (a :: ts'_)
                              -> (F.Pair (convertOnePro a, convertPro' ts'_))
                 in let r_ = recursion ts_ in r_ (convertPro' ts_);;
        end;;
    (* ctxSub (G, s) = (G', s')
