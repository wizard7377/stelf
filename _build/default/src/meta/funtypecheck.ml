      ...
      struct
        ...
        let rec conv (gs_, gs'_) =
          (let exception Conv 
           in let rec conv =
                function 
                         | ((null_, s), (null_, s')) -> (s, s')
                         | ((I.Decl (g_, I.Dec (_, v_)), s),
                            (I.Decl (g'_, I.Dec (_, v'_)), s'))
                             -> let (s1, s1') = conv ((g_, s), (g'_, s'))
                                  in let ((s2, s2') as ps) =
                                       (I.dot1 s1, I.dot1 s1') in begin
                                       if Conv.conv ((v_, s1), (v'_, s1'))
                                       then ps else raise Conv end
                         | _ -> raise Conv
                in try begin
                         conv (gs_, gs'_);true
                         end with 
        ...
        let rec validBlock (psi_, k, (l, g_)) =
          let rec skipBlock =
            ...
            in let rec validBlock' =
                 function 
                          | (I.Decl (psi_, F.Block (F.CtxBlock (l', g'_))),
                             0) -> begin
                              if (l' = l) && (conv ((g_, I.id), (g'_, I.id)))
                              then () else
                              raise
                              ((Error "Typecheck Error: Not a valid block"))
                              end
                          | (I.Decl (psi_, F.Prim _), 0)
                              -> raise
                                 ((Error
                                   "Typecheck Error: Not a valid block"))
                          | (null_, k)
                              -> raise
                                 ((Error
                                   "Typecheck Error: Not a valid block"))
                          | (I.Decl (psi_, F.Block (F.CtxBlock (l', g'_))),
                             k) -> validBlock' (psi_, skipBlock (g'_, k))
                          | (I.Decl (psi_, F.Prim d_), k)
                              -> validBlock' (psi_, k - 1)
                 in validBlock' (psi_, k);;
        let rec raiseSub (g_, psi'_) =
          let n = I.ctxLength g_
            in let m = I.ctxLength psi'_
                 in let rec args =
                      function 
                               | (0, a, s_) -> s_
                               | (n', a, s_)
                                   -> let I.Dec (_, v_) = I.ctxDec (g_, n')
                                        in begin
                                        if
                                        Subordinate.belowEq
                                        (I.targetFam v_, a) then
                                        args
                                        (n' - 1, a,
                                         (I.App
                                          ((I.Root ((I.BVar n'), I.nil_)),
                                           s_)))
                                        else args (n' - 1, a, s_) end
                      in let rec term m' =
                           let I.Dec (_, v_) = I.ctxDec (psi'_, m')
                             in (I.Exp
                                 ((I.Root
                                   ((I.BVar (n + m')),
                                    args (n, I.targetFam v_, I.nil_)))))
                           in let rec raiseSub'' =
                                function 
                                         | (0, s) -> s
                                         | (m', s)
          ...
          let rec raiseType'' =
            function 
                     ...
                     | (I.Decl (g'_, (I.Dec (_, v'_) as d_)), vn_, a)
                         ...
                         raiseType''
                         (g'_, Abstract.piDepend ((d_, I.maybe_), vn_), a)
                         else
                         raiseType''
                         (g'_, Weaken.strengthenExp (vn_, I.shift), a) end
            in let rec raiseType' =
                 function 
                          | (psi1_, []) -> []
                          | (psi1_,
                             (F.Prim ((I.Dec (x, v_) as d_)) :: psi1'_))
                              -> let s = raiseSub (g_, psi1_)
                                   in let vn_ = Whnf.normalize (v_, s)
                                        in let a = I.targetFam vn_
                                             in let d'_ =
                                                  (I.Dec
                                                   (x,
                                                    raiseType'' (g_, vn_, a)))
                                                  in ((F.Prim d'_)
                                                      :: raiseType'
                                                         ((I.Decl
                                                           (psi1_, d_)),
                                                          psi1'_))
                 in raiseType' (I.null_, psi'_);;
        let rec raiseM =
          function 
                   | (b_, []) -> []
                   | (b_, (F.MDec (xx, f_) :: l_))
                       -> ((F.MDec (xx, (F.All ((F.Block b_), f_))))
                           :: raiseM (b_, l_));;
        let rec psub =
          function 
                   | (k, null_, s) -> s
                   | (k, I.Decl (g_, _), s)
                       -> psub (k - 1, g_, (I.Dot ((I.Idx k), s)));;
        let rec deltaSub =
          function 
                   | (null_, s) -> I.null_
                   | (I.Decl (delta_, dd_), s)
        ...
        let rec check =
          function 
                   ...
                   | (psi_, delta_, F.Lam
                      ((F.Prim (I.Dec (_, v_)) as ld_), p_),
                      (F.All (F.Prim (I.Dec (_, v'_)), f'_), s')) -> begin
                       if Conv.conv ((v_, I.id), (v'_, s')) then
                       check
                       ((I.Decl (psi_, ld_)), shift delta_, p_,
                        (f'_, I.dot1 s'))
                       else
                       raise
                       ((Error "Typecheck Error: Primitive Abstraction")) end
                   | (psi_, delta_, F.Lam
                      ((F.Block ((F.CtxBlock (l, g_) as b_)) as ld_), p_),
                      (F.All (F.Block (F.CtxBlock (l', g'_)), f'_), s'))
                       -> begin
                       if (l = l') && (conv ((g_, I.id), (g'_, s'))) then
                       check
                       ((I.Decl (psi_, ld_)), shiftBlock (b_, delta_), p_,
                        (f'_, F.dot1n (g_, s')))
                       else
                       raise ((Error "Typecheck Error: Block Abstraction"))
                       end
                   | (psi_, delta_, F.Inx (m_, p_),
                      (F.Ex (I.Dec (_, v'_), f'_), s'))
                       -> begin
                            TypeCheck.typeCheck
                            (F.makectx psi_, (m_, (I.EClo (v'_, s'))));
                            check
                            (psi_, delta_, p_,
                             (f'_, (I.Dot ((I.Exp m_), s'))))
                            
                            end
                   | (psi_, delta_, F.Case (F.Opts o_), (f'_, s'))
                       -> checkOpts (psi_, delta_, o_, (f'_, s'))
                   | (psi_, delta_, F.Pair (p1_, p2_),
                      (F.And (f1'_, f2'_), s'))
                       -> begin
                   ...
                   | (psi_, delta_, F.Let (ds_, p_), (f'_, s'))
                       -> let (psi'_, delta'_, s'') =
                            assume (psi_, delta_, ds_)
                            in check
                               (extend (psi_, psi'_),
                                extend (delta_, delta'_), p_,
                                (f'_, I.comp (s', s'')))
                   | _
                       -> raise
                          ((Error "Typecheck Error: Term not well-typed"))
        ...
        and assume =
          function 
                   ...
                   | (psi_, delta_, F.Split (kk, ds_))
                       -> begin
                          match infer (delta_, kk)
                          with 
                               | (F.MDec (name, F.Ex (d_, f_)), s)
                                   -> let ld_ = (F.Prim (I.decSub (d_, s)))
                                        in let dd_ =
                                             (F.MDec
                                              (name, F.forSub (f_, I.dot1 s)))
                                             in let (psi'_, delta'_, s') =
                                                  assume
                                                  ((I.Decl (psi_, ld_)),
                                                   (I.Decl
                                                    (shift delta_, dd_)),
                               ...
                               | _
                                   -> raise
                                      ((Error "Typecheck Error: Declaration"))
                          end
                   | (psi_, delta_, F.New (b_, ds_))
                       -> let _ =
                            ...
                            in let (psi'_, delta'_, s') =
                                 assume
                                 ((I.Decl (psi_, (F.Block b_))),
                                  shiftBlock (b_, delta_), ds_)
                                 in (raiseType (b_, psi'_),
                                     raiseM (b_, delta'_), s')
                   | (psi_, delta_, F.App ((kk, u_), ds_))
                       -> begin
                          ...
                          with 
                               | (F.MDec
                                  (name, F.All (F.Prim (I.Dec (_, v_)), f_)),
                                  s)
                                   -> let _ =
                                        try TypeCheck.typeCheck
                                            (F.makectx psi_,
                                             (u_, (I.EClo (v_, s))))
                                        with 
                                             | TypeCheck.Error msg
                   ...
                   | (psi_, delta_, F.PApp ((kk, k), ds_))
                       -> begin
                          ...
                          with 
                               | (F.MDec
                                  (name, F.All
                                   (F.Block (F.CtxBlock (l, g_)), f_)),
                                  s)
                                   -> let _ = validBlock (psi_, k, (l, g_))
                                        in let dd_ =
                                             (F.MDec
                                              (name,
                                               F.forSub (f_, psub (k, g_, s))))
                                             in let (psi'_, delta'_, s') =
                                                  assume
                                                  (psi_,
                                                   (I.Decl (delta_, dd_)),
                               ...
                               | _
                                   -> raise
                                      ((Error
                                        "Typecheck Error: Declaration PApp"))
                          end
                       ...
                       -> begin
                          ...
                          with 
                               | (F.MDec (name, F.And (f1_, f2_)), s)
                                   -> let dd_ =
                                        (F.MDec (name, F.forSub (f1_, s)))
                                        in let (psi'_, delta'_, s') =
                                             assume
                                             (psi_, (I.Decl (delta_, dd_)),
                                              ds_)
                                             in (psi'_,
                                                 (F.mdecSub (dd_, s')
                                                  :: delta'_),
                               ...
                               | _
                                   -> raise
                                      ((Error
                                        "Typecheck Error: Declaration Left"))
                          end
                   ...
                   | (psi_, delta_, F.Lemma (cc, ds_))
                       -> let F.LemmaDec (names, _, f_) = F.lemmaLookup cc
                            in let name =
                                 foldr
                                 (fun (x__op, y__op) -> x__op ^ y__op)
                                 ""
                                 names
                                 in let dd_ = (F.MDec ((Some name), f_))
                                      in let (psi'_, delta'_, s') =
                                           assume
                                           (psi_, (I.Decl (delta_, dd_)),
                                            ds_)
        ...
        and checkSub =
          function 
                   ...
                   | (I.Decl (psi_, F.Prim d_), I.Shift k, null_) -> begin
                       if k > 0 then
                       checkSub (psi_, (I.Shift (k - 1)), I.null_) else
                       raise ((Error "Substitution not well-typed")) end
                   | (I.Decl (psi_, F.Block (F.CtxBlock (_, g_))), I.Shift k,
                      null_)
                       -> let g = I.ctxLength g_ in begin
                            if k >= g then
                            checkSub (psi_, (I.Shift (k - g)), I.null_) else
                            raise ((Error "Substitution not well-typed")) end
                   | (psi'_, I.Shift k, psi_)
                       -> checkSub
                          (psi'_,
                           ...
                           psi_)
                   | (psi'_, I.Dot (I.Idx k, s'), I.Decl
                      (psi_, F.Prim (I.Dec (_, v2_))))
                       -> let g'_ = F.makectx psi'_
                            in let I.Dec (_, v1_) = I.ctxDec (g'_, k)
                                 in begin
                                 if Conv.conv ((v1_, I.id), (v2_, s')) then
                                 checkSub (psi'_, s', psi_) else
                                 raise
                                 ((Error
                   ...
                   | (psi'_, I.Dot (I.Exp u_, s'), I.Decl
                      (psi_, F.Prim (I.Dec (_, v2_))))
                       -> let g'_ = F.makectx psi'_
                            in let _ =
                                 TypeCheck.typeCheck
                                 (g'_, (u_, (I.EClo (v2_, s'))))
                                 in checkSub (psi'_, s', psi_)
                   | (psi'_, (I.Dot (I.Idx k, _) as s), I.Decl
                      (psi_, F.Block (F.CtxBlock (l1, g_))))
                       -> let (F.Block (F.CtxBlock (l2, g'_)), w) =
                            F.lfctxLFDec (psi'_, k)
                            in let rec checkSub' =
                                 function 
                                          | ((null_, w1), s1, null_, _) -> s1
                                          | ((I.Decl (g'_, I.Dec (_, v'_)),
                                              w1),
                                             I.Dot (I.Idx k', s1), I.Decl
                                             (g_, I.Dec (_, v_)), m) -> begin
                                              if k' = m then begin
                                              if
                                              Conv.conv ((v'_, w1), (v_, s1))
                                              then
                                              checkSub'
                                              ((g'_, I.comp (w1, I.shift)),
                                               s1, g_, m + 1)
                                              else
                                              raise
                                              ((Error
                                                "ContextBlock assignment not well-typed"))
                                              end else
                                              raise
                                              ((Error
                                                "ContextBlock assignment out of order"))
                                              end
                                 in checkSub
                                    (psi'_, checkSub' ((g'_, w), s, g_, k),
                                     psi_)
        and checkOpts =
          function 
                   | (psi_, delta_, [], _) -> ()
                   | (psi_, delta_, ((psi'_, t, p_) :: o_), (f'_, s'))
                       -> begin
                            checkSub (psi'_, t, psi_);
                            begin
                              check
                              (psi'_, deltaSub (delta_, t), p_,
                               (f'_, I.comp (s', t)));
                              checkOpts (psi_, delta_, o_, (f'_, s'))
                              end
        ...
        let rec isFor =
          function 
                   ...
                   | (g_, F.And (f1_, f2_))
                       -> begin
                            isFor (g_, f1_);isFor (g_, f2_)
                            end
        and isForBlock =
          function 
                   | (g_, [], f_) -> isFor (g_, f_)
                   | (g_, (d_ :: g1_), f_)
                       -> isForBlock ((I.Decl (g_, d_)), g1_, f_);;
        let rec checkTags' =
          function 
                   | (v_, F.Ex _) -> ()
        ...
        let rec checkTags =
          function 
                   | (null_, null_) -> ()
                   | (I.Decl (g_, I.Dec (_, v_)), I.Decl (b_, T))
                       -> begin
                            checkTags (g_, b_);
                            begin match T with 
                                               | S.Lemma _ -> ()
        ...
        let rec isState (S.State (n, (g_, b_), (ih_, oh_), d, o_, h_, f_)) =
          begin
            ...
            begin
              ...
              begin
                begin
                if not (Abstract.closedCtx g_) then
                raise ((Error "State context not closed!")) else () end;
                begin
                  map (function 
                                | (n', f'_) -> isFor (g_, f'_)) h_;
                  isFor (g_, f_)
                  end
