    ...
    open!
      struct
          ...
          let rec formatTuple (g_, p_) =
            let rec formatTuple' =
              function 
                       ...
                       | F.Inx (m_, p'_)
                           -> (Print.formatExp (g_, m_) :: (Fmt.String ",")
                               :: Fmt.break_ :: formatTuple' p'_)
              in begin
                 match p_
                 with 
            ...
            in let S.State (n, (g_, b_), (ih_, oh_), d, o_, h_, f_) =
                 current ()
                 in TextIO.print
        ...
        let rec menu () = begin
          ...
          let s_ = current ()
            in let splitO_ = MTPSplitting.expand s_
                 in let infO_ = Inference.expand s_
                      in let recO_ = MTPRecursion.expand s_
                           in let fillO_ = MTPFilling.expand s_
                                in menu_ :=
                                     ((Some
                                       (fillingToMenu_
                                        (fillO_,
          ...
          let rec menuToString' =
            function 
                     ...
                     | (k, (Splitting o_ :: m_), ((None, None) as kOopt'))
                         -> let kOopt'' = begin
                              if MTPSplitting.applicable o_ then
                              ((Some k), (Some o_)) else kOopt' end
                              in let ((Some k'' as kopt), s) =
                                   menuToString' (k + 1, m_, kOopt'')
                                   in (kopt, begin
                                       if k = k'' then
                                       ((s ^ "\n* ") ^ (format k)) ^
                                         (MTPSplitting.menu o_)
                                       else
                                       ((s ^ "\n  ") ^ (format k)) ^
                                         (MTPSplitting.menu o_)
                                       end)
                     | (k, (Splitting o_ :: m_),
                        ((Some k', Some o'_) as kOopt'))
                         -> let kOopt'' = begin
                              if MTPSplitting.applicable o_ then
                              begin
                              match MTPSplitting.compare (o_, o'_)
                              with 
                                   | LESS -> ((Some k), (Some o_))
                                   | _ -> kOopt'
                              end else kOopt' end
                     ...
                     | (k, (Filling o_ :: m_), kOopt)
                         -> let (kopt, s) = menuToString' (k + 1, m_, kOopt)
                              in (kopt,
                                  ((s ^ "\n  ") ^ (format k)) ^
                                    (MTPFilling.menu o_))
            ...
            in begin
               ...
               with 
                    ...
                    | Some m_
                        -> let (kopt, s) =
                             menuToString' (1, m_, (None, None)) in s
               end;;
        let rec printMenu () = begin
          ...
          let s_ = current ()
            in let _ = begin
                 if ! Global.doubleCheck then FunTypeCheck.isState s_ else ()
                 end
                 in begin
                      print "\n";
                      begin
        ...
        let rec init (k, names) =
          let cL =
            ...
            names
            in let _ = MTPGlobal.maxFill := k
                 in let _ = reset ()
                      in let f_ = RelFun.convertFor cL
                           in let o_ =
                                transformOrder (I.null_, f_, map select cL)
                                in let slist_ = MTPInit.init (f_, o_)
                                     in let _ = begin
                                          if (List.length slist_) = 0 then
                                          raise Domain else () end
                                          in try begin
                                                   map
                                                   (function 
                                                             | s_
                     ...
                     | (1, (Recursion o_ :: _))
                         -> let s'_ =
                              ...
                              in let _ = pushHistory ()
                                   in let _ = delete ()
                                        in let _ =
                                             insert (MTPrint.nameState s'_)
                                             in begin
                                                  menu ();printMenu ()
                                                  end
                     | (1, (Inference o_ :: _))
                         -> let s'_ =
                              Timers.time Timers.recursion Inference.apply o_
                              in let _ = pushHistory ()
                                   in let _ = delete ()
                                        in let _ =
                                             insert (MTPrint.nameState s'_)
        ...
        let rec solve () = begin
          ...
          let s_ = current ()
            in let (open'_, solved'_) =
                 try MTPStrategy.run [s_]
                 with 
                      | MTPSplitting.Error s
                          -> abort ("MTPSplitting. Error: " ^ s)
                      | MTPFilling.Error s -> abort ("Filling Error: " ^ s)
                      | MTPRecursion.Error s
                          -> abort ("Recursion Error: " ^ s)
                      | Inference.Error s -> abort ("Inference Errror: " ^ s)
                      | Error s -> abort ("Mpi Error: " ^ s)
                 in let _ = pushHistory ()
                      in let _ = delete ()
        ...
        let rec check () = begin
          if empty () then raise ((Error "Nothing to check")) else
          let s_ = current () in FunTypeCheck.isState s_ end;;
        let rec auto () =
          let (open'_, solved'_) =
