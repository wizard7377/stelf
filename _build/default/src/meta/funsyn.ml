...
module type FUNSYN = sig
                     ...
                     type labelDec_ =
                       | LabelDec of
                         string * IntSyn.dec_ list * IntSyn.dec_ list 
                     (* ContextBody                *)
                     (* BB ::= l: SOME Theta. Phi  *)
                     type ctxBlock_ =
                     ...
                     val
                       convFor : ((for_ * IntSyn.sub_) * (for_ * IntSyn.sub_)) ->
                                 bool
...
module FunSyn(FunSyn__0: sig
  ...
  struct
    ...
    (* ContextBlocks              *);;
    (* B ::= l : Phi              *);;
    type lFDec_ = | Prim of IntSyn.dec_ 
                  | Block of ctxBlock_ ;;
    (* Contexts                   *);;
    (* LD ::= x :: A              *);;
    (*      | B                   *);;
    ...
    open!
      struct
        ...
        let rec listToCtx gin_ =
          let rec listToCtx' =
            function 
                     | (g_, []) -> g_
                     | (g_, (d_ :: ds_))
                         -> listToCtx' ((I.Decl (g_, d_)), ds_)
            in listToCtx' (I.null_, gin_);;
        ...
        let rec lfctxLFDec (psi_, k) =
          let rec lfctxLFDec' =
            function 
                     | (I.Decl (psi'_, (Prim (I.Dec (x, v'_)) as ld_)), 1)
                         -> (ld_, (I.Shift k))
                     | (I.Decl (psi'_, Prim _), k')
                         -> lfctxLFDec' (psi'_, k' - 1)
                     | (I.Decl (psi'_, (Block (CtxBlock (_, g_)) as ld_)),
                        k')
                         -> let l = I.ctxLength g_ in begin
                              if k' <= l then (ld_, (I.Shift ((k - k') + 1)))
                              else lfctxLFDec' (psi'_, k' - l) end
            in lfctxLFDec' (psi_, k);;
        let rec dot1n =
          function 
                   | (null_, s) -> s
          ...
          function 
                   ...
                   | ((All (Block (CtxBlock (_, g1_)), f1_), s1),
                      (All (Block (CtxBlock (_, g2_)), f2_), s2))
                       -> convForBlock
                          ((ctxToList g1_, f1_, s1),
                           (ctxToList g1_, f2_, s2))
                   | ((Ex (d1_, f1_), s1), (Ex (d2_, f2_), s2))
                       -> (Conv.convDec ((d1_, s1), (d2_, s2))) &&
                            (convFor ((f1_, I.dot1 s1), (f2_, I.dot1 s2)))
                   | ((And (f1_, f1'_), s1), (And (f2_, f2'_), s2))
                       -> (convFor ((f1_, s1), (f2_, s2))) &&
                            (convFor ((f1'_, s1), (f2'_, s2)))
                   | _ -> false
        and convForBlock =
          function 
                   | (([], f1_, s1), ([], f2_, s2))
                   ...
                   | (((d1_ :: g1_), f1_, s1), ((d2_ :: g2_), f2_, s2))
                       -> (Conv.convDec ((d1_, s1), (d2_, s2))) &&
                            (convForBlock
                             ((g1_, f1_, I.dot1 s1), (g2_, f2_, I.dot1 s2)))
                   | _ -> false;;
        let rec ctxSub =
          function 
                   | (null_, s) -> (I.null_, s)
        ...
        let rec forSub =
          function 
                   | (All (Prim d_, f_), s)
                       -> (All
                           ((Prim (I.decSub (d_, s))), forSub (f_, I.dot1 s)))
                   | (All (Block (CtxBlock (name, g_)), f_), s)
                       -> let (g'_, s') = ctxSub (g_, s)
                            in (All
                                ((Block ((CtxBlock (name, g'_)))),
                                 forSub (f_, s')))
                   | (Ex (d_, f_), s)
                       -> (Ex (I.decSub (d_, s), forSub (f_, I.dot1 s)))
                   | (True, s) -> True
                   | (And (f1_, f2_), s)
                       -> (And (forSub (f1_, s), forSub (f2_, s)));;
        let rec mdecSub (MDec (name, f_), s) = (MDec (name, forSub (f_, s)));;
        let rec normalizeFor =
          function 
                   | (All (Prim d_, f_), s)
                       -> (All
                           ((Prim (Whnf.normalizeDec (d_, s))),
                            normalizeFor (f_, I.dot1 s)))
                   | (Ex (d_, f_), s)
                       -> (Ex
                           (Whnf.normalizeDec (d_, s),
                            normalizeFor (f_, I.dot1 s)))
                   | (And (f1_, f2_), s)
                       -> (And
                           (normalizeFor (f1_, s), normalizeFor (f2_, s)))
                   | (True, _) -> True;;
        end;;
    (* hack!!! improve !!!! *);;
    (* union (G, G') = G''
