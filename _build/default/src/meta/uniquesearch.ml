      ...
      struct
        ...
        let rec solve =
          function 
                   ...
                   | (max, depth, (C.Impl (r, a_, h_, g), s), C.DProg
                      (g_, dPool), sc, acc)
        ...
        and rSolve =
          function 
                   | (max, depth, ps', (C.Eq q_, s), C.DProg (g_, dPool), sc,
                      acc) -> begin
                       if Unify.unifiable (g_, ps', (q_, s)) then
                       sc (I.nil_, acc) else acc end
                   | (max, depth, ps', (C.Assign (q_, eqns), s),
                      (C.DProg (g_, dPool) as dp), sc, acc)
                       -> begin
        ...
        and aSolve =
          function 
                   | ((trivial_, s), dp, cnstr, sc, acc) -> begin
                       if Assign.solveCnstr cnstr then sc () else acc end
                   | ((C.UnifyEq (g'_, e1, n_, eqns), s),
                      (C.DProg (g_, dPool) as dp), cnstr, sc, acc)
                          ...
                          -> begin
                               ...
                               let ge'_ =
                                 ...
                                 in let gE' = List.length ge'_ in begin
                                      ...
                                      (it - 1, depth)
                                      (ge'_, sc, acc') else
                                      raise ((Error "not found")) end else
                                      sc acc' end
        ...
        let rec search (maxFill, ge_, sc) =
          searchEx (1, maxFill) (ge_, sc, []);;
        end;;
