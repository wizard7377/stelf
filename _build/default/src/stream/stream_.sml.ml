...
module BasicStream : BASIC_STREAM =
  struct
    ...
    let empty = (Stream (function 
                                  | () -> Empty));;
    let rec cons (x, s) = (Stream (function 
                                            | () -> (Cons (x, s))));;
    end;;
...
module BasicMemoStream : BASIC_STREAM =
  struct
    ...
    let rec expose (Stream d) = d ();;
    let rec delay d =
      let memo = ref (function 
                               | () -> raise Uninitialized)
        in let rec memoFun () =
             try let r = d () in begin
                                   memo := (function 
                                                     | () -> r);r
                                   end
             with 
...
module Stream(Stream__0: sig module BasicStream : BASIC_STREAM end) : STREAM
  =
  struct
    open BasicStream;;
    exception EmptyStream ;;
    ...
    and tl' = function 
                       | Empty -> raise EmptyStream
                       | Cons (x, s) -> s;;
    let rec map f s = delay (function 
                                      | () -> map' f (expose s))
    ...
    let rec filter p s = delay (function 
                                         | () -> filter' p (expose s))
    ...
    let rec take (s, n) = begin
      if n < 0 then raise Subscript else takePos (s, n) end;;
    let rec fromList =
      function 
               | [] -> empty
    ...
    let rec drop (s, n) = begin
      if n < 0 then raise Subscript else dropPos (s, n) end;;
    let rec tabulate f = delay (function 
                                         | () -> tabulate' f)
    and tabulate' f = (Cons (f 0, tabulate (function 
                                                     | i -> f (i + 1))));;
    end;;
